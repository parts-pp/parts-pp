...
# ==========================================================
# Trader profile requirements (always enforced - 7 fields)
# ==========================================================
TRADER_PROFILE_REQUIRED_FIELDS = [
...
def _trader_profile_missing(tp: dict):
    """
    فحص اكتمال ملف التاجر (النسخة الحالية المعتمدة)
    يعتمد فقط على 3 حقول أساسية لبدء تقديم عروض السعر
    """
    try:
        tp = tp or {}
    except Exception:
    ...
    company = str(tp.get("company_name") or "").strip()
    iban = str(tp.get("iban") or "").strip()
    stc = str(tp.get("stc_pay") or "").strip()

    if not company:
        missing.append("اسم المتجر")

    if not iban:
        missing.append("IBAN")

    if not stc:
        missing.append("STC Pay")

    return missing
    
def _fmt_missing_fields_lines(missing: list[str]) -> str:
    if not missing:
        return ""
    out = []
    for i, k in enumerate(missing, start=1):
        out.append(f"{i}) {TRADER_PROFILE_FIELD_LABELS.get(k, k)}")
    return "\n".join(out)

def _is_trader_profile_strict(context: ContextTypes.DEFAULT_TYPE = None) -> bool:
    # ✅ تم حذف زر الإلزام بالكامل — الإلزام دائم وثابت
    return True
...
async def _need_complete_trader_profile_notice(
    ...
    """تنبيه موحد: الملف ناقص + عرض الحقول الناقصة + زر فتح لوحة التاجر (رابط)."""
    ...
    if missing is None:
        try:
            ...
            tp = {}
        missing = _trader_profile_missing(tp)

    miss_lines = ""
    if missing:
        miss_lines = _fmt_missing_fields_lines(missing)
        miss_lines = f"\n\n📌 <b>الحقول الناقصة:</b>\n<pre>{html.escape(miss_lines)}</pre>"

    head = (
        f"{name}\n"
        "⛔ <b>لا يمكنك تقديم/إرسال عرض سعر</b> قبل إكمال <b>جميع</b> حقول ملف التاجر.\n"
        "✅ أكملها من لوحة التاجر ثم أعد المحاولة."
    )
    ...
    try:
        ...
        try:
            await context.bot.send_message(chat_id=int(user_id), text=f"{name}\n⛔ أكمل ملف التاجر أولاً.", disable_web_page_preview=True)
        except Exception:
            pass
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ لا يبدأ/يكمل عرض سعر إلا بعد اكتمال ملف التاجر (ثابت: 7 حقول)
    try:
        tp = get_trader_profile(int(user_id or 0)) or {}
    except Exception:
        tp = {}
    ...
    missing = _trader_profile_missing(tp)
    if missing:
        # تنبيه واحد واضح + فتح لوحة التاجر فورًا
        try:
            await _alert(q, "⛔ ملف التاجر غير مكتمل. أكمل البيانات المطلوبة ثم أعد المحاولة.", force=True)
        except Exception:
            pass

        try:
            await show_trader_panel(q.message, context, user_id)
        except Exception:
            try:
        ...
        if field == "national_id":
            ...
            if not re.fullmatch(r"\d{10}", v):
                await _reply_html(
                    "رقم الهوية غير صحيح",
                    [
                        "⚠️ الصيغة المطلوبة: 10 أرقام فقط.",
                        "مثال: <code>10XXXXXXXX</code>",
                    ],
                )
                return
            val = v
        ...
        if field == "vat_number":
            ...
            if not re.fullmatch(r"\d{15}", v):
                await _reply_html(
                    ...
                    [
                        "⚠️ الصيغة المطلوبة: 15 رقم فقط.",
                        "مثال: <code>3XXXXXXXXXXXXXX</code>",
                    ],
                )
                return
            val = v
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "edit":
        ...
        if field == "iban":
            ...
            hint = "\n\n10 أرقام — مثال: <code>10XXXXXXXX</code>"
        elif field == "cr_number":
            hint = "\n\n10 أرقام — مثال: <code>10XXXXXXXX</code>"
        elif field == "vat_number":
            hint = "\n\n15 رقم — مثال: <code>3XXXXXXXXXXXXXX</code>"

        await q.message.reply_text(
            f"{_user_name(q)}\n🟦 <b>تعديل {html.escape(title)}</b>\nاكتب القيمة الآن وسيتم حفظها مباشرة{hint}",
            parse_mode="HTML",
        )
        return
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "tstrict":
        ...
        strict_on = mode == "on"
        try:
            set_setting("trader_profile_strict", "1" if strict_on else "0")
        except Exception as e:
            _swallow(e, "tstrict_set")

        # ✅ تنفيذ صامت: بدون نافذة Alert مزعجة
        try:
            await q.answer(
                "✅ تم تفعيل الإلزام" if strict_on else "✅ تم تعطيل الإلزام",
                show_alert=False,
            )
        except Exception as e:
            _swallow(e, "tstrict_answer")

        # ✅ رسالة خاصة للأدمن فقط لتأكيد التنفيذ
        try:
            await context.bot.send_message(
                chat_id=int(q.from_user.id),
                text=(
                    "✅ تم تفعيل الإلزام في لوحة التجار"
                    if strict_on
                    else "✅ تم تعطيل الإلزام في لوحة التجار"
                ),
                disable_web_page_preview=True,
            )
        except Exception as e:
            _swallow(e, "tstrict_dm")

        # ✅ تحديث نفس شاشة إدارة التجار (تحديث نص الحالة واسم الزر)
        await _admin_show_traders_manage()
        return
    ...
    if action == "trecord_pdf":
        ...
        try:
            ...
            val = ParagraphStyle(
                "val", parent=base, alignment=0
            )  # TA_LEFT (القيم على اليسار بالنمط العربي)
            ...
            # Data table (Key / Value) — RTL (العنوان يمين / القيمة يسار)
            rows = [
                ...
                [P(html.escape(sub_status), val), P(f"<b>حالة الاشتراك ({month})</b>", label)],
            ...
            # ✅ العمود الثاني (العناوين) على اليمين، والقيم على اليسار
            tbl = Table(rows, colWidths=[full_w * 0.62, full_w * 0.38])
            tbl.setStyle(
                TableStyle(
                    [
                        # ✅ صفوف شفافة حتى تظهر العلامة المائية بوضوح
                        (
                            "ROWBACKGROUNDS",
                            (0, 0),
                            (-1, -1),
                            [
                                colors.Color(1, 1, 1, alpha=0.00),
                                colors.Color(1, 1, 1, alpha=0.00),
                            ],
                        ),
                        # تمييز عمود العناوين (يمين) بخفة شفافة
                        (
                            "BACKGROUND",
                            (1, 0),
                            (1, -1),
...
def trader_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "🏷️ تعديل رقم السجل التجاري", callback_data="pp_tprof|edit|cr_number"
                )
            ],
            [
                InlineKeyboardButton(
                    "🪪 تعديل رقم الهوية", callback_data="pp_tprof|edit|national_id"
                )
            ],
            [
                InlineKeyboardButton(
                    "🧾 تعديل الرقم الضريبي (VAT)", callback_data="pp_tprof|edit|vat_number"
                )
            ],
...
async def show_trader_panel(
    ...
    # ✅ اكتمال الملف
    # ✅ اكتمال الملف (ثابت: 7 حقول)
    missing = _trader_profile_missing(tp)
    complete_txt = "مكتمل ✅" if not missing else f"ناقص ⛔ ({len(missing)})"

    def _code_or_placeholder(v: str, ph: str = "⛔ غير مدخل") -> str:
        v = (v or "").strip()
        return f"<code>{html.escape(v)}</code>" if v else ph

    txt = (
        f"{banner}"
        "🟩 <b>لوحة التاجر</b>\n"
        f"🔒 الحالة: <b>{status_txt}</b>\n"
        f"📌 اكتمال الملف: <b>{complete_txt}</b>\n\n"
        f"👤 الاسم المعروض: <b>{html.escape(dn)}</b>\n"
        f"🏢 اسم المتجر: <b>{html.escape(cn or '—')}</b>\n\n"
        "💳 <b>بيانات الدفع</b>\n"
        f"🏦 البنك: {_code_or_placeholder(tp.get('bank_name') or '')}\n"
        f"💳 IBAN: {_code_or_placeholder(tp.get('iban') or '')}\n"
        f"📱 STC Pay: {_code_or_placeholder(tp.get('stc_pay') or '')}\n\n"
        "📄 <b>البيانات النظامية</b>\n"
        f"🏷️ السجل التجاري: {_code_or_placeholder(tp.get('cr_number') or '')}\n"
        f"🪪 رقم الهوية: {_code_or_placeholder(tp.get('national_id') or '')}\n"
        f"🧾 الرقم الضريبي: {_code_or_placeholder(tp.get('vat_number') or '')}\n"
        "\nℹ️ يتم حفظ هذه البيانات داخل ملف المنصة وتبقى حتى بعد إعادة التشغيل.\n"
    )

    if missing:
        txt += (
            "\n🧩 <b>الحقول الناقصة:</b>\n"
            f"<pre>{html.escape(_fmt_missing_fields_lines(missing))}</pre>\n"
            "✅ أكملها من أزرار التعديل بالأسفل، ثم قدّم عرض السعر."
        )
    else:
        txt += "\n✅ ملفك مكتمل ويمكنك تقديم عروض الأسعار."
