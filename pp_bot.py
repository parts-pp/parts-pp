...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ضمان تحميل الاكسل قبل أي قراءة/تسعير (يحل مشكلة: "لا توجد بنود" رغم وجودها)
    try:
        ensure_workbook()
    except Exception:
        pass
    ...
    def _get_items(order_id: str):
        ...
        # إعادة ضمان تحميل الاكسل قبل القراءة (زيادة أمان)
        try:
            ensure_workbook()
        except Exception:
            pass
        ...
        # إعادة محاولة بسيطة (لو كانت القراءة الأولى رجعت فاضية بسبب تزامن/تحميل)
        if not its:
            try:
                ensure_workbook()
            except Exception:
    ...
    if action == "ppq_begin":
        ...
        if not its:
            await q.message.reply_text(
                ...
                f"رقم الطلب: {order_id}\n"
                "ملاحظة: لو أنت متأكد فيه بنود، غالبًا رقم الطلب واصل بمسافة/سطر جديد وتم إصلاحه بهذا التحديث.",
                disable_web_page_preview=True,
            )
            return
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            td["quote_step"] = "it_price"  # ✅ يتم استقباله في text_handler
            try:
            ...
            td["quote_step"] = "it_all_price"  # ✅ يتم استقباله في text_handler
            try:
                await q.message.edit_reply_markup(reply_markup=None)
            except Exception:
                pass
        ...
        try:
            await q.message.edit_reply_markup(reply_markup=_items_kb(order_id, its, pm))
        except Exception:
            pass
        await _alert(q, "تم")
        return
