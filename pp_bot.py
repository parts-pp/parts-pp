...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    3) عند الضغط (جاهز للشحن): يطلب رفع فاتورة المتجر (إجباري) في الخاص
       - قبل رفع الفاتورة: يبقى زر (جاهز للشحن) فقط
       - بعد رفع الفاتورة: يظهر (⏳ بانتظار سداد العميل) + زر (📞 الاتصال بالعميل)
    4) بعد تأكيد دفع البضاعة (goods_payment_status مؤكد/مدفوع): يظهر زرين فقط (🚚 تم الشحن + 📞 الاتصال بالعميل)
    5) بعد الشحن: يظهر (✅ تأكيد التسليم) + (📞 الاتصال بالعميل) لمدة 7 أيام (المنع يتم من هاندر المراسلة حسب chat_expires_at_utc)
    """
    try:
    ...
    # 2) بعد تجهيز -> يظهر الزر التالي فقط
    if ost in ("preparing", "prep"):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready|{order_id}")],
        ])

    # 3) مرحلة جاهز للشحن (لكن الفاتورة شرط)
    if ost in ("ready_to_ship", "ready"):
        # قبل رفع الفاتورة: نخليه يرجع لنفس زر جاهز للشحن (عشان يعيد التوجيه للخاص)
        if not inv_file:
            return InlineKeyboardMarkup([
                [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready|{order_id}")],
            ])

        # بعد رفع الفاتورة وقبل الدفع: انتظار + فتح المراسلة
        if not goods_paid:
            return InlineKeyboardMarkup([
    ...
    # 5) بعد التسليم/إغلاق: نخلي الاتصال إذا كان نظام الشات يسمح (المنع من هاندر الشات)
...
async def quote_ok_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    update_order_fields(order_id, {
        ...
        # ✅ إصلاح حالة الطلب: بعد قبول العرض لازم يتحول الطلب إلى accepted
        # حتى تعمل أزرار التسلسل (جاري تجهيز/جاهز للشحن) بدون تعطل
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== الإجمالي (القطع + الشحن إن وجد وغير مشمول) =====
    goods_total = 0
    shipping_fee = 0
    ...
    total_line = f"💰 إجمالي الطلب (شامل الشحن): {_money(grand_total)}" if str(grand_total).strip() else ""
    ...
    # ===== سيناريو التسلسل (بدون أي Alerts/تحذيرات) =====

    # 1) جاري تجهيز: مسموح فقط من البداية
    if new_status == "preparing":
        # ✅ إصلاح: بعض الطلبات تبقى quoted بعد قبول العرض، فكان الزر لا يعمل
        if ost_now not in ("", "new", "accepted", "quoted"):
            return

    # 2) جاهز للشحن: الضغط هنا يفتح مرحلة رفع الفاتورة (إجباري) في الخاص
    if new_status == "ready_to_ship":
        ...
        if not inv_file:
            ...
            # رسالة واحدة فقط للتاجر في الخاص (جزء من التدفق، بدون تنبيهات)
            try:
                await context.bot.send_message(
                    chat_id=actor_id,
                    text=(
                        "🧾 رفع فاتورة المتجر مطلوب لإكمال الطلب\n\n"
                        f"👤 العميل: {client_name}\n"
                        f"🧾 رقم الطلب: {order_id}\n"
                        + (f"{total_line}\n" if total_line else "")
                        + "📌 الحالة: 🟢 الطلب جاهز للشحن\n\n"
                        "➡️ الخطوة القادمة: ارسل فاتورة المتجر الرسمية (PDF أو صورة) هنا في الخاص.\n"
                        "بعد رفع الفاتورة سيظهر لك زر الاتصال بالعميل، وننتظر إيصال السداد من العميل."
                    ).strip(),
                    disable_web_page_preview=True,
                )
            except Exception:
        ...
        # إذا الفاتورة موجودة أصلاً: التحويل إلى ready_to_ship يتم من هاندر رفع الفاتورة (لا نكرر هنا)
        return
    ...
    if total_line:
        header_block += f"{total_line}\n"

    # ===== خطوة تالية (تظهر داخل نفس الرسالة بدل التنبيهات) =====
    next_step = ""
    if new_status in ("preparing", "prep"):
        next_step = "➡️ الخطوة التالية: اضغط (🟢 الطلب جاهز للشحن).\n"
    elif new_status in ("ready_to_ship", "ready"):
        next_step = "➡️ الخطوة التالية: ارفع فاتورة المتجر الرسمية في الخاص، ثم ننتظر إيصال السداد من العميل.\n"
    elif new_status == "shipped":
        next_step = "➡️ الخطوة التالية: بعد تسليم العميل اضغط (✅ تأكيد التسليم بنجاح).\n"
    elif new_status in ("delivered", "closed"):
        next_step = "✅ تم تأكيد التسليم وإغلاق الطلب.\n"
    ...
    if client_id:
        try:
            msg = (
                "📦 تحديث حالة الطلب\n"
                + header_block
                + ("\n" + parts_block if parts_block else "")
