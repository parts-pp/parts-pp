    ...
    if stage == STAGE_JOIN_VAT:
        ...
        async def _send_file_to_admins(kind: str, fid: str, cap: str):
            for aid in (ADMIN_IDS or []):
                try:
                    if kind == "photo":
                        await context.bot.send_photo(chat_id=int(aid), photo=fid, caption=cap, parse_mode="HTML")
                    else:
                        await context.bot.send_document(chat_id=int(aid), document=fid, caption=cap, parse_mode="HTML")
                except Exception:
                    pass
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💬 مراسلة المنصة", callback_data="support_open_cb")],
        ])
        await update.message.reply_text(
            f"{_join_portal_brand()}\n\n"
            "✅ تم إرسال مستنداتك للإدارة للمراجعة.\n"
