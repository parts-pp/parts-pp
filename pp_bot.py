...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    """لوحة خطوات التاجر (قفل بصري لبقية الأزرار حسب التسلسل المطلوب)."""
    ...
    # ✅ تطبيع بسيط لحالات مختلفة/قديمة
    if st in ("prep", "preparing_order", "preparingrequest"):
        st = "preparing"
    if st in ("ready", "readytoship"):
        st = "ready_to_ship"

    # ✅ التسلسل:
    # تجهيز -> جاهز للشحن -> تم الشحن -> (تسليم + مراسلة)
    allow_prep = st not in ("ready_to_ship", "shipped", "delivered", "closed")
    allow_ready = st in ("preparing",)
    allow_shipped = st in ("ready_to_ship",)
    allow_delivered = st in ("shipped",)
    allow_chat = st in ("shipped", "delivered")

    def _btn(label: str, ok: bool, cb_ok: str, need_hint: str):
        if ok:
            return InlineKeyboardButton(label, callback_data=cb_ok)
        return InlineKeyboardButton("🔒 " + label, callback_data=f"pp_trader_status|guard|{need_hint}|{oid}")

    return InlineKeyboardMarkup([
        [_btn("🟡 جاري تجهيز الطلب", allow_prep, f"pp_trader_status|prep|{oid}", "prep")],
        [_btn("🟢 الطلب جاهز للشحن", allow_ready, f"pp_trader_status|ready|{oid}", "ready")],
        [_btn("🚚 تم شحن الطلب", allow_shipped, f"pp_trader_status|shipped|{oid}", "shipped")],
        [_btn("✅ تم تسليم الطلب للعميل بنجاح", allow_delivered, f"pp_trader_status|delivered|{oid}", "delivered")],
        [_btn("💬 مراسلة العميل داخل المنصة", allow_chat, f"pp_chat_open|{oid}", "chat")],
    ])
...
async def chat_open_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    st_now = str(order.get("order_status") or "").strip().lower()
    if st_now not in ("shipped", "delivered", "closed"):
        try:
            await q.answer("⚠️ لا يمكن استخدام (💬 مراسلة العميل) قبل تنفيذ خطوة (🚚 تم شحن الطلب).", show_alert=True)
        except Exception:
            pass
        return
