...
async def send_trader_ledger_pdf(
    ...
    # ✅ (02) badges: إصلاح الانعكاس + الصيغة المطلوبة
    RLM = "\u200f"
    trader_name = (tname or str(tid)).strip()
    trader_badge = f"{trader_name}" + (f" {tuser}" if tuser else "")

    badges = Table([[
        P(f"{RLM}الوقت: <b>{now.strftime('%H:%M')}</b>", tiny_c),
        P(f"{RLM}التاريخ: <b>{now.strftime('%Y-%m-%d')}</b>", tiny_c),
        P(f"{RLM}التاجر: <b>{trader_badge}</b>", tiny_c),
    ...
    def kv_table(rows: list[tuple[str, str]]):
        """
        ✅ (04) حدود/شبكة شفافة لإظهار الخلفية المائية خلف الجدول
        """
        data = []
        for k, v in rows:
    ...
    # ✅ (01) Footer ثابت أسفل الصفحة: آخر سطر في الـPDF
    ...
    def _wm(canvas, docx):
        try:
            if logo_path and os.path.exists(logo_path):
                ...
                wm_h = 16.8 * cm
                x = (page_w - wm_w) / 2.0
                y = (page_h - wm_h) / 2.0 + (1.2 * cm)
                try:
                    canvas.setFillAlpha(0.12)
                except Exception:
                    pass
                canvas.drawImage(logo_path, x, y, width=wm_w, height=wm_h, mask='auto',
