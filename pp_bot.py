...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    # ✅ اقرأ الحالة الحالية لتفعيل زر واحد فقط (حسب التسلسل)
    ost = ""
    try:
    ...
    # تطبيع بسيط
    if ost in ("ready", "readytoship"):
        ost = "ready_to_ship"

    # ✅ المرحلة الحالية: زر واحد فقط بدون قفل، والبقية مقفولة
    step = "prep"  # الافتراضي: أول خطوة
    if ost == "preparing":
        step = "ready"
    elif ost == "ready_to_ship":
        step = "shipped"
    elif ost in ("shipped", "delivered", "closed"):
        step = "delivered"  # بعد الشحن: التسليم + المراسلة متاحة

    def _status_btn(label: str, action: str, need: str):
        if action == step or (step == "delivered" and action == "delivered"):
            return InlineKeyboardButton(label, callback_data=f"pp_trader_status|{action}|{oid}")
        return InlineKeyboardButton("🔒 " + label, callback_data=f"pp_trader_status|guard|{need}|{oid}")

    def _chat_btn(label: str, need: str):
        if step == "delivered":
            return InlineKeyboardButton(label, callback_data=f"pp_chat_open|{oid}")
        return InlineKeyboardButton("🔒 " + label, callback_data=f"pp_trader_status|guard|{need}|{oid}")

    return InlineKeyboardMarkup([
        [_status_btn("🟡 جاري تجهيز الطلب", "prep", "prep")],
        [_status_btn("🟢 الطلب جاهز للشحن", "ready", "prep")],
        [_status_btn("🚚 تم شحن الطلب", "shipped", "ready")],
        [_status_btn("✅ تم تسليم الطلب للعميل بنجاح", "delivered", "shipped")],
        [_chat_btn("💬 مراسلة العميل داخل المنصة", "shipped")],
    ])
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # عادي: pp_trader_status|<st>|<order_id>
    # حارس: pp_trader_status|guard|<need>|<order_id>
    st = (parts[1] or "").strip()
    if st == "guard":
        need = (parts[2] or "").strip().lower()
        order_id = (parts[3] if len(parts) >= 4 else "").strip()
    else:
        need = ""
    ...
    if st == "guard":
        ...
        btn = need_map.get(need, "الخطوة السابقة")
        try:
            await q.answer(f"⚠️ لازم تبدأ أولاً بـ: {btn}", show_alert=True)
        except Exception:
            pass
        return
    ...
    # ===== ✅ حارس التسلسل: يمنع تخطي الخطوات (Popup فقط) =====
    # تطبيع سريع
    ost_seq = ost_now
    if ost_seq in ("ready", "readytoship"):
        ost_seq = "ready_to_ship"

    # (تم الشحن) لا يعمل إلا بعد (الطلب جاهز للشحن)
    if new_status == "shipped":
        if ost_seq not in ("ready_to_ship", "shipped", "delivered", "closed"):
            try:
                await q.answer("⚠️ قبل (🚚 تم شحن الطلب) اضغط: 🟢 الطلب جاهز للشحن", show_alert=True)
            except Exception:
                pass
            return

    # (تأكيد التسليم) لا يعمل إلا بعد (تم الشحن)
    if new_status == "delivered":
        if ost_seq not in ("shipped", "delivered", "closed"):
            try:
                await q.answer("⚠️ قبل (✅ تم التسليم) اضغط: 🚚 تم شحن الطلب", show_alert=True)
            except Exception:
                pass
            return
...
async def chat_open_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ حارس: مراسلة العميل لا تعمل إلا بعد (تم الشحن)
    try:
        ost = str(order.get("order_status") or "").strip().lower()
    except Exception:
        ost = ""
    if ost not in ("shipped", "delivered", "closed"):
        try:
            await q.answer("⚠️ المراسلة تفتح بعد: 🚚 تم شحن الطلب", show_alert=True)
        except Exception:
            pass
        return
