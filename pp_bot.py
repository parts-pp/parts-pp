...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "ppq_begin":
        ...
        # ✅ جديد: خطوة اختيار البنود المشمولة قبل المبلغ
        td["quote_order_id"] = order_id
        td["quote_step"] = "items"
        ...
        sel = set(range(1, len(its) + 1))  # افتراضيًا: الكل محدد
        td["quote_item_sel"] = sorted(sel)

        await q.message.reply_text(
            f"{name}\n"
            "🧩 اختر البنود المتوفرة لهذا الطلب ثم اضغط (التالي)\n"
            "⬜ = غير متوفر | ✅ = متوفر",
            reply_markup=trader_quote_items_kb(order_id, its, sel),
            disable_web_page_preview=True,
        )
        return
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        n = len(its)
        cur = set(int(x) for x in (td.get("quote_item_sel") or []) if str(x).isdigit())
        ...
        if action == "ppq_it":
            ...
            if not cur:
                await _alert(q, "حدد بند واحد على الأقل")
                return

            td["quote_item_sel"] = sorted(cur)
        ...
        try:
            await q.message.edit_reply_markup(
                reply_markup=trader_quote_items_kb(order_id, its, cur)
            )
        except Exception:
            pass
        await _alert(q, "تم")
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # ✅ جديد: إضافة البنود المشمولة في العرض (حسب اختيار التاجر)
    try:
        obx = get_order_bundle(order_id) or {}
    ...
    sel = [int(x) for x in (td.get("quote_item_sel") or []) if str(x).isdigit()]
    if not sel:
        sel = list(range(1, len(itemsx) + 1))

    sel_nums = ",".join(str(i) for i in sel)
    include_line = f"🧩 البنود المشمولة في العرض: {sel_nums}\n"

    # (اختياري) أسماء مختصرة جداً (حتى 6 بنود)
    name_lines = []
    for i in sel[:6]:
        if 1 <= i <= len(itemsx):
            nm = (itemsx[i-1].get("name") or itemsx[i-1].get("item_name") or "").strip()
            if nm:
                name_lines.append(f"• {i}- {nm[:32]}")
    if name_lines:
        include_line += "\n".join(name_lines) + "\n"
    ...
    fields_to_update = {
        ...
        # ✅ جديد: حفظ البنود المشمولة (كنص بسيط بدون تعديل هيكل الشيت)
