...
def _paymode_label(v: str) -> str:
    try:
        x = str(v or "").strip().lower()
    except Exception:
        x = ""
    if x == "ecom":
        return "متجر إلكتروني (رابط دفع متغير)"
    return "تحويل بنكي / STC Pay"

def _paymode_note_html(v: str) -> str:
    try:
        x = str(v or "").strip().lower()
    except Exception:
        x = ""
    if x == "ecom":
        return "📝 سيتم طلب رابط الدفع لكل طلب عند تحديث الحالة: <b>الطلب جاهز للشحن</b>."
    return ""
...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    try:
        ...
        if not ost:
            ost = str(order.get("status") or "").strip().lower()
    ...
    if ost in ("preparing", "prep"):
        return InlineKeyboardMarkup([
            # ✅ كان ready — لازم يكون ready_to_ship عشان يتطابق مع المعالج
            [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready_to_ship|{order_id}")],
        ])
    ...
    if ost in ("ready_to_ship", "ready"):
        ...
        if not inv_file:
            return InlineKeyboardMarkup([
                # ✅ كان ready — لازم يكون ready_to_ship عشان يكمل التدفق (فاتورة/رابط)
                [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready_to_ship|{order_id}")],
            ])
...
async def start_trader_deeplink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ فحص كامل لملف التاجر (نفس حقول شرط تقديم العرض)
        # ✅ فحص كامل لملف التاجر (نفس حقول شرط تقديم العرض) — حسب نوع الدفع
    pay_mode = (tp.get("payment_profile") or "").strip().lower()
    if pay_mode not in ("ecom", "manual"):
        pay_mode = "manual"
            ...
            for aid in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        ...
                        text=admin_ping,
                        parse_mode="HTML",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔗 فتح رابط الدفع", url=pay_link)]]),
                        disable_web_page_preview=True,
                    )
                except Exception as e:
                    _swallow(e)
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ اختيار طريقة تجهيز الدفع عند جاهز للشحن (اختياري للتاجر)
    ready_mode = ""
    if st_norm in ("ready_mode_inv", "ready_mode_link"):
        ready_mode = st_norm
        st_norm = "ready"
...
# ===== الحالة الحالية =====
    if not ost_now:
# ===== الحالة الحالية =====
        ost_now = str(order.get("status") or "").strip().lower()
    ...
    # 2) جاهز للشحن: يطلب (فاتورة المتجر) أو (رابط دفع إلكتروني) — اختيار للتاجر
    if new_status == "ready_to_ship":
        if ost_now not in ("preparing", "prep", "ready_to_ship", "ready"):
            return

        inv_file = (str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "")).strip()

        # ✅ بديل الفاتورة: رابط دفع خاص بالمتجر (بعض المتاجر لا تصدر فاتورة قبل السداد)
        try:
            gpm = str(order.get("goods_payment_method") or "").strip().lower()
        except Exception:
            gpm = ""
        pay_link_saved = (str(order.get("goods_payment_link") or "")).strip()
        has_trader_link = (gpm == "trader_link" and bool(pay_link_saved))

        # إذا الفاتورة موجودة مسبقاً: لا نعيد نفس التدفق هنا
        if inv_file:
            return

        # ✅ إذا رابط الدفع موجود مسبقاً لكن الحالة ما زالت "preparing" (حصل تعارض/توقف في تدفق الرابط)
        # هنا نخلي زر "جاهز للشحن" يكمل التدفق ويرسل الرابط للعميل مباشرة.
        if has_trader_link:
            try:
                # تحديث حالة الطلب (نثبتها كـ ready_to_ship)
                update_order_fields(order_id, {
                    "goods_payment_method": "trader_link",
                    "goods_payment_link": pay_link_saved,
            ...
            # إشعار للعميل + نقل لمرحلة انتظار الإيصال
            try:
                client_id2 = int(order.get("user_id") or 0)
            except Exception:
                client_id2 = 0
            ...
            if client_id2:
                try:
                    ...
                    udc["goods_order_id"] = order_id
                    set_stage(context, client_id2, STAGE_AWAIT_GOODS_RECEIPT)
                except Exception as e:
                    _swallow(e)

                try:
                    ship_included2 = _is_yes(order.get("ship_included") or order.get("shipping_included") or "")
                    ship_fee_s2 = str(order.get("shipping_fee_sar") or order.get("shipping_fee") or "").strip()
                    ship_fee_n2 = _num_float(ship_fee_s2)

                    goods_s2 = str(order.get("goods_amount_sar") or order.get("quote_goods_amount") or "").strip()
                    goods_n2 = _num_float(goods_s2)
                    total_n2 = goods_n2 + (0.0 if ship_included2 else ship_fee_n2)

                    def _fmt_num2(n: float) -> str:
                        try:
                            if abs(n - int(n)) < 1e-9:
                                return f"{int(n)} ر.س"
                    ...
                    goods_txt2 = _fmt_money(goods_s2) or (_fmt_num2(goods_n2) if goods_n2 > 0 else "—")
                    ship_txt2 = "مشمول" if ship_included2 else (_fmt_money(ship_fee_s2) or (_fmt_num2(ship_fee_n2) if ship_fee_n2 > 0 else "—"))
                    total_txt2 = _fmt_num2(total_n2) if (goods_n2 > 0 or ship_fee_n2 > 0 or ship_included2) else "—"

                    client_name2 = (str(order.get("user_name") or "").strip() or "—")
                    store_name2 = (str(trader_store or "").strip() or "—")
                    tname2 = (str(accepted_name or "").strip() or "—")

                    client_lines = [
                        "🔗 <b>رابط دفع إلكتروني من المتجر</b>",
                        "",
                        f"🧾 رقم الطلب: <b>{html.escape(order_id)}</b>",
                        f"👤 العميل: <b>{html.escape(client_name2)}</b>",
                        f"🧑‍💼 التاجر: <b>{html.escape(tname2)}</b>",
                        f"🏪 المتجر: <b>{html.escape(store_name2)}</b>",
                        "",
                        "📌 <b>تفاصيل المبالغ</b>:",
                        f"🧩 قيمة القطع: <b>{html.escape(str(goods_txt2))}</b>",
                        f"🚚 الشحن: <b>{html.escape(str(ship_txt2))}</b>",
                        f"💰 الإجمالي (قطع + شحن): <b>{html.escape(str(total_txt2))}</b>",
                        "",
                        "🔗 <b>رابط دفع إلكتروني</b>: اضغط الزر بالأسفل للسداد",
                        "",
                        "بعد الدفع ارسل <b>إيصال السداد</b> هنا داخل المنصة لإكمال الشحن.",
                    ]
                    ...
                    await context.bot.send_message(
                        ...
                        text="\n".join(client_lines),
                        parse_mode="HTML",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔗 اضغط للسداد", url=pay_link_saved)]]),
                        disable_web_page_preview=True,
                    )
                except Exception as e:
                    _swallow(e)
            ...
            for aid in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        chat_id=int(aid),
                        text=(
                            "🔗 <b>إعادة إرسال رابط دفع (بدون فاتورة)</b>\n"
                            f"🧾 الطلب: <b>{html.escape(order_id)}</b>\n"
                            f"🧑‍💼 التاجر: <b>{html.escape(str(accepted_name or '—'))}</b> (<code>{accepted_tid}</code>)\n"
                            "🔗 الرابط: بالزر أسفل الرسالة"
                        ),
                        parse_mode="HTML",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔗 فتح رابط الدفع", url=pay_link_saved)]]),
                        disable_web_page_preview=True,
                    )
                except Exception as e:
                    _swallow(e)

            # تحديث لوحة التاجر برسالة قصيرة
            try:
                await _admin_edit_or_send(
                    q,
                    (
                        "✅ تم اعتماد خيار <b>رابط دفع إلكتروني</b>\n"
                        f"🧾 الطلب: <b>{html.escape(order_id)}</b>\n"
                        "تم إرسال الرابط للعميل، وننتظر إيصال السداد."
                    ),
                    kb=trader_received_notice_kb(order_id),
                )
            except Exception as e:
                _swallow(e)
        ...
        # 2.1) لو ضغط "جاهز للشحن" بدون اختيار: اعرض خيارين
        if not ready_mode:
            try:
                msg_private = (
                    "📦 <b>جاهز للشحن</b>\n\n"
                    ...
                    + "\n"
                    + f"{goods_line}\n{ship_line}\n{total_line}\n\n"
                    + "⬇️ <b>اختر طريقة المتجر لطلب سداد قيمة القطع</b>:\n"
                    + "1) <b>رفع فاتورة المتجر</b> (يظهر للعميل كيبورد (ايبان + stc pay) لاختيار طريقة الدفع)\n"
                    + "2) <b>رابط دفع إلكتروني فقط</b> (بدون فاتورة وبدون كيبورد الدفع — سيتم إرسال الرابط للعميل مباشرة)"
                )
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🧾 رفع فاتورة المتجر", callback_data=f"pp_trader_status|ready_mode_inv|{order_id}")],
                    [InlineKeyboardButton("🔗 رابط دفع إلكتروني فقط", callback_data=f"pp_trader_status|ready_mode_link|{order_id}")],
                ])
                await context.bot.send_message(
                    chat_id=actor_id,
                    text=msg_private,
        ...
        # 2.2) خيار: رفع فاتورة المتجر (النظام الحالي)
        if ready_mode == "ready_mode_inv":
        ...
        # 2.3) خيار: رابط دفع إلكتروني فقط (بدون فاتورة)
        if ready_mode == "ready_mode_link":
            try:
                ud2 = get_ud(context, actor_id)
                ud2["tsu_kind"] = "seller_pay_link"
                ud2["tsu_order_id"] = order_id
                set_stage(context, actor_id, STAGE_TRADER_STATUS_UPDATE)
            except Exception as e:
                _swallow(e)
            ...
            try:
                msg_private = (
                    "🔗 <b>رابط دفع إلكتروني (بدون فاتورة)</b>\n\n"
                    f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>\n"
                    f"👤 العميل: <b>{client_name}</b>\n"
                    f"🧑‍💼 التاجر: <b>{accepted_name}</b>\n"
                    f"🏪 المتجر: <b>{trader_store}</b>\n"
                    ...
                    + f"{goods_line}\n{ship_line}\n{total_line}\n\n"
                    + "➡️ <b>الخطوة القادمة</b>:\n"
                    + "ارسل رابط الدفع الإلكتروني هنا (مثال: https://...)\n"
                    + "بعد إرسال الرابط سيتم إرساله للعميل مباشرة بدون كيبورد (ايبان + stc pay)، ثم ننتظر إيصال السداد."
                )
                await context.bot.send_message(
                    chat_id=actor_id,
                    text=msg_private,
    ...
    if new_status == "shipped":
        ...
        # ✅ بديل الفاتورة: رابط دفع التاجر (بدون فاتورة)
        try:
            gpm2 = str(order.get("goods_payment_method") or "").strip().lower()
        except Exception:
            gpm2 = ""
        has_trader_link2 = (gpm2 == "trader_link" and bool(str(order.get("goods_payment_link") or "").strip()))

        if not inv_file and not has_trader_link2:
            return
    ...
    # ===== تنفيذ التحديث =====
    fields: dict = {"order_status": new_status, "status": new_status}
    ...
    if PP_PAY_LINK_URL:
        try:
            await q.message.reply_text(
                ...
                f"💰 إجمالي القطع + الشحن: <b>{html.escape(total_txt)}</b>\n\n"
                "🔗 <b>رابط دفع إلكتروني</b>: اضغط الزر بالأسفل للسداد\n\n"
                "بعد الدفع ارسل صورة/ملف (PDF) لإيصال الدفع هنا داخل المنصة (الإيصال إلزامي).",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔗 اضغط للسداد", url=PP_PAY_LINK_URL)]]),
                disable_web_page_preview=True,
            )
        except Exception as e:
            _swallow(e)
...
async def goods_receipt_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # ✅ تأكيد واضح للعميل مباشرة بعد استلام الإيصال (مهم جداً خصوصاً عند رابط الدفع)
        try:
            b0 = get_order_bundle(order_id) or {}
            o0 = (b0.get("order") or {}) if isinstance(b0, dict) else {}
        except Exception:
            o0 = {}

        try:
            client_nm0 = (str(o0.get("user_name") or "").strip() or str(update.effective_user.full_name or "").strip() or "—")
        except Exception:
            client_nm0 = "—"

        try:
            t_id0 = int(o0.get("accepted_trader_id") or o0.get("assigned_trader_id") or 0)
        except Exception:
            t_id0 = 0

        try:
            t_label0 = _trader_label(t_id0, "—")
        except Exception:
            t_label0 = "—"

        goods_txt0 = _money(o0.get("goods_amount_sar") or "") or "—"
        ship_txt0 = "—"
        total_txt0 = "—"
        try:
            ship_incl0 = str(o0.get("ship_included") or o0.get("ship_included") or "").strip().lower() in ("yes","true","1","included","مشمول","مشمولة")
            ship_fee0 = o0.get("shipping_fee_sar")
            if ship_incl0:
                ship_txt0 = "مشمول"
                total_txt0 = _money(o0.get("goods_amount_sar") or "") or "—"
            else:
                ship_txt0 = _money(ship_fee0 or "") or "—"
                # total = goods + ship
                def _to_f0(x):
                    try:
                        ...
                        return 0.0
                total_txt0 = _money(_to_f0(o0.get("goods_amount_sar") or 0) + _to_f0(ship_fee0 or 0)) or "—"
        except Exception:
            pass
        ...
        await context.bot.send_message(
            ...
            text=(
                f"{_user_name(update)}\n"
                "✅ <b>تم استلام إيصال السداد</b>\n\n"
                f"🧾 رقم الطلب: <b>{html.escape(order_id)}</b>\n"
                f"👤 العميل: <b>{html.escape(str(client_nm0))}</b>\n"
                f"🧑‍💼 التاجر: <b>{html.escape(str(t_label0))}</b>\n\n"
                "📌 <b>المبالغ</b>:\n"
                f"🧩 قيمة القطع: <b>{html.escape(str(goods_txt0))}</b>\n"
                f"🚚 الشحن: <b>{html.escape(str(ship_txt0))}</b>\n"
                f"💰 الإجمالي: <b>{html.escape(str(total_txt0))}</b>\n\n"
                "⏳ جاري إشعار التاجر للتأكيد ثم متابعة الشحن."
            ),
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_goods_confirm":
        ...
        # ✅ بديل الفاتورة: رابط دفع التاجر (بدون فاتورة)
        try:
            gpm_now = str(order.get("goods_payment_method") or "").strip().lower()
        except Exception:
            gpm_now = ""
        has_trader_link_now = (gpm_now == "trader_link" and bool(str(order.get("goods_payment_link") or "").strip()))

        # لو الطلب وصل مراحل متقدمة لا نرجعه للخلف
        if ost_now in ("shipped", "delivered", "closed"):
            next_ost = ost_now
        else:
            if inv_file_now or has_trader_link_now:
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg:
        # ✅ FIX (حاسم): لا تمرر الوسائط في مراحل التدفق التي لها معالجات خاصة أسفل الدالة
        # لأن هذا البلوك كان يلتقط الميديا ويعمل return قبل وصولها لمعالجات:
        # - فاتورة التاجر (seller_invoice)
        # - إيصالات الدفع (المنصة/القطع/الاشتراك)
        # - صورة القطعة الاختيارية
        skip_forward = False
        try:
            if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
                skip_forward = True
            elif stage in (STAGE_AWAIT_GOODS_RECEIPT, STAGE_TRADER_SUB_AWAIT_RECEIPT, STAGE_AWAIT_RECEIPT, STAGE_ASK_ITEM_PHOTO):
                skip_forward = True
        except Exception:
            skip_forward = False
            ...
            if kind and file_id:
                ...
                if isinstance(sess, dict):
                    ...
                    if peer_id and order_id_sess:
                        ...
                        # ✅ FIX: إرفاق نفس كيبورد (انهاء/مراسلة) حتى لو الرد وسائط فقط
                        kb_sess = None
                        try:
                            if role == "client":
                                kb_sess = trader_reply_kb(order_id_sess, user_id)
                            elif role == "trader":
                                kb_sess = client_trader_chat_kb(order_id_sess)
                        except Exception:
                            kb_sess = None

                        await _send_media(peer_id, caption=caption, kb=kb_sess)
                        return
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
        ...
        try:
            ...
            client_name2 = (str(o2.get("user_name") or o2.get("client_name") or o2.get("customer_name") or "")).strip() or "العميل"
            car_name2 = (str(o2.get("car_name") or o2.get("vehicle_name") or o2.get("car") or o2.get("car_model") or "")).strip() or "—"
            availability2 = (str(o2.get("availability_days") or o2.get("quote_availability") or o2.get("availability") or "")).strip()
            ship_eta2 = (str(o2.get("ship_eta") or o2.get("shipping_eta") or o2.get("ship_days") or "")).strip()
            ...
            if client_id2:
                ...
                # ✅ رسالة العميل: موحدة + كل التفاصيل
                client_lines = [
                ...
                elif str(o2.get('ship_included') or '').strip().lower() in ('yes','true','1','included','مشمولة','مشمول'):
                    client_lines.append("🚚 مبلغ الشحن: <b>مشمول</b>")
                if am.get("total_txt"):
                    client_lines.append(f"🧾 المبلغ الإجمالي: <b>{html.escape(am['total_txt'])}</b>")
                client_lines.append("")
                client_lines.append("🧩 <b>تفاصيل القطع</b>:")
                try:
                    _items2 = b2.get('items', []) or []
                    if _items2:
                        for _i, _it in enumerate(_items2, start=1):
                            _nm = str((_it or {}).get('name') or (_it or {}).get('item_name') or '').strip()
                            _pn = str((_it or {}).get('part_no') or (_it or {}).get('item_part_no') or '').strip()
                            if not _nm:
                                continue
                            _line = f"• <b>{html.escape(_nm)}</b>" + (f" <i>({html.escape(_pn)})</i>" if _pn else "")
                            client_lines.append(_line)
                            if _i >= 12:
                                client_lines.append("• …")
                                break
                    else:
        ...
        # ✅ رسالة للتاجر (موحدة)
        try:
            car3 = (str(o3.get("car_name") or o3.get("vehicle_name") or o3.get("car") or o3.get("car_model") or "")).strip() or "—"
            av3 = (str(o3.get("availability_days") or o3.get("quote_availability") or o3.get("availability") or "")).strip()
            se3 = (str(o3.get("ship_eta") or o3.get("shipping_eta") or o3.get("ship_days") or "")).strip()
        except Exception:
            car3, av3, se3 = "—", "", ""

        lines_t = [
            f"{name}",
            "✅ <b>تم تسجيل الفاتورة وتحديث الحالة</b>",
            "",
            f"🧾 رقم الطلب: <b>{order_id2}</b>",
            f"🧑‍💼 التاجر: <b>{html.escape(str(_user_name(update)))}</b>",
            f"🚗 السيارة: <b>{html.escape(car3)}</b>",
            f"📌 الحالة: <b>{_pay_status_ar('ready_to_ship')}</b>",
        ]
        if av3:
            lines_t.append(f"🛠 مدة التجهيز: <b>{html.escape(av3)}</b>")
        if se3:
            lines_t.append(f"⏱ مدة الشحن: <b>{html.escape(se3)}</b>")
        lines_t.append("")
        if amt_block2.strip():
            lines_t.append(amt_block2.strip())
        lines_t.append("")
        lines_t.append("⏳ <b>متابعة</b>: بانتظار إكمال دفع قيمة القطع من العميل")
...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, viewer_id: int, order_id: str):
    ...
    for idx, it in enumerate(items or [], start=1):
        ...
        if _is_unavailable(it):
            unavailable_cnt += 1
            parts_lines.append(f"  {idx}) ❌ <b>{html.escape(nm)}</b>" + (f" — <code>{html.escape(partno)}</code>" if partno else "") + " — <i>غير متوفر</i>")
        elif price:
            priced_cnt += 1
            parts_lines.append(f"  {idx}) ✅ <b>{html.escape(nm)}</b>" + (f" — <code>{html.escape(partno)}</code>" if partno else "") + f" — <b>{html.escape(str(price))}</b>")
        else:
            unpriced_cnt += 1
            parts_lines.append(f"  {idx}) 🟡 <b>{html.escape(nm)}</b>" + (f" — <code>{html.escape(partno)}</code>" if partno else "") + " — <i>غير مسعر</i>")
    ...
    if role == "admin":
        ...
        rows.append([InlineKeyboardButton("🛎️ مراسلة المنصة", callback_data="pp_support_open")])
        rows.append([InlineKeyboardButton("📜 سجل الطلب / آخر تحديث", callback_data=f"pp_order_legal|{order_id}")])
        rows.append([InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")])
...
async def _resolve_and_show_order(context: ContextTypes.DEFAULT_TYPE, user_id: int, tail4: str):
    ...
    # نجمع مطابقات النهاية مع معلومات مختصرة لشرح سبب التعدد (قد تتكرر النهاية عبر أيام مختلفة)
    matches_info = []
    for o in uniq:
        oid = str(o.get("order_id") or "").strip()
        if not oid:
            continue
        if not oid.endswith(tail4):
            continue
    ...
    rows = []
    for it in matches_info[:10]:
        oid = it.get("order_id")
        # نوضح سبب التعدد: نفس النهاية + تواريخ/حالة مختلفة
        label_bits = [f"{oid}"]
        if it.get("created"):
            label_bits.append(it.get("created"))
        if it.get("status") and it.get("status") != "—":
            label_bits.append(it.get("status"))
        # اسم العميل يظهر فقط للأدمن (سرية)
        if uid in (ADMIN_IDS or []):
            label_bits.append(it.get("client") or "—")

        btn_label = " • ".join([b for b in label_bits if b])
        rows.append([
            InlineKeyboardButton(f"🧾 {btn_label}", callback_data=f"pp_open_order|{oid}"),
        ])
        rows.append([
            InlineKeyboardButton("📜 سجل الطلب / آخر تحديث", callback_data=f"pp_order_legal|{oid}"),
        ])
    ...
    await context.bot.send_message(
        chat_id=uid,
        text=(
            "🔎 وُجد أكثر من طلب بنفس النهاية (" + tail4 + ")\n"
            "هذا طبيعي إذا كان العدّاد يتكرر عبر أيام مختلفة أو أكثر من طلب.\n\n"
            "اختر الطلب المطلوب من الأزرار بالأسفل:"
        ),
        reply_markup=kb,
    )
...
# ==================================================
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE:
        ...
        if kind == "seller_pay_link":
            pay_link = (text or "").strip()

            # ✅ تحقق بسيط من الرابط
            if not pay_link or not (pay_link.startswith("http://") or pay_link.startswith("https://")):
                await _reply_html("رابط غير صحيح", ["⚠️ ارسل رابط الدفع بصيغة صحيحة يبدأ بـ <code>https://</code>"])
                return

            # ✅ حفظ رابط الدفع + تجهيز الدفع للعميل (بدون كيبورد ايبان/stc pay)
            update_order_fields(order_id2, {
                "goods_payment_method": "trader_link",
                "goods_payment_link": pay_link,
                "goods_payment_status": "awaiting_receipt",
                "order_status": "ready_to_ship",
            })

                        # ✅ إشعار للعميل: تفاصيل كاملة + زر رابط (بدون معاينة + بدون إظهار الرابط كنص)
            if client_id2:
                try:
                    udc = get_ud(context, client_id2)
                    ...
                    try:
                        store_name2 = (str(tprof.get("company_name") or "").strip() or "—")
                    except Exception:
                        store_name2 = "—"

                    ship_included2 = _is_yes(o2.get("ship_included") or o2.get("shipping_included") or "")
                    ship_fee_s2 = str(o2.get("shipping_fee_sar") or o2.get("shipping_fee") or "").strip()
                    ship_fee_n2 = _num_float(ship_fee_s2)

                    goods_n2 = _num_float(goods_amt)
                    total_n2 = goods_n2 + (0.0 if ship_included2 else ship_fee_n2)
                    ...
                    goods_txt2 = _fmt_money(goods_amt) or (_fmt_num2(goods_n2) if goods_n2 > 0 else "—")
                    ship_txt2 = "مشمول" if ship_included2 else (_fmt_money(ship_fee_s2) or (_fmt_num2(ship_fee_n2) if ship_fee_n2 > 0 else "—"))
                    total_txt2 = _fmt_num2(total_n2) if (goods_n2 > 0 or ship_fee_n2 > 0 or ship_included2) else "—"
                    ...
                    await context.bot.send_message(
                        ...
                        text="\n".join(client_lines),
                        parse_mode="HTML",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔗 اضغط للسداد", url=pay_link)]]),
                        disable_web_page_preview=True,
                    )
                except Exception as e:
                    _swallow(e)
            ...
            for aid in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        chat_id=aid,
                        text=(
                            "🔗 <b>تاجر أرسل رابط دفع (بدون فاتورة)</b>\n"
                            f"🧾 الطلب: <b>{html.escape(order_id2)}</b>\n"
            ...
            await _reply_html(
                "تم حفظ الرابط",
                [
                    f"✅ تم حفظ رابط الدفع للطلب: <b>{html.escape(order_id2)}</b>",
                    "تم إرسال الرابط للعميل، وننتظر إيصال السداد.",
                ],
            )
            return
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "paymode":
        # pp_tprof|paymode|open  OR pp_tprof|paymode|set_manual / set_ecom
        try:
            tp = get_trader_profile(int(uid or 0)) or {}
        except Exception:
            tp = {}

        cur_mode = (tp.get("payment_profile") or "").strip().lower()
        if cur_mode not in ("manual", "ecom"):
            cur_mode = "manual"

        if sub == "open":
            txt = (
                f"{_user_name(q)}\n"
                "💳 <b>اختيار طريقة الدفع للمتجر</b>\n\n"
                "اختر طريقة الدفع الافتراضية لعملائك:\n\n"
                "1️⃣ 🏦 <b>تحويل بنكي / STC Pay</b>\n"
                "2️⃣ 🔗 <b>متجر إلكتروني (رابط دفع متغير)</b>\n\n"
                "📝 ملاحظة: في خيار <b>متجر إلكتروني</b> سيتم طلب رابط الدفع لكل طلب عند تحديث الحالة <b>جاهز للشحن</b>."
            )
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("1️⃣ 🏦 تحويل بنكي / STC Pay", callback_data="pp_tprof|paymode|set_manual")],
                [InlineKeyboardButton("2️⃣ 🔗 متجر إلكتروني (رابط متغير)", callback_data="pp_tprof|paymode|set_ecom")],
                [InlineKeyboardButton("↩️ رجوع للوحة التاجر", callback_data="pp_tprof|home")],
            ])
            try:
                await q.message.reply_text(
                    txt,
                ...
                try:
                    await _admin_edit_or_send(q, txt, kb)
                except Exception as e:
                    _swallow(e)
            return

        if sub in ("set_manual", "set_ecom"):
            new_mode = "manual" if sub == "set_manual" else "ecom"
            try:
                upsert_trader_profile(int(uid or 0), {"payment_profile": new_mode})
            except Exception as e:
                _swallow(e)
                await _pop("❌ تعذر حفظ طريقة الدفع")
                return

            # UX: لو اختار متجر إلكتروني نسمح يظل محتفظ ببيانات البنك (لا نحذفها)
            await _toast("تم حفظ طريقة الدفع ✅")
            try:
                await show_trader_panel(q.message, context, uid)
            except Exception as e:
                _swallow(e)
...
def trader_panel_kb(tid: int, payment_profile: str = "manual") -> InlineKeyboardMarkup:
    mode = (payment_profile or "").strip().lower()
    if mode not in ("manual", "ecom"):
        mode = "manual"
    ...
    rows = [
        ...
        # ✅ اختيار نوع طريقة الدفع للتاجر
        [InlineKeyboardButton("💳 طريقة الدفع (متجر إلكتروني / تحويل)", callback_data="pp_tprof|paymode|open")],
    ]

    # ✅ تحسين UX: إذا كان متجر إلكتروني => لا نعرض أزرار البنك/IBAN/STC داخل اللوحة
    if mode == "manual":
        rows += [
...
async def show_trader_panel(update_or_q, context: ContextTypes.DEFAULT_TYPE, trader_id: int):
    ...
    pm = (tp.get("payment_profile") or "manual")
    pm_line = _paymode_label(pm)
    pm_note = _paymode_note_html(pm)
    ...
    txt = (
        ...
        f"💳 طريقة الدفع الحالية: <b>{html.escape(pm_line)}</b>\n"
        f"{pm_note}\n"
    ...
    try:
        kb = trader_panel_kb(int(trader_id or 0), (tp.get("payment_profile") or "manual"))
    except Exception:
        kb = None
