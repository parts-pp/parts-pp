...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _items_kb(order_id: str, items: list, price_map: dict):
        ...
        # أزرار التحكم (كل واحد سطر لحاله)
        rows.append([InlineKeyboardButton("🧾 تسعير الكل", callback_data=f"ppq_it_all|{order_id}")])
        rows.append([InlineKeyboardButton("✅ اكمل خطوات العرض", callback_data=f"ppq_it_done|{order_id}")])
        return InlineKeyboardMarkup(rows)
    ...
    if action == "ppq_begin":
        ...
        # ✅ (تعديل المطلوب): بدل متوفر/غير متوفر -> تسعير القطع
        td["quote_order_id"] = order_id
        ...
        # تهيئة تسعير القطع
        td["quote_item_prices"] = td.get("quote_item_prices") if isinstance(td.get("quote_item_prices"), dict) else {}
    ...
    # ✅ (تعديل المطلوب): أكشنات تسعير القطع (نستخدم نفس أسماء الأكشنات بدون نفخ)
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            td["quote_step"] = "it_price"  # ✅ سيتم استقباله لاحقاً في text_handler
            try:
            ...
            td["quote_step"] = "it_all_price"  # ✅ سيتم استقباله لاحقاً في text_handler
            try:
                await q.message.edit_reply_markup(reply_markup=None)
            except Exception:
                pass
            ...
            # نتركه مدعوم (لو كان في أي مكان قديم) لكن بدون إظهار زر له
            td["quote_item_prices"] = {}
            pm = {}
            td["quote_step"] = "it_pick"

        elif action == "ppq_it_done":
            # لازم يكون على الأقل قطعة واحدة مسعّرة
            ...
            td["quote_goods_amount"] = total  # ✅ إجمالي القطع
            td["quote_step"] = "type"
