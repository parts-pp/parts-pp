...
# ===== Order ID clickable link (HTML) =====
# الهدف: جعل رقم الطلب يظهر كرابط أزرق قابل للنقر داخل رسائل HTML
# ملاحظة: في الرسائل بدون parse_mode HTML لن يظهر الرابط كنص قابل للنقر (إلا إذا كان URL صريح).
import urllib.parse
...
def _order_id_link_html(order_id: str, context: ContextTypes.DEFAULT_TYPE = None) -> str:
    ...
    if not oid:
        return "—"
    bot = _bot_username(context)
    # payload قصير وآمن للـ start
    payload = f"pp_order_{oid}"
    url = f"https://t.me/{bot}?start={urllib.parse.quote(payload)}"
    return f'<a href="{url}">{html.escape(oid)}</a>'

def _order_id_link_url(order_id: str, context: ContextTypes.DEFAULT_TYPE = None) -> str:
    oid = ("" if order_id is None else str(order_id)).strip()
    if not oid:
        return ""
    bot = _bot_username(context)
    payload = f"pp_order_{oid}"
    return f"https://t.me/{bot}?start={urllib.parse.quote(payload)}"
# ===== End Order ID link =====

# ===== Global Order-ID Clickable Patch (SAFE) =====
# الهدف: جعل رقم الطلب نفسه قابل للنقر والنسخ في أي رسالة نصية تُرسل (reply_text / send_message)
# - لا يغير منطق البوت
# - يحافظ على رسائل HTML الموجودة
# - يحمي الرسائل النصية العادية عبر html.escape تلقائيًا
# ملاحظة: لا يمس الوسائط (صور/ملفات) إلا إذا كان هناك caption نصي (يتم التعامل معه عند استخدام send_message/reply_text فقط)

import telegram

# أنماط أرقام الطلب الممكنة (PP-YYMMDD-0001) أو مختصر PP1234
_ORDER_ID_PATTERNS = [
    r"PP-\d{6}-\d{4}",
    r"PP\d{4}",
]

_ORDER_ID_RE = re.compile(r"(" + "|".join(_ORDER_ID_PATTERNS) + r")", flags=re.IGNORECASE)

def _text_has_html_tags(s: str) -> bool:
    if not s:
        return False
    # مؤشر بسيط: وجود وسوم HTML الشائعة المستخدمة في البوت
    return bool(re.search(r"</?(b|i|u|s|code|pre|a)\b", s, flags=re.I))

def _wrap_order_ids_with_links(s: str) -> str:
    '''
    يستبدل أي رقم طلب في النص بـ <a href="...">ORDER_ID</a>
    مع محاولة تجنب اللف المكرر إذا كان داخل <a> بالفعل.
    '''
    if not s:
        return s

    if not _ORDER_ID_RE.search(s):
        return s

    def _is_inside_anchor(txt: str, pos: int) -> bool:
        before = txt[:pos]
    ...
    out = []
    last = 0
    for m in _ORDER_ID_RE.finditer(s):
        a, b = m.start(), m.end()
        out.append(s[last:a])
        oid = m.group(1)
        if _is_inside_anchor(s, a):
            out.append(oid)
        else:
            try:
                out.append(_order_id_link_html(oid))
            except Exception:
                out.append(oid)
        last = b
    out.append(s[last:])
...
def _make_text_clickable(text: str, parse_mode: str | None) -> tuple[str, str]:
    '''
    يعيد (text_new, parse_mode_new)
    - إذا النص أصلاً HTML أو parse_mode=HTML => لا نهرب HTML، فقط نضيف روابط للـ order_id.
    - إذا النص عادي => نعمل escape ثم نضيف روابط.
    '''
    pm = (parse_mode or "").strip().upper()
    is_html = (pm == "HTML") or _text_has_html_tags(text)

    if is_html:
        t = _wrap_order_ids_with_links(text)
        return t, "HTML"

    safe = html.escape(text or "", quote=False)
    safe = _wrap_order_ids_with_links(safe)
    return safe, "HTML"
...
async def _patched_send_message(self, *args, **kwargs):
    ...
    if isinstance(text_arg, str):
        pm_in = kwargs.get("parse_mode", None)
        new_text, new_pm = _make_text_clickable(text_arg, pm_in)
...
async def _patched_reply_text(self, *args, **kwargs):
    text_arg = kwargs.get("text") if "text" in kwargs else (args[0] if args else None)
...
if _ORIG_SEND_MESSAGE and not hasattr(telegram.Bot, "_pp_clickable_oid_patched"):
    telegram.Bot.send_message = _patched_send_message
    telegram.Bot._pp_clickable_oid_patched = True

if _ORIG_REPLY_TEXT and not hasattr(telegram.Message, "_pp_clickable_oid_patched"):
    telegram.Message.reply_text = _patched_reply_text
    telegram.Message._pp_clickable_oid_patched = True

# ===== End Global Order-ID Clickable Patch =====
...
def _order_tag_html(order_id: str) -> str:
    cn, tn = _order_parties(order_id)
    return (
        f"🧾 رقم الطلب: <code>{html.escape(str(order_id))}</code> | "
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _hdr(
        ...
        try:
            url = _order_id_link_url(order_id, context)
            if url:
                lines.append(f"🔗 {url}")
        except Exception:
            pass
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        def _quote_hdr(oid: str, goods_total: str = "") -> list:
            ...
            lines = [
                ...
                f"🔗 {_order_id_link_url(oid, context)}",
