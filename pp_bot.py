...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # ✅ تشخيص واضح بدل الصمت
    if not client_id:
        try:
            await message.reply_text(
                f"{_user_name(message)}\n"
                f"⚠️ لا يوجد user_id للعميل داخل الطلب {order_id} (الطلب غير مربوط بعميل بشكل صحيح)."
            )
        except Exception:
            pass
    else:
        try:
            ...
            # ✅ دليل قاطع في اللوق: لمن تم الإرسال؟
            try:
                log.info("QUOTE_SENT order_id=%s client_id=%s trader_id=%s", order_id, client_id, trader_id)
            except Exception:
                pass
            ...
            try:
                await message.reply_text(
                    f"{_user_name(message)}\n"
                    "⚠️ لم استطع إرسال العرض للعميل: العميل لم يفتح البوت في الخاص أو قام بحظر البوت.\n"
                    "اطلب منه يفتح البوت /start ثم أعد إرسال العرض."
                )
            except Exception:
                pass
        except Exception as e:
            try:
                await message.reply_text(
                    f"{_user_name(message)}\n⚠️ فشل إرسال العرض للعميل بسبب خطأ: {e}"
                )
            except Exception:
                pass
    ...
    # ✅ لا تقل "تم ارسال" إلا إذا تم فعلاً
    try:
        if client_id:
            await message.reply_text(f"{_user_name(message)}\n✅ تم إرسال عرض السعر للعميل\n\n{trader_copy}")
        else:
            await message.reply_text(f"{_user_name(message)}\n⚠️ لم يتم إرسال العرض للعميل (لا يوجد user_id)\n\n{trader_copy}")
    except Exception:
        pass
...
def build_app():
    ...
    # 🟢 [TASK] Backup Loop (بدون JobQueue) - تشغيل بعد بدء التطبيق (Polling)
    async def _post_init(application):
        try:
            # منع التشغيل المكرر لو صار restart / init
            if application.bot_data.get("_backup_loop_started"):
                return
            application.bot_data["_backup_loop_started"] = True

            # ✅ بدل Application.create_task (يسبب التحذير) نستخدم asyncio.create_task
            asyncio.create_task(_backup_loop(application))
        except Exception as e:
            try:
                log.error(f"Backup loop start error: {e}")
            except Exception:
                pass

    try:
        # تشغيل مهام الخلفية بعد بدء التطبيق لتجنب RuntimeWarning/Warnings
