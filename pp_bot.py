...
async def open_order_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    # ✅ Toast خفيف بدل الرد الصامت لأن فتح اللوحة قد يتأخر قليلاً مع قراءة الإكسل
    try:
    ...
    if stage == STAGE_JOIN_VAT:
        ...
        try:
            upsert_trader_profile(user_id, {
                ...
                "license_kind": str(jd.get("license_kind") or "document").strip().lower(),
                "updated_at_utc": utc_now_iso(),
            })
        except Exception as e:
            _swallow(e)
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "tview":
        ...
        kb_rows = [
            ...
            [
                InlineKeyboardButton("🧾 السجل التجاري", callback_data=f"pp_admin|tcrdoc|{tid}"),
                InlineKeyboardButton("🏪 رخصة المحل", callback_data=f"pp_admin|tlicensedoc|{tid}"),
            ],
    ...
    # ===== TRADER PROFILE DOCS (tcrdoc / tlicensedoc) =====
    if action in ("tcrdoc", "tlicensedoc"):
        tid = 0
        if len(parts) >= 3:
            try:
                tid = int(parts[2] or 0)
        ...
        if action == "tcrdoc":
            ...
            kind = str(prof.get("cr_kind") or "document").strip().lower()
            title = "السجل التجاري"
        else:
            file_id = str(prof.get("license_file_id") or "").strip()
            kind = str(prof.get("license_kind") or "document").strip().lower()
            title = "رخصة المحل"
