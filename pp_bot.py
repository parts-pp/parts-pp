...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================================================
    # ✅ خيارات جاهز للشحن (فاتورة متجر / رابط دفع إلكتروني)
    # هذه الأزرار لا تغيّر الحالة مباشرة — فقط تفتح مرحلة الإدخال.
    # =========================================================
    if st_norm in ("inv", "plink"):
        # اجلب الطلب
        try:
            b = get_order_bundle(order_id)
            order = b.get("order", {}) or {}
        ...
        try:
            ...
            ud2["tsu_kind"] = "seller_invoice" if st_norm == "inv" else "seller_pay_link"
        ...
        if st_norm == "inv":
            await context.bot.send_message(
                ...
                text=(
                    ...
                    "أرسل <b>فاتورة المتجر الرسمية</b> (PDF أو صورة) هنا في الخاص."
                ),
                parse_mode="HTML",
                disable_web_page_preview=True,
            )
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>\n\n"
                    "أرسل <b>رابط الدفع</b> هنا (http/https).\n"
                    "سيتم استبدال أي رابط سابق بهذا الرابط تلقائياً."
                ),
                parse_mode="HTML",
                disable_web_page_preview=True,
            )
    ...
    if new_status == "ready_to_ship":
        ...
        inv_file = (str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "")).strip()
        if not inv_file:
            # ✅ اختيار طريقة الإرسال (فاتورة متجر أو رابط دفع)
            try:
                has_prev_link = bool((str(order.get("seller_invoice_pay_link") or order.get("shop_invoice_pay_link") or "")).strip())
            except Exception:
                has_prev_link = False

            try:
                kb_rows = [
                    [InlineKeyboardButton("📎 إرسال فاتورة متجر", callback_data=f"pp_trader_status|inv|{order_id}")],
                    [InlineKeyboardButton("🔗 إرسال رابط دفع إلكتروني", callback_data=f"pp_trader_status|plink|{order_id}")],
                ]
                kb = InlineKeyboardMarkup(kb_rows)

                msg_private = (
                    "📦 <b>تحديث الحالة إلى: جاهز للشحن</b>\n\n"
                    f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>\n"
                    ...
                    + "\n"
                    + f"{goods_line}\n{ship_line}\n{total_line}\n\n"
                    + "⬇️ <b>اختر طريقة الدفع للعميل</b>:\n"
                    + "• إذا عندك <b>فاتورة متجر</b>: أرسلها (PDF أو صورة).\n"
                    + "• إذا الدفع عندك عبر <b>رابط إلكتروني</b>: أرسل رابط الدفع.\n\n"
                    + ("ℹ️ يوجد رابط دفع محفوظ سابقاً — سيتم استبداله تلقائياً عند إرسال رابط جديد.\n\n" if has_prev_link else "")
                    + "بعد الإرسال سيُطلب من العميل إرسال إيصال السداد لإكمال الشحن."
                )
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() in ("seller_invoice", "seller_pay_link"):
        ...
        kind2 = (ud.get("tsu_kind") or "").strip()

        # ✅ قبول رابط دفع إلكتروني (بدل ملف الفاتورة)
        try:
        ...
        if msg_text:
            try:
                m_url = re.search(r"(https?://\S+)", msg_text)
                if m_url:
                    pay_url = (m_url.group(1) or "").strip().strip(")>].,")
            except Exception:
                pay_url = ""

        if pay_url and kind2 in ("seller_pay_link", "seller_invoice"):
            # ✅ حفظ رابط الدفع (واستبدال أي رابط سابق) + اعتبار "جاهز للشحن"
            try:
                update_order_fields(order_id2, {
                    "seller_invoice_pay_link": pay_url,
                    "shop_invoice_pay_link": pay_url,

                    # placeholder لتجاوز شرط وجود الفاتورة قبل الشحن
                    "seller_invoice_file_id": "PAY_LINK",
                    "shop_invoice_file_id": "PAY_LINK",
                    "seller_invoice_mime": "pay_link",
                    "shop_invoice_mime": "pay_link",
                    "seller_invoice_at": utc_now_iso(),
                    "shop_invoice_at": utc_now_iso(),

                    "order_status": "ready_to_ship",

                    # ✅ نعتبر طريقة الدفع "رابط" وننتظر الإيصال مباشرة (بدون كيبورد)
                    "goods_payment_method": "pay_link",
                    "goods_payment_status": "awaiting_receipt",
                })
            except Exception as e:
            ...
            # ✅ إرسال الرابط للعميل + نقل مرحلة الإيصال مباشرة
            try:
                ...
                if client_id2:
                    ...
                    if availability2:
                        lines.append(f"🛠 مدة التجهيز: <b>{html.escape(availability2)}</b>")
                    if ship_eta2:
                        lines.append(f"⏱ مدة الشحن: <b>{html.escape(ship_eta2)}</b>")
                    if am.get("goods_txt"):
                        lines.append("")
                        lines.append(f"💰 قيمة القطع: <b>{html.escape(am['goods_txt'])}</b>")
                    if am.get("ship_txt"):
                        lines.append(f"🚚 الشحن: <b>{html.escape(am['ship_txt'])}</b>")
                    if am.get("total_txt"):
                        lines.append(f"🧾 الإجمالي: <b>{html.escape(am['total_txt'])}</b>")
                    lines.append("")
                    lines.append(f"🔗 رابط الدفع: <a href=\"{html.escape(pay_url)}\">اضغط هنا</a>")
                    lines.append("")
                    lines.append("📸 بعد الدفع أرسل <b>صورة إيصال السداد</b> هنا لإكمال الشحن (الإيصال إلزامي).")
            ...
            try:
                await update.message.reply_text(f"{_user_name(update)}\n✅ تم إرسال رابط الدفع للعميل بنجاح", disable_web_page_preview=True)
            except Exception as e:
                _swallow(e)
