...
def notice_kb_for(
    ...
    """كيبورد إشعارات موحد: سجل + لوحة + (مراسلة التاجر/المنصة) + إنهاء."""
    oid = ("" if order_id is None else str(order_id)).strip()
    ...
    if include_chat_trader and oid and oid.upper() != "JOIN":
        rows.append([InlineKeyboardButton("💬 مراسلة التاجر", callback_data=f"pp_chat_trader|{oid}")])

    if include_support and oid and oid.upper() != "JOIN":
        rows.append([InlineKeyboardButton("🔎 مراسلة المنصة", callback_data=f"pp_track|{oid}")])

    try:
        nav = chat_nav_kb_for(context, int(actor_id or 0), oid, fallback_end_cb)
        end_row = (nav.inline_keyboard or [])[-1] if nav else None
        if end_row:
            rows.append(end_row)
        else:
            rows.append([InlineKeyboardButton("✖️ إنهاء", callback_data=str(fallback_end_cb or "ui_close").strip() or "ui_close")])
    except Exception:
        rows.append([InlineKeyboardButton("✖️ إنهاء", callback_data=str(fallback_end_cb or "ui_close").strip() or "ui_close")])
