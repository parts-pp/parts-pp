...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _days_txt(x: str) -> str:
        ...
        if not x:
            return "—"
        # keep digits only if it looks numeric
        d = re.sub(r"[^0-9]+", "", x)
        if d:
            return f"{int(d)} يوم"
        return x
...
asasync def _rebroadcast_noquote_orders_job(context: ContextTypes.DEFAULT_TYPE):
    """
    ...
    if msg:
        ...
        if kind and file_id:
                ...
                if peer_id and order_id_sess:
                    try:
                        ...
                        caption = (
                            ...
                            + (_order_notify_plain(order_id_sess) + "\n")
            ...
            if stage == STAGE_CHAT_TRADER:
                ...
                try:
                    head.append(_order_notify_plain(order_id))
                except Exception:
                    pass
                caption = "\n".join(head) + "\n" + (cap_raw if cap_raw else "📎 مرفق")
