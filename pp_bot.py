...
async def send_trader_ledger_pdf(
    ...
    # ✅ BADGES 6-COLS styles (separate from other tables)
    badge_lbl = ParagraphStyle("badge_lbl", parent=base, alignment=TA_RIGHT, fontSize=8.9, leading=10.6)
    # ✅ تعديل: القيمة يمين (نمط عربي)
    ...
    # =========================
    # ✅ (01) BADGES FIX: 6 columns (label/value) with paired color blocks
    # - ترتيب الأعمدة من اليسار لليمين: قيمة الوقت | كلمة الوقت | قيمة التاريخ | كلمة التاريخ | قيمة التاجر | كلمة التاجر
    # - القيم (وقت/تاريخ/يوزر) تُجبر LTR داخل خلية مستقلة لمنع الانعكاس
    # =========================
    LRM = "\u200e"
    def _val_ltr(x: str) -> str:
        s = "" if x is None else str(x)
        s = s.strip()
        return f"{LRM}{s}{LRM}" if s else "—"

    trader_name = (tname or str(tid)).strip() or "—"
    trader_value = trader_name
    if tuser:
        trader_value = f"{trader_name}  {_val_ltr(tuser)}"

    time_value = _val_ltr(now.strftime("%H:%M"))
    date_value = _val_ltr(now.strftime("%Y-%m-%d"))
    ...
    badges.setStyle(TableStyle([
        ...
        # محاذاة دقيقة (كلها يمين)
        ("ALIGN", (1, 0), (1, 0), "RIGHT"),  # كلمة الوقت
        ("ALIGN", (0, 0), (0, 0), "RIGHT"),  # ✅ قيمة الوقت يمين
        ("ALIGN", (3, 0), (3, 0), "RIGHT"),  # كلمة التاريخ
        ("ALIGN", (2, 0), (2, 0), "RIGHT"),  # ✅ قيمة التاريخ يمين
        ("ALIGN", (5, 0), (5, 0), "RIGHT"),  # كلمة التاجر
        ("ALIGN", (4, 0), (4, 0), "RIGHT"),  # ✅ قيمة التاجر يمين
    ]))
    story.append(badges)
