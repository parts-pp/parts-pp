...
# ===== Auto Restore on Boot (Last Pinned Backup) =====
# الهدف: عند كل اقلاع جديد يتم استرجاع آخر نسخة اكسل "مثبتة" في مجموعة النسخ تلقائيًا،
# ثم ضمان إضافة أي أعمدة/شيتات جديدة عبر ensure_workbook() بدون فقدان البيانات.
PP_AUTO_RESTORE_ON_BOOT = str(os.getenv("PP_AUTO_RESTORE_ON_BOOT", "1")).strip().lower() not in ("0", "false", "no", "off")

async def _auto_restore_last_pinned_on_boot(app: Application) -> bool:
    """يرجع True إذا تم الاسترجاع بنجاح. لا يقوم بإنشاء ملف فارغ إطلاقاً."""
    if not PP_AUTO_RESTORE_ON_BOOT:
        return False
    ...
    try:
        ...
        if not doc or not (doc.file_name or "").lower().endswith(".xlsx"):
            # لا يوجد ملف مثبت صالح — نكتفي بتجهيز الملف الحالي/إنشاءه لو غير موجود
            try:
                ensure_workbook()
            except Exception:
                pass
        ...
        # تنزيل آخر نسخة مثبتة فوق ملف الاكسل المحلي
        path = _excel_path()
        try:
            f = await app.bot.get_file(doc.file_id)
        except Exception:
            # بعض الإصدارات تسمح مباشرة من doc
            try:
        ...
        # بعد الاسترجاع: نضيف الأعمدة/الشيتات الجديدة تلقائيًا (ترقية غير مدمرة)
        try:
            ensure_workbook()
        except Exception:
            pass
        ...
        try:
            log_event("AUTO_RESTORE_OK", file=(doc.file_name or ""), file_id=str(doc.file_id))
        except Exception:
            pass
        return True

    except Exception as e:
        # مهم: لا ننشئ ملف فارغ على الفشل
        try:
            log.exception("AUTO_RESTORE_FAILED: %s", e)
        except Exception:
            pass
        try:
            ensure_workbook()
...
async def _post_init(app: Application) -> None:
    # يُنفذ قبل بدء polling/webhook — آمن للاسترجاع والترقية + بدء مهام النسخ الاحتياطي
    try:
        await _auto_restore_last_pinned_on_boot(app)
    except Exception as e:
        try:
            log.exception("AUTO_RESTORE_ON_BOOT error: %s", e)
        except Exception:
            pass

    # ✅ تشغيل مهام النسخ الاحتياطي (لو موجودة) بعد تجهيز قاعدة البيانات
    try:
        _start_backup_tasks(app)
    except Exception as e:
        try:
...
def build_app():
    ...
    # ✅ تجهيز/استرجاع الاكسل يتم في post_init قبل تشغيل البوت (Auto-restore + upgrade)
