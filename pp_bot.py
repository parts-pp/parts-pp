...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== بيانات الشحن + المدد + ملخص القطع =====
    ship_method = (str(order.get("delivery_type") or order.get("ship_method") or order.get("delivery_choice") or "").strip() or "—")
    ship_city = (str(order.get("ship_city") or order.get("pickup_city") or "").strip() or "—")

    avail_days_raw = str(order.get("availability_days") or order.get("prep_days") or "").strip()
    ship_eta_raw = str(order.get("ship_eta") or order.get("shipping_days") or order.get("ship_days") or "").strip()

    # ✅ إصلاح 3-5 => لا تتحول 35
    def _days_txt(x: str) -> str:
        x = (x or "").strip()
        if not x:
            return "—"

        # مدى: 3-5 / 7-14 ... إلخ
        m = re.match(r"^\s*(\d+)\s*-\s*(\d+)\s*(?:يوم|ايام|أيام)?\s*$", x)
        if m:
            a = int(m.group(1))
            b = int(m.group(2))
            if a > 0 and b > 0:
        ...
        # رقم مفرد
        m2 = re.match(r"^\s*(\d+)\s*(?:يوم|ايام|أيام)?\s*$", x)
        if m2:
            n = int(m2.group(1))
            if n <= 0:
                return "—"
    ...
    prep_days_txt = _days_txt(avail_days_raw)
    ship_days_txt = _days_txt(ship_eta_raw)

    # ملخص سريع للقطع المطلوبة
    parts_names = []
    try:
        if isinstance(items, list) and items:
            for it in items:
                nm = str((it or {}).get("name") or (it or {}).get("item_name") or "").strip()
                if nm:
                    parts_names.append(nm)
    except Exception:
        parts_names = []

    parts_short = "، ".join(parts_names[:6]) if parts_names else "—"
    if len(parts_names) > 6:
        parts_short = parts_short + f" (+{len(parts_names) - 6})"
    ...
    if client_id:
        try:
            ...
            client_msg_lines.append(f"🚚 نوع الشحن: <b>{ship_method}</b>")
            client_msg_lines.append(f"⏱ مدة التجهيز: <b>{prep_days_txt}</b>")
            client_msg_lines.append(f"📦 مدة الشحن: <b>{ship_days_txt}</b>")
            client_msg_lines.append(f"📍 مدينة الشحن: <b>{ship_city}</b>")
            client_msg_lines.append(f"🧩 القطع المطلوبة: <b>{parts_short}</b>")
    ...
    try:
        ...
        admin_msg_lines.append(f"🚚 نوع الشحن: <b>{ship_method}</b>")
        admin_msg_lines.append(f"📍 مدينة الشحن: <b>{ship_city}</b>")
        admin_msg_lines.append(f"🧩 القطع المطلوبة: <b>{parts_short}</b>")
    ...
    try:
        ...
        trader_msg_lines.append(f"🚚 نوع الشحن: <b>{ship_method}</b>")
        trader_msg_lines.append(f"⏱ مدة التجهيز: <b>{prep_days_txt}</b>")
        trader_msg_lines.append(f"📦 مدة الشحن: <b>{ship_days_txt}</b>")
        trader_msg_lines.append(f"📍 مدينة الشحن: <b>{ship_city}</b>")
        trader_msg_lines.append(f"🧩 القطع المطلوبة: <b>{parts_short}</b>")
...
async def tsu_skip_tracking_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------- أسماء/بيانات ----------
    client_name = _s(order.get("user_name") or order.get("client_name") or order.get("customer_name")) or "العميل"

    trader_name = _s(order.get("accepted_trader_name") or order.get("quoted_trader_name"))
    trader_store = _s(order.get("accepted_store_name") or order.get("shop_name") or order.get("store_name"))

    if (not trader_name or not trader_store) and accepted_tid:
        try:
            tp = get_trader_profile(int(accepted_tid)) or {}
        except Exception:
            tp = {}
        if not trader_name:
            trader_name = _s(tp.get("display_name") or tp.get("name") or tp.get("full_name") or tp.get("company_name"))
        if not trader_store:
            trader_store = _s(tp.get("company_name") or tp.get("store_name") or tp.get("shop_name") or tp.get("display_name"))

    trader_name = trader_name or "التاجر"
    trader_store = trader_store or "المتجر"

    car_name = _s(order.get("car_name") or order.get("vehicle_name") or order.get("car") or order.get("car_model")) or "—"

    ship_method = _s(order.get("delivery_type") or order.get("ship_method") or order.get("delivery_choice")) or "—"
    ship_city = _s(order.get("ship_city") or order.get("pickup_city")) or "—"

    prep_days = _s(order.get("prep_days") or order.get("prepare_days") or order.get("prep_eta_days") or order.get("prepare_eta_days"))
    ship_days = _s(order.get("ship_days") or order.get("shipping_days") or order.get("shipping_eta_days") or order.get("ship_eta_days"))

    # ---------- مبالغ ----------
    goods_total = 0.0
    shipping_fee_raw = 0.0
    try:
        goods_total = _to_float(order.get("goods_total_sar") or order.get("goods_amount_sar") or 0)
    except Exception:
        goods_total = 0.0
    try:
        shipping_fee_raw = _to_float(order.get("shipping_fee_sar") or 0)
    except Exception:
        shipping_fee_raw = 0.0
    ...
    shipping_included = ship_inc_raw in ("yes", "1", "true", "on", "included", "مشمولة", "مشمول")
    shipping_fee_effective = 0.0 if shipping_included else float(shipping_fee_raw or 0.0)
    grand_total = float(goods_total or 0.0) + float(shipping_fee_effective or 0.0)

    goods_line = f"💰 مبلغ القطع: <b>{_money_tail(goods_total, fb='—') if goods_total else '—'}</b>"
    if shipping_included:
        ship_line = "🚚 الشحن: <b>مشمولة ✅</b>"
    else:
        ship_line = f"🚚 الشحن: <b>{_money_tail(shipping_fee_effective, fb='—') if shipping_fee_effective else '—'}</b>"
    total_line = f"🧾 الإجمالي: <b>{_money_tail(grand_total, fb='—') if grand_total else '—'}</b>"
    ...
    try:
        ...
        if isinstance(items, list) and items:
            ...
            lines = []
            lines.append("🧩 <b>تفاصيل القطع</b>")
            lines.append(f"📦 عدد القطع: <b>{total_items}</b>")
            lines.append(f"✅ المتوفر: <b>{priced_count}</b>")
            if unavail_count:
                lines.append(f"⚠️ غير متوفر: <b>{unavail_count}</b>")
            if priced_count:
                lines.append(f"💰 إجمالي القطع المتوفرة: <b>{_money_tail(parts_total, fb='0')}</b>")
            if priced_lines:
                lines.append("")
                lines.append("✅ <b>القطع المتوفرة:</b>")
                lines.extend(priced_lines)
            if unavail_lines:
                lines.append("")
                lines.append("⚠️ <b>القطع غير المتوفرة:</b>")
                lines.extend(unavail_lines)
    ...
    if already_shipped:
        done_text = (
            ...
            f"👤 العميل: <b>{client_name}</b>\n"
            f"🧑‍💼 التاجر: <b>{trader_name}</b>\n"
            f"🏪 المتجر: <b>{trader_store}</b>\n"
            f"🚚 الشحن: <b>{ship_method}</b>\n"
            f"🏙️ مدينة الشحن: <b>{ship_city}</b>\n\n"
            f"{goods_line}\n{ship_line}\n{total_line}"
        )
        try:
            await q.message.edit_text(
                done_text,
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")]]),
                disable_web_page_preview=True,
            )
        except Exception:
            try:
    ...
    # ===== الرسائل =====
    # رسالة موحدة بتنسيق مثل trader_status_cb
    prep_line = f"🛠️ مدة التجهيز: <b>{prep_days} يوم</b>" if prep_days else "🛠️ مدة التجهيز: <b>—</b>"
    shipdays_line = f"⏱️ مدة الشحن: <b>{ship_days} يوم</b>" if ship_days else "⏱️ مدة الشحن: <b>—</b>"

    # إشعار العميل
    if client_id:
        try:
            lines = []
            lines.append("📦 <b>تحديث حالة الطلب</b>")
            lines.append("")
            lines.append(f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>")
            lines.append(f"👤 العميل: <b>{client_name}</b>")
            lines.append(f"🧑‍💼 التاجر: <b>{trader_name}</b>")
            lines.append(f"🏪 المتجر: <b>{trader_store}</b>")
            lines.append(f"🚗 السيارة: <b>{car_name}</b>")
            lines.append("📌 الحالة: <b>تم الشحن</b>")
            lines.append("🔎 رقم التتبع: <b>غير متوفر</b>")
            lines.append("")
            lines.append(f"🚚 نوع الشحن: <b>{ship_method}</b>")
            lines.append(f"🏙️ مدينة الشحن: <b>{ship_city}</b>")
            lines.append(prep_line)
            lines.append(shipdays_line)
            lines.append("")
            ...
            show_client_chat = bool(_assigned_trader_id(order_id))
            await context.bot.send_message(
                chat_id=client_id,
                text=msg,
                parse_mode="HTML",
    ...
    try:
        ...
        alines = []
        alines.append("📌 <b>تحديث حالة من التاجر</b>")
        alines.append("")
        alines.append(f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>")
        alines.append(f"👤 العميل: <b>{client_name}</b>")
        alines.append(f"🧑‍💼 التاجر: <b>{trader_name}</b>")
        alines.append(f"🏪 المتجر: <b>{trader_store}</b>")
        alines.append("📌 الحالة: <b>تم الشحن</b>")
        alines.append("🔎 التتبع: <b>غير متوفر</b>")
        alines.append("")
        alines.append(f"🚚 نوع الشحن: <b>{ship_method}</b>")
        alines.append(f"🏙️ مدينة الشحن: <b>{ship_city}</b>")
        alines.append("")
        alines.append(goods_line)
        alines.append(ship_line)
        alines.append(total_line)
        alines.append("")
        alines.append(f"🧑‍💼 تم التحديث بواسطة: <b>{_user_name(q)}</b>")

        if parts_block:
            alines.append("")
            alines.append(parts_block)
    ...
    # إشعار التاجر (بدون زر مراسلة العميل داخل إشعار الحالة)
    try:
        tlines = []
        tlines.append(f"{_user_name(q)}")
        tlines.append("✅ <b>تم تحديث حالة الطلب</b>")
        tlines.append("")
        tlines.append(f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>")
        tlines.append(f"👤 العميل: <b>{client_name}</b>")
        tlines.append(f"🧑‍💼 التاجر: <b>{trader_name}</b>")
        tlines.append(f"🏪 المتجر: <b>{trader_store}</b>")
        tlines.append("📌 الحالة: <b>تم الشحن</b>")
        tlines.append("🔎 رقم التتبع: <b>غير متوفر</b>")
        tlines.append("")
        tlines.append(f"🚚 نوع الشحن: <b>{ship_method}</b>")
        tlines.append(f"🏙️ مدينة الشحن: <b>{ship_city}</b>")
        tlines.append(prep_line)
        tlines.append(shipdays_line)
        tlines.append("")
        tlines.append(goods_line)
        tlines.append(ship_line)
        tlines.append(total_line)

        if parts_block:
            tlines.append("")
            tlines.append(parts_block)

        tlines.append("")
        tlines.append("➡️ <b>الخطوة القادمة</b>: بعد التسليم اضغط (✅ تأكيد التسليم بنجاح).")

        trader_msg = "\n".join(tlines).strip()

        # نفس منطق strip chat buttons المستخدم في trader_status_cb
        def _strip_chat_buttons(kb: InlineKeyboardMarkup) -> InlineKeyboardMarkup:
            try:
                if not kb or not getattr(kb, "inline_keyboard", None):
                    return kb
...
async def goods_pay_bank_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(
        ...
        f"{warn}"
        "⚠️ يرجى التأكد من فاتورة التاجر المعتمدة قبل الدفع\n"
        "📸 بعد التحويل يرجى ارسال <b>صورة ايصال الدفع</b> هنا مباشرة\n"
...
async def goods_receipt_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== أسماء العميل/التاجر/المتجر =====
    client_name = (str(o.get("user_name") or o.get("client_name") or o.get("customer_name") or ud.get("user_name") or "").strip() or "العميل")
    ...
    accepted_name = (str(o.get("accepted_trader_name") or o.get("quoted_trader_name") or "").strip())
    trader_store = (str(o.get("accepted_store_name") or o.get("shop_name") or o.get("store_name") or "").strip())
    ...
    if (not accepted_name or not trader_store) and accepted_tid:
        try:
            ...
            tp = {}
        if not accepted_name:
            accepted_name = (str(tp.get("display_name") or "").strip() or str(tp.get("company_name") or "").strip())
        if not trader_store:
            trader_store = (str(tp.get("company_name") or "").strip() or str(tp.get("store_name") or "").strip() or str(tp.get("display_name") or "").strip())
    ...
    # ===== الشحن/العنوان =====
    ship_city = (str(o.get("ship_city") or o.get("pickup_city") or "").strip())
    ship_district = (str(o.get("ship_district") or "").strip())
    ship_short = (str(o.get("ship_short_address") or "").strip())
    ship_method = (str(o.get("delivery_type") or o.get("ship_method") or o.get("delivery_choice") or "").strip())
    ship_prep = (str(o.get("prep_time") or o.get("prep_days") or o.get("prepare_days") or o.get("prepare_time") or "").strip())
    ship_eta = (str(o.get("ship_days") or o.get("shipping_days") or o.get("shipping_time") or o.get("eta_days") or "").strip())

    delivery_details = (str(o.get("delivery_details") or o.get("address_text") or o.get("full_address") or o.get("address") or "").strip())
    if delivery_details:
        # لا نرسل رقم الجوال هنا (يبقى سري)
        try:
            delivery_details = re.sub(r"(\+?9665\d{8}|9665\d{8}|05\d{8})", "*********", delivery_details)
        except Exception:
            pass

    addr_lines = []
    if ship_method:
        addr_lines.append(f"🚚 نوع التسليم: <b>{ship_method}</b>")
    if ship_city:
        addr_lines.append(f"🏙 المدينة: <b>{ship_city}</b>")
    if ship_district:
        addr_lines.append(f"📍 الحي: <b>{ship_district}</b>")
    if ship_short:
        addr_lines.append(f"🧭 العنوان المختصر: <b>{ship_short}</b>")
    if ship_prep:
        addr_lines.append(f"🛠 مدة التجهيز: <b>{ship_prep}</b>")
    if ship_eta:
        addr_lines.append(f"⏱ مدة الشحن: <b>{ship_eta}</b>")
    if delivery_details:
        addr_lines.append(f"📝 تفاصيل العنوان: {delivery_details}")
    ...
    goods_amt_raw = o.get("goods_amount_sar") or o.get("goods_total_sar") or ""
    ship_included_norm = str(o.get("ship_included") or o.get("shipping_included") or o.get("shipping_fee_included") or "").strip().lower()
    ship_fee_raw = o.get("shipping_fee_sar")
    ...
    goods_line = f"💰 مبلغ القطع: <b>{_money_sar(goods_val)}</b>" if goods_val else "💰 مبلغ القطع: —"
    if ship_included_norm in ("yes", "true", "1", "included", "مشمولة", "مشمول"):
        ship_line = "🚚 الشحن: <b>مشمولة ✅</b>"
    else:
        ship_line = f"🚚 الشحن: <b>{_money_sar(ship_val)}</b>" if ship_val else "🚚 الشحن: —"
    total_line = f"🧾 الإجمالي: <b>{_money_sar(total_val)}</b>" if total_val else "🧾 الإجمالي: —"

    # ===== تفاصيل القطع (مختصر ومرتب) =====
    parts_lines = []
    try:
        if isinstance(items, list) and items:
            parts_lines.append("🧩 <b>القطع المطلوبة</b>")
            for i, it in enumerate(items, start=1):
                nm = str((it or {}).get("name") or (it or {}).get("item_name") or "—").strip() or "—"
                pn = str((it or {}).get("part_no") or (it or {}).get("item_part_no") or (it or {}).get("number") or "").strip()
                if pn:
                    parts_lines.append(f"• {i}- {nm} <code>({pn})</code>")
                else:
                    parts_lines.append(f"• {i}- {nm}")
    except Exception:
        parts_lines = []
    ...
    caption_lines = []
    caption_lines.append("🧾 <b>إيصال سداد قيمة القطع</b>")
    caption_lines.append("")
    caption_lines.append(f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>")
    caption_lines.append(f"👤 العميل: <b>{client_name}</b>")
    caption_lines.append(f"🧑‍💼 التاجر: <b>{accepted_name}</b>")
    caption_lines.append(f"🏪 المتجر: <b>{trader_store}</b>")
    caption_lines.append(f"🚗 السيارة: <b>{car_name}</b>")
    caption_lines.append("")
    caption_lines.append(goods_line)
    caption_lines.append(ship_line)
    caption_lines.append(total_line)
    caption_lines.append("")
    caption_lines.append("📦 <b>تفاصيل التسليم</b>")
    caption_lines.append(addr_block)

    if parts_block:
        caption_lines.append("")
        caption_lines.append(parts_block)

    caption_lines.append("")
    caption_lines.append("✅ الخطوة التالية: اضغط (تأكيد الاستلام)")

    caption = "\n".join([x for x in caption_lines if x is not None]).strip()
...
async def _rebroadcast_noquote_orders_job(context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ يعيد نشر الطلب لمجموعة التجار كل 3 ساعات إذا:
      - لا يوجد عروض
      - أو يوجد عرض/تم قبول ولكن لم يتم سداد قيمة البضاعة (goods_payment_status غير مؤكد)

    ✅ تنبيه العميل بعد ساعة إذا:
      - تم إرسال فاتورة التاجر (seller_invoice_at / shop_invoice_at) ولم يسدد

    ✅ تنبيه التاجر بعد ساعة إذا:
      - تم تأكيد سداد قيمة البضاعة (goods_payment_confirmed_at_utc) ولم يحدّث الحالة (لم يشحن)

    ✅ تنبيه الأدمن بعد 24 ساعة على الطلبات العالقة (بدون عروض/بدون سداد)
    """
    ...
    for o in orders:
        ...
        has_quote = (qtid > 0) or (qs in ("sent", "accepted"))
        no_quote = not has_quote

        # حالة سداد قيمة البضاعة (سداد للمتجر/التاجر)
        gps = str(o.get("goods_payment_status") or "").strip().lower()
        goods_paid = gps in ("confirmed", "paid", "success", "successful", "done", "ok")

        unpaid_goods = has_quote and (not goods_paid)

        # اشتغل فقط على: بدون عروض أو غير مدفوع
        if not (no_quote or unpaid_goods):
            continue
        ...
        # =========================================================
        # (A) إعادة النشر لمجموعة التجار كل 3 ساعات (No quote / Unpaid)
        # =========================================================
        if (now - base_ts) >= three_hours:
            last_b = _parse_utc_iso(str(o.get("last_group_broadcast_at_utc") or "")) or None
            last_b = _as_utc_aware(last_b)

            if (not last_b) or ((now - last_b) >= three_hours):
                # جلب بيانات الطلب
                try:
                    b = get_order_bundle(order_id)
                    order = b.get("order", {}) or {}
                ...
                ud_payload = {
                    ...
                    "items": items,
                    "_reminder": True,
                    "_reminder_reason": "unpaid" if unpaid_goods else "noquote",
                }

                try:
                    log_event(
                        "إعادة نشر طلب (بدون عروض)" if no_quote else "إعادة نشر طلب (غير مدفوع)",
                        order_id=order_id
                    )
                except Exception:
                    pass
        ...
        # =========================================================
        # (B) تنبيه العميل بعد ساعة: فاتورة التاجر وصلت ولم يتم السداد
        # =========================================================
        try:
            b = get_order_bundle(order_id)
            order = b.get("order", {}) or {}
        ...
        client_id = 0
        try:
            client_id = int(order.get("user_id") or o.get("user_id") or 0)
        except Exception:
            client_id = 0

        # وقت إرسال فاتورة التاجر (موجود فعليا عندك)
        inv_at = (
            _parse_utc_iso(str(o.get("seller_invoice_at") or "")) or
            _parse_utc_iso(str(o.get("shop_invoice_at") or "")) or
            _parse_utc_iso(str(order.get("seller_invoice_at") or "")) or
            _parse_utc_iso(str(order.get("shop_invoice_at") or "")) or
            None
        )
        inv_at = _as_utc_aware(inv_at)

        # إذا عنده فاتورة + لم يُسدّد بعد
        if client_id and inv_at and (not goods_paid):
            if (now - inv_at) >= one_hour:
                last_ping = _parse_utc_iso(str(o.get("last_unpaid_invoice_user_ping_at_utc") or "")) or None
                ...
                if (not last_ping) or ((now - last_ping) >= one_day):
                    try:
                        await context.bot.send_message(
                            ...
                            text=(
                                "🔔 تذكير من المنصة\n"
                                f"🧾 رقم الطلب: {_order_id_link_html(order_id)}\n\n"
                                "طلبك تم إعطاؤه أولوية وهو قيد المتابعة.\n"
                                "وصلتك فاتورة الدفع لإكمال الطلب، وبمجرد إتمام السداد سيتم تسريع الإسناد واستكمال الإجراء.\n\n"
                                "🛟 إذا واجهتك أي مشكلة اكتب: منصة"
                            ),
                            reply_markup=track_kb(order_id),
                            disable_web_page_preview=True,
                        )
                    ...
                    try:
                        update_order_fields(order_id, {"last_unpaid_invoice_user_ping_at_utc": utc_now_iso()})
                    except Exception:
                        pass

        # =========================================================
        # (C) تنبيه التاجر بعد ساعة: السداد وصل ولم يتم تحديث الحالة
        # =========================================================
        accepted_tid = 0
        try:
            accepted_tid = int(o.get("accepted_trader_id") or order.get("accepted_trader_id") or 0)
        except Exception:
            accepted_tid = 0

        paid_at = (
            _parse_utc_iso(str(o.get("goods_payment_confirmed_at_utc") or "")) or
            _parse_utc_iso(str(order.get("goods_payment_confirmed_at_utc") or "")) or
            None
        )
        paid_at = _as_utc_aware(paid_at)

        shipped_at = _parse_utc_iso(str(o.get("shipped_at_utc") or order.get("shipped_at_utc") or "")) or None
        shipped_at = _as_utc_aware(shipped_at)

        ost_now = str(o.get("order_status") or order.get("order_status") or "").strip().lower()

        # نعتبر "لم يحدث" إذا: دفع مؤكد + مرّت ساعة + لم يتم الشحن + مازال ready_to_ship/ready/preparing
        not_updated = (ost_now in ("ready_to_ship", "ready", "preparing", "prep")) and (not shipped_at)

        if accepted_tid and goods_paid and paid_at and not_updated:
            if (now - paid_at) >= one_hour:
                last_t_ping = _parse_utc_iso(str(o.get("last_paid_trader_ping_at_utc") or "")) or None
                last_t_ping = _as_utc_aware(last_t_ping)

                if (not last_t_ping) or ((now - last_t_ping) >= one_day):
                    try:
                        await context.bot.send_message(
                            chat_id=int(accepted_tid),
                            text=(
                                "✅ تم تأكيد سداد الطلب لديك\n"
                                f"🧾 رقم الطلب: {_order_id_link_html(order_id)}\n\n"
                                "فضلا حدّث حالة الطلب داخل المنصة (تجهيز / جاهز للشحن / تم الشحن) حتى تصل إشعارات العميل تلقائيا.\n"
                                "في حال احتجت مساعدة اكتب: منصة"
                            ),
                            disable_web_page_preview=True,
                        )
                    except Exception:
                    ...
                    try:
                        update_order_fields(order_id, {"last_paid_trader_ping_at_utc": utc_now_iso()})
                    except Exception:
                        pass

        # =========================================================
        # (D) تنبيه الأدمن بعد 24 ساعة: طلبات عالقة
        # =========================================================
    ...
    if admin_need_list:
        ...
        text = (
            "⏰ <b>تنبيه إداري</b>\n"
            "طلبات مضى عليها 24 ساعة بدون اكتمال (عروض/سداد): \n\n"
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== مكان التنفيذ =====
    try:
        chat_id = q.message.chat_id if (q.message and q.message.chat_id) else 0
    except Exception:
        chat_id = 0

    try:
        chat_type = q.message.chat.type if (q.message and q.message.chat) else None
    except Exception:
        chat_type = None

    in_team_group = bool(TEAM_CHAT_ID and chat_id and int(chat_id) == int(TEAM_CHAT_ID))
    in_private = bool(chat_type == ChatType.PRIVATE)

    if not (in_team_group or in_private):
        return

    # ===== داخل المجموعة: أزرار محددة فقط =====
    # (حتى لو أنت تقول كل شيء من الخاص — هذا تحصين ما يضر)
    if in_team_group and action not in (
        "pp_team_quote",
        "pp_trader_open",
        "pp_team_locked",
        "pp_team_quote_locked",
        "pp_team_goods_confirm",  # ✅ تحصين
    ):
        return
    ...
    def _order_notify_plain(_oid: str, max_parts: int = 6) -> str:
        _oid = (_oid or "").strip()
        if not _oid:
            return ""
        try:
            _b = get_order_bundle(_oid) or {}
            _o = _b.get("order", {}) or {}
            _items = _b.get("items", []) or []
        except Exception:
            _o, _items = {}, []

        _client = (str(_o.get("user_name") or _o.get("client_name") or _o.get("customer_name") or "").strip() or "العميل")

        _tid = 0
        try:
            _tid = int(_o.get("accepted_trader_id") or 0)
        except Exception:
            _tid = 0

        _tname = (str(_o.get("accepted_trader_name") or _o.get("quoted_trader_name") or "").strip())
        _store = (str(_o.get("accepted_store_name") or _o.get("shop_name") or _o.get("store_name") or "").strip())

        if (_tid and (not _tname or not _store)):
            try:
                _tp = get_trader_profile(int(_tid)) or {}
            except Exception:
                _tp = {}
            if not _tname:
                _tname = (str(_tp.get("display_name") or "").strip() or str(_tp.get("company_name") or "").strip())
            if not _store:
                _store = (str(_tp.get("company_name") or "").strip() or str(_tp.get("store_name") or "").strip() or str(_tp.get("display_name") or "").strip())

        _tname = _tname or "التاجر"
        _store = _store or "المتجر"

        _goods = 0
        _ship_raw = 0
        try:
            _goods = int(float(_o.get("goods_total_sar") or _o.get("goods_amount_sar") or 0))
        except Exception:
            _goods = 0
        try:
            _ship_raw = int(float(_o.get("shipping_fee_sar") or 0))
        except Exception:
            _ship_raw = 0
        try:
            _ship_inc = str(_o.get("shipping_included") or _o.get("ship_included") or _o.get("shipping_fee_included") or "").strip().lower()
        except Exception:
            _ship_inc = ""
        _ship_included = _ship_inc in ("yes","1","true","on","included","مشمولة","مشمول")
        _ship = 0 if _ship_included else int(_ship_raw or 0)
        _total = int((_goods or 0) + (_ship or 0))

        _ship_line = "🚚 الشحن: مشمول ✅" if _ship_included else (f"🚚 الشحن: {_money(_ship) if _ship else '—'}")
        _lines = [
            f"🧾 الطلب: {_oid}",
            f"👤 العميل: {_client}",
            f"🧑‍💼 التاجر: {_tname}",
            f"🏪 المتجر: {_store}",
            f"💰 مبلغ القطع: {_money(_goods) if _goods else '—'}",
            _ship_line,
            f"🧾 الإجمالي: {_money(_total) if _total else '—'}",
        ]

        _ship_method = (str(_o.get("delivery_type") or _o.get("ship_method") or _o.get("delivery_choice") or "").strip())
        _ship_city = (str(_o.get("ship_city") or _o.get("pickup_city") or "").strip())
        _prep = (str(_o.get("prep_days") or _o.get("prepare_days") or _o.get("prep_time_days") or "").strip())
        _ship_days = (str(_o.get("shipping_days") or _o.get("ship_days") or _o.get("delivery_days") or "").strip())

        if _ship_method:
            _lines.append(f"📦 نوع الشحن: {_ship_method}")
        if _prep:
            _lines.append(f"🛠 مدة التجهيز: {_prep}")
        if _ship_days:
            _lines.append(f"🚚 مدة الشحن: {_ship_days}")
        if _ship_city:
            _lines.append(f"🏙 مدينة الشحن: {_ship_city}")

        try:
            _p = []
            for _it in (_items or [])[:max_parts]:
                _nm = (str((_it or {}).get("name") or (_it or {}).get("item_name") or "").strip())
                if _nm:
                    _p.append(_nm)
            if _p:
                _lines.append("🧩 القطع: " + "، ".join(_p) + (" …" if len(_items or []) > max_parts else ""))
        except Exception:
            pass
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    "💰 تقديم عرض سعر\n\n"
                    f"{order_snapshot}\n\n"
                    f"{_order_notify_plain(order_id)}\n\n"
    ...
    # ==========================================================
    # 🔐 باقي الأوامر: خاص فقط (مع استثناء تحصيني لتأكيد السداد)
    # ==========================================================
    if not in_private:
        if not (in_team_group and action == "pp_team_goods_confirm"):
            return
    ...
    if action == "pp_team_goods_confirm":
        try:
            ...
            if not (order.get("goods_amount_sar") or order.get("goods_total_sar")):
                try:
            ...
            # ✅ تحديد الحالة الصحيحة بعد التأكيد (بدون in_progress)
            try:
                ost_now = str(order.get("order_status") or "").strip().lower()
            except Exception:
                ost_now = ""
            ...
            if ost_now in ("shipped", "delivered", "closed"):
                ...
                next_ost = "ready_to_ship" if inv_file_now else (ost_now if ost_now else "preparing")
            ...
            # ✅ تحصين: أعد جلب بعد التحديث (عشان tid + الحقول تتأكد)
            b3 = get_order_bundle(order_id) or {}
            o3 = b3.get("order", {}) or {}

            # ✅ tid: من الطلب ثم fallback للـ assigned
            tid3 = 0
            try:
            ...
            if not tid3:
                try:
                    tid3 = int(_assigned_trader_id(order_id) or 0)
                except Exception:
                    tid3 = 0

            # 🔒 قفل زر المجموعة بصريًا (لو عندك بوست بالمجموعة)
            try:
                await _lock_team_post_keyboard(context, order_id, reason="🔒 تم إيقاف العروض الطلب منتهي")
            except Exception as e:
                _swallow(e)

            # ✅ إرسال فاتورة التاجر للعميل (كما عندك)
            try:
            ...
            # ✅ إشعار التاجر + لوحة الحالة
            # (هذا أهم شيء عندك الآن)
            try:
                ...
                addr_lines = []
                if ship_city: addr_lines.append(f"المدينة: {ship_city}")
                if ship_dist: addr_lines.append(f"الحي: {ship_dist}")
                if ship_short: addr_lines.append(f"العنوان المختصر: {ship_short}")
                if delivery_details: addr_lines.append(f"تفاصيل إضافية: {delivery_details}")
                if ship_phone: addr_lines.append(f"📞 رقم الجوال: {ship_phone}")
                addr_block = "\n".join(addr_lines) if addr_lines else "—"
                ...
                try:
                    ...
                    ship_num3 = 0.0
                try:
                    ship_included = str(o3.get("ship_included") or o3.get("shipping_included") or "").strip().lower() in ("yes","y","true","1","مشمول","included")
                except Exception:
                    ship_included = False

                total_num3 = goods_num3 + (0.0 if ship_included else ship_num3)
                amt3 = _money(goods_num3)
                ship3 = _money(ship_num3)
                total3 = _money(total_num3)

                ship_line = "🚚 <b>الشحن</b>: <b>مشمول</b>\n" if ship_included else f"🚚 <b>الشحن</b>: <b>{html.escape(str(ship3))}</b>\n"
                total_label = "إجمالي القطع (الشحن مشمول)" if ship_included else "إجمالي القطع + الشحن"
                ...
                if tid3:
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            "🚀 <b>ابدأ التجهيز الآن</b>\n"
                            "وعند الشحن حدّث الحالة من الأزرار.\n\n"
                            "📍 <b>عنوان الشحن</b>:\n"
                    ...
                    # ✅ تحديث لوحة التاجر إن كانت مفتوحة
                    try:
                        await _show_order_panel_private(context, int(tid3), order_id)
                    except Exception as e:
                        _swallow(e)
                else:
                    # ✅ لو ما قدرنا نحدد التاجر: بلّغ الأدمن بدل الصمت
                    for aid in ADMIN_IDS:
                        try:
                            await context.bot.send_message(
                                chat_id=int(aid),
                                text=f"⚠️ تعذر تحديد accepted_trader_id لإرسال إشعار السداد\n🧾 الطلب: {order_id}",
                                disable_web_page_preview=True,
                            )
                        except Exception:
                            pass
            ...
            # ✅ إشعار الإدارة النصي (كما عندك) — أبقيته بدون تغيير جوهري
            try:
                ...
                try:
                    ...
                    ship_num4 = 0.0
                try:
                    ship_included4 = str(o4.get("ship_included") or o4.get("shipping_included") or "").strip().lower() in ("yes","y","true","1","مشمول","included")
                except Exception:
                ...
                ship_line_admin = "🚚 <b>الشحن</b>: <b>مشمول</b>\n" if ship_included4 else f"🚚 <b>الشحن</b>: <b>{html.escape(str(ship4))}</b>\n"
                total_label_admin = "إجمالي القطع (الشحن مشمول)" if ship_included4 else "إجمالي القطع + الشحن"

                client_name_admin = (o4.get("user_name") or "").strip() or "العميل"
                trader_name_admin = (o4.get("accepted_trader_name") or o4.get("quoted_trader_name") or "").strip() or "—"
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text=(
                                ...
                                f"🧾 <b>رقم الطلب</b>: {html.escape(order_id)}\n"
                                f"👤 <b>العميل</b>: <b>{html.escape(client_name_admin)}</b>\n"
            ...
            for aid in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        chat_id=int(aid),
                        text=f"❌ خطأ داخل pp_team_goods_confirm\n🧾 الطلب: {order_id}\n⚠️ {e}",
                        disable_web_page_preview=True,
                    )
                except Exception:
                    pass
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الهدف: إذا وصلت رسالة وسائط (صورة/فيديو/ملف) داخل مراحل المراسلة
    # يتم تمرير نفس الوسائط للطرف الآخر مع النص (caption) + هيدر بسيط.
    # لا نلمس مراحل التسعير/الإيصالات الأخرى.
    # =========================================================
    ...
    if msg:
        ...
        if kind and file_id:
            ...
            # حد الكابتشن في تيليجرام ~1024، نخليه آمن
            if len(cap_raw) > 900:
                cap_raw = cap_raw[:900].rstrip() + "…"

            def _safe_int2(x) -> int:
                try:
                    return int(x or 0)
                except Exception:
                    return 0

            async def _send_media(chat_id: int, caption: str = "", kb=None):
                try:
                    if kind == "photo":
                        await context.bot.send_photo(
                            chat_id=chat_id,
            ...
            # 1) pp_chat_sessions (مراسلة مباشرة داخل جلسة)
            try:
            ...
            if isinstance(sess, dict):
                peer_id = _safe_int2(sess.get("peer_id"))
                ...
                if peer_id and order_id_sess:
                    try:
                        ...
                        caption = (
                            ...
                            + (f"💬 {cap_raw}" if cap_raw else "📎 مرفق")
                        )
                    except Exception:
                        caption = (f"💬 {cap_raw}" if cap_raw else "📎 مرفق")
                    await _send_media(peer_id, caption=caption)
                    return

            # 2) مراحل المراسلة المعروفة (توجيه وسائط)
            # ملاحظة: نحن هنا نمرر الوسائط فقط (مع كابتشن)، ثم نثبت نفس سلوك التأكيد/إعادة الستيج.
            try:
            ...
            if user_id in ADMIN_IDS and stage == STAGE_SUPPORT_ADMIN_REPLY:
                target_uid = _safe_int2(ud.get("support_reply_to_uid"))
                ...
                await _send_media(target_uid, caption=caption, kb=_support_kb())
                try:
            ...
            if user_id in ADMIN_IDS and stage == STAGE_ADMIN_TRADER_MSG:
                tid = _safe_int2(ud.get("admin_msg_to_trader_id"))
                if not tid:
                    return
                caption = f"{PP_SUPPORT_LABEL}\n💬 رسالة من الإدارة:\n" + (cap_raw if cap_raw else "📎 مرفق")
                await _send_media(tid, caption=caption, kb=_admin_to_trader_reply_kb(user_id))
                try:
            ...
            if stage == STAGE_CHAT_TRADER:
                order_id = str(ud.get("chat_trader_order_id") or "").strip()
                ...
                await _send_media(tid, caption=caption, kb=trader_reply_kb(order_id, user_id))
                try:
                    await msg.reply_text("✅ تم إرسال رسالتك للتاجر.")
                except Exception as e:
                    _swallow(e)
                return
            ...
            if stage == STAGE_TRADER_REPLY:
                ...
                to_uid = _safe_int2(td_local.get("trader_reply_user_id"))
                order_id = str(td_local.get("trader_reply_order_id") or "").strip()
                ...
                tname = (tprof.get("display_name") or "").strip() or (name_now or "").strip() or "التاجر"
                tcompany = (tprof.get("company_name") or "").strip()
                tlabel = tname + (f" ({tcompany})" if tcompany else "")
                ...
                await _send_media(to_uid, caption=caption, kb=client_trader_chat_kb(order_id))
                try:
            ...
            # ===== STAGE_ADMIN_CHAT (إدارة → عميل/تاجر) =====
            if stage == STAGE_ADMIN_CHAT and user_id in ADMIN_IDS:
                order_id = str(ud.get("admin_chat_order_id") or "").strip()
                peer_id = _safe_int2(ud.get("admin_chat_peer_id"))
                role = str(ud.get("admin_chat_role") or "").strip()  # client / trader
                if not order_id or not peer_id:
                    return
                ...
                kb = track_kb(order_id) if role == "client" else trader_chat_admin_kb(order_id, int(user_id))
                await _send_media(peer_id, caption=caption, kb=kb)
            ...
            if stage == STAGE_TRADER_CHAT_ADMIN:
                ...
                admin_id = _safe_int2(ud.get("trader_chat_admin_id"))
                if not order_id or not admin_id:
                    return
                ...
                await _send_media(admin_id, caption=caption, kb=admin_contact_kb(order_id))
                try:
            ...
            if stage == STAGE_ADMIN_REPLY and user_id in ADMIN_IDS:
                ad = context.user_data.setdefault(user_id, {})
                to_uid = _safe_int2(ad.get("reply_user_id"))
                ...
                await _send_media(to_uid, caption=caption, kb=track_kb(order_id))
                try:
            ...
            if stage == STAGE_TRACK_ORDER:
                ...
                for aid in ADMIN_IDS:
                    await _send_media(int(aid), caption=caption, kb=admin_reply_kb(order_id, user_id))
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
        ...
        try:
            ...
            # ✅ اسم التاجر/المتجر لإظهاره للعميل (قبل الدفع)
            try:
                tprof = get_trader_profile(user_id) or {}
            except Exception:
                tprof = {}

            tname = (str(tprof.get("display_name") or "").strip() or _user_name(update) or "التاجر")
            tcompany = (str(tprof.get("company_name") or "").strip())
            tlabel = tname + (f" ({tcompany})" if tcompany else "")
            ...
            # 1) إرسال فاتورة التاجر للعميل فقط (مع توضيح المبالغ + اسم التاجر)
            if client_id2:
                try:
                    cap_client = (
                        f"🧾 فاتورة التاجر - الطلب {order_id2}\n"
                        f"🧑‍💼 التاجر: {tlabel}"
                        f"{money_block}"
                    )
                    if is_photo:
            ...
            # 2) نسخة فاتورة التاجر للإدارة (مع توضيح المبالغ + اسم التاجر)
            for aid in ADMIN_IDS:
                try:
                    cap_admin = (
                    ...
                    if is_photo:
                        await context.bot.send_photo(chat_id=aid, photo=file_id, caption=cap_admin)
                    else:
                        await context.bot.send_document(chat_id=aid, document=file_id, caption=cap_admin)
                except Exception as e:
                    _swallow(e)
            ...
            if client_id2:
                ...
                # ✅ رسالة العميل لازم تحتوي المبالغ الصحيحة + اسم التاجر
                client_lines = [
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_REPLY:
        ...
        try:
            ...
            await _reply_html("تم الإرسال", ["✅ تم إرسال ردّك للعميل."], kb=trader_reply_done_kb())
        except Exception:
    ...
    if stage == STAGE_ASK_CAR:
        ...
        await _reply_html("سنة الموديل", ["📌 اكتب سنة الموديل فقط (4 أرقام). مثال: <code>2023</code>"])
        return

    if stage == STAGE_ASK_MODEL:
        s = (text or "").strip()
        if not re.fullmatch(r"(19|20)\d{2}", s):
            await _reply_html("صيغة غير صحيحة", ["⚠️ اكتب سنة الموديل 4 أرقام فقط. مثال: <code>2023</code>"])
            return

        ud["car_model"] = s
        set_stage(context, user_id, STAGE_ASK_VIN)
        await _reply_html("رقم الهيكل VIN", ["🔎 اكتب رقم الهيكل (17 خانة). مثال: <code>LVVDC12B4RD012345</code>"])
        return

    if stage == STAGE_ASK_VIN:
        vin = (text or "").replace(" ", "").upper()
        if not _looks_like_vin(vin):
            await _reply_html("VIN غير صحيح", ["⚠️ رقم الهيكل يجب أن يكون 17 خانة. مثال: <code>LVVDC12B4RD012345</code>"])
            return
...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, trader_id: int, order_id: str):
    order_id = (order_id or "").strip()
    if not order_id:
        return
    ...
    if int(trader_id or 0) not in (ADMIN_IDS or []) and int(trader_id or 0) != int(accepted_tid or 0):
        try:
            await context.bot.send_message(
                chat_id=int(trader_id or 0),
                text=(
                    "🔒 هذا الطلب ليس على حسابك.\n"
                    f"🧾 رقم الطلب: {order_id}"
                ),
                disable_web_page_preview=True,
            )
