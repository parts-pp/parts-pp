...
# ✅ Strict Mode: إلزام كل حقول ملف التاجر (قابل للتبديل من لوحة الأدمن)
def _trader_profile_strict_mode() -> bool:
    try:
        v = (get_setting(TRADER_PROFILE_STRICT_KEY, "0") or "").strip().lower()
        return v in ("1", "on", "yes", "true", "enabled")
    except Exception:
        return False

def _trader_profile_missing_fields(trader_id: int, strict: bool | None = None) -> list[str]:
    """Return list of missing required labels for trader profile."""
    if strict is None:
        strict = _trader_profile_strict_mode()
    try:
...
# ✅ إعداد: تفعيل/إلغاء إلزام كل حقول ملف التاجر (Strict Mode)
TRADER_PROFILE_STRICT_KEY = "trader_profile_strict_mode"
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # ✅ شرط ملف التاجر (إلزام أساسي/صارم حسب إعداد الأدمن)
    try:
        missing = _trader_profile_missing_fields(int(trader_id or 0))
    except Exception:
        missing = []

    if missing:
        bot_username = getattr(context.bot, "username", "") or ""
        panel_url = f"https://t.me/{bot_username}?start=trader_{int(trader_id)}"
        lines = "\n".join([f"• {x}" for x in missing])
        strict_on = _trader_profile_strict_mode()
        hdr = "⛔ لا يمكنك إرسال عرض سعر قبل إكمال ملف التاجر." + ("\n(وضع الإلزام مفعل)" if strict_on else "")
        msg = (
            f"{_user_name(message)}\n"
            f"{hdr}\n\n"
            "الحقول الناقصة:\n"
            f"{lines}\n\n"
            "افتح لوحة التاجر وأكمل البيانات ثم أعد المحاولة."
        )
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🧑‍💼 فتح لوحة التاجر", url=panel_url)]])
        try:
            await message.reply_text(msg, reply_markup=kb, disable_web_page_preview=True)
        except Exception as e:
            _swallow(e)
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_PROFILE_EDIT:
        ...
        # ✅ السماح بمسح الحقل (اكتب - لمسح القيمة)
        if val in ("-", "—", "none", "None", "0"):
            val = ""

        if field not in ("display_name", "company_name", "bank_name", "iban", "stc_pay", "national_id", "cr_number", "vat_number"):
            set_stage(context, user_id, STAGE_NONE)
        ...
        if field == "national_id":
            ...
            if v and (not re.fullmatch(r"\d{10}", v)):
                await _reply_html(
                    "رقم الهوية غير صحيح",
                    [
                        "⚠️ الصيغة المطلوبة: 10 أرقام.",
                        "مثال: <code>10XXXXXXXX</code>",
                        "أو اكتب <code>-</code> لمسح الحقل.",
                    ],
                )
                return
            val = v
        ...
        if field == "cr_number":
            v = re.sub(r"\s+", "", val)
            # غالباً 10 أرقام، نسمح 7-15 لتفادي كسر إدخالات مختلفة
            if v and (not re.fullmatch(r"\d{7,15}", v)):
                await _reply_html(
                    "رقم السجل غير صحيح",
                    [
                        "⚠️ اكتب أرقام السجل فقط (بدون مسافات).",
                        "أو اكتب <code>-</code> لمسح الحقل.",
                    ],
                )
                return
        ...
        if field == "vat_number":
            v = re.sub(r"\s+", "", val)
            # الرقم الضريبي بالسعودية 15 رقم غالباً
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "edit":
        ...
        labels = {
            ...
            "cr_number": "رقم السجل التجاري (اختياري)",
            "vat_number": "الرقم الضريبي (اختياري)",
        }
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _admin_show_traders_manage():
        ...
        # ✅ Strict Mode toggle
        strict_on = _trader_profile_strict_mode()
        strict_txt = "🔒 إلزام كل الحقول: ON" if strict_on else "🔓 إلزام كل الحقول: OFF"
        strict_cb = "pp_admin|tstrict|off" if strict_on else "pp_admin|tstrict|on"
        rows.append([InlineKeyboardButton(strict_txt, callback_data=strict_cb)])
    ...
    # ✅ Strict Mode toggle (إلزام كل حقول ملف التاجر)
    if action == "tstrict":
        mode = (parts[2].strip().lower() if len(parts) >= 3 else "")
        try:
            set_setting(TRADER_PROFILE_STRICT_KEY, "1" if mode == "on" else "0")
        except Exception as e:
            _swallow(e)
        await _toast("تم تحديث وضع الإلزام")
        await _admin_show_traders_manage()
        return
    ...
    if action == "tview":
        ...
        national_id = (prof.get("national_id") or "").strip()
        cr_number = (prof.get("cr_number") or "").strip()
        vat_number = (prof.get("vat_number") or "").strip()
        ...
        # 📅 تاريخ الانضمام: من ملف التاجر أو أقدم طلب كتقدير
        joined_at = created
        if not joined_at and orders:
            try:
                orders_sorted_old = sorted(orders, key=_dt, reverse=False)
                joined_at = str((orders_sorted_old[0] or {}).get("created_at_utc") or "").strip()
            except Exception:
                joined_at = ""
        msg = (
            "👤 <b>ملف التاجر</b>\n\n"
            ...
            f"📅 تاريخ الانضمام: <b>{html.escape(joined_at or '—')}</b>\n\n"
            ...
            f"🆔 الهوية: <b>{html.escape(national_id or '—')}</b>\n"
            f"🧾 السجل التجاري: <b>{html.escape(cr_number or '—')}</b>\n"
            f"🏷️ الرقم الضريبي: <b>{html.escape(vat_number or '—')}</b>\n"
        ...
        kb_rows = [
            ...
            [InlineKeyboardButton("📄 سجل التاجر PDF", callback_data=f"pp_admin|tprofile_pdf|{tid}")],
    ...
    # ===== TRADER PROFILE PDF (tprofile_pdf) =====
    if action == "tprofile_pdf":
        tid = 0
        if len(parts) >= 3:
            try:
                tid = int(parts[2] or 0)
        ...
        if not tid:
            ...
            return
        try:
            await send_trader_record_pdf(context, chat_id=int(uid), trader_id=int(tid))
...
def trader_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🆔 رقم الهوية (اختياري)", callback_data="pp_tprof|edit|national_id")],
        [InlineKeyboardButton("🧾 السجل التجاري (اختياري)", callback_data="pp_tprof|edit|cr_number")],
        [InlineKeyboardButton("🏷️ الرقم الضريبي (اختياري)", callback_data="pp_tprof|edit|vat_number")],
...
async def send_trader_record_pdf(context: ContextTypes.DEFAULT_TYPE, chat_id: int, trader_id: int):
    """
    📄 سجل التاجر PDF (للإدارة)
    - نفس هوية الفاتورة قدر الإمكان: شعار + خط Amiri + تذييل GO
    - صفحة واحدة خفيفة بدون نفخ
    """
    import os, tempfile
    ...
    # ✅ بيانات التاجر
    try:
        tp = get_trader_profile(trader_id) or {}
    except Exception:
        tp = {}
    ...
    # تاريخ الانضمام: من الملف أو أقدم طلب
    created = str(tp.get("created_at_utc") or "").strip()
    if not created:
        try:
        ...
        if orders:
            try:
                created = str(sorted(orders, key=_dt, reverse=False)[0].get("created_at_utc") or "").strip()
            except Exception:
                created = ""

    # حقول
    display_name = str(tp.get("display_name") or "").strip() or "—"
    company_name = str(tp.get("company_name") or "").strip() or "—"
    bank_name = str(tp.get("bank_name") or "").strip() or "—"
    iban = str(tp.get("iban") or "").strip() or "—"
    stc_pay = str(tp.get("stc_pay") or "").strip() or "—"
    national_id = str(tp.get("national_id") or "").strip() or "—"
    cr_number = str(tp.get("cr_number") or "").strip() or "—"
    vat_number = str(tp.get("vat_number") or "").strip() or "—"

    # ✅ إنشاء PDF
    tmpdir = tempfile.gettempdir()
    fn = os.path.join(tmpdir, f"PP_TRADER_RECORD_{trader_id}.pdf")
    ...
    try:
        ...
        c.translate(w/2, h/2)
        c.rotate(25)
        c.drawCentredString(0, 0, "PParts")
        c.restoreState()
    except Exception:
        pass
    ...
    # شعار
    try:
        if os.path.exists("pparts.jpg"):
            c.drawImage("pparts.jpg", 40, h-130, width=140, height=90, preserveAspectRatio=True, mask='auto')
    except Exception:
        pass
    ...
    c.drawRightString(w-40, h-70, "سجل التاجر")

    c.setFont(font_name, 12)
    c.drawRightString(w-40, h-95, f"تاريخ الإصدار: {datetime.now(timezone.utc).astimezone().strftime('%Y-%m-%d %H:%M')}")
    ...
    def draw_kv(label, value):
        nonlocal y
        c.setFont(font_name, 12)
        c.drawRightString(w-40, y, f"{label}:")
        c.setFont(font_name, 12)
        c.drawString(60, y, str(value))
        y -= line_h

    draw_kv("🆔 ID", trader_id)
    draw_kv("👤 اسم التاجر", display_name)
    draw_kv("🏪 اسم المتجر", company_name)
    draw_kv("📅 تاريخ الانضمام", created or "—")
    draw_kv("🔘 حالة التفعيل", enabled_txt)
    draw_kv(f"💳 حالة الاشتراك ({month})", sub_status)

    y -= 10
    c.line(40, y, w-40, y)
    y -= 20

    draw_kv("🏦 البنك", bank_name)
    draw_kv("🏷️ IBAN", iban)
    draw_kv("📱 STC Pay", stc_pay)

    y -= 10
    c.line(40, y, w-40, y)
    y -= 20

    draw_kv("🆔 رقم الهوية", national_id)
    draw_kv("🧾 السجل التجاري", cr_number)
    draw_kv("🏷️ الرقم الضريبي", vat_number)

    # تذييل مشابه للفواتير
    try:
        c.setFont(font_name, 11)
        c.setFillGray(0.35)
        c.drawCentredString(w/2, 45, "الخدمات المساندة GO")
        c.setFont(font_name, 9)
        c.drawCentredString(w/2, 30, "PParts منصة قطع غيار السيارات — سجل داخلي")
    except Exception:
        pass
    ...
    try:
        with open(fn, "rb") as f:
            await context.bot.send_document(
                ...
                caption=f"📄 سجل التاجر — ID {trader_id}",
            )
    except Exception as e:
        _swallow(e)
