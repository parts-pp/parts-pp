...
async def send_invoice_pdf(
    ...
    def _load_item_prices_map() -> dict:
        ...
        - أو خريطة محفوظة في order (json أو dict) مثل: quote_item_prices / item_prices / goods_item_prices
        المفاتيح المتوقعة: "1","2"... (حسب ترتيب البنود)
        """
        pm = {}

        # 1) من حقول order (JSON/dict)
        for k in ("quote_item_prices", "item_prices", "goods_item_prices", "quote_items_prices"):
            ...
            try:
                if isinstance(raw, dict):
                    ...
                    sraw = str(raw).strip()
                    src = json.loads(sraw) if (sraw.startswith("{") and sraw.endswith("}")) else None
    ...
    if invoice_for_norm == "trader":
        ...
        if items:
            for i, it in enumerate(items, start=1):
                ...
                # ✅ جديد: إذا ما له سعر نكتب غير متوفرة (بدون حذف البند)
                if pr:
        ...
        # ✅ أبعاد نفس الجدول السابق + عمود جديد بدون تغيير الشكل العام
        col_w = [0.18 * full_w, 0.28 * full_w, 0.46 * full_w, 0.08 * full_w]
    else:
    ...
    parts_tbl.setStyle(TableStyle([
        ...
        ("BOTTOMPADDING", (0, 0), (-1, -1), 1),
        # رقم # (آخر عمود) نفس تضييق الحشو السابق
        ("RIGHTPADDING", (len(col_w) - 1, 0), (len(col_w) - 1, -1), 1.5),
        ("LEFTPADDING", (len(col_w) - 1, 0), (len(col_w) - 1, -1), 1.5),
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _money_tail_simple(x: object, fb: str = "0") -> str:
        try:
            s = _s(x).replace(",", "").strip()
            if not s:
                s = fb
            # تنظيف رموز محتملة
            s = re.sub(r'^\s*(ر\.?\s*س|ر\.س|SAR|SR)\s*', '', s, flags=re.I).strip()
            s = re.sub(r'\s*(ر\.?\s*س|ر\.س|SAR|SR)\s*$', '', s, flags=re.I).strip()
            s = s or fb
            return f"{s} ﷼"
        except Exception:
    ...
    if items:
        for i, it in enumerate(items, start=1):
            ...
            pr = _pick_item_price_local(i, it, pm)
            if pr:
                priced += 1
                parts_lines.append(f"✅ {i}) {nm} | {pn} | {_money_tail_simple(pr, fb='0')}")
            else:
                unpriced += 1
                parts_lines.append(f"❌ {i}) {nm} | {pn} | غير متوفرة")
    else:
        parts_lines.append("—")

    total_cnt = priced + unpriced
    parts_block = "\n".join(parts_lines)

    summary_block = (
        f"📌 ملخص العرض:\n"
        f"✅ المشمول بالعرض: {priced} من {total_cnt}\n"
        f"❌ غير متوفر: {unpriced}"
    )

    # حماية من طول رسالة تيليجرام
    try:
        if len(parts_block) > 2600:
            parts_block = parts_block[:2550].rstrip() + "\n…"
    except Exception:
        pass
