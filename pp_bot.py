...
async def send_invoice_pdf(
    ...
    # ✅ (تعديل مطلوب): عمود سعر القطعة + إظهار القطع المسعّرة فقط في فاتورة التاجر
    ...
    if invoice_for_norm == "trader":
        ...
        if items:
            for i, it in enumerate(items, start=1):
                ...
                # ✅ فقط المسعّر يظهر في فاتورة التاجر
                if not pr:
                    continue

                shown_any = True
                parts_rows.append([
                    Paragraph(_ar(html.escape(_money_tail(pr, fb="0"))), parts_cell_r),
