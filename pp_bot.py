    ...
    if msg:
        ...
        if isinstance(sess, dict):
            ...
            if expired:
                ...
                if peer_id and order_id_sess and kind and file_id:
                    # تحديث آخر تفاعل
                    try:
                    ...
                    try:
                        ...
                        if kind == "photo":
                            await context.bot.send_photo(chat_id=peer_id, photo=file_id, caption=caption, reply_markup=kb_end)
                        elif kind == "video":
                            await context.bot.send_video(chat_id=peer_id, video=file_id, caption=caption, reply_markup=kb_end)
                        elif kind == "document":
                            await context.bot.send_document(chat_id=peer_id, document=file_id, caption=caption, reply_markup=kb_end)
                        elif kind == "voice":
                            await context.bot.send_voice(chat_id=peer_id, voice=file_id, caption=caption, reply_markup=kb_end)
                        elif kind == "audio":
                            await context.bot.send_audio(chat_id=peer_id, audio=file_id, caption=caption, reply_markup=kb_end)
                        elif kind == "video_note":
                            await context.bot.send_video_note(chat_id=peer_id, video_note=file_id, reply_markup=kb_end)
                        else:
                            await context.bot.send_message(chat_id=peer_id, text=caption, reply_markup=kb_end)
                ...
                if stage in (STAGE_JOIN_CR, STAGE_JOIN_LICENSE):
                    ...
                    if stage == STAGE_JOIN_CR:
                        ...
                        await msg.reply_text(
                            ...
                            parse_mode="HTML",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("✖️ إنهاء", callback_data="pp_join_done")]
                            ]),
                            disable_web_page_preview=True,
                        )
                        return
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
        ...
        try:
            ...
            if client_id2:
                ...
                # ✅ تحديد وضع دفع التاجر (manual/link) لعزل مسارات التحويل بالكامل
                trader_pay_mode = "manual"
                try:
                    tid2 = int(o2.get("accepted_trader_id") or 0)
                    tp2 = get_trader_profile(tid2) if tid2 else {}
                    trader_pay_mode = (str((tp2 or {}).get("payment_mode") or "")).strip().lower() or "manual"
                except Exception:
                    trader_pay_mode = "manual"

                if trader_pay_mode in ("link", "pay_link", "payment_link"):
                    link2 = (str(o2.get("goods_payment_link") or "")).strip()
                    # في وضع الرابط: لا نظهر كيبورد التحويل نهائياً
                    if link2.startswith("http://") or link2.startswith("https://"):
                        client_lines_link = list(client_lines)
                        try:
                            if client_lines_link:
                                client_lines_link[-1] = "⬇️ <b>الخطوة القادمة</b>: افتح رابط الدفع ثم أرسل إيصال السداد هنا لإكمال الإجراء"
                        except Exception:
                            pass

                        # اربط مرحلة ايصال قيمة القطع عند العميل مباشرة
                        try:
                        ...
                        # إذا التاجر في وضع رابط ولم يرسل الرابط بعد: لا نرسل خيارات التحويل للعميل
                        client_lines_wait = list(client_lines)
                        try:
                            if client_lines_wait:
                                client_lines_wait[-1] = "⏳ <b>بانتظار رابط الدفع من المتجر</b>
سيصلك رابط الدفع هنا داخل المنصة، وبعد الدفع أرسل الإيصال لإكمال الإجراء."
                        except Exception:
                            pass
                        await context.bot.send_message(
                            chat_id=client_id2,
                            text="\n".join(client_lines_wait),
                            parse_mode="HTML",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("💬 مراسلة التاجر", callback_data=f"pp_chat_trader|{order_id2}")],
                            ]),
...
async def _resolve_and_show_order(context: ContextTypes.DEFAULT_TYPE, user_id: int, tail4: str):
    ...
    for it in matches_info[:12]:
        kb_rows.append([InlineKeyboardButton(f"فتح {it['order_id']}", callback_data=f"ord_open|{it['order_id']}")])
...
async def pp_rb_stop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=uid,
            text=f"⛔ تم إيقاف إعادة النشر تلقائيًا لهذا الطلب\n🧾 رقم الطلب: {_order_id_link_html(order_id)}",
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
    except Exception as e:
