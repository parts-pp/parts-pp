...
async def _send_client_payment_preview(
    ...
    ship_inc_txt = _s(order.get("ship_included"))  # قد يكون "مشمولة"/"غير مشمولة" أو yes/no
    ...
    # ✅ اسم التاجر + اسم المتجر (للعرض أعلى الرسالة) + تحديد trader_id للإرسال له أيضاً
    trader_name = _s(order.get("accepted_trader_name") or order.get("quoted_trader_name"))
    trader_company = ""
    try:
    ...
    # ✅ حساب الشحن رقمياً (لإضافته إلى إجمالي الفاتورة قبل الدفع الأخير)
    ship_included_norm = str(order.get("ship_included") or "").strip().lower()
    raw_shipping_fee = order.get("shipping_fee_sar")
    if raw_shipping_fee is None or str(raw_shipping_fee).strip() == "":
        raw_shipping_fee = 0 if ship_included_norm in ("yes", "true", "1", "included", "مشمولة") else 25
    ship_fee_val = _to_float(raw_shipping_fee)
    ship_fee_total_txt = _money(raw_shipping_fee)

    # عنوان/حالة حسب النطاق
    if pay_scope == "goods":
    ...
    # ✅ إجمالي الفاتورة ليظهر دائمًا مع رقم الطلب (للعميل/التاجر)
    if pay_scope == "goods":
    ...
    # ---------- small summary for goods (بدون تكرار مبالغ) ----------
    parts_summary = ""
    ...
    if pay_scope == "goods" and trader_id0:
        try:
            tid_int = int(trader_id0)
            if tid_int and tid_int not in targets:
                targets.append(tid_int)
        except Exception:
            pass
    ...
    if client_id:
        try:
            await context.bot.send_message(
                ...
                text=(
                    "👋 عزيزي العميل\n"
                    "✅ وصلنا عرض السعر الرسمي للطلب أدناه:\n\n"
                    f"{official_with_trader}\n\n"
                    "🟩 للإجراء:\n"
                    "• ✅ اضغط (أقبل العرض) لإكمال الشراء\n"
                    "• ❌ اضغط (أرفض العرض) لرفضه"
                ),
...
async def goods_receipt_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ship_fee_raw is None or str(ship_fee_raw).strip() == "":
        ship_fee_raw = 0 if ship_included_norm in ("yes", "true", "1", "included", "مشمولة") else 25
...
async def _resolve_and_show_order(context: ContextTypes.DEFAULT_TYPE, trader_id: int, tail4: str):
    tail4 = str(tail4).strip()
    if not tail4.isdigit():
        return

    # اجلب كل طلبات التاجر
    try:
        orders = list_orders_for_trader(int(trader_id or 0)) or []
    except Exception:
        orders = []
    ...
    # لو فيه طلب واحد فقط → اعرضه
    if len(matches) == 1:
        await _show_order_panel_private(context, trader_id, matches[0])
        return
        ...
        if _support_should_close_by_time(ud):
            ...
            if cur_stage and cur_stage != STAGE_NONE:
                ...
                if msg:
                    ...
                    # ✅ استخراج/تحديد رقم الطلب (شرط إلزامي حتى لا تصبح مراسلات عامة)
                    ...
                    try:
                        m = re.search(r"(PP-\d{6}-\d{4})", msg, flags=re.IGNORECASE)
                        if m:
                            order_id_support = (m.group(1) or "").strip()
                    except Exception:
                        pass

                    # ✅ إذا ما فيه رقم طلب: لا نرسل للإدارة (منع التكدس)
                    if not order_id_support:
                        try:
                            await update.message.reply_text(
                                "🟥 لإرسال طلبك للمنصة اكتب رقم الطلب داخل رسالتك مثل:\nPP-260208-0003\nثم اكتب سؤالك.",
                                reply_markup=_support_kb(),
                                disable_web_page_preview=True,
                            )
                        except Exception:
                    ...
                    # حفظه للرسائل القادمة داخل نفس جلسة الدعم
                    try:
                            ...
                            await context.bot.send_message(
                                ...
                                reply_markup=InlineKeyboardMarkup(
                                    [[
                                        InlineKeyboardButton(
                                            "✉️ رد",
                                            callback_data=f"pp_support_reply|{order_id_support}|{user_id}",
                                        )
                                    ]]
                                ),
                                disable_web_page_preview=True,
