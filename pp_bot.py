...
STAGE_ADMIN_CHAT = globals().get("STAGE_ADMIN_REPLY", STAGE_SUPPORT_ADMIN_REPLY)
STAGE_TRADER_CHAT_ADMIN = STAGE_ADMIN_TRADER_MSG
...
def trader_quote_type_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✔️ وكالة           ", callback_data=f"ppq_type|{order_id}|agency")],
        [InlineKeyboardButton("✔️ وكلاء محليين    ", callback_data=f"ppq_type|{order_id}|local_dealers")],
        [InlineKeyboardButton("✔️ تجاري           ", callback_data=f"ppq_type|{order_id}|aftermarket")],
        [InlineKeyboardButton("✔️ مختلط           ", callback_data=f"ppq_type|{order_id}|mixed")],
    ])
...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    try:
        ...
        raw = str(order.get("accepted_at_utc") or "").strip()
        accept_key = (re.sub(r"[^0-9]", "", raw)[:14] or "0")
    except Exception:
        ost = ""
        inv_file = ""
        gps = ""
    ...
    if ost in ("", "new", "accepted", "quoted"):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("🟡 جاري تجهيز الطلب", callback_data=f"pp_trader_status|prep|{order_id}|{accept_key}")],
        ])

    # 2) بعد تجهيز
    if ost in ("preparing", "prep"):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready|{order_id}|{accept_key}")],
        ])
    ...
    if ost in ("ready_to_ship", "ready"):
        ...
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("🚚 تم شحن الطلب", callback_data=f"pp_trader_status|shipped|{order_id}|{accept_key}")],
        ])

    # 4) بعد الشحن
    if ost in ("shipped",):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ تأكيد التسليم بنجاح", callback_data=f"pp_trader_status|delivered|{order_id}|{accept_key}")],
...
def trader_received_notice_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✅ إشعار الإدارة: تم التسليم بنجاح", callback_data=f"pp_trader_status|delivered|{order_id}|{accept_key}")],
        [InlineKeyboardButton("🔒 إنهاء / إقفال الطلب (منجز)", callback_data=f"pp_order_finish|{order_id}")],
    ])
...
async def quote_ok_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تحصين: ثبّت التاجر المعتمد في أكثر من عمود (بدون إضافة أعمدة جديدة)
    update_order_fields(order_id, {
        ...
        "assigned_trader_name": tname,

        "quoted_trader_id": tid,
        "quoted_trader_name": tname,
    ...
    try:
        ...
        # ===== إشعار التاجر السابق عند استبدال القبول =====
    try:
        prev_tid = int(str(order.get("accepted_trader_id") or 0).strip() or 0)
    except Exception:
        prev_tid = 0

    # التاجر الجديد (اللي العميل بيقبله الآن)
    new_tid = int(cb_tid or 0)

    if prev_tid and new_tid and prev_tid != new_tid:
        try:
            await context.bot.send_message(
                chat_id=prev_tid,
                text=(
                    "عزيزي التاجر 🤝\n\n"
                    "تم إلغاء موافقة العميل على عرضك بسبب اختيار عرض آخر.\n"
                    "يمكنك تقديم عرض جديد ومحاولة المنافسة في هذا الطلب ✅"
                ),
                disable_web_page_preview=True,
            )
        except Exception:
    ...
    # ✅ إغلاق بصري للتاجر السابق (بدون إرسال رسالة نصية جديدة)
    if switched and prev_tid and prev_tid != tid:
        try:
            # نحاول تعديل الكيبورد لآخر رسالة عنده إن كانت نفس رسالة Callback (أحياناً تكون في الخاص)
            # إذا فشل، ما يهم لأن القفل الحقيقي داخل trader_status_cb
            try:
                kb_locked = InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ تم إلغاء عرضك لصالح عرض آخر", callback_data="pp_ui_close")]
                ])
            except Exception:
                kb_locked = None

            # إن كانت رسالة القبول/العدول وصلت للتاجر السابق وكانت قابلة للتعديل عبر bot (نحاول)
            # لا نعرف message_id هنا، لذلك نكتفي بمحاولة تحديث نفس رسالة الـcallback الحالية إن كانت تخصه (نادراً).
            try:
                if q.message and q.from_user and int(q.from_user.id) == int(prev_tid):
                    await q.message.edit_reply_markup(reply_markup=kb_locked)
            except Exception:
                pass
        except Exception as e:
            _swallow(e)
...
async def quote_no_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        update_order_fields(order_id, {
            ...
            # ✅ تحصين: تصفير أي مراجع قد تُستخدم كمرجع للتاجر المعتمد
            "assigned_trader_id": "",
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # parts: ["pp_trader_status", st, order_id, accept_key?]
    st = parts[1]
    order_id = (parts[2] or "").strip()
    btn_accept_key = (parts[3] if len(parts) >= 4 else "").strip()
    ...
    # =========================
    # ✅ قفل الأزرار القديمة (بعد تبديل القبول لتاجر آخر)
    # =========================
    raw_now = str(order.get("accepted_at_utc") or "").strip()
    now_accept_key = (re.sub(r"[^0-9]", "", raw_now)[:14] or "")
    if now_accept_key:
        if (not btn_accept_key) or (btn_accept_key != now_accept_key):
            try:
                await _alert(
                    q,
                    "⚠️ زر قديم: تم تحويل القبول لتاجر آخر.\n"
                    "افتح آخر رسالة قبول للطلب ثم حدّث الحالة منها.",
                    force=True,
                )
            except Exception:
                pass
    ...
    # =========================
    # ✅ تحصين: تحديد التاجر المعتمد بشكل صارم (Fail-Closed)
    # =========================
    def _int0(x: object) -> int:
        try:
    ...
    try:
        accepted_tid = _int0(order.get("accepted_trader_id"))
    except Exception:
        accepted_tid = 0

    # Fallbacks (بدون أعمدة جديدة)
    if not accepted_tid:
        accepted_tid = _int0(order.get("assigned_trader_id")) or _int0(order.get("quoted_trader_id"))

    # أقوى مرجع إذا موجود عندك
    try:
        at2 = _assigned_trader_id(order_id)
        at2 = _int0(at2)
        if at2:
            accepted_tid = at2
    except Exception:
    ...
    # ✅ سماح للتاجر المقبول فقط أو الأدمن (Popup لطيف بدل رسالة نصية)
    if actor_id not in ADMIN_IDS and actor_id != accepted_tid:
        try:
            await _alert(
                q,
                "عزيزي التاجر 🤝\n\n"
                "تم إلغاء العرض المقدم منك لصالح عرض آخر.\n"
                "بإمكانك تقديم عرض أفضل في الطلبات القادمة ✅",
                force=True,
            )
        except Exception as e:
            _swallow(e)
    ...
    if new_status == "preparing":
        if ost_now not in ("", "new", "accepted", "quoted"):
            await _alert(q, "🔒 لا يمكن تحديث الحالة في هذه المرحلة", force=True)
            return
    ...
    if new_status == "shipped":
        if ost_now not in ("ready_to_ship", "ready"):
            await _alert(q, "🔒 لا يمكن الشحن قبل الوصول لمرحلة (جاهز للشحن)", force=True)
            return

        inv_file = (str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "")).strip()
        if not inv_file:
            await _alert(q, "⚠️ يلزم رفع فاتورة المتجر قبل الشحن", force=True)
            return

        if not goods_paid:
            await _alert(q, "🔒 لا يمكن الشحن قبل تأكيد سداد قيمة القطع", force=True)
            return

    if new_status == "delivered":
        if ost_now not in ("shipped",):
            await _alert(q, "🔒 لا يمكن تأكيد التسليم قبل الشحن", force=True)
            return
...
async def order_finish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ صلاحيات: التاجر المقبول أو الأدمن فقط (Popup لطيف)
    if actor_id not in ADMIN_IDS and actor_id != accepted_tid:
        try:
            await _alert(
                q,
...
async def chat_open_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trader_id = int(accepted_tid or 0)

    if not client_id:
        await _alert(q, "لا يوجد عميل مرتبط بالطلب")
        return

    # لا يوجد تاجر معتمد/مقبول بعد
    ...
    # السماح فقط: العميل صاحب الطلب / التاجر المعتمد / الادمن
    if actor_id not in ADMIN_IDS and actor_id not in (client_id, trader_id):
        # ✅ إذا كان المتطفل تاجر قديم (غالباً) نعطي Popup لطيف بدل نص مصرح
        try:
            await _alert(
                q,
                "عزيزي التاجر 🤝\n\n"
...
async def confirm_received_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    accepted_tid = int(accepted_tid or 0)
