...
def _effective_order_status(o: dict) -> str:
    ...
    يعتمد على حقول واقعية (إلغاء/شحن/استلام/قبول/دفع) بدون أي عشوائية.
    """
    try:
    ...
    # ===== Inference for legacy rows with empty order_status =====
    try:
    ...
    for k in ("delivered_confirmed_at_utc", "delivered_at_utc", "closed_at_utc", "closed_at"):
        try:
            if str((o or {}).get(k) or "").strip():
                # لو وصلنا لمرحلة إغلاق/تأكيد استلام نعتبرها delivered (قانونياً انتهت)
                return "delivered"
        except Exception:
            pass
    ...
    # دفع/تأكيد/تعيين تاجر = دخلت مرحلة تجهيز
    try:
