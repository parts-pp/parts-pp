...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pfee_broadcast":
        ...
        for t in traders:
            ...
            total_registered += 1
            try:
                if await _is_trader_group_member(context, tid):
        ...
        trader_sub_fee = _trader_sub_fee_amount()
        msg = (
            "📣 <b>إعلان سياسة الرسوم للتجار</b>\n\n"
            "هذه الشاشة خاصة بالإدارة فقط.\n"
            "من هنا يمكنك أولًا تعديل رسم الاشتراك الشهري، ثم إرسال الإعلان إلى التجار المسجلين الموجودين في مجموعة التجار، ويصلهم الإشعار في الخاص.\n\n"
            f"💳 قيمة اشتراك التاجر الحالية: <b>{trader_sub_fee}</b> ريال\n"
            f"👥 التجار المسجلون: <b>{total_registered}</b>\n"
            f"✅ الموجودون حاليًا في مجموعة التجار: <b>{total_group_members}</b>"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💳 تعديل اشتراك التاجر", callback_data="pp_admin|setfee|sub")],
            [InlineKeyboardButton("📨 إرسال الإشعار للتجار الآن", callback_data="pp_admin|pfee_broadcast_send")],
            [InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|fees")],
        ])
        await _admin_edit_or_send(q, msg, kb)
        return

    if action == "pfee_broadcast_send":
        sent = 0
        failed = 0
        skipped = 0
        try:
        ...
        for t in traders:
            ...
            try:
                in_group = await _is_trader_group_member(context, tid)
            except Exception:
                in_group = False
            ...
            try:
                ok = await _send_platform_fee_notice_message(
                    context,
                    tid,
                    profile=(get_trader_profile(tid) or t),
                    broadcast=True,
                )
                if ok:
                    sent += 1
        ...
        await _admin_edit_or_send(
            q,
            "📣 <b>تم تنفيذ إعلان سياسة الرسوم للتجار</b>\n\n"
            f"✅ تم الإرسال بالخاص: <b>{sent}</b>\n"
            f"⚠️ تعذر الإرسال: <b>{failed}</b>\n"
            f"⏭️ تم التجاوز لعدم وجوده في مجموعة التجار: <b>{skipped}</b>",
            InlineKeyboardMarkup([
                [InlineKeyboardButton("↩️ رجوع لإعلان السياسة", callback_data="pp_admin|pfee_broadcast")],
                [InlineKeyboardButton("🏠 الرئيسية", callback_data="pp_admin|home")],
            ])
        )
        return
