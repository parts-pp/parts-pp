...
async def goods_pay_stc_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(
        ...
        f"🧾 <b>رقم الطلب</b>: <code>{html.escape(str(order_id))}</code>\n\n"
        "📌 <b>تفاصيل المبالغ</b>:\n"
        f"🧩 قيمة القطع: <b>{html.escape(goods_txt)}</b>\n"
        f"🚚 الشحن: <b>{html.escape(ship_txt)}</b>\n"
        f"💰 إجمالي القطع + الشحن: <b>{html.escape(total_txt)}</b>\n"
...
async def notify_admins_goods_receipt(
    ...
    def _money_txt(x: object) -> str:
        # يعتمد على _fmt_money الموجود عندك إن توفر
        try:
            t = _fmt_money(_s(x))
            return (t or "").strip()
        except Exception:
    ...
    try:
        ...
        # ✅ لا نستخدم price_sar نهائياً هنا (لأنه رسوم منصة)
        # نحسب الإجمالي = القطع + (الشحن إن لم يكن مشمولاً)
        ship_for_total = "0" if ship_included else (ship_fee or "0")

        try:
            # موجود عندك في نفس الملف حسب التعديلات السابقة
            g_num, s_num, t_num = _calc_totals(goods_amt or "0", ship_for_total or "0")
            total_amt = _s(t_num)
        except Exception:
            # fallback آمن بدون أي خلط برسوم المنصة
            try:
    ...
    if ship_included:
        ...
        ship_txt = _money_txt(ship_fee) or "—"

    total_txt = _money_txt(total_amt) or "—"

    # تحويل الحالة عربي (إن كانت الدالة موجودة عندك)
    try:
    ...
    caption = (
        ...
        f"👤 العميل: {user_name or '—'}{f' ({user_id})' if user_id else ''}\n"
        f"📞 الجوال: {phone or '—'}\n"
        f"🏙 المدينة: {city or '—'}\n"
