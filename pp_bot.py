...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_status == "ready_to_ship":
        ...
        # ✅ افتح مرحلة إدخال وسيلة الدفع (فاتورة PDF/صورة أو رابط دفع إلكتروني)
        # أي خيار جديد يستبدل أي بيانات سابقة لنفس الطلب (حتى بعد إعادة التشغيل).
        try:
            ud2 = get_ud(context, actor_id)
            ud2["tsu_kind"] = "seller_invoice"
            ud2["tsu_order_id"] = order_id
            ud2.pop("tsu_paylink_replace", None)
        ...
        try:
            ...
            msg_private = (
                "🟢 <b>الطلب جاهز للشحن</b> — <b>اختر طريقة الدفع</b>\n\n"
                ...
                "• 📄 فاتورة متجر: ارفع <b>PDF/صورة</b> (فاتورة أولية/صورة شاشة).\n"
                "• 🔗 رابط دفع: ارسل <b>رابط الدفع</b> كنص.\n\n"
                "✅ <b>مهم</b>: أي فاتورة/رابط جديد سيتم اعتماده مباشرة ويستبدل السابق لنفس الطلب."
            )
