...
FEE_FREE_PREV_SUB_KEY = "platform_fee_free_prev_trader_sub_sar"
FEE_FREE_PREV_LOW_KEY = "platform_fee_free_prev_low_sar"
FEE_FREE_PREV_HIGH_KEY = "platform_fee_free_prev_high_sar"

def _safe_amount_int(v, default: int = 0) -> int:
    try:
...
def _is_free_amount(v) -> bool:
    return _safe_amount_int(v, 0) <= 0

def _platform_fees_are_free() -> bool:
    try:
        if _is_platform_fee_free_mode():
            return True
    except Exception:
        pass
    return _is_free_amount(_platform_fee_low_amount()) and _is_free_amount(_platform_fee_high_amount())

def _trader_subscription_fees_are_free() -> bool:
    return _is_free_amount(_trader_sub_fee_amount())

def _sync_platform_fee_free_flag_from_amounts() -> bool:
    enabled = _platform_fees_are_free()
    try:
        _set_platform_fee_free_mode(bool(enabled))
    except Exception:
        pass
    return bool(enabled)

def _apply_fee_free_snapshot(enable: bool) -> tuple[bool, int, int, int]:
    current_sub = _safe_amount_int(_trader_sub_fee_amount(), 0)
    current_low = _safe_amount_int(_platform_fee_low_amount(), 0)
    current_high = _safe_amount_int(_platform_fee_high_amount(), 0)

    if enable:
        try:
            if not _is_platform_fee_free_mode():
                set_setting(FEE_FREE_PREV_SUB_KEY, str(current_sub))
                set_setting(FEE_FREE_PREV_LOW_KEY, str(current_low))
                set_setting(FEE_FREE_PREV_HIGH_KEY, str(current_high))
        except Exception:
            pass

        set_setting(TRADER_SUB_FEE_KEY, "0")
        set_setting(PLATFORM_FEE_LOW_KEY, "0")
        set_setting(PLATFORM_FEE_HIGH_KEY, "0")
        _set_platform_fee_free_mode(True)
        return True, 0, 0, 0

    prev_sub = _safe_amount_int(get_setting(FEE_FREE_PREV_SUB_KEY, str(current_sub)), current_sub)
    prev_low = _safe_amount_int(get_setting(FEE_FREE_PREV_LOW_KEY, str(current_low)), current_low)
    prev_high = _safe_amount_int(get_setting(FEE_FREE_PREV_HIGH_KEY, str(current_high)), current_high)

    set_setting(TRADER_SUB_FEE_KEY, str(prev_sub))
    set_setting(PLATFORM_FEE_LOW_KEY, str(prev_low))
    set_setting(PLATFORM_FEE_HIGH_KEY, str(prev_high))
    _set_platform_fee_free_mode(False)
    return False, prev_sub, prev_low, prev_high

async def _activate_zero_trader_subscription(context: ContextTypes.DEFAULT_TYPE, trader_id: int, month: str, actor_name: str = "النظام") -> dict:
    month = str(month or month_key_utc()).strip()
    actor_name = (actor_name or "النظام").strip() or "النظام"
    paid_at = utc_now_iso()
    row = {}
    try:
        row = upsert_trader_subscription(int(trader_id or 0), month, {
            "amount_sar": 0,
            "payment_method": "free",
            "payment_status": "confirmed",
            "paid_at_utc": paid_at,
            "confirmed_by_admin_id": "0",
            "confirmed_by_admin_name": actor_name,
            "receipt_file_id": "",
            "receipt_kind": "",
        }) or {}
    except Exception as e:
    ...
    try:
        set_trader_enabled(int(trader_id or 0), True)
    except Exception as e:
        _swallow(e)
...
async def send_trader_subscription_invoice_pdf(
    ...
    """فاتورة اشتراك التاجر بنفس محرك وشكل فاتورة المنصة تقريبًا، لكن ببيانات الاشتراك بدل القطع."""
    try:
        import tempfile
    ...
    month = _s(month) or month_key_utc()
    try:
        amount_sar = int(float(amount_sar or _trader_sub_fee_amount()))
    except Exception:
        amount_sar = _trader_sub_fee_amount()

    try:
        sub_row = get_trader_subscription(int(trader_id or 0), month) or {}
    except Exception as e:
        _swallow(e)
        sub_row = {}
    ...
    invoice_no = _s(sub_row.get("invoice_no")) or f"SUBINV-{month.replace('-', '')}-{int(trader_id or 0)}"
    paid_at_txt = _s(paid_at_utc) or _s(sub_row.get("paid_at_utc")) or utc_now_iso()
    confirmed_by_name = _s(sub_row.get("confirmed_by_admin_name")) or "الإدارة"
    payment_method_norm = _s(payment_method) or _s(sub_row.get("payment_method"))
    pay_method_txt = _pay_method_ar(payment_method_norm or "")
    payment_status_txt = _pay_status_ar(_s(sub_row.get("payment_status")) or "confirmed")

    trader_name = _s(tp.get("display_name") or tp.get("name") or tp.get("full_name")) or "التاجر"
    trader_company = _s(tp.get("company_name") or tp.get("shop_name") or tp.get("store_name")) or "—"
    trader_phone = _s(tp.get("shop_phone") or tp.get("phone") or tp.get("mobile") or tp.get("shop_mobile") or tp.get("store_phone")) or "—"
    trader_cr_no = _s(tp.get("cr_no") or tp.get("cr") or tp.get("cr_number") or tp.get("commercial_register") or tp.get("commercial_registration")) or "—"
    trader_vat_no = _s(tp.get("vat_no") or tp.get("vat") or tp.get("vat_number") or tp.get("tax_no") or tp.get("tax_number")) or "—"
    trader_iban = _s(tp.get("iban")) or "—"
    trader_bank = _s(tp.get("bank_name")) or "—"
    trader_stc = _s(tp.get("stc_pay")) or "—"
    ...
    inv_title = "فاتورة اشتراك منصة للتاجر"
    platform_bar = "منصة قطع غيار PPARTS"
    ...
    def _inv_no_display(full: str) -> str:
        ...
        if not s:
            return "—"
        tail4 = s[-4:] if len(s) >= 4 else s
        return f"SUB-{month.replace('-', '')}-{tail4}"

    inv_no_disp = _inv_no_display(invoice_no)

    tmpdir = tempfile.gettempdir()
    pdf_path = os.path.join(tmpdir, f"pp_trader_sub_invoice_{trader_id}_{month.replace('-', '')}_{uuid.uuid4().hex[:6]}.pdf")
    ...
    C_BORDER = colors.HexColor("#CBD5E1")
    C_TEXT = colors.HexColor("#0B1220")
    C_DARK = colors.HexColor("#4B5563")
    C_DARK_2 = colors.HexColor("#6B7280")
    BADGE_BG = colors.HexColor("#F3F4F6")
    SEC_HDR = colors.HexColor("#374151")
    SEC_HDR_2 = colors.HexColor("#6B7280")
    STAMP = colors.HexColor("#DC2626")
    ROW_TINT1 = "#F9FAFB"
    ROW_TINT2 = "#F3F4F6"
    ...
    styles = getSampleStyleSheet()
    kv_label = ParagraphStyle("sub_kv_label", parent=styles["Normal"], alignment=TA_RIGHT, fontSize=8.6, leading=10.2, fontName=font_name, textColor=C_TEXT)
    kv_value = ParagraphStyle("sub_kv_value", parent=styles["Normal"], alignment=TA_RIGHT, fontSize=8.6, leading=10.2, fontName=font_name, textColor=C_TEXT)
    center = ParagraphStyle("sub_center", parent=styles["Normal"], alignment=TA_CENTER, fontSize=11.4, leading=12.6, fontName=font_name)
    tiny_c = ParagraphStyle("sub_tiny_c", parent=styles["Normal"], alignment=TA_CENTER, fontSize=8.8, leading=10.2, fontName=font_name)
    ...
    title_bar = Table([[[P(f"<b>{platform_bar}</b>    |    <b>{inv_title}</b>", ParagraphStyle("sub_tbar", parent=center, textColor=colors.white, fontSize=10.6, leading=12.0, fontName=font_name))]]], colWidths=[full_w])
    title_bar.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), C_DARK),
        ("BOX", (0, 0), (-1, -1), 0.0, colors.white),
        ("LINEBELOW", (0, 0), (-1, 0), 1.6, _with_alpha(C_DARK_2, 0.95)),
    ...
    badges = Table([[[
        P(f"رقم الفاتورة: <b>{inv_no_disp}</b>", tiny_c),
        P(f"الشهر: <b>{html.escape(month)}</b>", tiny_c),
        P(f"{inv_date}  {inv_time} (KSA)", tiny_c),
    ]]], colWidths=[0.40 * full_w, 0.30 * full_w, 0.30 * full_w])
    badges.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), _with_alpha(BADGE_BG, 0.58)),
    ...
    def section_kv(title: str, rows: list):
        hdr = Table([[[P(f"<b>{title}</b>", ParagraphStyle("sub_sh", parent=kv_label, fontSize=9.1, leading=10.6, textColor=colors.white, fontName=font_name))]]], colWidths=[full_w])
        hdr.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), _with_alpha(SEC_HDR, 0.92)),
            ("LINEBELOW", (0, 0), (-1, 0), 1.1, _with_alpha(SEC_HDR_2, 0.92)),
            ("BOX", (0, 0), (-1, -1), 0.6, C_BORDER),
    ...
    rows_trader = [
        ("اسم التاجر", trader_name),
        ("اسم المتجر", trader_company),
        ("رقم معرف التاجر", str(int(trader_id or 0))),
        ("رقم اتصال المتجر", trader_phone),
        ("رقم السجل التجاري", trader_cr_no),
        ("الرقم الضريبي", trader_vat_no),
    ]
    section_kv("بيانات التاجر", rows_trader)

    rows_payment = [
        ("نوع الفاتورة", "اشتراك منصة للتاجر"),
        ("الشهر", month),
        ("طريقة السداد", pay_method_txt),
        ("حالة السداد", payment_status_txt),
        ("تاريخ التحقق", _fmt_dt_ksa(paid_at_txt)),
        ("تم التحقق بواسطة", confirmed_by_name),
        ("رقم المرجع", f"SUB-{int(trader_id or 0)}-{month}"),
    ]
    section_kv("بيانات السداد", rows_payment)

    sec_accounts = Table([[[P("<b>معرّفات الحساب ووسائل السداد</b>", ParagraphStyle("sub_sh2", parent=kv_label, fontSize=9.0, leading=10.5, textColor=colors.white, fontName=font_name))]]], colWidths=[full_w])
    sec_accounts.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), _with_alpha(SEC_HDR, 0.92)),
        ("LINEBELOW", (0, 0), (-1, 0), 1.1, _with_alpha(SEC_HDR_2, 0.92)),
        ("BOX", (0, 0), (-1, -1), 0.6, C_BORDER),
        ("LEFTPADDING", (0, 0), (-1, -1), 6),
    ...
    story.append(sec_accounts)

    parts_cell_r = ParagraphStyle("sub_parts_cell_r", parent=styles["Normal"], alignment=TA_RIGHT, fontSize=8.2, leading=9.6, fontName=font_name)
    parts_cell_num = ParagraphStyle("sub_parts_cell_num", parent=styles["Normal"], alignment=TA_RIGHT, fontSize=8.2, leading=9.6, fontName=font_name)

    accounts_rows = [[
        P("<b>القيمة</b>", ParagraphStyle("sub_ph0", parent=parts_cell_r, textColor=colors.white)),
        P("<b>البيان</b>", ParagraphStyle("sub_ph1", parent=parts_cell_r, textColor=colors.white)),
        P("<b>#</b>", ParagraphStyle("sub_ph2", parent=parts_cell_r, textColor=colors.white)),
    ]]

    account_items = [
        (PP_BENEFICIARY or "—", "اسم المستفيد"),
        (PP_BANK_NAME or "—", "البنك"),
        (PP_IBAN or "—", "IBAN المنصة"),
        (PP_STC_PAY or "—", "STC Pay المنصة"),
        (trader_bank or "—", "بنك التاجر"),
        (trader_iban or "—", "IBAN التاجر"),
        (trader_stc or "—", "STC Pay التاجر"),
    ]

    for i, (val, lbl) in enumerate(account_items, start=1):
        accounts_rows.append([
            Paragraph(_ar(html.escape(_cell_clip(val, 70) or "—")), parts_cell_r),
            Paragraph(_ar(html.escape(lbl)), parts_cell_r),
            Paragraph(_ar(str(i)), parts_cell_num),
        ])

    accounts_tbl = Table(accounts_rows, colWidths=[0.56 * full_w, 0.36 * full_w, 0.08 * full_w], rowHeights=[0.62 * cm] * len(accounts_rows), repeatRows=1)
    accounts_tbl.setStyle(TableStyle([
        ...
        ("TOPPADDING", (0, 0), (-1, -1), 1),
        ("BOTTOMPADDING", (0, 0), (-1, -1), 1),
        ("RIGHTPADDING", (2, 0), (2, -1), 0.8),
        ("LEFTPADDING", (2, 0), (2, -1), 0.8),
        ("ROWBACKGROUNDS", (0, 1), (-1, -1), [_hexA(ROW_TINT1, 0.18), _hexA(ROW_TINT2, 0.12)]),
    ]))
    story.append(accounts_tbl)
    story.append(Spacer(1, 3))

    sec_fin = Table([[[P("<b>الملخص المالي</b>", ParagraphStyle("sub_sh3", parent=kv_label, fontSize=9.0, leading=10.5, textColor=colors.white, fontName=font_name))]]], colWidths=[full_w])
    sec_fin.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), _with_alpha(SEC_HDR, 0.92)),
        ("LINEBELOW", (0, 0), (-1, 0), 1.1, _with_alpha(SEC_HDR_2, 0.92)),
        ("BOX", (0, 0), (-1, -1), 0.6, C_BORDER),
    ...
    fin_lbl_w = ParagraphStyle("sub_fin_lbl_w", parent=tiny_c, alignment=TA_RIGHT, fontSize=9.0, leading=10.2, fontName=font_name, textColor=colors.white)
    fin_lbl_d = ParagraphStyle("sub_fin_lbl_d", parent=tiny_c, alignment=TA_RIGHT, fontSize=9.0, leading=10.2, fontName=font_name, textColor=C_TEXT)
    fin_amt_w = ParagraphStyle("sub_fin_amt_w", parent=tiny_c, alignment=TA_RIGHT, fontSize=10.0, leading=11.0, fontName=font_name, textColor=colors.white)
    fin_amt_d = ParagraphStyle("sub_fin_amt_d", parent=tiny_c, alignment=TA_RIGHT, fontSize=10.0, leading=11.0, fontName=font_name, textColor=C_TEXT)

    money_box = Table([
        [P("<b>الإجمالي</b>", fin_lbl_w), P("<b>رسوم الاشتراك</b>", fin_lbl_d)],
        [Paragraph(_money_tail(amount_sar, fb="0"), fin_amt_w), Paragraph(_money_tail(amount_sar, fb="0"), fin_amt_d)],
    ], colWidths=[0.45 * full_w, 0.55 * full_w])
    money_box.setStyle(TableStyle([
        ("BOX", (0, 0), (-1, -1), 0.8, C_BORDER),
        ("INNERGRID", (0, 0), (-1, -1), 0.35, C_BORDER),
        ...
        ("TOPPADDING", (0, 0), (-1, -1), 6),
        ("BOTTOMPADDING", (0, 0), (-1, -1), 6),
        ("BACKGROUND", (0, 0), (0, 1), _with_alpha(C_DARK, 0.82)),
        ("BACKGROUND", (1, 0), (1, 1), _hexA("#E5E7EB", 0.25)),
        ("LINEABOVE", (0, 1), (-1, 1), 0.6, C_BORDER),
    ]))
    story.append(money_box)
    story.append(Spacer(1, 2))
        ...
        try:
            if logo_path and os.path.exists(logo_path):
                ...
                try:
                    ...
                    pass
                canvas.drawImage(img, x, y, width=wm_w, height=wm_h, mask='auto', preserveAspectRatio=True, anchor='c')
                try:
                    canvas.setFillAlpha(1)
                except Exception:
                    pass
    ...
    caption = f"📄 {inv_title}\nالشهر: {month}\nرقم الفاتورة: {inv_no_disp}"
    filename = f"PP_Trader_Subscription_{invoice_no}.pdf"
        ...
        try:
            with open(pdf_path, "rb") as f:
                await context.bot.send_document(
                    ...
                    document=f,
                    filename=filename,
                    caption=f"(نسخة) {caption} — trader_id {trader_id}",
                    disable_content_type_detection=False,
                )
        except Exception as e:
            _swallow(e)
...
async def pay_bank_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_SUB_AWAIT_PAY_METHOD:
        ...
        amount = _safe_amount_int(ud.get("sub_amount_sar") or _trader_sub_fee_amount(), 0)
        ud["sub_payment_method"] = "bank_transfer"

        if _is_free_amount(amount):
            await _activate_zero_trader_subscription(context, user_id, month, actor_name="النظام")
            set_stage(context, user_id, STAGE_NONE)
            await q.message.reply_text(
                f"🤍 اهلا {_user_name(q)}\n\n✅ تم تفعيل اشتراكك مباشرة لأن قيمة الاشتراك الحالية <b>0</b> ريال، ولا يلزم أي إيصال.",
                parse_mode="HTML",
                disable_web_page_preview=True,
            )
            return
...
async def pay_link_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_SUB_AWAIT_PAY_METHOD:
        ...
        if _is_free_amount(amount):
            ...
            set_stage(context, user_id, STAGE_NONE)
            await q.message.reply_text(
                f"🤍 اهلا {_user_name(q)}\n\n✅ تم تفعيل اشتراكك مباشرة لأن قيمة الاشتراك الحالية <b>0</b> ريال، ولا يلزم أي رابط دفع أو إيصال.",
                parse_mode="HTML",
                disable_web_page_preview=True,
            )
            return
...
def _trader_can_start_quote_now(user_id: int) -> tuple[bool, str]:
    ...
    1) صلاحية التاجر للتسعير منفصلة عن مجاني رسوم المنصة للعميل
    2) إذا التاجر عليه استثناء/سماح إداري -> يسمح مباشرة
    3) إذا الإدارة وضعت منعًا صريحًا -> يمنع
    4) إذا اشتراك التاجر الشهري = 0 -> يعتبر مجانيًا ومفعلًا
    5) إذا النظام المدفوع مفعل ولا يوجد استثناء -> يشترط اشتراك شهري صالح
    """
    ...
    # ===== 1) إذا اشتراك التاجر مجاني حاليا -> يسمح =====
    if _trader_subscription_fees_are_free():
        return True, ""
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in ADMIN_IDS and stage in (STAGE_ADMIN_SET_TRADER_SUB_FEE, STAGE_ADMIN_SET_PLATFORM_FEE_LOW, STAGE_ADMIN_SET_PLATFORM_FEE_HIGH):
        ...
        try:
            _sync_platform_fee_free_flag_from_amounts()
        except Exception as e:
            _swallow(e)
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in ADMIN_IDS and _trader_is_disabled(uid) and action in ("edit", "orders"):
        # Popup واضح
    ...
    if action == "sub":
        month = month_key_utc()
        amount = _safe_amount_int(_trader_sub_fee_amount(), 0)
        ud["sub_month"] = month
        ud["sub_amount_sar"] = amount
        ud["sub_kind"] = "trader_subscription"

        if _is_free_amount(amount):
            await _activate_zero_trader_subscription(context, uid, month, actor_name="النظام")
            set_stage(context, uid, STAGE_NONE)
            await q.message.reply_text(
                (
                    "💳 <b>اشتراك المنصة للتاجر</b>\n"
                    f"📅 الشهر: <b>{html.escape(month)}</b>\n"
                    "🆓 قيمة الاشتراك الحالية: <b>0</b> ريال\n\n"
                    "✅ تم تفعيل اشتراكك مباشرة بدون دفع أو إيصال لأن قيمة الاشتراك الحالية مجانية."
                ),
                parse_mode="HTML",
                disable_web_page_preview=True,
            )
...
def _trader_quote_access_mode_text(prof: dict) -> str:
    raw = str((prof or {}).get("quote_access_override") or "").strip().lower()
    if raw in ("allow", "allowed", "yes", "true", "1", "on", "enabled", "manual", "admin", "free", "exempt"):
        return "سماح يدوي"
    if raw in ("deny", "denied", "block", "blocked", "forbid", "forbidden", "off", "disabled", "no"):
        return "منع يدوي"
    return "تلقائي"


def _trader_has_quote_restrictions(prof: dict) -> bool:
    p = prof or {}
    return any([
        str(p.get("quote_access_override") or "").strip(),
        str(p.get("subscription_exempt") or "").strip(),
        str(p.get("subscription_exempt_month") or "").strip(),
        str(p.get("subscription_override_until") or "").strip(),
    ])


async def _notify_trader_profile_action(context: ContextTypes.DEFAULT_TYPE, trader_id: int, title: str, body_lines: list[str] | None = None):
    if not trader_id:
        return
    lines = [f"👤 {title}"]
    for ln in (body_lines or []):
        ...
        if txt:
            lines.append(txt)
    lines.append("")
    lines.append("راجع لوحة التاجر لمعرفة الحالة الحالية.")
    lines.append("وللاستفسار استخدم زر مراسلة الإدارة من داخل اللوحة.")
    try:
        await context.bot.send_message(
            chat_id=int(trader_id),
            text="\n".join(lines),
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "fee_free_on":
        _, sub_fee, fee_low, fee_high = _apply_fee_free_snapshot(True)
        await _toast("تم تفعيل العرض المجاني")
        await _admin_edit_or_send(
            q,
            (
                "✅ تم تفعيل العرض المجاني كاختصار إداري.\n\n"
                f"• اشتراك التاجر: <b>{sub_fee}</b> ريال\n"
                f"• رسوم المنصة حتى 5 قطع: <b>{fee_low}</b> ريال\n"
                f"• رسوم المنصة من 6 قطع فأكثر: <b>{fee_high}</b> ريال\n\n"
                "لن يتم طلب دفع أو إيصال لأي قيمة أصبحت 0."
            ),
    ...
    if action == "fee_free_off":
        _, sub_fee, fee_low, fee_high = _apply_fee_free_snapshot(False)
        await _toast("تم إلغاء العرض المجاني")
        await _admin_edit_or_send(
            q,
            (
                "⛔ تم إلغاء العرض المجاني واستعادة آخر القيم المحفوظة.\n\n"
                f"• اشتراك التاجر: <b>{sub_fee}</b> ريال\n"
                f"• رسوم المنصة حتى 5 قطع: <b>{fee_low}</b> ريال\n"
                f"• رسوم المنصة من 6 قطع فأكثر: <b>{fee_high}</b> ريال"
            ),
    ...
    if action == "fees":
        ...
        free_now = "مفعل" if _sync_platform_fee_free_flag_from_amounts() else "غير مفعل"
        msg = (
    ...
    if action == "tview":
        ...
        for srow in (subs or []):
            try:
                x = int(srow.get("trader_id") or 0)
            except Exception:
                x = 0
            if x != int(tid):
                continue
            stv = str(srow.get("payment_status") or "").strip().lower()
            if stv == "confirmed":
                sub_status = "🟩 مدفوع"
            elif stv in ("pending", "awaiting"):
                sub_status = "🟨 قيد التحقق"
        ...
        quote_mode_txt = _trader_quote_access_mode_text(prof)
        perm_exempt = str(prof.get("subscription_exempt") or "").strip().lower() in ("1", "true", "yes", "on", "enabled", "active", "allow", "allowed", "ok", "paid", "approved", "confirmed", "free", "exempt")
        month_exempt_raw = str(prof.get("subscription_exempt_month") or "").strip()
        month_override_until_raw = str(prof.get("subscription_override_until") or "").strip()
        has_restrictions = _trader_has_quote_restrictions(prof)
        ...
        pricing_state_lines = [
            f"🎛️ وضع التسعير: <b>{html.escape(quote_mode_txt)}</b>",
            f"💳 اشتراك الشهر ({html.escape(month)}): <b>{sub_status}</b>",
        ]
        if perm_exempt:
            pricing_state_lines.append("🪪 إعفاء دائم: <b>مفعل</b>")
        if month_exempt_raw:
            pricing_state_lines.append(f"🗓️ إعفاء الشهر: <b>{html.escape(month_exempt_raw)}</b>")
        if month_override_until_raw:
            pricing_state_lines.append(f"⏳ السماح حتى: <b>{html.escape(month_override_until_raw)}</b>")
        if not has_restrictions:
            pricing_state_lines.append("🧹 القيود والاستثناءات: <b>لا يوجد</b>")

        msg = (
            "👤 <b>ملف التاجر الإداري</b>\n\n"
            f"🆔 ID: <b>{tid}</b>\n"
            f"👤 الاسم: <b>{html.escape(label)}</b>\n"
            f"🔘 حالة الحساب: <b>{'🟩 مفعل' if enabled else '🟥 موقوف'}</b>\n"
            + (f"🧾 الرقم الضريبي: <b>{html.escape(vat_no)}</b>\n" if vat_no else "")
            + "\n"
            + "\n".join(pricing_state_lines)
            + "\n\n"
            f"📦 عدد الطلبات: <b>{total_orders}</b>\n"
            f"✅ الطلبات المؤكدة: <b>{confirmed_orders}</b>\n"
        ...
        toggle_quote_label = "🟥 تقييد التسعير" if quote_mode_txt != "منع يدوي" else "🟩 فتح التسعير"
        toggle_quote_mode = "restrict" if quote_mode_txt != "منع يدوي" else "open"

        kb_rows = [
            [InlineKeyboardButton("💬 مراسلة التاجر", callback_data=f"pp_admin|tmsg|{tid}")],
            [InlineKeyboardButton("🧾 السجل التجاري", callback_data=f"pp_admin|tcr|{tid}"), InlineKeyboardButton("🏪 رخصة المحل", callback_data=f"pp_admin|tlic|{tid}")],
            [InlineKeyboardButton(
                "⛔ إيقاف الحساب" if enabled else "✅ تفعيل الحساب",
                callback_data=f"pp_admin|tset|{tid}|{'off' if enabled else 'on'}"
            )],
            [InlineKeyboardButton(toggle_quote_label, callback_data=f"pp_admin|tquote|{tid}|{toggle_quote_mode}")],
            [InlineKeyboardButton(f"🟨 تفعيل التسعير حتى نهاية {month}", callback_data=f"pp_admin|tquote|{tid}|grace_month")],
            [InlineKeyboardButton("🧹 إعادة الوضع الافتراضي", callback_data=f"pp_admin|tquote|{tid}|clear")],
            [InlineKeyboardButton("📚 إيصالات الاشتراك", callback_data=f"pp_admin|tsubs|{tid}"), InlineKeyboardButton("📦 آخر طلبات التاجر", callback_data=f"pp_admin|torders|{tid}")],
            [InlineKeyboardButton("📤 كشف معاملات (CSV)", callback_data=f"pp_admin|texport|{tid}"), InlineKeyboardButton("🧾 سجل التاجر (PDF)", callback_data=f"pp_admin|tledgerpdf|{tid}")],
    ...
    if action == "tquote":
        ...
        fields = {}
        toast = "✅ تم تحديث إعدادات التاجر"
        trader_title = "تحديث إداري"
        trader_lines = []

        if mode in ("allow", "open"):
            fields = {"quote_access_override": "allow"}
            toast = "✅ تم فتح التسعير يدويًا"
            trader_title = "تم فتح التسعير لك"
            trader_lines = ["يمكنك الآن بدء عروض السعر الجديدة."]
        elif mode in ("deny", "restrict"):
            fields = {"quote_access_override": "deny"}
            toast = "⛔ تم تقييد التسعير على التاجر"
            trader_title = "تم تقييد التسعير على حسابك"
            trader_lines = ["لن تتمكن من بدء عروض سعر جديدة حتى إشعار آخر."]
        elif mode in ("month_on", "grace_month"):
            fields = {
                "quote_access_override": "allow",
                "subscription_exempt_month": month,
                "subscription_override_until": month,
            }
            toast = f"🟨 تم منح سماح تشغيلي حتى نهاية {month}"
            trader_title = "تم منحك سماح تشغيلي مؤقت"
            trader_lines = [f"تم فتح التسعير لك حتى نهاية {month}."]
        elif mode in ("perm_on",):
            fields = {"subscription_exempt": "yes", "quote_access_override": "allow"}
            toast = "✅ تم تفعيل الإعفاء الدائم"
            trader_title = "تم تفعيل إعفاء دائم على حسابك"
            trader_lines = ["أصبح حسابك في وضع إعفاء دائم من قيد الاشتراك."]
        elif mode in ("perm_off",):
            fields = {"subscription_exempt": ""}
            toast = "🧹 تم إلغاء الإعفاء الدائم"
            trader_title = "تم إلغاء الإعفاء الدائم"
            trader_lines = ["يخضع حسابك الآن لسياسة الاشتراك المعتادة."]
        elif mode in ("month_off",):
            fields = {
                "subscription_exempt_month": "",
                "subscription_override_until": "",
            }
            toast = "🧹 تم إلغاء السماح الشهري"
            trader_title = "تم إلغاء السماح المؤقت"
            trader_lines = ["ألغت الإدارة السماح التشغيلي المؤقت على حسابك."]
        elif mode in ("reset", "clear"):
            fields = {
            ...
            toast = "♻️ تم إعادة ملف التاجر للوضع الافتراضي"
            trader_title = "تمت إعادة إعدادات الحساب للوضع الافتراضي"
            trader_lines = ["أزالت الإدارة القيود والاستثناءات اليدوية من ملفك."]
        else:
        ...
        try:
            await _notify_trader_profile_action(context, tid, trader_title, trader_lines)
        except Exception as e:
            _swallow(e)
    ...
    if action == "tset":
        ...
        try:
            ...
            try:
                await _notify_trader_profile_action(
                    context,
                    tid,
                    "تم تحديث حالة حسابك",
                    [f"الحالة الحالية: {'مفعل' if enable else 'موقوف مؤقتًا'}."]
                )
            except Exception as e:
                _swallow(e)

            await _toast("✅ تم تحديث حالة الحساب")
        except Exception:
        ...
        await _admin_edit_or_send(
            q,
            f"✅ تم {'تفعيل' if enable else 'إيقاف'} حساب التاجر بنجاح",
            InlineKeyboardMarkup([
                [InlineKeyboardButton("👤 العودة لملف التاجر", callback_data=f"pp_admin|tview|{tid}")],
                [InlineKeyboardButton("↩️ رجوع لقائمة التجار", callback_data="pp_admin|traders_manage")],
                [InlineKeyboardButton("🏠 الرئيسية", callback_data="pp_admin|home")],
            ])
        )
...
async def pp_rb_stop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================================================
    # ✅ مسار إداري مستقل: إيقاف إعادة النشر فقط بدون إلغاء الطلب
    # =========================================================
    if is_admin:
        now_iso = utc_now_iso()
        try:
            update_order_fields(order_id, {
                "rebroadcast_disabled": "1",
                "rebroadcast_disabled_at_utc": now_iso,
                "rebroadcast_disabled_by_id": str(uid),
                "admin_noquote_24h_sent_at_utc": now_iso,
            })
        except Exception as e:
            _swallow(e)
        ...
        for aid in (ADMIN_IDS or []):
            try:
                await context.bot.send_message(
                    chat_id=int(aid),
                    text=(
                        "⛔ <b>تم إيقاف إعادة النشر التلقائي لهذا الطلب</b>\n"
                        f"🧾 رقم الطلب: <b>{html.escape(order_id)}</b>\n"
                        f"👤 بواسطة: <b>{html.escape(_user_name(q))}</b> (<code>{uid}</code>)"
                    ),
                    parse_mode="HTML",
                    reply_markup=notice_kb_for(context, int(aid), order_id, include_chat_trader=False, include_support=True),
                    disable_web_page_preview=True,
        ...
        try:
            await _alert(q, "⛔ تم إيقاف إعادة النشر التلقائي لهذا الطلب", force=True)
        except Exception as e:
            _swallow(e)
    ...
    # ✅ السماح لصاحب الطلب فقط في مسار العميل
    owner_id = 0
    try:
