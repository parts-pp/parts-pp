...
async def traders_join_request_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        _pending_join_store(context, uid, int(jr.chat.id))
    except Exception:
        pass

    full_name = (" ".join([getattr(u, "first_name", "") or "", getattr(u, "last_name", "") or ""])).strip()
    username = (getattr(u, "username", None) or "").strip()
    who = full_name or (f"@{username}" if username else f"ID:{uid}")

    dm_sent = False
    try:
        await context.bot.send_message(
            chat_id=uid,
            text=_join_applicant_text(),
            reply_markup=_join_applicant_kb(uid),
            disable_web_page_preview=True,
        )
        dm_sent = True
    except Forbidden:
    ...
    if TEAM_CHAT_ID:
        try:
            mention = html.escape(who, quote=False)
            admin_lines = []
            admin_lines.append(f"🛡️ <b>{html.escape(_brand_title(), quote=False)}</b>")
            admin_lines.append("")
            admin_lines.append("📌 <b>طلب انضمام جديد لمجموعة التجار</b>")
            admin_lines.append("")
            admin_lines.append(f"👤 الاسم: <b>{mention}</b>")
            admin_lines.append(f"🆔 ID: <code>{uid}</code>")
            if username:
                admin_lines.append(f"🔗 المستخدم: @{html.escape(username, quote=False)}")
            admin_lines.append("")
            admin_lines.append("📎 المطلوب قبل القبول:")
            admin_lines.append("السجل التجاري + الرقم الضريبي + رخصة/إثبات المتجر (PDF/صور) عبر البوت.")
            admin_lines.append("")
            admin_lines.append(_only_bot_line())
            admin_lines.append("")
            admin_lines.append("✅ تم إرسال التعليمات له بالخاص." if dm_sent else "⚠️ لم نستطع مراسلته بالخاص (لم يفتح البوت غالباً).")

            await context.bot.send_message(
                chat_id=TEAM_CHAT_ID,
                text="\n".join(admin_lines),
                parse_mode="HTML",
                reply_markup=_join_admin_kb(uid),
                disable_web_page_preview=True,
            )
        except Exception as e:
            _swallow(e)
...
async def join_admin_action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data.startswith("pp_join_"):
        return
    ...
    info = _pending_join_get(context, uid)
    chat_id = int(info.get("chat_id") or 0)
    if not chat_id:
        try:
            await _alert(q, "⚠️ لم أجد طلب الانضمام (قد يكون تم التعامل معه مسبقاً).", force=True)
        except Exception:
            pass
        return
    ...
    if action == "pp_join_appr":
        ...
        if ok:
            _pending_join_drop(context, uid)
            try:
                await q.message.reply_text(f"✅ تم قبول طلب الانضمام للمتقدم: {uid}")
            except Exception:
                pass
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        f"{_brand_title()}\n\n"
                        "✅ تم قبول طلب انضمامك لمجموعة التجار.\n"
                        "اهلا بك، ونتمنى لك مبيعات موفقة.\n\n"
                        + _only_bot_line()
                    ),
                    disable_web_page_preview=True,
                )
            ...
            try:
                await _alert(q, "⚠️ تعذر قبول الطلب (تحقق من صلاحيات البوت كأدمن).", force=True)
            except Exception:
                pass
        return

    if action == "pp_join_decl":
        ok = False
        try:
            await context.bot.decline_chat_join_request(chat_id=chat_id, user_id=uid)
            ok = True
        except Exception as e:
            _swallow(e)
            ok = False
        ...
        if ok:
            _pending_join_drop(context, uid)
            try:
                await q.message.reply_text(f"⛔ تم رفض طلب الانضمام للمتقدم: {uid}")
            except Exception:
                pass
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        f"{_brand_title()}\n\n"
                        "⛔ تم رفض طلب الانضمام حالياً.\n"
                        "يمكنك إعادة التقديم لاحقاً بعد تجهيز المستندات المطلوبة.\n\n"
                        + _only_bot_line()
                    ),
                    disable_web_page_preview=True,
                )
            ...
            try:
                await _alert(q, "⚠️ تعذر رفض الطلب (تحقق من صلاحيات البوت كأدمن).", force=True)
            except Exception:
                pass
        return
...
# ===== بوابة انضمام التجار (طلبات انضمام المجموعة) =====
STAGE_TRADER_JOIN_DOCS = "pp_trader_join_docs"

def _brand_title() -> str:
    return "🚗 P Parts | بوابة التجار"

def _only_bot_line() -> str:
    try:
        bn = (PP_BOT_USERNAME or "ppartsbot").strip().lstrip("@")
    except Exception:
        bn = "ppartsbot"
    return f"✅ التقديم والطلبات عبر @{bn} فقط"

def _join_open_bot_url(applicant_id: int) -> str:
    bn = _bot_username()
    aid = int(applicant_id or 0)
    return f"https://t.me/{bn}?start=join_{aid}"

def _join_applicant_text() -> str:
    return "\n".join([
        _brand_title(),
        "",
        "هذه المجموعة خاصة بالتجار وموردي قطع الغيار فقط.",
        "",
        "لإكمال طلب الانضمام يرجى رفع المستندات التالية عبر البوت (PDF/صور):",
        "• السجل التجاري",
        "• الرقم الضريبي",
        "• رخصة/إثبات المتجر",
        "",
        "بعد المراجعة سيتم قبول الطلب.",
        "",
        _only_bot_line(),
    ])

def _join_applicant_kb(applicant_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 فتح البوت ورفع المستندات", url=_join_open_bot_url(applicant_id))],
    ])

def _join_admin_kb(applicant_id: int) -> InlineKeyboardMarkup:
    uid = int(applicant_id or 0)
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("👤 مراسلة المتقدم", url=f"tg://user?id={uid}"),
            InlineKeyboardButton("🔗 فتح البوت", url=_join_open_bot_url(uid)),
        ],
        [
            InlineKeyboardButton("✅ قبول الطلب", callback_data=f"pp_join_appr|{uid}"),
            InlineKeyboardButton("⛔ رفض الطلب", callback_data=f"pp_join_decl|{uid}"),
        ],
    ])

def _pending_join_store(context: ContextTypes.DEFAULT_TYPE, applicant_id: int, chat_id: int) -> None:
    try:
        bd = context.application.bot_data.setdefault("pp_pending_join", {})
        bd[int(applicant_id)] = {"chat_id": int(chat_id)}
    except Exception:
        pass

def _pending_join_get(context: ContextTypes.DEFAULT_TYPE, applicant_id: int) -> dict:
    try:
        bd = context.application.bot_data.setdefault("pp_pending_join", {})
        v = bd.get(int(applicant_id)) or {}
        return v if isinstance(v, dict) else {}
    except Exception:
        return {}

def _pending_join_drop(context: ContextTypes.DEFAULT_TYPE, applicant_id: int) -> None:
    try:
        bd = context.application.bot_data.setdefault("pp_pending_join", {})
        bd.pop(int(applicant_id), None)
    except Exception:
        pass

# ===== End بوابة انضمام التجار =====
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================
    # (0) join_ : بوابة انضمام التجار (رفع مستندات)
    # =========================
    if args and isinstance(args[0], str):
        a0 = args[0].strip()
        if a0 == "trader_join":
            ud = get_ud(context, update.effective_user.id)
            set_stage(context, update.effective_user.id, STAGE_TRADER_JOIN_DOCS)
            ud["join_applicant_id"] = int(update.effective_user.id)
            try:
                await update.effective_message.reply_text(
                    _join_applicant_text()
                    + "\n\n⬇️ ارسل المستندات هنا الآن (PDF/صور) ويمكنك كذلك إرسال رسالة نصية توضح اسم المتجر/المدينة.",
                    reply_markup=_join_applicant_kb(int(update.effective_user.id)),
                    disable_web_page_preview=True,
                )
            except Exception:
                try:
                    await update.effective_message.reply_text(_join_applicant_text())
                except Exception:
                    pass
            return

        if a0.startswith("join_"):
            try:
                target_id = int(a0.split("_", 1)[1].strip())
            except Exception:
                target_id = 0

            if target_id and int(update.effective_user.id) != int(target_id):
                try:
                    await update.effective_message.reply_text(
                        f"{_brand_title()}\n\n"
                        "⚠️ هذا الرابط مخصص للمتقدم فقط.\n\n"
                        + _only_bot_line(),
                        disable_web_page_preview=True,
                    )
                except Exception:
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== بوابة انضمام التجار: استقبال مستندات (PDF/صور) ضمن مرحلة رفع المستندات =====
    if stage == STAGE_TRADER_JOIN_DOCS:
        ...
        if msg:
            ...
            try:
                if getattr(msg, "document", None):
                    kind = "document"
                    fid = getattr(msg.document, "file_id", "") or ""
                elif getattr(msg, "photo", None):
                    kind = "photo"
                    fid = getattr(msg.photo[-1], "file_id", "") or ""
            except Exception:
                kind = ""
                fid = ""
            ...
            if kind and fid:
                if TEAM_CHAT_ID:
                    try:
                        head = (
                            f"📎 <b>مستند جديد من متقدم الانضمام</b>\n"
                            f"🛡️ <b>{html.escape(_brand_title(), quote=False)}</b>\n"
                            f"🆔 <code>{uid}</code>\n"
                            + _only_bot_line()
                        )
                        if kind == "document":
                            await context.bot.send_document(
                                chat_id=TEAM_CHAT_ID,
                                document=fid,
                                caption=(head + (f"\n\n📝 {html.escape(cap, quote=False)}" if cap else "")),
                                parse_mode="HTML",
                                reply_markup=_join_admin_kb(uid),
                            )
                        else:
                ...
                try:
                    await msg.reply_text(
                        "✅ تم استلام المستند وتحويله للإدارة للمراجعة.\n"
                        "إذا لديك مستندات أخرى أرسلها الآن.",
                        disable_web_page_preview=True,
                    )
                except Exception:
                    pass
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== بوابة انضمام التجار: استقبال نصوص ضمن مرحلة رفع المستندات =====
    try:
        ud = get_ud(context, user_id)
        stage = ud.get(STAGE_KEY)
    except Exception:
    ...
    if stage == STAGE_TRADER_JOIN_DOCS:
        txt_in = (update.message.text or "").strip()
        if txt_in:
            if TEAM_CHAT_ID:
                try:
                    uid = int(user_id)
                    mention = html.escape(_user_name(update), quote=False)
                    admin_text = (
                        f"📝 <b>ملاحظة من المتقدم</b>\n"
                        f"🛡️ <b>{html.escape(_brand_title(), quote=False)}</b>\n"
                        f"👤 {mention}\n"
                        f"🆔 <code>{uid}</code>\n\n"
                        f"{html.escape(txt_in, quote=False)}\n\n"
                        + _only_bot_line()
                    )
                    await context.bot.send_message(
                        chat_id=TEAM_CHAT_ID,
            ...
            try:
                await update.message.reply_text(
                    "✅ تم استلام رسالتك وتحويلها للإدارة.\n"
                    "⬇️ الآن ارسل المستندات (PDF/صور) هنا، وسيتم تحويلها مباشرة.",
                    disable_web_page_preview=True,
                )
            except Exception:
                pass
...
def build_app():
    ...
    # 🟢 [HANDLER] Traders Group Join Requests
    app.add_handler(ChatJoinRequestHandler(traders_join_request_cb))
    app.add_handler(CallbackQueryHandler(join_admin_action_cb, pattern=r"^pp_join_(appr|decl)\|"))
