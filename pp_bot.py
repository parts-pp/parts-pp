...
async def send_trader_ledger_pdf(
    ...
    # ✅ (02) fallback: fetch username from Telegram if not stored in excel/profile
    if not tuser:
        try:
    ...
    # ✅ (04) صفوف شفافة لإظهار العلامة المائية خلف الجداول
    ...
    # ✅ (01) badges: حل جذري (فصل الكلمة عن القيمة داخل خلايا) لمنع الانعكاس نهائيًا
    LRM = "\u200e"

    def _badge_cell(label_ar: str, value: str, value_ltr: bool = True):
        """
        جدول صغير داخل الخلية: [الكلمة] [القيمة]
        - الكلمة RTL يمين
        - القيمة (أرقام/تاريخ/وقت/يوزر) تُجبر LTR داخل خلية مستقلة
        """
        ...
        cell_w = full_w / 3.0
        inner = Table(
            [[P(f"<b>{label_ar}</b>", tiny_r), P(v or "—", tiny_l)]],
            colWidths=[cell_w * 0.42, cell_w * 0.58],
        )
    ...
    trader_val = trader_name
    if tuser:
        # مسافة آمنة + إجبار LTR على اليوزر فقط داخل نفس القيمة
        trader_val = f"{trader_name}  {tuser}"

    badges = Table([[
        _badge_cell("الوقت", now.strftime("%H:%M"), value_ltr=True),
        _badge_cell("التاريخ", now.strftime("%Y-%m-%d"), value_ltr=True),
        _badge_cell("التاجر", trader_val, value_ltr=False),  # الاسم عربي، داخل نفس خلية القيمة
    ...
    def kv_table(rows: list[tuple[str, str]]):
        """
        ✅ (04) حدود/شبكة شفافة + خلفية صفوف شفافة لإظهار العلامة المائية خلف الجدول
        """
        data = []
        for k, v in rows:
    ...
    def _wm(canvas, docx):
        try:
            # ✅ (04) علامة مائية كبيرة على قد مساحة المحتوى/الجداول
            if logo_path and os.path.exists(logo_path):
                page_w, page_h = A4
                wm_w = page_w * 0.86
                wm_h = wm_w
