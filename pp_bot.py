...
async def _need_complete_trader_profile_notice(
    ...
    if missing is None:
        try:
            ...
            tp = {}
        missing = _trader_profile_missing(tp, strict=strict)
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _admin_show_traders_manage():
        ...
        strict = _is_trader_profile_strict(context)
        strict_txt = "🔒 إلزام كل الحقول: <b>مفعل</b>" if strict else "🔓 إلزام كل الحقول: <b>غير مفعل</b>"
        rows2 = [
            [InlineKeyboardButton(("🔓 تعطيل الإلزام" if strict else "🔒 تفعيل الإلزام"),
                                  callback_data=f"pp_admin|tstrict|{'off' if strict else 'on'}")],
        ]

        msg = "🧑‍💼 <b>إدارة التجار</b>" + strict_txt + "اختر تاجر لفتح ملفه أو تفعيل/تعطيل:"
        kb = InlineKeyboardMarkup(rows2 + rows + [[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|home")]])
        await _admin_edit_or_send(q, msg, kb)
    ...
    if action == "tview":
        ...
        kb_rows = [
            ...
            [InlineKeyboardButton("📄 سجل التاجر PDF", callback_data=f"pp_admin|trecord_pdf|{tid}")],
    ...
    if action == "trecord_pdf":
        ...
        try:
            ...
            styles = getSampleStyleSheet()
            base = ParagraphStyle("base", parent=styles["Normal"], fontName=font_name, fontSize=11, leading=14, alignment=2)
            center = ParagraphStyle("center", parent=base, alignment=1)
            small = ParagraphStyle("small", parent=base, fontSize=9, leading=12, textColor=colors.gray)
            ...
            doc = SimpleDocTemplate(out_path, pagesize=A4, leftMargin=1.6*cm, rightMargin=1.6*cm, topMargin=1.4*cm, bottomMargin=1.6*cm)
            ...
            header_tbl = Table([[logo_cell if logo_cell else P("<b>PPARTS</b>", center)]], colWidths=[full_w])
            header_tbl.setStyle(TableStyle([
                ("ALIGN", (0,0), (-1,-1), "CENTER"),
                ("VALIGN", (0,0), (-1,-1), "MIDDLE"),
                ("BOTTOMPADDING", (0,0), (-1,-1), 6),
            ...
            # Title bar
            title_bar = Table([[P("<b>سجل التاجر</b>", ParagraphStyle("tb", parent=center, textColor=colors.white, fontName=font_name, fontSize=12))]], colWidths=[full_w])
            title_bar.setStyle(TableStyle([
                ...
                ("TOPPADDING", (0,0), (-1,-1), 6),
                ("BOTTOMPADDING", (0,0), (-1,-1), 6),
                ("BOX", (0,0), (-1,-1), 0.25, colors.HexColor("#0B3D91")),
            ]))
            story.append(title_bar)
            ...
            rows = [
                [P("<b>ID</b>"), P(str(tid))],
                [P("<b>اسم التاجر</b>"), P(html.escape(tname))],
                [P("<b>اسم المتجر</b>"), P(html.escape(tcompany))],
                [P("<b>تاريخ الانضمام</b>"), P(html.escape(created))],
                [P("<b>حالة التفعيل</b>"), P(html.escape(enabled_txt))],
                [P(f"<b>حالة الاشتراك ({month})</b>"), P(html.escape(sub_status))],
                [P("<b>اسم البنك</b>"), P(html.escape(bank))],
                [P("<b>IBAN</b>"), P(html.escape(iban))],
                [P("<b>STC Pay</b>"), P(html.escape(stc))],
                [P("<b>رقم الهوية</b>"), P(html.escape(nid))],
                [P("<b>السجل التجاري</b>"), P(html.escape(crn))],
                [P("<b>الرقم الضريبي</b>"), P(html.escape(vat))],
                [P("<b>آخر تحديث</b>"), P(html.escape(upd))],
            ]

            tbl = Table(rows, colWidths=[full_w*0.34, full_w*0.66])
            tbl.setStyle(TableStyle([
                ("BACKGROUND", (0,0), (-1,0), colors.whitesmoke),
                ("BOX", (0,0), (-1,-1), 0.35, colors.lightgrey),
            ...
            try:
                with open(out_path, "rb") as f:
                    await context.bot.send_document(chat_id=q.message.chat_id, document=f, caption=cap)
            except Exception as e:
                _swallow(e)
        except Exception as e:
            _swallow(e)
