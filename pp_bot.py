...
async def _send_client_payment_preview(
    ...
    # ✅ اسم التاجر (اختياري) ليظهر للعميل (خصوصاً عند pay_scope=goods)
    trader_name = _s(order.get("accepted_trader_name") or order.get("quoted_trader_name"))
    if not trader_name:
        try:
            trader_id0 = int(order.get("quoted_trader_id") or order.get("accepted_trader_id") or 0)
        except Exception:
            trader_id0 = 0
        if trader_id0:
            try:
    ...
    if pay_scope == "goods":
        ...
        who_line = "👤 المستفيد: <b>التاجر</b> (قيمة القطع)"
        next_line = "بعد التحقق سيتم اعتماد الدفع وإرسال بيانات الشحن للتاجر."
        icon_scope = "🧩"
    else:
        ...
        who_line = "🏢 المستفيد: <b>المنصة</b> (رسوم الخدمة)"
    ...
    if pay_scope == "goods" and isinstance(items, list) and items:
        ...
        parts_summary = (
            ...
            + (f"🚚 <b>قيمة الشحن</b>: <b>{ship_fee_total_txt}</b>\n" if ship_fee_total_txt and ship_fee_total_txt != "—" else "")
            + (f"🧾 <b>إجمالي الفاتورة</b>: <b>{inv_total_txt}</b>\n" if inv_total_txt and inv_total_txt != "—" else "")
            + "━━━━━━━━━━━━\n"
        )
    # ---------- message ----------
    msg = (
        ...
        f"💳 <b>قيمة الفاتورة</b>: <b>{inv_total_txt}</b>\n"
        + (f"👤 <b>التاجر</b>: <b>{html.escape(trader_name)}</b>\n" if (pay_scope == "goods" and trader_name) else "")
        + f"{who_line}\n"
        f"💳 <b>طريقة الدفع</b>: <b>{method_txt}</b>\n"
        f"💰 <b>المبلغ</b>: <b>{amt_txt}</b>\n"
        + (f"🚚 <b>الشحن</b>: <b>{ship_fee_total_txt}</b>\n" if (pay_scope == "goods" and ship_fee_total_txt and ship_fee_total_txt != "—") else "")
        + (f"🧾 <b>الإجمالي</b>: <b>{inv_total_txt}</b>\n" if pay_scope == "goods" else "")
        ...
        if client_id:
            try:
                ...
                await context.bot.send_message(
                    ...
                    text="\n".join(lines),
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("💬 مراسلة التاجر داخل المنصة", callback_data=f"pp_chat_open|{order_id}")],
                    ]),
                    disable_web_page_preview=True,
                )
            except Exception:
