...
async def _alert(q, text: str | None = None, force: bool = False):
    """
    ✅ Popup توجيهي واضح:
    - يميّز (مسؤول منصة / تاجر / عميل) بشكل مضمون من الإكسل عبر get_trader_profile()
    - إذا text فارغ/None => Popup توجيهي حسب callback_data
    - إذا text موجود => Toast للعادي + Popup للأهم
    """
    ...
    def _parse_cb(q):
        try:
            ...
            raw = ""
        head = raw.split("|", 1)[0].strip().lower() if raw else ""
        rest = raw.split("|")[1:] if "|" in raw else []
        return raw, head, rest

    def _maybe_order_id(parts: list[str]) -> str:
        for p in parts:
            s = (p or "").strip()
            if not s:
                continue
    ...
    def _role_and_display_name(uid: int, fallback_name: str) -> tuple[str, str]:
        # 1) إدارة
        try:
            if uid and uid in ADMIN_IDS:
                return ("مسؤول المنصة", fallback_name)
        except Exception:
            pass

        # 2) تاجر (مضمون من شيت traders عبر get_trader_profile)
        try:
            prof = get_trader_profile(int(uid or 0)) or {}
            # إذا رجع ملف فيه trader_id فعلاً => تاجر
            if prof.get("trader_id"):
                dn = str(prof.get("display_name") or "").strip()
                cn = str(prof.get("company_name") or "").strip()
                # اسم يظهر بشكل جميل
                show = dn or cn or fallback_name
                return ("التاجر", show)
        except Exception:
            pass

        # 3) عميل
        return ("العميل", fallback_name)

    def _default_guidance(q) -> str:
        raw, head, parts = _parse_cb(q)
        oid = _maybe_order_id(parts)
        ...
        base_name = _user_name_from_q(q)
        role, show_name = _role_and_display_name(uid, base_name)

        prefix = f"عزيزي {role}: {show_name}\n"
        if oid:
            prefix += f"🧾 رقم الطلب: {oid}\n\n"
        else:
            prefix += "\n"

        # إغلاق
        if head == "pp_ui_close":
            return prefix + "✅ تم إغلاق النافذة."

        # إلغاء
        if head == "pp_cancel":
            return prefix + (
                "⚠️ تم إلغاء العملية الحالية.\n"
                "إذا رغبت بالمتابعة، اكتب رقم الطلب في الخاص أو ارجع للقائمة."
            )

        # تحديث حالة التاجر
        if head == "pp_trader_status":
            action = (parts[0] if len(parts) >= 1 else "").strip().lower()
            mp = {
                "prep": "جاري تجهيز الطلب",
                "ready": "الطلب جاهز للشحن",
                "shipped": "تم شحن الطلب",
            ...
            st = mp.get(action, "تحديث الحالة")
            return prefix + (
                f"⛔ لا يمكن تنفيذ ({st}) الآن.\n\n"
                "🔎 السبب الشائع:\n"
                "- الطلب ليس في المرحلة الصحيحة\n"
                "- أو ينقصه إجراء سابق (فاتورة / سداد)\n\n"
                "✅ الإجراء الصحيح:\n"
                "1) افتح لوحة الطلب من الخاص\n"
                "2) أكمل الخطوة المطلوبة\n"
                "3) ثم حاول مرة أخرى"
            )

        # أزرار الإدارة / الفريق
        if head.startswith(("pp_admin_", "pp_team_")):
            return prefix + (
                "⛔ هذا الإجراء مخصص للإدارة أو فريق المنصة فقط.\n\n"
                "✅ للتصرف:\n"
                "استخدم زر (📩 مراسلة الإدارة) أو اكتب رقم الطلب في الخاص."
            )

        # عروض الأسعار
        if head.startswith(("ppq_", "pp_quote_")):
            return prefix + (
                "🔒 هذا الخيار غير متاح في هذه المرحلة.\n"
                "✅ افتح عرض السعر من الخاص وأكمل الخطوات بالترتيب."
            )

        # الدعم
        if head.startswith("pp_support"):
            return prefix + (
                "📩 للتواصل مع الإدارة:\n"
                "اضغط زر (اتصل بالمنصة) من لوحة التحكم."
            )

        # افتراضي عام
        return prefix + (
            "⛔ هذا الزر غير متاح حالياً.\n\n"
            "✅ للتصرف الصحيح:\n"
            "1) اكتب رقم الطلب في الخاص (مثال: PP-260208-0001)\n"
            "2) أو ارجع للقائمة الرئيسية\n"
            "3) وإذا استمرت المشكلة تواصل مع الإدارة"
        )

    try:
        # ✅ النص الفارغ/None => Popup توجيهي
        if text is None or not str(text).strip():
            msg = _default_guidance(q)
            await q.answer(text=msg, show_alert=True)
            return

        s = str(text).strip()

        # ترقية تلقائية إلى Popup للرسائل المهمة
        if not force:
            s_low = s.lower()
            important_phrases = (
                ...
                "تعذر", "فشل", "خطأ", "خطا",
                "blocked", "unauthorized", "forbidden", "error", "failed",
            )
            important_prefixes = ("⛔", "🔒", "🟥", "⚠️", "🚫")
            if (
                s.startswith(important_prefixes)
                or any(p in s for p in important_phrases)
                or any(p in s_low for p in important_phrases)
            ):
                force = True
        ...
        # ✅ fallback: لا تترك المستخدم على تحميل
        try:
            await q.answer()
        except Exception:
            pass
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_goods_confirm":
        ...
        try:
            ...
            client_username_admin = (o4.get("user_username") or o4.get("username") or o4.get("client_username") or "").strip()
            if client_username_admin and not client_username_admin.startswith("@"):
                client_username_admin = f"@{client_username_admin}"
            if not client_username_admin:
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            f"🧾 <b>رقم الطلب</b>: <code>{html.escape(order_id)}</code>\n"
                            f"👤 <b>العميل</b>: <b>{html.escape(client_name_admin)}</b> ({html.escape(client_username_admin)})\n"
