...
async def pp_join_admin_action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if ok and approved:
            await context.bot.send_message(chat_id=applicant_id, text=f"{brand}\n\n✅ تم قبول طلب انضمامك.\n\n✅ التقديم والطلبات عبر @{PP_BOT_USERNAME} فقط", disable_web_page_preview=True)
        elif ok and (not approved):
            await context.bot.send_message(chat_id=applicant_id, text=f"{brand}\n\n⛔ تم رفض طلب الانضمام. يمكنك إعادة التقديم.\n\n✅ التقديم والطلبات عبر @{PP_BOT_USERNAME} فقط", disable_web_page_preview=True)
    except Exception:
        pass

    # إشعار الإدارة بالنتيجة + زر مراسلة
    admin_txt = (
        f"{brand}\n\n"
        + ("✅ تم قبول الطلب\n" if ok and approved else "⛔ تم رفض الطلب\n" if ok else "⚠️ تعذر تنفيذ الإجراء\n")
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg:
        ...
        # =========================================================
        # ✅ Join Portal (بوابة التجار) — استقبال المستندات بالترتيب
        # (1) السجل التجاري → (2) رخصة/إثبات المتجر
        # =========================================================
        try:
            if getattr(msg, "chat", None) and getattr(msg.chat, "type", None) == ChatType.PRIVATE:
                if stage in (STAGE_JOIN_CR, STAGE_JOIN_LICENSE):
                    ...
                    if stage == STAGE_JOIN_LICENSE:
                        ...
                        await msg.reply_text(
                            f"{_join_portal_brand()}\n\n✅ تم استلام رخصة/إثبات المتجر.\n\nالخطوة 3/3\n🧾 اكتب الآن <b>الرقم الضريبي</b> (أرقام فقط).\n\n✅ التقديم والطلبات عبر @{PP_BOT_USERNAME} فقط",
                            parse_mode="HTML",
                            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✖️ إنهاء", callback_data="pp_join_done")]]),
                            disable_web_page_preview=True,
                        )
        ...
        if stage == STAGE_APPLICANT_CHAT_ADMIN:
            ...
            try:
                if kind == "photo":
                    await context.bot.send_photo(chat_id=admin_id, photo=file_id, caption=caption, reply_markup=_join_portal_admin_kb(user_id))
                else:
                    await context.bot.send_document(chat_id=admin_id, document=file_id, caption=caption, reply_markup=_join_portal_admin_kb(user_id))
            except Exception:
                pass
            try:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_JOIN_VAT:
        ...
        if not vat or len(vat) < 6:
            await update.message.reply_text("اكتب الرقم الضريبي بشكل صحيح (أرقام فقط).")
            return

        jd = _join_ud(context, user_id)
        jd["vat_no"] = vat
        set_stage(context, user_id, STAGE_JOIN_WAIT)

        admin_text = (
            f"{_join_portal_brand()}\n\n"
            "📥 <b>تم استلام مستندات متقدم جديد</b>\n\n"
            f"🆔 ID: <code>{user_id}</code>\n"
            f"🧾 الرقم الضريبي: <code>{vat}</code>\n\n"
            "📎 سيتم إرسال الملفات بالأسفل."
        )
        await _notify_admins_private(context, admin_text, kb=_join_portal_admin_kb(user_id))

        async def _send_file(kind: str, fid: str, cap: str):
            for aid in (ADMIN_IDS or []):
                try:
                    if kind == "photo":
                        await context.bot.send_photo(chat_id=int(aid), photo=fid, caption=cap)
                    else:
                        await context.bot.send_document(chat_id=int(aid), document=fid, caption=cap)
                except Exception:
                    pass
        ...
        if cr_fid:
            await _send_file(cr_kind, cr_fid, f"{_join_portal_brand()} | السجل التجاري | ID:{user_id}")
        if lic_fid:
            await _send_file(lic_kind, lic_fid, f"{_join_portal_brand()} | رخصة/إثبات المتجر | ID:{user_id}")

        admin_for_reply = int((ADMIN_IDS or [0])[0] or 0)
        kb = _join_portal_applicant_reply_kb(admin_for_reply) if admin_for_reply else None
        await update.message.reply_text(
            f"{_join_portal_brand()}\n\n✅ تم استلام مستنداتك وإرسالها للإدارة للمراجعة.\n⏳ سيتم إشعارك عند قبول الطلب أو رفضه.\n\n✅ التقديم والطلبات عبر @{PP_BOT_USERNAME} فقط",
            reply_markup=kb,
            disable_web_page_preview=True,
        )
        return
