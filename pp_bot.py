...
async def send_trader_record_pdf(context: ContextTypes.DEFAULT_TYPE, trader_id: int, admin_chat_id: int):
    """
    ✅ PDF ثالث: سجل التاجر (ثيم برتقالي) — بدون ختم مدفوع
    يحتوي: بيانات التاجر + الاشتراك + التفعيل + ملخص المبيعات (منجزة/غير منجزة) + إجماليات.
    """
    ...
    # خطوط عربية (نفس الفواتير)
    font_name = "Amiri"
    try:
        # يحاول نفس مسار الفواتير عندك
        pdfmetrics.registerFont(TTFont(font_name, "Amiri-Regular.ttf"))
    except Exception:
        font_name = "Helvetica"

    # ✅ جلب بيانات التاجر + الاشتراك + الطلبات
    try:
        ensure_workbook()
    except Exception:
        pass
    ...
    try:
        prof = get_trader_profile(trader_id) or {}
    except Exception:
        prof = {}

    try:
        enabled = is_trader_enabled(trader_id)
    except Exception:
        enabled = True
    ...
    try:
        orders = list_orders_for_trader(trader_id) or []
    except Exception:
        orders = []
    ...
    for o in (orders or []):
        ...
        try:
            amt = float(re.sub(r"[^0-9.]+", "", raw) or 0)
        except Exception:
            amt = 0.0

        is_done = (gps == "confirmed") or (ost in ("closed", "delivered"))
        if is_done:
            done_orders += 1
            done_amt += amt
        else:
    ...
    # ✅ تحضير محتوى
    dn = (prof.get("display_name") or "").strip() or str(trader_id)
    cn = (prof.get("company_name") or "").strip() or "—"
    phone = (prof.get("shop_phone") or "").strip() or "—"
    cr_no = (prof.get("cr_no") or "").strip() or "—"
    vat_no = (prof.get("vat_no") or "").strip() or "—"
    bank = (prof.get("bank_name") or "").strip() or "—"
    iban = (prof.get("iban") or "").strip() or "—"
    stc = (prof.get("stc_pay") or "").strip() or "—"
    joined = (prof.get("joined_at_utc") or "").strip() or "—"
    upd = (prof.get("updated_at_utc") or "").strip() or "—"

    status_txt = "مفعل ✅" if enabled else "موقوف ⛔"

    # ✅ PDF tempfile
    fd, tmp_path = tempfile.mkstemp(prefix=f"pp_trader_record_{trader_id}_", suffix=".pdf")
    os.close(fd)
    ...
    try:
        ...
        # ثيم برتقالي
        ORANGE = colors.Color(1.0, 0.55, 0.0)  # تقريبي
        ORANGE_L = colors.Color(1.0, 0.92, 0.80)

        story = []
        story.append(Paragraph(_ar("سجل التاجر"), title_style))
        story.append(Paragraph(_ar(f"التاريخ: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"), base_style))
        story.append(Spacer(1, 10))

        # جدول بيانات التاجر (KV)
        kv = [
            [_ar(str(trader_id)), _ar("ID")],
            [_ar(status_txt), _ar("الحالة")],
            [_ar(sub_status), _ar("الاشتراك (" + str(month) + ")")],
            [_ar(dn), _ar("اسم التاجر")],
            [_ar(cn), _ar("اسم المتجر")],
            [_ar(phone), _ar("رقم المتجر")],
            [_ar(cr_no), _ar("السجل التجاري")],
            [_ar(vat_no), _ar("الرقم الضريبي")],
            [_ar(bank), _ar("اسم البنك")],
            [_ar(iban), _ar("IBAN")],
            [_ar(stc), _ar("STC Pay")],
            [_ar(joined), _ar("تاريخ الانضمام")],
            [_ar(upd), _ar("آخر تحديث")],
        ]

        # ✅ RTL: المعرفات يمين (العمود الثاني) والقيم يسار (العمود الأول)
        t1 = Table(kv, colWidths=[12.0*cm, 5.2*cm])
        ...
        # ملخص المبيعات
        kv2 = [
            [_ar(str(total_orders)), _ar("إجمالي الطلبات")],
            [_ar(str(done_orders)), _ar("منجزة/مؤكدة")],
            [_ar(str(open_orders)), _ar("غير منجزة")],
            [_ar(_money(done_amt)), _ar("إجمالي منجز (قيمة القطع)")],
            [_ar(_money(open_amt)), _ar("إجمالي غير منجز (قيمة القطع)")],
        ]
        t2 = Table(kv2, colWidths=[10.2*cm, 7.0*cm])
        ...
        story.append(Paragraph(_ar("ملخص المبيعات"), ParagraphStyle("h2", parent=base_style, fontSize=13, leading=18, spaceAfter=8)))
        story.append(t2)
        ...
        try:
            with open(tmp_path, "rb") as f:
                await context.bot.send_document(
                    ...
                    document=f,
                    filename=f"سجل_التاجر_{trader_id}.pdf",
                    caption=f"🧾 سجل التاجر: {dn} (ID: {trader_id})",
                )
        except Exception as e:
            _swallow(e)
...
async def _need_complete_trader_profile_notice(context: ContextTypes.DEFAULT_TYPE, user_id: int, name: str, order_id: str = "", missing_fields: list[str] | None = None):
    ...
    # خريطة تعليمات مختصرة لكل حقل (بدون نفخ)
    field_help = {
        "display_name": ("اسم التاجر", "مثال: <code>أبو محمد</code>"),
        "company_name": ("اسم المتجر", "مثال: <code>مؤسسة قطع الغيار</code>"),
        "shop_phone": ("رقم اتصال المتجر", "10 أرقام ويبدأ بـ 05 — مثال: <code>05XXXXXXXX</code>"),
        "cr_no": ("رقم السجل التجاري", "أرقام فقط (6-15) — مثال: <code>1010123456</code>"),
        "vat_no": ("الرقم الضريبي", "15 رقم — مثال: <code>310XXXXXXXXXXXX</code>"),
        "bank_name": ("اسم البنك", "مثال: <code>الراجحي</code>"),
        "iban": ("رقم IBAN", "يبدأ بـ SA ثم 22 رقم — مثال: <code>SA1234567890123456789012</code>"),
        "stc_pay": ("رقم STC Pay", "10 أرقام ويبدأ بـ 05 — مثال: <code>05XXXXXXXX</code>"),
    }
    ...
    missing_line = ""
    if missing_fields:
        # ✅ اعرض جميع الحقول الناقصة بوضوح (ليس فقط أول حقل)
        try:
            missing_titles = [field_help.get(k, (k, ""))[0] for k in missing_fields]
        except Exception:
            missing_titles = [str(k) for k in missing_fields]
        missing_line = "\n\n⛔ الحقول الناقصة: <b>" + "، ".join(missing_titles) + "</b>"

        # ✅ مع تلميح للحقل الأول لتسهيل الإكمال خطوة بخطوة
        if first_missing:
            title, hint = field_help.get(first_missing, (first_missing, ""))
            if hint:
                missing_line += f"\n\n🔎 ابدأ بـ: <b>{title}</b>\n{hint}"

    steps = []
    for k in ["display_name","company_name","shop_phone","cr_no","vat_no","bank_name","iban","stc_pay"]:
        title, hint = field_help.get(k, (k, ""))
        steps.append(f"• {title} — {hint}")

    msg = (
        f"{name}\n"
        "⛔ لا يمكنك تقديم عرض سعر قبل إكمال ملف التاجر بالكامل.\n\n"
        "✅ أكمل البيانات التالية من لوحة التاجر:" "\n"
        + "\n".join(steps)
        + missing_line
...
async def start_trader_deeplink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ إلزام جديد: كل الحقول مطلوبة قبل تقديم عرض سعر
    required_fields = ["display_name","company_name","shop_phone","cr_no","vat_no","bank_name","iban","stc_pay"]
    missing_fields = [k for k in required_fields if not str((tp or {}).get(k) or "").strip()]

    # ✅ لا نرسل رسالة مضللة "بياناتك مسجلة" إذا كان الملف ناقص
    # - لو ناقص: نرسل تنبيه واضح + نفتح اللوحة مباشرة
    # - لو مكتمل: نفتح اللوحة مباشرة بدون أي رسالة إضافية
    if missing_fields:
        try:
            await _send_trader_profile_notice(context, user_id, name, missing_fields=missing_fields)
        except Exception as e:
            _swallow(e)
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all((tp.get(f) or "").strip() for f in required_fields):
        missing = [f for f in required_fields if not (str(tp.get(f) or "").strip())]
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_PROFILE_EDIT:
        ...
        if field == "shop_phone":
            v = re.sub(r"\s+", "", val)
            # السعودية غالباً: 10 أرقام ويبدأ بـ 05
            if not re.fullmatch(r"05\d{8}", v):
                await _reply_html(
                    "رقم اتصال غير صحيح",
                    [
        ...
        if field == "vat_no":
            v = re.sub(r"\s+", "", val)
            # الرقم الضريبي بالسعودية غالباً 15 رقم
            if not re.fullmatch(r"\d{15}", v):
                await _reply_html(
                    "الرقم الضريبي غير صحيح",
                    [
                        "⚠️ الصيغة المطلوبة: 15 رقم.",
                        "مثال: <code>310XXXXXXXXXXXX</code>",
                    ],
                )
                return
            val = v
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "backup_now":
        ...
        try:
            ...
            if not sent:
                try:
                    await q.message.reply_text("❌ فشل إرسال النسخة الاحتياطية إلى مجموعة النسخ.")
                except Exception as e:
                    _swallow(e)
                return
    ...
    if action == "tview":
        ...
        shop_phone = (prof.get("shop_phone") or "").strip()
        cr_no = (prof.get("cr_no") or "").strip()
        vat_no = (prof.get("vat_no") or "").strip()
        joined = (prof.get("joined_at_utc") or "").strip()
        ...
        msg = (
            ...
            f"📞 رقم المتجر: <b>{html.escape(shop_phone or '—')}</b>\n"
            f"🏷️ السجل التجاري: <b>{html.escape(cr_no or '—')}</b>\n"
            f"🧾 الرقم الضريبي: <b>{html.escape(vat_no or '—')}</b>\n"
            f"📅 تاريخ الانضمام: <b>{html.escape(joined or '—')}</b>\n\n"
        ...
        kb_rows = [
            ...
            [InlineKeyboardButton("🧾 سجل التاجر (PDF)", callback_data=f"pp_admin|trecordpdf|{tid}")],
    ...
    # ===== TRADER RECORD PDF (trecordpdf) =====
    if action == "trecordpdf":
        tid = 0
        if len(parts) >= 3:
            try:
                tid = int(parts[2] or 0)
        ...
        try:
            await send_trader_record_pdf(context, trader_id=tid, admin_chat_id=int(q.message.chat_id if q.message else uid))
        except Exception as e:
            _swallow(e)
            await _pop("تعذر إنشاء سجل التاجر PDF")
        return
...
def trader_panel_kb() -> InlineKeyboardMarkup:
    # ✅ ترتيب حسب الأولوية + الحقول الجديدة
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏷️ تعديل السجل التجاري", callback_data="pp_tprof|edit|cr_no")],
        ...
        [InlineKeyboardButton("📩 مراسلة الإدارة", callback_data="pp_support_open")],
        [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
    ])
...
async def show_trader_panel(update_or_q, context: ContextTypes.DEFAULT_TYPE, trader_id: int):
    ...
    shop_phone = (tp.get("shop_phone") or "").strip()
    cr_no = (tp.get("cr_no") or "").strip()
    vat_no = (tp.get("vat_no") or "").strip()
    joined = (tp.get("joined_at_utc") or "").strip()

    required_fields = ["display_name","company_name","shop_phone","cr_no","vat_no","bank_name","iban","stc_pay"]
    missing = [f for f in required_fields if not (str(tp.get(f) or "").strip())]
    complete_txt = "✅ مكتمل" if not missing else ("⛔ ناقص: " + "، ".join([{
        ...
        "iban":"IBAN",
        "stc_pay":"STC Pay",
    }.get(x, x) for x in missing[:3]]) + ("…" if len(missing) > 3 else ""))
    ...
    txt = (
        ...
        f"🧾 الرقم الضريبي: <b>{html.escape(vat_no or '—')}</b>\n"
        f"📅 تاريخ الانضمام: <b>{html.escape(joined or '—')}</b>\n"
        f"✅ اكتمال الملف: <b>{html.escape(complete_txt)}</b>\n\n"
