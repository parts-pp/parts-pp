...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if missing and bool(strict):
        # ✅ عند تفعيل الإلزام: منع تقديم/تعديل العرض مع تنبيه فقط (بدون رسائل إضافية)
        await _alert(q, "⛔ لا يمكنك تقديم عرض قبل استكمال بيانات التاجر")
        return
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _admin_show_traders_manage(q):
    ...
    if action == "traders_manage":
        await _admin_show_traders_manage(q)
        return
    ...
    if action == "tstrict":
        ...
        # ✅ تنفيذ صامت بدون نافذة Alert
        try:
            await q.answer("تم التنفيذ", show_alert=False)
        except Exception as e:
            _swallow(e)

        # ✅ رسالة خاصة للأدمن فقط
        try:
            await context.bot.send_message(
    ...
    if action == "trecord_pdf":
        ...
        try:
            ...
            tbl = Table(rows, colWidths=[full_w * 0.58, full_w * 0.42])
            tbl.setStyle(
                TableStyle(
                    [
                        (
                            ...
                            [
                                colors.whitesmoke,
                                colors.Color(1, 0.6, 0.2, alpha=0.15),
                            ],
                        ),
