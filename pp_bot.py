...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_goods_confirm":
        ...
        if ost_now in ("shipped", "delivered", "closed"):
            ...
            # إذا فاتورة المتجر موجودة: نرفعه مباشرة لمرحلة ready_to_ship
            # عشان تظهر أزرار (🚚 تم الشحن + 📞 مراسلة العميل)
            if inv_file_now:
                next_ost = "ready_to_ship"
            else:
                # بدون فاتورة: نخليه في وضع التجهيز/البداية حسب الموجود
        ...
        update_order_fields(order_id, {
            ...
            "quote_locked": "yes",
            "order_status": next_ost,   # ✅ أهم تعديل: لا تستخدم in_progress
        })
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ==================================================================
    # 🔒 زر إغلاق الطلب (بعد 7 أيام من لحظة الشحن فقط) — Alert مربع
    # ==================================================================
    if action == "pp_order_finish":
        ...
        # قبل انتهاء 7 أيام → Alert مربع توضيحي
        if now_utc < expires_dt:
            ...
            try:
                await q.answer(
                    ...
                    "🔒 يتم تفعيل زر الإغلاق تلقائيًا بعد انتهاء مدة المتابعة (7 أيام من الشحن).\n"
        ...
        # ✅ بعد انتهاء 7 أيام → يسمح بالإغلاق
        update_order_fields(order_id, {
            "order_status": "closed",
            "closed_at_utc": utc_now_iso(),
        })
