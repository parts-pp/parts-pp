...
# ==================================================
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE:
        ...
        if kind == "seller_pay_link":
            ...
            # ✅ إشعار للعميل: رابط فقط + ننتظر الإيصال
            if client_id2:
                try:
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ==========================================================
    # ✅ طريقة الدفع للتاجر (يدوي / متجر إلكتروني)
    # ==========================================================
    if action == "paymode":
...
async def show_trader_panel(update_or_q, context: ContextTypes.DEFAULT_TYPE, trader_id: int):
    ...
    # ✅ عرض طريقة الدفع الحالية داخل لوحة التاجر + تنبيه عند وضع "متجر إلكتروني"
    pay_mode = (tp.get("payment_profile") or "manual")
    pay_mode = (str(pay_mode).strip().lower())
    if pay_mode not in ("manual", "ecom"):
        pay_mode = "manual"

    pay_mode_txt = "تحويل بنكي / STC Pay" if pay_mode == "manual" else "متجر إلكتروني (رابط دفع متغير)"
    pay_mode_note = ""
    if pay_mode == "ecom":
        pay_mode_note = "ℹ️ سيتم طلب رابط الدفع لكل طلب عند تحديث الحالة: <b>الطلب جاهز للشحن</b>\n"
    ...
    txt = (
        ...
        f"💳 طريقة الدفع الحالية: <b>{html.escape(pay_mode_txt)}</b>\n"
        f"{pay_mode_note}"
