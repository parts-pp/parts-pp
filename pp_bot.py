...
# ✅ جديد: كيبورد اختيار البنود المشمولة في عرض السعر
def trader_quote_items_kb(order_id: str, items: list[dict], selected: set[int]) -> InlineKeyboardMarkup:
    ...
    for i, it in enumerate(items or [], start=1):
        ...
        label = f"{mark} {i}- {nm[:28]}"
        rows.append([InlineKeyboardButton(label, callback_data=f"ppq_it|{order_id}|{i}")])

    rows.append([
        InlineKeyboardButton("☑️ تحديد الكل", callback_data=f"ppq_it_all|{order_id}"),
        InlineKeyboardButton("🧹 مسح الكل", callback_data=f"ppq_it_none|{order_id}"),
    ])
    rows.append([InlineKeyboardButton("✅ التالي", callback_data=f"ppq_it_done|{order_id}")])
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ كيبورد البنود: تسعير بند-بند
    def _items_price_kb(order_id: str, items: list, prices_map: dict) -> InlineKeyboardMarkup:
        rows = []
        ...
        for i, it in enumerate(items or [], start=1):
            ...
            p = prices_map.get(str(i))
            if p not in (None, "", "None"):
                label = f"✅ {nm[:26]} ({p} ر.س)"
            else:
                label = f"🔹 {nm[:30]}"

            rows.append([InlineKeyboardButton(label, callback_data=f"ppq_item|{order_id}|{i}")])

        # ✅ زر واحد واضح + زر تسعير الكل (اختياري)
        rows.append([
            InlineKeyboardButton("☑️ تسعير الكل", callback_data=f"ppq_item_all|{order_id}"),
            InlineKeyboardButton("✅ اكمل خطوات العرض", callback_data=f"ppq_item_done|{order_id}"),
        ])

        return InlineKeyboardMarkup(rows)

    # ===========================
    # بداية بناء عرض السعر
    # ===========================
    if action == "ppq_begin":
        if len(parts) < 2:
            return
        ...
        td["quote_order_id"] = order_id
        td["quote_step"] = "item_pick"
        td["quote_started_at_utc"] = utc_now_iso()
        set_stage(context, user_id, STAGE_TRADER_SET_QUOTE)

        # ✅ تهيئة تخزين تسعير البنود
        td["quote_item_prices"] = {}         # {"1":"120","2":"80"}
        td["quote_item_last_idx"] = None
        td["quote_item_all_mode"] = False
        ...
        td["quote_items_count"] = len(its)

        await q.message.reply_text(
            f"{name}\n"
            f"🧾 رقم الطلب: {order_id}\n"
            "🧩 اختر القطعة لتسعيرها (ثم اكتب سعرها)\n"
            "بعد الانتهاء اضغط (اكمل خطوات العرض)",
            reply_markup=_items_price_kb(order_id, its, td.get("quote_item_prices") or {}),
            disable_web_page_preview=True,
        )
        return
    ...
    # ===========================
    # اختيار قطعة لتسعيرها
    # ===========================
    if action in ("ppq_item", "ppq_item_all", "ppq_item_done"):
        try:
            obx = get_order_bundle(order_id) or {}
            its = (obx.get("items") or []) if isinstance(obx, dict) else []
        except Exception:
        ...
        prices_map = td.get("quote_item_prices") or {}
        ...
        if action == "ppq_item":
            ...
            if idx < 1 or idx > len(its):
                await _alert(q, "بند غير صحيح")
                return

            # خزّن القطعة المختارة واطلب السعر عبر رسالة نصية (سيتم ربطها في text_handler لاحقاً)
            td["quote_step"] = "item_price"
            td["quote_item_last_idx"] = idx
            ...
            await q.message.reply_text(
                ...
                f"🧾 رقم الطلب: {order_id}\n"
                f"🧩 القطعة: {nm}\n"
                "💰 اكتب سعر هذه القطعة بالريال (أرقام فقط)",
                disable_web_page_preview=True,
            )
            return

        if action == "ppq_item_all":
            # ✅ نمط تسعير الكل: سنبدأ بأول قطعة غير مسعّرة
            td["quote_item_all_mode"] = True

            # ابحث عن أول بند غير مسعّر
            next_idx = 0
            for i in range(1, len(its) + 1):
                if str(i) not in prices_map or not str(prices_map.get(str(i)) or "").strip():
                    next_idx = i
                    break

            if not next_idx:
                await _alert(q, "كل البنود تم تسعيرها")
                return

            td["quote_step"] = "item_price"
            td["quote_item_last_idx"] = next_idx
            try:
                it = its[next_idx - 1] or {}
                nm = (it.get("name") or it.get("item_name") or "").strip()
            except Exception:
                nm = ""
            if not nm:
        ...
        if action == "ppq_item_done":
            # ✅ تأكد تم تسعير بند واحد على الأقل
            clean_prices = {}
            for k, v in (prices_map or {}).items():
                sv = str(v or "").strip()
                if sv:
                    clean_prices[str(k)] = sv

            if not clean_prices:
                await _alert(q, "سعّر بند واحد على الأقل ثم اكمل")
                return

            # ✅ احسب إجمالي القطع من مجموع البنود المسعّرة
            total = 0.0
            for _, v in clean_prices.items():
                try:
                    total += float(str(v).replace(",", "").strip())
                except Exception:
                    pass

            # خزّن الإجمالي كسلوك قديم (عشان بقية الويزارد ما تتأثر)
            td["quote_goods_amount"] = str(int(total)) if abs(total - int(total)) < 1e-9 else str(total)

            # جهّز الانتقال لبقية الخطوات
            td["quote_step"] = "type"
            td["quote_item_prices"] = clean_prices
            ...
            await q.message.reply_text(
                f"{name}\nاختر نوع القطع",
                reply_markup=trader_quote_parts_type_kb(order_id),
            )
            return

    # ===========================
    # بقية الويزارد كما هو
    # ===========================
    if action == "ppq_type":
        if len(parts) < 3:
            return
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # ✅ جديد: تفصيل سعر كل قطعة + إجمالي القطع (من تسعير بند-بند)
    try:
        obx = get_order_bundle(order_id) or {}
    ...
    # البنود المسعّرة فقط
    sel = []
    for k in list(prices_map.keys()):
        try:
            kk = int(k)
        except Exception:
            continue
        if 1 <= kk <= len(itemsx) and str(prices_map.get(str(kk)) or "").strip():
            sel.append(kk)
    sel = sorted(set(sel))
    ...
    for i in sel:
        ...
        pv = str(prices_map.get(str(i)) or "").strip()
        try:
            total_calc += float(pv.replace(",", ""))
        except Exception:
            pass

        detail_lines.append(f"• {i}- {nm}: {pv} ر.س")

    # لو في عدم توافق بين المحسوب والموجود، نخلي الموجود الرسمي هو quote_goods_amount (عشان الثبات)
    detail_block = ""
    if detail_lines:
        detail_block = (
            "🧩 تفاصيل عرض القطع:\n"
            + "\n".join(detail_lines)
            + "\n"
            + f"🧾 إجمالي القطع: {goods_amount} ر.س\n"
        )

    # احقن التفاصيل تحت رقم الطلب
    official2 = official
    try:
    ...
    fields_to_update = {
        ...
        # ✅ حفظ البنود المسعّرة كنص بسيط (لا يعتمد على أعمدة جديدة)
        "quote_items_included": ",".join(str(i) for i in sel),
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        if txt in ("الغاء", "إلغاء", "cancel", "خروج", "رجوع", "انهاء", "إنهاء"):
            ...
            # ✅ جديد: تنظيف تسعير بند-بند
            td.pop("quote_item_prices", None)
            td.pop("quote_item_last_idx", None)
            td.pop("quote_item_last_name", None)
            td.pop("quote_item_all_mode", None)
            td.pop("quote_items_count", None)
        ...
        # ✅ مساعد: كيبورد البنود (زر لكل قطعة + اكمل)
        def _items_price_kb_local(order_id_local: str, items_local: list, prices_map_local: dict) -> InlineKeyboardMarkup:
            rows = []
            pm = prices_map_local or {}
            for i, it in enumerate(items_local or [], start=1):
                try:
                    nm = (it.get("name") or it.get("item_name") or "").strip()
                except Exception:
                    nm = ""
                ...
                pv = str(pm.get(str(i)) or "").strip()
                if pv:
                    label = f"✅ {nm[:26]} ({pv} ر.س)"
                else:
                    label = f"🔹 {nm[:30]}"

                rows.append([InlineKeyboardButton(label, callback_data=f"ppq_item|{order_id_local}|{i}")])

            rows.append([
                InlineKeyboardButton("☑️ تسعير الكل", callback_data=f"ppq_item_all|{order_id_local}"),
                InlineKeyboardButton("✅ اكمل خطوات العرض", callback_data=f"ppq_item_done|{order_id_local}"),
            ])
            return InlineKeyboardMarkup(rows)
        ...
        # ✅ جديد: إدخال سعر قطعة محددة (بند-بند)
        if step == "item_price":
            # القطعة المختارة من ppq_cb
            try:
                idx = int(td.get("quote_item_last_idx") or 0)
            except Exception:
                idx = 0
            ...
            if idx <= 0:
                ...
                await update.message.reply_text(
                    f"{name}\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    "🧩 اختر القطعة لتسعيرها من الأزرار:",
                    reply_markup=_items_price_kb_local(order_id, its, td.get("quote_item_prices") or {}),
                    disable_web_page_preview=True,
                )
                return
            ...
            if not m_amt:
                nm0 = str(td.get("quote_item_last_name") or "").strip() or f"قطعة {idx}"
                await update.message.reply_text(
                    f"{name}\n"
                    "🟥 *السعر غير صحيح*\n"
                    f"القطعة: {nm0}\n"
            ...
            # حفظ السعر
            prices_map = td.setdefault("quote_item_prices", {})
            prices_map[str(idx)] = amount
            td["quote_item_prices"] = prices_map
            ...
            # ✅ إذا وضع "تسعير الكل" شغال: انتقل تلقائيًا لأول قطعة غير مسعّرة
            all_mode = bool(td.get("quote_item_all_mode"))
            if all_mode:
                next_idx = 0
                ...
                if next_idx:
                    ...
                    td["quote_item_last_idx"] = next_idx
                    try:
                        itn = its[next_idx - 1] or {}
                        nm = (itn.get("name") or itn.get("item_name") or "").strip()
                    except Exception:
                        nm = ""
                    if not nm:
                        nm = f"قطعة {next_idx}"
                ...
                # لا يوجد بنود متبقية -> أوقف نمط الكل وارجع للكيبورد
                td["quote_item_all_mode"] = False

            # ✅ رجوع للكيبورد بعد حفظ السعر
            td["quote_step"] = "item_pick"
            nm_last = str(td.get("quote_item_last_name") or "").strip()
            nm_last = nm_last or f"قطعة {idx}"
            await update.message.reply_text(
                f"{name}\n"
                f"✅ تم حفظ سعر القطعة: {nm_last}\n"
                f"🧾 رقم الطلب: {order_id}\n"
                "🧩 اختر القطعة التالية أو اضغط (اكمل خطوات العرض)",
                reply_markup=_items_price_kb_local(order_id, its, prices_map),
                disable_web_page_preview=True,
            )
            return

        # ✅ جديد: لو التاجر كتب نص وهو في وضع اختيار القطع
        if step == "item_pick":
            try:
                obx = get_order_bundle(order_id) or {}
                its = (obx.get("items") or []) if isinstance(obx, dict) else []
            ...
            await update.message.reply_text(
                f"{name}\n"
                f"🧾 رقم الطلب: {order_id}\n"
                "🟦 استخدم الأزرار لاختيار القطعة ثم اكتب سعرها.",
                reply_markup=_items_price_kb_local(order_id, its, td.get("quote_item_prices") or {}),
                disable_web_page_preview=True,
            )
            return
