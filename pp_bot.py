...
async def send_trader_record_pdf(context: ContextTypes.DEFAULT_TYPE, trader_id: int, admin_chat_id: int):
    ...
    try:
        ...
        kv = [
            [_ar("ID"), _ar(str(trader_id))],
            [_ar("الحالة"), _ar(status_txt)],
            [_ar("الاشتراك (" + str(month) + ")"), _ar(sub_status)],
            [_ar("اسم التاجر"), _ar(dn)],
            [_ar("اسم المتجر"), _ar(cn)],
            [_ar("رقم المتجر"), _ar(phone)],
            [_ar("السجل التجاري"), _ar(cr_no)],
            [_ar("الرقم الضريبي"), _ar(vat_no)],
            [_ar("اسم البنك"), _ar(bank)],
            [_ar("IBAN"), _ar(iban)],
            [_ar("STC Pay"), _ar(stc)],
            [_ar("تاريخ الانضمام"), _ar(joined)],
            [_ar("آخر تحديث"), _ar(upd)],
        ]

        t1 = Table(kv, colWidths=[5.2*cm, 12.0*cm])
        ...
        # ملخص المبيعات
        kv2 = [
            [_ar("إجمالي الطلبات"), _ar(str(total_orders))],
            [_ar("منجزة/مؤكدة"), _ar(str(done_orders))],
            [_ar("غير منجزة"), _ar(str(open_orders))],
            [_ar("إجمالي منجز (قيمة القطع)"), _ar(_money(done_amt))],
            [_ar("إجمالي غير منجز (قيمة القطع)"), _ar(_money(open_amt))],
        ]
        t2 = Table(kv2, colWidths=[7.0*cm, 10.2*cm])
