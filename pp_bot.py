...
async def _send_client_payment_preview(
    ...
    # ✅ إرسال للتاجر أيضاً عند دفع "قيمة القطع" (حتى يرى الإجمالي الصحيح مع الشحن إن وُجد)
    # (بدون تكرار مزعج لمسار رسوم المنصة)
    if pay_scope == "goods" and trader_id0:
        try:
            kb_trader = trader_status_kb(order_id) if "trader_status_kb" in globals() else None
        except Exception:
            kb_trader = None

        try:
            await context.bot.send_message(
                chat_id=int(trader_id0),
                text=msg,
                parse_mode="HTML",
...
async def _alert(q, text: str | None = None, force: bool = False):
    ...
    ✅ ذكي: لو النص مهم (غير مصرح/مقفول/تعذر/غير متاح/تحذير أحمر...) يتحول Popup تلقائياً
    """
    try:
        if text is None:
            await q.answer()
        ...
        # ✅ ذكاء: ترقية تلقائية إلى Popup للرسائل "المهمة"
        if not force:
            s_low = s.lower()

            # كلمات/عبارات مهمة (عربي + انجليزي)
            important_phrases = (
                "غير مصرح", "غير مسموح", "ممنوع", "لا يمكنك", "لا يمكن",
                "مقفول", "مغلق", "منتهي", "غير متاح", "غير متوفر",
                "تعذر", "فشل", "خطأ", "خطا",
                "blocked", "not allowed", "unauthorized", "forbidden", "denied", "error", "failed",
            )

            # بدايات مهمة (رموز تنبيه)
            important_prefixes = ("⛔", "🔒", "🟥", "⚠️", "🚫")

            # قرار الترقية
            if s.startswith(important_prefixes) or any(p in s for p in important_phrases) or any(p in s_low for p in important_phrases):
                force = True

        await q.answer(text=s, show_alert=bool(force))
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _hdr(
        ...
        ship_now_raw = (str(ship_fee or "").strip() or str(snap.get("shipping_fee") or "").strip())
        total_now_raw = (str(total_amt or "").strip() or str(snap.get("total_amount") or "").strip())
    ...
    if action == "ppq_begin":
        ...
        g, s, t = _save_amounts_to_order(order_id, its, pm)

        snap = _order_snapshot(order_id)
        await q.message.reply_text(
            _hdr(order_id, snap=snap, goods_total=g, ship_fee=s, total_amt=t) +
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            # (الشحن قد يُضاف لاحقاً، لكن نثبت الآن إجمالي القطع الصحيح)
            ship_fee_existing = snap.get("shipping_fee") or ""
            ship_inc = snap.get("ship_included") or ""
            g, s, t = _save_amounts_to_order(order_id, its, pm, shipping_fee=ship_fee_existing, ship_included=ship_inc)
    ...
    if action == "ppq_shipinc":
        ...
        if v_inc == "yes":
            ...
            await q.message.reply_text(
                _hdr(order_id, snap=_order_snapshot(order_id), goods_total=g, ship_fee=s, total_amt=t) +
                "\n\n⏳ اختر مدة التجهيز:",
        ...
        # نخلي shipping_fee_sar فاضي إلى أن يكتبها التاجر
        try:
    ...
    if action == "ppq_eta":
        ...
        # ✅ قبل الإرسال النهائي: نثبت آخر أرقام صحيحة (قطع + شحن + إجمالي)
        try:
            its = _get_items(order_id)
            pm = _get_price_map_for_order(order_id)
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_status in ("preparing", "prep"):
        ...
        next_step_client = "➡️ <b>الخطوة القادمة</b>: عند الاستلام اضغط (✅ تم الاستلام) لإغلاق الطلب."
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
        ...
        try:
            ...
            for aid in ADMIN_IDS:
                try:
                    cap_admin = (
                        f"🧾 فاتورة تاجر (نسخة للادمن) - الطلب {order_id2}"
                        f"{money_block}"
                    )
                    if is_photo:
            ...
            if client_id2:
                ...
                # ✅ إشعار إداري إضافي مختصر (لو تحب يكون أوضح من الكابشن)
                for aid in ADMIN_IDS:
                    try:
                        admin_lines = [
                            "📣 <b>تم رفع فاتورة التاجر</b>",
                            f"🧾 الطلب: <code>{html.escape(order_id2)}</code>",
                        ]
                        if am.get("goods_txt"):
                            admin_lines.append(f"💰 قيمة القطع: <b>{html.escape(am['goods_txt'])}</b>")
                        if am.get("ship_val", 0) > 0 and am.get("ship_txt"):
                            admin_lines.append(f"🚚 رسوم الشحن: <b>{html.escape(am['ship_txt'])}</b>")
                        if am.get("total_txt"):
                            admin_lines.append(f"🧾 الإجمالي: <b>{html.escape(am['total_txt'])}</b>")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await _reply_html(
                ...
                [
                    ...
                    f"🧾 رقم الطلب: <code>{html.escape(order_id_reply or '—')}</code>",
                ],
            )
        except Exception:
                    ...
                    if order_id_support:
                        await _reply_html(
                            ...
                            [
                                ...
                                f"🧾 رقم الطلب: <code>{html.escape(order_id_support)}</code>",
                            ],
                            kb=_support_kb(),
                        )
                    else:
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_SEND_PAYLINK:
        ...
        await _reply_html("تم الإرسال", [f"✅ تم إرسال رابط الدفع للعميل.", f"🧾 رقم الطلب: <code>{html.escape(order_id)}</code>"])
        set_stage(context, user_id, STAGE_NONE)
        ud.pop("paylink_order_id", None)
        ud.pop("paylink_client_id", None)
        return
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_FIND_ORDER:
        ...
        o = ob.get("order", {})
        msg = (
            f"📦 <b>الطلب</b> <code>{html.escape(oid)}</code>\n"
                    ...
                    try:
                        ...
                        await context.bot.send_message(
                            ...
                            text=(
                                ...
                                f"🧾 رقم الطلب: <code>{html.escape(order_id2)}</code>\n"
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        def _quote_hdr(oid: str, goods_total: str = "") -> list:
            ...
            if not goods_now:
                goods_now = str(
                    ...
                    or (snap or {}).get("goods_amount")
                    or (snap or {}).get("goods_amount_sar")
                    or ""
                ).strip()

            ship_fee = str(td.get("quote_shipping_fee") or (snap or {}).get("shipping_fee") or (snap or {}).get("shipping_fee_sar") or "").strip()
        ...
        if step == "eta_custom":
            v = (text or "").strip()
            if len(v) < 2:
                await _reply_html("مدة غير واضحة", _quote_hdr(order_id) + ["ℹ️ اكتبها بصيغة مفهومة.", "مثال: <code>2-3 أيام</code>"])
                return
            td["quote_ship_eta"] = v
            td["quote_step"] = "done"
        ...
        if step == "avail_custom":
            v = (text or "").strip()
            if len(v) < 2:
                await _reply_html("مدة غير واضحة", _quote_hdr(order_id) + ["ℹ️ اكتبها بصيغة مفهومة.", "مثال: <code>5 أيام</code>"])
                return
            td["quote_availability"] = v
            td["quote_step"] = "eta"
            await _reply_html(
        ...
        await _reply_html("تنبيه", _quote_hdr(order_id) + ["ℹ️ أكمل الخطوات باستخدام الأزرار حتى لا تتداخل المراحل."])
        return
    ...
    if stage == STAGE_CHAT_TRADER:
        ...
        snap = _order_snapshot(order_id)
        amt = _fmt_money(snap.get("goods_amount") or "")
    ...
    if stage == STAGE_TRADER_REPLY:
        ...
        snap = _order_snapshot(order_id)
        amt_txt = _fmt_money(snap.get("goods_amount") or "")
        ...
        try:
            if role == "client":
                msg = (
                    ...
                    f"💰 الإجمالي: <b>{html.escape(_fmt_money(snap.get('total_amount') or '') or '—')}</b>\n"
    ...
    if stage == STAGE_TRADER_CHAT_ADMIN:
        ...
        snap = _order_snapshot(order_id)
        total_txt = _fmt_money(snap.get("total_amount") or "")
        goods_txt = _fmt_money(snap.get("goods_amount") or "")
        ship_txt = _fmt_money(snap.get("shipping_fee") or "")
        ...
        if ship_txt:
            lines.append(f"🚚 قيمة الشحن: <b>{html.escape(ship_txt)}</b>")
        if total_txt:
            lines.append(f"🧾 الإجمالي: <b>{html.escape(total_txt)}</b>")
    ...
    if stage == STAGE_ASK_PICKUP_PHONE:
        ...
        try:
            ...
            await _reply_html("اختيار الدفع", ["👇 اختر طريقة الدفع من الأزرار."], kb=pay_method_kb())
        return
    ...
    if stage == STAGE_ASK_SHIP_CITY:
        ...
        await _reply_html("العنوان المختصر", ["🏠 اكتب العنوان الوطني المختصر."])
        return

    if stage == STAGE_ASK_SHIP_STREET:
        if len(text) < 3:
            await _reply_html("عنوان غير واضح", ["⚠️ اكتب العنوان المختصر بشكل أوضح."])
            return
        ud.setdefault("ship", {})["short"] = text.strip()
        set_stage(context, user_id, STAGE_ASK_SHIP_PHONE)
        await _reply_html("رقم الاتصال", ["📱 اكتب رقم الاتصال. مثال: <code>05xxxxxxxx</code>"])
        return
    ...
    if stage == STAGE_ASK_SHIP_PHONE:
        ...
        order_id = (ud.get("order_id") or "").strip()
        if not order_id:
            await _reply_html("تعذر الربط", ["⚠️ تعذر ربط عنوان الشحن بالطلب. اعد المحاولة من البداية."])
            set_stage(context, user_id, STAGE_NONE)
            return
