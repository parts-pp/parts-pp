...
# ==================================================
# ✅ Unified status update notification builder (SAFE PATCH)
# الهدف: توحيد مضمون رسائل تحديث الحالة (عميل/تاجر/إدارة) بدون تغيير منطق الأزرار أو تدفق الحالات
# يستخدم نفس الحقول/المعادلات الموجودة في trader_status_cb
# ==================================================

def _kb_strip_chat_buttons(kb: InlineKeyboardMarkup | None) -> InlineKeyboardMarkup | None:
    """يحذف أزرار الشات من كيبورد التاجر إن وجدت (يحافظ على بقية الأزرار)."""
    try:
        if not kb or not getattr(kb, "inline_keyboard", None):
            return kb
        out = []
...
def _build_status_message_lines(
    order_id: str,
    client_name: str,
    car_name: str,
    accepted_name: str,
    ...
    total_line: str,
    parts_block: str = "",
    header: str = "📦 <b>تحديث حالة الطلب</b>",
    updated_by: str = "",
    next_step: str = "",
    tracking_number: str = "",
    include_updated_by: bool = False,
) -> str:
    """يبني نص رسالة HTML موحد بنفس شكل المثال (بدون تغيير محتوى الأزرار)."""
    lines = []
    lines.append(header)
    lines.append("")
    lines.append(f"🧾 رقم الطلب: <b>{_esc(order_id)}</b>")
    lines.append(f"👤 العميل: <b>{_esc(client_name)}</b>")
    if car_name:
        lines.append(f"🚗 السيارة: <b>{_esc(car_name)}</b>")
    lines.append(f"🧑‍💼 التاجر: <b>{_esc(accepted_name)}</b>")
    lines.append(f"🏪 المتجر: <b>{_esc(trader_store)}</b>")
    lines.append(f"📌 الحالة: <b>{_esc(display_status)}</b>")

    trk = (tracking_number or "").strip()
    if trk:
        lines.append(f"📦 رقم التتبع: <b>{_esc(trk)}</b>")

    lines.append("")
    lines.append(str(goods_line or "").strip())
    lines.append(str(ship_line or "").strip())
    lines.append(str(total_line or "").strip())

    if include_updated_by and updated_by:
        lines.append("")
        lines.append(f"🧑‍💼 تم التحديث بواسطة: <b>{_esc(updated_by)}</b>")
...
async def _notify_status_update_all(
    context: ContextTypes.DEFAULT_TYPE,
    order_id: str,
    new_status: str,
    updated_by_q=None,
    tracking_number: str = "",
    # allow preserving existing keyboard behavior per-caller
    client_kb=None,
    trader_kb=None,
    admin_kb=None,
    strip_trader_chat_buttons: bool = False,
) -> None:
    """
    يرسل إشعارات تحديث الحالة بشكل موحد:
    - العميل
    - الإدارة
    - التاجر
    مع الحفاظ على كيبورد كل جهة حسب ما يمرره المُستدعي.
    """
    order_id = (order_id or "").strip()
    if not order_id:
        return
    ...
    car_name = (order.get("car_name") or order.get("vehicle_name") or order.get("car") or order.get("car_model") or "")
    car_name = (str(car_name).strip() or "—")
    ...
    goods_line = f"💰 مبلغ القطع: {_money(goods_total) if goods_total else '—'}"
    ship_line = "🚚 الشحن: مشمولة ✅" if shipping_included else f"🚚 الشحن: {_money(shipping_fee_effective) if shipping_fee_effective else '—'}"
    total_line = f"🧾 الإجمالي: {_money(grand_total) if grand_total else '—'}"

    # parts block (same spirit as trader_status_cb: available/unavailable + sums)
    parts_block = ""
    try:
        priced_lines = []
        unavail_lines = []
        ...
        for it in (items or []):
            ...
            if not nm:
                continue
            pr_raw = (it.get("price") or it.get("unit_price") or it.get("item_price") or it.get("quote_price") or "")
            pr_s = str(pr_raw).strip()
            if not pr_s or pr_s in ("0", "0.0", "—", "-", "غير متوفر", "غير متوفرة"):
                unavail_count += 1
                unavail_lines.append(f"• {html.escape(nm, quote=False)}")
                continue
            try:
                p = float(re.sub(r"[^0-9.]+", "", pr_s) or 0)
            except Exception:
                p = 0.0
            if p <= 0:
                unavail_count += 1
            ...
            priced_count += 1
            parts_total += p
            priced_lines.append(f"• {html.escape(nm, quote=False)} — <b>{_money(p)}</b>")

        total_items = priced_count + unavail_count
        if total_items:
            parts_block_lines = []
            parts_block_lines.append("🧩 <b>تفاصيل القطع</b>")
            parts_block_lines.append(f"متوفر: <b>{priced_count}</b> / غير متوفر: <b>{unavail_count}</b> / {total_items}")
            if priced_count:
                parts_block_lines.append(f"💰 إجمالي القطع المتوفرة: <b>{_money(parts_total)}</b>")
            parts_block_lines.append("")
            if priced_lines:
                parts_block_lines.append("✅ <b>القطع المتوفرة:</b>")
                parts_block_lines.extend(priced_lines)
    ...
    # next steps (نفس المنطق المبسط الموجود)
    next_step_client = ""
    next_step_trader = ""
    next_step_admin = ""
    ...
    updated_by_name = ""
    try:
        if updated_by_q is not None:
            updated_by_name = _user_name(updated_by_q)
    except Exception:
        updated_by_name = ""

    # defaults for keyboards (preserve caller behavior if provided)
    if admin_kb is None:
        try:
            admin_kb = admin_contact_kb(order_id)
        except Exception:
    ...
    if client_kb is None:
        # نفس قرار إظهار زر مراسلة التاجر للعميل (حسب التدفق الحالي)
        try:
            show_client_chat = (new_status in ("preparing", "prep", "ready_to_ship", "ready", "shipped", "delivered", "closed")) and bool(_assigned_trader_id(order_id))
        except Exception:
            show_client_chat = False
        client_kb = client_trader_chat_kb(order_id) if show_client_chat else None

    if trader_kb is None:
        try:
            trader_kb = trader_status_kb(order_id)
        except Exception:
            trader_kb = None

    if strip_trader_chat_buttons:
        trader_kb = _kb_strip_chat_buttons(trader_kb)

    # --- client ---
    if client_id:
        try:
            msg = _build_status_message_lines(
                order_id=order_id,
                client_name=client_name,
                car_name=car_name,
                ...
                header="📦 <b>تحديث حالة الطلب</b>",
                updated_by="",
                next_step=next_step_client,
                tracking_number=tracking_number,
    ...
    # --- admins ---
    try:
        admin_msg = _build_status_message_lines(
            order_id=order_id,
            client_name=client_name,
            car_name=car_name,
            accepted_name=accepted_name,
            ...
            total_line=total_line,
            parts_block=parts_block,
            header="📌 <b>تحديث حالة من التاجر</b>",
            updated_by=updated_by_name,
            next_step=(f"🧭 <b>متابعة</b>: {next_step_admin}" if next_step_admin else ""),
            tracking_number=tracking_number,
    ...
    to_trader = 0
    try:
        to_trader = int(accepted_tid or 0)
    except Exception:
        to_trader = 0
    if to_trader:
        try:
            trader_msg = _build_status_message_lines(
                order_id=order_id,
                client_name=client_name,
                car_name=car_name,
                accepted_name=accepted_name,
                ...
                total_line=total_line,
                parts_block=parts_block,
                header=(f"{_user_name(updated_by_q)}\n✅ <b>تم تحديث حالة الطلب</b>" if updated_by_q is not None else "✅ <b>تم تحديث حالة الطلب</b>"),
                updated_by="",
                next_step=next_step_trader,
                tracking_number=tracking_number,
                include_updated_by=False,
            )
            await context.bot.send_message(
...
# ==================================================
# ✅ End unified status update notifications
# ==================================================
    ...
    if not already_shipped:
        ...
        try:
            ...
            try:
                ...
                # ✅ إشعارات موحدة (عميل/تاجر/إدارة) بنفس تنسيق تحديث الحالة
        try:
            await _notify_status_update_all(
                context,
                order_id=order_id,
                new_status="shipped",
                updated_by_q=q,
                tracking_number="غير متوفر",
                client_kb=client_trader_chat_kb(order_id),
                trader_kb=trader_received_notice_kb(order_id),
                admin_kb=admin_contact_kb(order_id),
                strip_trader_chat_buttons=False,
            )
        except Exception as e:
            _swallow(e)
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE:
        ...
        if kind == "tracking":
            ...
            if tracking and tracking.lower() in [w.lower() for w in skip_words]:
                ...
                # ✅ إشعارات موحدة (عميل/تاجر/إدارة) — مرة واحدة فقط
                if not already_shipped:
                    try:
                        await _notify_status_update_all(
                            context,
                            ...
                            updated_by_q=update,
                            tracking_number="غير متوفر",
                            client_kb=client_trader_chat_kb(order_id2),
                            trader_kb=trader_received_notice_kb(order_id2),
                            admin_kb=admin_contact_kb(order_id2),
                            strip_trader_chat_buttons=False,
                        )
                    except Exception as e:
                        _swallow(e)
...
async def confirm_received_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await _notify_status_update_all(
            ...
            updated_by_q=q,
            tracking_number="",
            client_kb=None,  # نفس قرار إظهار زر مراسلة التاجر للعميل
            trader_kb=trader_received_notice_kb(order_id),
            admin_kb=admin_contact_kb(order_id),
            strip_trader_chat_buttons=False,
        )
