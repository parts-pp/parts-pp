...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "restore_unlock":
        ...
        await _alert(q, "تم تفعيل الاسترجاع")
        # إعادة عرض صفحة الاسترجاع بتحديث الحالة
        try:
            q.data = "pp_admin|restore_db"  # لو ممنوع تعديل q.data في نسختك احذف السطر هذا
        except Exception:
            pass
        # الأفضل: نعيد فتحها مباشرة
        try:
            await _admin_edit_or_send(q, "✅ تم تفعيل الاسترجاع مؤقتًا.\nالآن أرسل ملف الإكسل.", InlineKeyboardMarkup([
                [InlineKeyboardButton("🗂 فتح صفحة الاسترجاع", callback_data="pp_admin|restore_db")],
                [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
            ]))
        except Exception:
            pass
    ...
    if action == "restore_lock":
        ...
        try:
            await _admin_edit_or_send(q, "🔒 تم تعطيل الاسترجاع.", InlineKeyboardMarkup([
                [InlineKeyboardButton("🗂 فتح صفحة الاسترجاع", callback_data="pp_admin|restore_db")],
                [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
            ]))
        except Exception:
