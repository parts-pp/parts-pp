...
def build_official_quote_text(
    ...
    if ship_included:
        ...
        shipping_txt = _money(shipping_val) if shipping_fee_sar else "—"
    ...
    return (
        ...
        "📦 <b>تفاصيل المبالغ</b>\n"
        f"🧩 قيمة القطع: <b>{goods_txt}</b>\n"
        f"🚚 قيمة الشحن: <b>{shipping_txt}</b>\n"
        f"💰 إجمالي القطع + الشحن: <b>{total_txt}</b>\n\n"
        "🔧 <b>نوع القطع</b>:\n"
        f"✔️ {_ppq_type_label(parts_type)}\n\n"
        f"{ship_block}\n\n"
        f"⏳ مدة التجهيز: <b>{availability}</b>\n\n"
