...
async def send_trader_ledger_pdf(
    ...
    # --- Arabic RTL + shaping (FIX: keep tags, shape text parts) ---
    try:
        ...
        def _ar(s: str) -> str:
            ...
            if not s:
                return s
            try:
                # قسم النص إلى (وسوم) و(نص عادي) وطبّق التشكيل على النص فقط
                ...
                for part in parts:
                    ...
                    if part.startswith("<") and part.endswith(">"):
                        out.append(part)  # اترك الوسم كما هو
                    else:
                        try:
    ...
    def _pick(*keys: str) -> str:
        """يرجع أول قيمة غير فاضية من مفاتيح متعددة (fallback keys)."""
        for k in keys:
            try:
    ...
    def kv_table(rows: list[tuple[str, str]]):
        ...
        - القيم يسار
        - المعرفات يمين
        (أي: عمود القيمة أولاً، ثم عمود المعرّف)
        """
        data = []
        for k, v in rows:
            vtxt = v if v else "—"
            # [القيمة, المعرّف]
            data.append([P(vtxt, left), P(f"<b>{k}</b>", right)])

        t = Table(data, colWidths=[full_w*0.66, full_w*0.34])
        t.setStyle(TableStyle([
            ("BOX", (0, 0), (-1, -1), 0.8, C_BORDER),
    ...
    kv_table([
        ("رقم السجل", inv_no),  # ✅ يضمن وجوده داخل السجل نفسه
