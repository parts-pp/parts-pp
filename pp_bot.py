...
async def pp_rb_stop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إشعار صاحب الإجراء
    try:
        who_txt = "الإدارة" if is_admin else "صاحب الطلب"
        await context.bot.send_message(
            chat_id=uid,
            text=(
                f"⛔ تم إيقاف إعادة النشر تلقائيًا لهذا الطلب ({who_txt})\n"
                f"🧾 رقم الطلب: {_order_id_link_html(order_id)}"
            ),
            parse_mode="HTML",
            disable_web_page_preview=True,
    ...
    # إذا أوقفها العميل: نبلغ الإدارة برسالة مختصرة (حتى يتوقفوا عن المتابعة على طلبه)
