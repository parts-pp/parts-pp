...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if missing:
        ...
        try:
            ...
            await _alert(q, f"⛔ أكمل بيانات لوحة التاجر أولاً.
📌 الناقص: {miss_txt}", force=True)
        except Exception as e:
            _swallow(e)
            try:
...
if action == "tstrict":
    ...
    try:
        await q.answer("✅ تم تفعيل الإلزام" if strict_on else "✅ تم تعطيل الإلزام", show_alert=False)
    except Exception as e:
        _swallow(e)
    ...
    try:
        await context.bot.send_message(
            ...
            text=("✅ تم تفعيل الإلزام في لوحة التجار" if strict_on else "✅ تم تعطيل الإلزام في لوحة التجار"),
            disable_web_page_preview=True,
        )
    except Exception as e:
        _swallow(e)
