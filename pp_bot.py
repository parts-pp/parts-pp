...
async def send_invoice_pdf(
    ...
    def _inv_no_display(full: str) -> str:
        ...
        if not s:
            return "—"
        tail_suffix = s[-4:] if len(s) >= 4 else s
        kn = "PRE" if kind_norm == "preliminary" else "SHP"
        return f"{order_id_disp}-{kn}-{tail_suffix}"
...
def _parse_order_search_input(s: str) -> dict:
    '''
    يقبل:
      - pp0012 / PP0012 / pp000012 / pp10000 / pp0100000 ...
      - 0012 / 000012 / 10000 ...
      - 240217-0012 / 240217-000012 / 240217-10000 ...
      - PP-240217-0012 / PP-240217-000012 / PP-240217-10000 ...
    ويرجع:
      {"kind": "tail", "tail": "..."} أو {"kind":"full","order_id":"..."} أو {"kind":"none"}

    ملاحظة أمان (منع التداخل مع أرقام التسعير):
      - الرقم الصافي (بدون pp وبدون تاريخ) بطول 4 يُقبل فقط إذا يبدأ بـ 0 (مثال: 0012)
        لتجنب التداخل مع أرقام مثل 1200 أثناء مراحل التسعير.
      - بطول 5 فأكثر يُقبل كما هو (لأن العداد قد يتجاوز 9999: مثل 10000).
    '''
    t = (s or "").strip()
    if not t:
        return {"kind": "none"}
    ...
    # digits only (allow 4..9) — but 4 digits must start with 0
    m = re.match(r"^(\d{4,9})$", u)
    if m:
        tail = m.group(1)
        if len(tail) == 4 and not tail.startswith("0"):
            return {"kind": "none"}
        return {"kind": "tail", "tail": tail}

    # 240217-0012 or PP-240217-0012 (seq 4..9)
    ...
    if m:
        ...
        if len(seq) == 4:
            seq = seq.zfill(4)
        return {"kind": "full", "order_id": f"{d6}-{seq}"}

    # direct exact id as-is (best effort)
    if re.match(r"^\d{6}-\d{4,9}$", u):
        return {"kind": "full", "order_id": u}
...
async def _resolve_and_show_order(context: ContextTypes.DEFAULT_TYPE, user_id: int, tail_suffix: str):
    tail_suffix = str(tail_suffix).strip()
    if not tail_suffix.isdigit():
        return
    ...
    for o in uniq:
        ...
        if not oid:
            continue
        if not oid.endswith(tail_suffix):
            continue
    ...
    if not matches_info:
        await context.bot.send_message(
            chat_id=uid,
            text=f"🔍 لا يوجد طلب ينتهي بالرقم {tail_suffix}",
        )
        return
    ...
    for it in matches_info[:10]:
        ...
        rows.append([
            InlineKeyboardButton("📜 السجل القانوني / آخر تحديث", callback_data=f"pp_order_legal|{oid}"),
        ])
    ...
    await context.bot.send_message(
        chat_id=uid,
        text=(
            "🔎 وُجد أكثر من طلب بنفس النهاية (" + tail_suffix + ")\n"
...
async def open_order_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ العميل صاحب الطلب: نعرض “سجل الطلب القانوني / آخر تحديث” (أوضح له من لوحة التاجر)
    if client_id and uid == client_id:
        try:
            msg, kb = build_order_legal_message(order_id, uid)
            await q.message.reply_text(
                msg,
                parse_mode="HTML",
                reply_markup=kb,
                ...
                if msg:
                    ...
                    if not order_id_support:
                        try:
                            ...
                            tail_suffix = (m4.group(1) or "").strip() if m4 else ""
                        except Exception:
                            tail_suffix = ""
