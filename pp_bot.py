...
def _parse_order_search_input(s: str) -> dict:
    ...
    يقبل:
      ...
      - PP-240217-0012
    ويرجع:
      {"kind": "tail", "tail": "0012"} أو {"kind":"full","order_id":"240217-0012"} أو {"kind":"none"}
    '''
    t = (s or "").strip()
    if not t:
        return {"kind": "none"}
    ...
    # pp0012
    m = re.match(r"^PP(\d{4})$", u, flags=re.I)
    if m:
    ...
    # 240217-0012 or PP-240217-0012
    m = re.match(r"^(?:PP-)?(\d{6})-(\d{4,})$", u, flags=re.I)
    if m:
        return {"kind": "full", "order_id": f"{m.group(1)}-{m.group(2).zfill(4)}"}

    # allow direct exact id as-is (best effort)
    if re.match(r"^\d{6}-\d{4,}$", u):
...
def _viewer_role_for_order(viewer_id: int, order: dict) -> str:
    try:
        ...
        vid = 0
    if vid and vid in (ADMIN_IDS or []):
        return "admin"

    try:
        cid = int(order.get("user_id") or 0)
    except Exception:
        cid = 0
...
def _fmt_utc_ts_for_humans(s: str) -> str:
    ...
    try:
        ...
        try:
            ...
            dt2 = dt.astimezone(ksa)
        except Exception:
            dt2 = dt.astimezone(timezone(timedelta(hours=3)))
...
def build_order_legal_message(order_id: str, viewer_id: int) -> tuple[str, InlineKeyboardMarkup | None]:
    """رسالة قانونية كاملة (ملخص حالة الطلب + خط زمني) + أزرار حسب الصلاحية."""
    try:
        b = get_order_bundle(order_id) or {}
        o = (b.get("order") or {}) if isinstance(b, dict) else {}
    ...
    lines.append("سجل الطلب القانوني")
    lines.append("")
...
async def _resolve_and_show_order(context: ContextTypes.DEFAULT_TYPE, user_id: int, tail4: str):
    ...
    if len(matches) == 1:
        try:
            msg, kb = build_order_legal_message(matches[0], uid)
            await context.bot.send_message(
                chat_id=uid,
                text=msg,
                parse_mode="HTML",
    ...
    kb = InlineKeyboardMarkup([
        [
            ...
            InlineKeyboardButton("📜", callback_data=f"pp_order_legal|{oid}"),
        ]
