...
# ==========================================================
# Trader profile requirements (always enforced)
# ==========================================================
...
def _trader_profile_missing(tp: dict) -> list:
    tp = tp or {}
    missing = []
    for f in TRADER_PROFILE_REQUIRED_FIELDS:
        if not str(tp.get(f) or "").strip():
            missing.append(f)
    return missing


def _trader_profile_complete(tp: dict) -> bool:
    return not _trader_profile_missing(tp or {})


def _fmt_missing_fields_lines(missing: list) -> str:
    if not missing:
        return ""
    lines = []
    for i, f in enumerate(missing, start=1):
        lines.append(f"{i}) {TRADER_PROFILE_FIELD_LABELS.get(f, f)}")
    return "\n".join(lines)
...
async def _need_complete_trader_profile_notice(
    ...
    """رسالة واحدة واضحة بدون روابط/أزرار (لتجنب التوهان)."""
    try:
    ...
    extra = f"\n\n🧾 رقم الطلب: {html.escape(str(order_id))}" if order_id else ""

    msg = (
        f"{name}\n"
        "⛔ لا يمكنك تقديم/إرسال عرض سعر قبل إكمال <b>ملف التاجر</b>.\n\n"
        "📌 الحقول المطلوبة:\n"
        f"{html.escape(_fmt_missing_fields_lines(missing) or '—')}"
        f"{extra}\n\n"
        "✅ لإكمال الملف: افتح لوحة التاجر في الخاص (اكتب: <code>تاجر</code>) ثم أدخل البيانات من الأزرار."
    )
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        missing = _trader_profile_missing(tp)
    if missing:
        # ✅ رسالة واحدة واضحة (بدون تكرار رسائل/أزرار)
        await _need_complete_trader_profile_notice(
            context,
            user_id,
            _user_name(q),
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_quote":
        ...
        if missing:
            await _need_complete_trader_profile_notice(
                context, actor_id, _user_name(q), order_id, missing=missing
            )
            return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_PROFILE_EDIT:
        ...
        # ==========================
        # ✅ قواعد إدخال بسيطة وواضحة
        # ==========================
        if field in ("display_name", "company_name", "bank_name"):
            if len(val) < 2:
                await _reply_html("قيمة غير واضحة", ["⚠️ اكتب النص بشكل أوضح."])
                return
            if len(val) > 80:
                await _reply_html("قيمة طويلة", ["⚠️ اختصر النص (حد أقصى 80 حرف)."])
                return
        ...
        if field == "national_id":
            ...
            if not re.fullmatch(r"\d{10}", v):
                await _reply_html(
                    "رقم الهوية غير صحيح",
                    [
                        "⚠️ الصيغة المطلوبة: 10 أرقام (بدون مسافات).",
        ...
        if field == "vat_number":
            ...
            if not re.fullmatch(r"\d{15}", v):
                await _reply_html(
                    ...
                    [
                        "⚠️ الصيغة المطلوبة: 15 رقم (بدون مسافات).",
        ...
        if missing:
            await _reply_html(
                ...
                [
                    ...
                    "• الرقم الضريبي",
                    "",
                    "✅ افتح لوحة التاجر في الخاص (اكتب: تاجر) ثم أكمل البيانات.",
                ],
            )
            return
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _admin_show_traders_manage():
        ...
        msg = "🧑‍💼 <b>إدارة التجار</b>\n🔒 إلزام ملف التاجر: <b>ثابت</b> (7 حقول)\n\nاختر تاجر لفتح ملفه أو تفعيل/تعطيل:"
        kb = InlineKeyboardMarkup(
            rows + [[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|home")]]
        )
        await _admin_edit_or_send(q, msg, kb)
...
def trader_panel_kb() -> InlineKeyboardMarkup:
    # ✅ ترتيب واضح + تقسيم منطقي حتى لا يتوه التاجر
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    "💳 تعديل رقم الايبان (IBAN)", callback_data="pp_tprof|edit|iban"
                )
            ],
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    "🧾 تعديل الرقم الضريبي (VAT)",
                    callback_data="pp_tprof|edit|vat_number",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    if not enabled:
        ...
        if missing:
            txt += (
                ...
                "✅ أكملها من أزرار التعديل بالأسفل، ثم ارجع وقدّم عرض السعر."
            )
        else:
