...
async def send_trader_ledger_pdf(
    ...
    # username (from profile if exists)
    tuser = _pick(
        "username",
        "tg_username",
        "user_name",
    ...
    # ✅ (01) بدج الشريط فقط للحجز في التخطيط (بدون نص نهائياً)
    badges = Table([[P(" ", tiny_c), P(" ", tiny_c), P(" ", tiny_c)]], colWidths=[full_w / 3.0] * 3)
    ...
    # ===== watermark + badge-text + footer on pages =====
    def _wm(canvas, docx):
        try:
            page_w, page_h = A4
            ...
            # ✅ (01) رسم سطر التاجر/التاريخ/الوقت يدويًا (بدون Paragraph) لمنع القلب نهائيًا
            try:
                seg_w = full_w / 3.0
                left_x = doc.leftMargin
                # هذا Y مضبوط ليمر داخل شريط الـbadges (تحت العنوان مباشرة)
                badge_h = 0.62 * cm
                badge_y = page_h - (0.65 * cm + 3.00 * cm + 0.30 * cm + 0.80 * cm + badge_h)  # ثابت مع نفس تصميمك

                # حدود خفيفة (مطابقة للشريط)
                canvas.saveState()
                try:
                    canvas.setFillAlpha(1.0)
                except Exception:
                ...
                canvas.rect(left_x, badge_y, full_w, badge_h, stroke=1, fill=1)

                # خطوط تقسيم
                canvas.line(left_x + seg_w, badge_y, left_x + seg_w, badge_y + badge_h)
                canvas.line(left_x + 2 * seg_w, badge_y, left_x + 2 * seg_w, badge_y + badge_h)

                canvas.setFont(font_name, 8.8)
                canvas.setFillColor(colors.HexColor("#0B1220"))

                pad = 6  # مسافة ثابتة تمنع الالتصاق
                mid_y = badge_y + (badge_h / 2.0) - 3

                # يمين: التاجر
                trader_label = _ar("التاجر")
                trader_val = (tname or str(tid)).strip()
                if tuser:
                    trader_val = f"{trader_val}  {tuser}"  # مسافة واضحة

                seg3_x1 = left_x + 2 * seg_w
                seg3_x2 = left_x + 3 * seg_w
                canvas.drawRightString(seg3_x2 - pad, mid_y, trader_label)
                canvas.drawString(seg3_x1 + pad, mid_y, trader_val)

                # وسط: التاريخ
                date_label = _ar("التاريخ")
                date_val = now.strftime("%Y-%m-%d")
                seg2_x1 = left_x + 1 * seg_w
                seg2_x2 = left_x + 2 * seg_w
                canvas.drawRightString(seg2_x2 - pad, mid_y, date_label)
                canvas.drawString(seg2_x1 + pad, mid_y, date_val)

                # يسار: الوقت
                time_label = _ar("الوقت")
                time_val = now.strftime("%H:%M")
                seg1_x1 = left_x + 0 * seg_w
                seg1_x2 = left_x + 1 * seg_w
                canvas.drawRightString(seg1_x2 - pad, mid_y, time_label)
                canvas.drawString(seg1_x1 + pad, mid_y, time_val)
