...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    # ✅ قفل بصري ديناميكي: يفتح الزر الحالي ويقفل البقية (لكن تبقى قابلة للضغط لعرض Popup توجيهي)
    try:
    ...
    # السماح حسب التسلسل
    allow_prep = ost not in ("shipped", "delivered", "closed")
    allow_ready = ost in ("preparing", "prep")
    allow_shipped = ost in ("ready_to_ship", "ready")
    allow_delivered = ost in ("shipped",)
    allow_chat = ost in ("shipped", "delivered", "closed")

    def _lbl(ok: bool, txt: str) -> str:
        return txt if ok else f"🔒 {txt}"

    return InlineKeyboardMarkup([
        [InlineKeyboardButton(_lbl(allow_prep, "🟡 جاري تجهيز الطلب"), callback_data=f"pp_trader_status|prep|{order_id}")],
        [InlineKeyboardButton(_lbl(allow_ready, "🟢 الطلب جاهز للشحن"), callback_data=f"pp_trader_status|ready|{order_id}")],
        [InlineKeyboardButton(_lbl(allow_shipped, "🚚 تم شحن الطلب"), callback_data=f"pp_trader_status|shipped|{order_id}")],
        [InlineKeyboardButton(_lbl(allow_delivered, "✅ تم تسليم الطلب للعميل بنجاح"), callback_data=f"pp_trader_status|delivered|{order_id}")],
        [InlineKeyboardButton(_lbl(allow_chat, "💬 مراسلة العميل داخل المنصة"), callback_data=f"pp_chat_open|{order_id}")],
    ])



def trader_received_notice_kb(order_id: str) -> InlineKeyboardMarkup:
    # إشعار "تأكيد الاستلام" يصل للتاجر بدون لوحة الطلب
    return InlineKeyboardMarkup([
...
async def _send_client_payment_preview(
    ...
    ship_fee_txt = _money(order.get("shipping_fee_sar") or "")
    ...
    if missing_ship:
        raw_shipping_fee = 0 if ship_included_norm in ("yes", "true", "1", "included", "مشمولة") else 0

    ship_fee_val = _to_float(raw_shipping_fee)

    # ✅ عرض الشحن: إذا فاضي وغير مشمول -> "يحددها التاجر"
    if missing_ship and ship_included_norm not in ("yes", "true", "1", "included", "مشمولة"):
        ship_fee_total_txt = "يحددها التاجر"
    else:
    ...
    if pay_scope == "goods":
        ...
        next_line = "بعد التحقق سيتم اعتماد الدفع وإرسال بيانات الشحن."
        icon_scope = "🧩"
    else:
    ...
    if pay_scope == "goods" and isinstance(items, list) and items:
        ...
        parts_summary = (
            "━━━━━━━━━━━━\n"
            + f"📌 <b>ملخص القطع</b>\n"
            + f"✅ مسعّرة: <b>{priced_count}</b>  |  🟥 غير متوفرة: <b>{unpriced_count}</b>  |  📦 الإجمالي: <b>{total_items}</b>\n"
            + "━━━━━━━━━━━━\n"
        )
    ...
    msg = (
        ...
        f"🧾 <b>إجمالي الفاتورة</b>: <b>{inv_total_txt}</b>\n"
        + (f"💰 <b>قيمة القطع</b>: <b>{amt_txt}</b>\n" if pay_scope == "goods" else f"💰 <b>رسوم المنصة</b>: <b>{amt_txt}</b>\n")
        + (f"🚚 <b>قيمة الشحن</b>: <b>{ship_fee_total_txt}</b>\n" if (pay_scope == "goods" and ship_fee_total_txt and ship_fee_total_txt != "—") else "")
    ...
    # ✅ إرسال للعميل فقط (منع التكرار على التاجر)
    try:
...
async def _need_complete_trader_profile_notice(context: ContextTypes.DEFAULT_TYPE, user_id: int, name: str, order_id: str = ""):
    ...
    msg = (
        f"{name}\n"
        "⛔ لا يمكنك تقديم عرض سعر قبل إكمال ملف التاجر واكمال طرق الدفع.\n\n"
        "✅ الخطوات المختصرة:\n"
...
async def quote_ok_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # fallback مضبوط لو ملف التاجر ناقص
    if not tname:
    ...
    update_order_fields(order_id, {
        ...
        # ✅ أهم سطرين: تثبيت الاسم (والشركة) حتى يظهر بعد إرسال الإيصال
    ...
    # ✅ جديد: جدولة نسخة احتياطية بعد قبول العرض (بدون بطء + بدون تكرار سريع)
    try:
        app = getattr(context, "application", None)
        if app:
            if not app.bot_data.get("_backup_touch_quote_ok"):
                app.bot_data["_backup_touch_quote_ok"] = True

                async def _bk_job():
                    try:
                        await asyncio.sleep(5)
                        await _send_backup_excel(app, reason="quote_accepted")
                    finally:
                        try:
                            app.bot_data["_backup_touch_quote_ok"] = False
                        except Exception:
                            pass
    ...
    # تثبيت أنه تم إشعار التاجر
    try:
        update_order_fields(order_id, {"accepted_trader_notified": "yes"})
    except Exception:
        pass

    # إشعار التاجر السابق عند العدول
    if switched:
        try:
            await context.bot.send_message(
                chat_id=prev_tid,
                text=(
                    ...
                    f"🧾 رقم الطلب: {order_id}"
                    f"{city_line}\n\n"
                    "وصل للعميل عرض آخر وتم اختيار عرض مختلف\n"
                    "يمكنك تقديم عرض جديد طالما لم يتم الدفع\n\n"
                    "🔒 تنبيه: لا يتم عرض رقم العميل قبل الدفع"
                ),
                reply_markup=trader_quote_start_kb(order_id),
                disable_web_page_preview=True,
            )
    ...
    try:
        msg = (
            ...
            "🟦 الخطوة التالية: سيتم فتح خطوات الدفع داخل الطلب"
        )
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all((tp.get(f) or "").strip() for f in required_fields):
        ...
        try:
            await q.message.reply_text("⬆️ أكمل ملف التاجر من الزر ثم ارجع وابدأ العرض.")
        except Exception:
            pass
        return
    ...
    def _get_price_map_for_order(order_id: str):
        # ✅ الأسعار الآن داخل جلسة مرتبطة بالطلب
        qs = _qget(td, order_id, create=True)  # يعتمد على Helpers رقم (1)
    ...
    def _set_price_map_for_order(order_id: str, new_pm: dict):
        ...
        # ✅ للحفاظ على التوافق مع بقية الكود (finalize_quote_send / الفاتورة)
        # نخزن نسخة "للطلب الحالي فقط"
    ...
    def _items_kb(order_id: str, items: list, price_map: dict):
        ...
        rows.append([InlineKeyboardButton("🧾 تسعير الكل", callback_data=f"ppq_it_all|{oid}")])
        rows.append([InlineKeyboardButton("🧹 تصفير الأسعار لهذا الطلب", callback_data=f"ppq_it_none|{oid}")])
        rows.append([InlineKeyboardButton("✅ اكمل خطوات العرض", callback_data=f"ppq_it_done|{oid}")])
        return InlineKeyboardMarkup(rows)
    ...
    if action == "ppq_begin":
        ...
        # ✅ جلسة مستقلة لهذا الطلب: تصفير جذري لمنع انتقال أسعار طلب سابق
        _qreset(td, order_id)                 # يمسح جلسة هذا الطلب إن وجدت
        qs = _qget(td, order_id, create=True)
        qs["item_prices"] = {}
        ...
        # ✅ توافقي: نخزن نسخة للطلب الحالي فقط
        ...
        total_now = _calc_total(its, pm)

        await q.message.reply_text(
            f"{name}\n"
            "🧩 اختر القطعة لتسعيرها\n"
            f"🧾 رقم الطلب: {order_id}\n"
            f"💰 إجمالي القطع الحالي: {total_now} ر.س",
    ...
    # ✅ ربط أي تفاعل على كيبورد بجلسته الخاصة
    td["quote_order_id"] = order_id
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            # ✅ حفظ الـ pending داخل الجلسة (وليس عام للتاجر)
            qs = _qget(td, order_id, create=True)
            qs["pending_item_idx"] = idx
            qs["pending_item_name"] = nm
            qs["last_touch"] = int(_dt_utc_now().timestamp())

            # ✅ توافقي مع text_handler الحالي (إلى أن نعدّله بالخطوة التالية)
            ...
            # ✅ تصفير أسعار هذا الطلب فقط
            pm = {}
            _set_price_map_for_order(order_id, pm)

            # تصفير pending داخل الجلسة
            qs = _qget(td, order_id, create=True)
            qs.pop("pending_item_idx", None)
            qs.pop("pending_item_name", None)
            qs["last_touch"] = int(_dt_utc_now().timestamp())
            ...
            total = _calc_total(its, pm)

            # ✅ توافقي + مهم للفواتير: نخزن أسعار القطع للطلب الحالي
            _set_price_map_for_order(order_id, pm)
            td["quote_goods_amount"] = total
            td["quote_step"] = "type"
        ...
        try:
            await q.message.edit_reply_markup(reply_markup=_items_kb(order_id, its, _get_price_map_for_order(order_id)))
        except Exception:
            pass
        await _alert(q, "تم")
        return
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # بناء بلوك تفصيلي (يعرض المسعّر + غير متوفر لغير المسعّر)
    items_block = ""
    try:
        if isinstance(itemsx, list) and itemsx:
            ...
            lines.append("━━━━━━━━━━━━")
            lines.append(f"💰 إجمالي القطع (المسعّر): {goods_amount} ر.س")
            lines.append(f"📦 حالة الشحن: {'مشمولة' if ship_inc == 'yes' else 'غير مشمولة'} | قيمة الشحن: {fee_sar} ر.س")
            items_block = "\n".join(lines) + "\n"
    except Exception:
        items_block = ""
    ...
    fields_to_update = {
        ...
        # ✅ احتفاظ اختياري: نخزن الأسعار كـ نص (بدون تغيير هيكل الشيت)
    ...
    # ✅ جديد: نسخة احتياطية ذكية بعد حفظ العرض في الاكسل (بدون بطء + بدون تكرار سريع)
    try:
        app = getattr(context, "application", None)
        if app:
    ...
    try:
        ...
        if old_line in trader_copy:
            trader_copy = trader_copy.replace(
                old_line,
                "📌 في حال قبول العميل لعرضك سيتم إرسال لوحة الطلب لك على الخاص لإكمال البيع",
                1
            )
    except Exception:
        pass
    ...
    try:
        await message.reply_text(
            ...
            "📌 تذكير: قيمة القطع التي ستقبضها هي (مبلغ القطع) + (الشحن إن كان غير مشمول).\n\n"
            f"{trader_copy}"
        )
    except Exception:
        pass
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ترجمة الزر (st) إلى حالة داخلية فقط (للحفظ) + اسم عرض عربي من الهيلبر الموحد
    ...
    # ✅ اسم العرض العربي الموحد (بدون تكرار خرائط داخل الدالة)
    ...
    # ===== أسماء العميل/التاجر (لازم بكل إشعار) =====
    client_name = (order.get("user_name") or order.get("client_name") or order.get("customer_name") or "").strip()
    if not client_name:
        client_name = "العميل"
    ...
    # ===== ✅ حساب الإجمالي الموحد (القطع + الشحن إذا غير مشمول) =====
    goods_total = 0
    shipping_fee = 0
    ...
    grand_total = int((goods_total or 0) + (shipping_fee or 0))
    grand_total_line = f"💰 إجمالي الطلب (شامل الشحن): {_money(grand_total)}" if str(grand_total).strip() else ""

    # ===== حالة دفع البضاعة (لازم لمنع الشحن قبل السداد) =====
    gps = str(order.get("goods_payment_status") or "").strip().lower()
    goods_paid = gps in ("confirmed", "paid", "success", "successful", "done", "ok")

    # ===== ✅ حارس: منع إعادة استخدام الأزرار المنتهية (Popup فقط) =====
    ost_now = str(order.get("order_status") or "").strip().lower()

    # ===== Helper: Popup قصير + رسالة تفصيلية بالخاص (لأن Popup تيليجرام محدود ~200 حرف) =====
    async def _popup_and_pm(short_text: str, long_text: str):
        # Popup قصير (أسود + زر "حسنا")
        try:
            await _alert(q, (short_text or "")[:180], force=True)
        except Exception:
            try:
                await q.answer((short_text or "")[:180], show_alert=True)
            except Exception:
                pass

        # تفاصيل بالخاص
        try:
            await context.bot.send_message(chat_id=actor_id, text=long_text, disable_web_page_preview=True)
        except Exception:
            pass

    # ===== ✅ حارس تسلسل الأزرار (Popup + تفاصيل بالخاص) =====
    # تجهيز -> جاهز للشحن -> تم الشحن (ممنوعة قبل الدفع 100%) -> تأكيد التسليم (بعد الشحن + بعد الدفع)
    if new_status == "ready_to_ship" and ost_now not in ("preparing", "prep"):
        await _popup_and_pm(
            "⚠️ ترتيب خاطئ: ابدأ بـ (جاري تجهيز) ثم (جاهز للشحن).",
            (
                f"{actor_name}\n"
                "⚠️ لا يمكن اختيار (🟢 الطلب جاهز للشحن) قبل تنفيذ (🟡 جاري تجهيز الطلب).\n\n"
                f"🧾 رقم الطلب: {order_id}\n"
                f"👤 العميل: {client_name}\n"
                f"🧑‍💼 التاجر: {accepted_name}\n"
                f"{grand_total_line}\n"
                f"📌 الحالة الحالية: {_pay_status_ar(ost_now)}"
            ).strip()
        )
        return

    if new_status == "shipped":
        # ✅ ممنوع الشحن قبل (جاهز للشحن)
        if ost_now not in ("ready_to_ship", "ready"):
            await _popup_and_pm(
                "⚠️ ترتيب خاطئ: لازم (جاهز للشحن) قبل (تم الشحن).",
                (
                    f"{actor_name}\n"
                    "⚠️ لا يمكن اختيار (🚚 تم شحن الطلب) قبل تنفيذ (🟢 الطلب جاهز للشحن).\n\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"👤 العميل: {client_name}\n"
                    f"🧑‍💼 التاجر: {accepted_name}\n"
                    f"{grand_total_line}\n"
        ...
        # ✅ ممنوع الشحن قبل دفع قيمة البضاعة 100% (حتى لو أدمن)
        if not goods_paid:
            await _popup_and_pm(
                "⛔ ممنوع (تم الشحن) قبل تأكيد دفع البضاعة.",
                (
                    f"{actor_name}\n"
                    "⛔ لا يمكن تحديث الحالة إلى (🚚 تم شحن الطلب) قبل تأكيد سداد العميل لقيمة البضاعة.\n"
                    "هذا المنع إلزامي حتى للإدارة.\n\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"👤 العميل: {client_name}\n"
                    f"🧑‍💼 التاجر: {accepted_name}\n"
                    f"{grand_total_line}\n"
                    f"💳 حالة دفع البضاعة: {_pay_status_ar(gps or 'unpaid')}\n\n"
                    "الخطوة الصحيحة: انتظر وصول إيصال الدفع للتاجر وتأكيد الدفع ثم سيتم فتح زر (تم الشحن)."
                ).strip()
            )
            return

    if new_status == "delivered":
        # ✅ ممنوع التسليم قبل الشحن
        if ost_now not in ("shipped",):
            await _popup_and_pm(
                "⚠️ ترتيب خاطئ: لازم (تم الشحن) قبل (تأكيد التسليم).",
                (
                    f"{actor_name}\n"
                    "⚠️ لا يمكن اختيار (✅ تأكيد التسليم) قبل تنفيذ (🚚 تم شحن الطلب).\n\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"👤 العميل: {client_name}\n"
                    f"🧑‍💼 التاجر: {accepted_name}\n"
                    f"{grand_total_line}\n"
        ...
        # ✅ ممنوع التسليم قبل دفع قيمة البضاعة 100% (حتى لو أدمن)
        if not goods_paid:
            await _popup_and_pm(
                "⛔ ممنوع (تأكيد التسليم) قبل تأكيد دفع البضاعة.",
                (
                    f"{actor_name}\n"
                    "⛔ لا يمكن اختيار (✅ تأكيد التسليم) قبل تأكيد سداد العميل لقيمة البضاعة.\n"
                    "هذا المنع إلزامي حتى للإدارة.\n\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"👤 العميل: {client_name}\n"
                    f"🧑‍💼 التاجر: {accepted_name}\n"
    ...
    # ===== ✅ شرط: قبل (جاهز للشحن) لازم فاتورة المتجر الرسمية =====
    if new_status == "ready_to_ship":
        ...
        if not inv_file:
            ...
            await _popup_and_pm(
                "⚠️ ارفع فاتورة المتجر أولاً قبل (جاهز للشحن).",
                (
                    f"{actor_name}\n"
                    "⚠️ قبل اختيار (🟢 الطلب جاهز للشحن) لازم ترفع *فاتورة المتجر الرسمية* (PDF أو صورة).\n\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"👤 العميل: {client_name}\n"
                    f"🧑‍💼 التاجر: {accepted_name}\n"
                    f"{grand_total_line}\n\n"
    ...
    # ✅ (1) عند (تم الشحن) اطلب رقم التتبع أو تخطي ثم نكمل الإشعار
    # (ملاحظة: هذا لن يصل له إلا بعد goods_paid=True بسبب الحارس أعلاه)
    if new_status == "shipped":
        try:
            already_tracking = (str(order.get("shipping_tracking") or order.get("tracking_number") or "").strip())
        except Exception:
            already_tracking = ""
        ...
        try:
            already_requested = str(udx.get("tsu_kind") or "").strip().lower() == "tracking" and str(udx.get("tsu_order_id") or "").strip() == str(order_id)
        except Exception:
            already_requested = False

        if (not already_tracking) and (not already_requested):
            try:
                udx["tsu_kind"] = "tracking"
                udx["tsu_order_id"] = order_id
                set_stage(context, actor_id, STAGE_TRADER_STATUS_UPDATE)
            except Exception:
                pass
            ...
            try:
                await q.message.reply_text(
                    f"{_user_name(q)}\n"
                    "📦 لتحديث الحالة إلى (تم الشحن):\n"
                    "ارسل *رقم التتبع* الآن هنا بالخاص.\n\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"👤 العميل: {client_name}\n"
                    f"🧑‍💼 التاجر: {accepted_name}\n"
                    f"{grand_total_line}\n\n"
                    "إذا لا يوجد رقم تتبع، اضغط (تخطي) لإرسال التحديث بدون تتبع.",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("⏭️ تخطي وإرسال التحديث بدون تتبع", callback_data=f"pp_tsu_skip_tracking|{order_id}")],
                        [InlineKeyboardButton("✖️ إلغاء", callback_data="pp_ui_close")],
                    ]),
                    disable_web_page_preview=True,
                )
            except Exception:
            ...
            await _alert(q, "ارسل رقم التتبع أو اضغط تخطي")
            return
    ...
    # ===== ✅ لوحة الطلب عند التاجر: تسلسل أزرار + إخفاء/إظهار حسب الدفع =====
    # ✅ (مهم): (الاتصال بالعميل + تأكيد التسليم) مختفية من البداية ولا تظهر إلا بعد دفع البضاعة
    # ✅ (مهم): زر (تم الشحن) لا يظهر إلا بعد دفع البضاعة كذلك
    def _trader_order_kb_local(ost: str, goods_paid_flag: bool) -> InlineKeyboardMarkup:
        ost2 = (ost or "").strip().lower()

        def _contact_row():
            return [InlineKeyboardButton("📞 الاتصال بالعميل", callback_data=f"pp_chat_open|{order_id}")]

        # تجهيز (دائمًا متاح)
        if ost2 in ("", "new", "accepted", "preparing", "prep"):
            rows = [
                [InlineKeyboardButton("🟡 جاري تجهيز الطلب", callback_data=f"pp_trader_status|prep|{order_id}")],
            ]
            # الاتصال مخفي قبل الدفع
            if goods_paid_flag:
                rows.append(_contact_row())
            return InlineKeyboardMarkup(rows)

        # جاهز للشحن (دائمًا بعد التجهيز)
        if ost2 in ("ready_to_ship", "ready"):
            rows = [
                [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready|{order_id}")],
            ]
            # قبل الدفع: لا نظهر زر الشحن ولا الاتصال
            if not goods_paid_flag:
                rows.append([InlineKeyboardButton("⏳ بانتظار سداد العميل لقيمة البضاعة", callback_data="pp_ui_close")])
                return InlineKeyboardMarkup(rows)

            # بعد الدفع: الآن نسمح بالمرحلة التالية (تم الشحن) + (الاتصال)
            rows.append([InlineKeyboardButton("🚚 تم الشحن", callback_data=f"pp_trader_status|shipped|{order_id}")])
            rows.append(_contact_row())
            return InlineKeyboardMarkup(rows)

        # تم الشحن
        if ost2 in ("shipped",):
            # قبل الدفع: (نادرًا) لو حصلت حالات قديمة، نخفي كل شيء
            if not goods_paid_flag:
                return InlineKeyboardMarkup([
                    [InlineKeyboardButton("⏳ بانتظار سداد العميل لقيمة البضاعة", callback_data="pp_ui_close")],
                ])
            # بعد الدفع: يظهر تأكيد التسليم + الاتصال
            return InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ تأكيد التسليم", callback_data=f"pp_trader_status|delivered|{order_id}")],
                _contact_row(),
            ])

        # تم التسليم / مغلق
        if ost2 in ("delivered", "closed"):
            if goods_paid_flag:
                return InlineKeyboardMarkup([_contact_row()])
            return InlineKeyboardMarkup([
                [InlineKeyboardButton("⏳ بانتظار سداد العميل لقيمة البضاعة", callback_data="pp_ui_close")],
            ])
    ...
    # ✅ تحديث الكيبورد مباشرة بعد كل ضغط زر
    try:
        await q.message.edit_reply_markup(reply_markup=_trader_order_kb_local(new_status, goods_paid))
    except Exception:
        pass

    # ---------- ✅ تجهيز نص القطع للعميل (مسعّر / غير متوفرة) ----------
    def _s(x: object) -> str:
        return ("" if x is None else str(x)).strip()
    ...
    try:
        if total_items > 0:
            parts_block_lines = []
            parts_block_lines.append("🧩 تفاصيل القطع لدى التاجر:")
            parts_block_lines.append(f"📌 المتوفر: {priced_count} من أصل {total_items}")
            if unavail_count:
                parts_block_lines.append(f"📌 غير متوفر: {unavail_count} من أصل {total_items}")
            if priced_count:
                parts_block_lines.append(f"💰 إجمالي القطع المتوفرة: {_money_tail(parts_total, fb='0')}")
            parts_block_lines.append("")
            if priced_lines:
                parts_block_lines.append("✅ القطع المتوفرة:")
                parts_block_lines.extend(priced_lines)
            if unavail_lines:
                parts_block_lines.append("")
                parts_block_lines.append("⛔ القطع غير المتوفرة:")
                parts_block_lines.extend(unavail_lines)
            parts_block = "\n".join(parts_block_lines)
    except Exception:
        parts_block = ""

    # ===== سطور مشتركة لكل إشعار =====
    header_block = (
        f"🧾 رقم الطلب: {order_id}\n"
        f"👤 العميل: {client_name}\n"
        f"🧑‍💼 التاجر: {accepted_name}\n"
    ...
    if grand_total_line:
        header_block += f"{grand_total_line}\n"
    ...
    for aid in ADMIN_IDS:
        try:
            admin_msg = (
                ...
                + header_block
                + f"🧑‍💼 تم التحديث بواسطة: {_user_name(q)}"
            )
            if parts_block:
                admin_msg += "\n\n" + parts_block
            await context.bot.send_message(chat_id=aid, text=admin_msg.strip(), disable_web_page_preview=True)
        except Exception:
            pass
        ...
        if client_id:
            try:
                ...
                await context.bot.send_message(
                    ...
                    text="\n".join(lines),
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("💬 متابعة الطلب ", callback_data=f"pp_chat_open|{order_id}")],
                    ]),
                    disable_web_page_preview=True,
                )
            except Exception:
...
async def order_finish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # منع الإقفال قبل الشحن/التسليم (حسب سياستك)
    st_now = str(order.get("order_status") or "").strip().lower()
    if st_now not in ("delivered", "closed"):
        try:
            await _alert(
                q,
                "⚠️ لا يمكن إنهاء الطلب قبل (تم التسليم)\n"
                f"🧾 رقم الطلب: {order_id}\n"
                f"{amount_line}",
                force=True
            )
    ...
    try:
        await _alert(
            q,
            "✅ تم إنهاء الطلب وإقفاله بنجاح\n"
            f"🧾 رقم الطلب: {order_id}\n"
            f"{amount_line}\n"
            "🔒 تم تعطيل الأزرار لهذا الطلب.",
            force=True
        )
    except Exception:
        pass
...
async def chat_open_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ============================
    # ✅ سطر واحد للمبلغ (إجمالي شامل الشحن إذا يوجد/مشمول، وإلا مبلغ القطع فقط)
    # ============================
    try:
        goods_amt_num = _to_float(order.get("goods_amount_sar") or order.get("goods_total_sar") or 0)
    except Exception:
        goods_amt_num = 0.0

    try:
        ship_fee_num = _to_float(order.get("shipping_fee_sar") or 0)
    except Exception:
        ship_fee_num = 0.0
    ...
    # إذا يوجد شحن (مبلغ>0) أو الشحن مشمول -> نظهر إجمالي شامل الشحن
    has_shipping = (ship_fee_num > 0.0) or shipping_included

    if has_shipping:
        total_num = goods_amt_num + (ship_fee_num if ship_fee_num > 0.0 else 0.0)
        amount_line = f"💰 إجمالي الطلب (شامل الشحن): {_money(total_num)}"
    else:
        amount_line = f"💰 مبلغ القطع: {_money(goods_amt_num)}"
    ...
    if actor_id == client_id and actor_id not in ADMIN_IDS:
        ...
        try:
            await q.message.reply_text(
                ...
                f"🧾 رقم الطلب: {order_id}\n"
                f"{amount_line}\n"
                "اكتب رسالتك للتاجر الآن وسيتم إرسالها عبر المنصة.",
    ...
    # ============================
    # ✅ مسار التاجر/الأدمن: تفعيل وضع الرد (Relay) للتاجر
    # ============================
    ...
    msg = (
        ...
        f"💰 <b>{html.escape(amount_line.replace('💰 ', ''))}</b>\n"
    ...
    for o in orders:
        ...
        if (now - base_ts) >= one_hour:
            ...
            if (not last_b) or ((now - last_b) >= one_hour):
                ...
                try:
                    log_event("إعادة نشر طلب بدون عروض (تذكير للتجار)", order_id=order_id)
                except Exception:
                    pass
...
async def notify_admins_goods_receipt(context: ContextTypes.DEFAULT_TYPE, ud: dict, file_id: str, mime: str = ""):
    ...
    try:
        ...
        total_amt = str(o.get("total_amount_sar") or "").strip()
        if not total_amt:
            total_amt = str(o.get("price_sar") or "").strip()

        status_txt = str(o.get("goods_payment_status") or "").strip()
        if not status_txt:
            status_txt = str(o.get("order_status") or "").strip()
    ...
    trader_name = trader_name or "—"
    goods_amt = goods_amt or "—"
    total_amt = total_amt or "—"
    status_txt = status_txt or "بانتظار تأكيد الاستلام"

    caption = (
        "📌 إشعار إداري\n"
        "🧾 تم استلام إيصال قيمة القطع\n"
        f"🧾 رقم الطلب: {order_id}\n"
        f"👤 العميل: {str(user_name) or '—'}{f' ({user_id})' if user_id else ''}\n"
        f"📞 الجوال: {phone or '—'}\n"
        f"🏙 المدينة: {city or '—'}\n"
        f"🧑‍💼 التاجر: {trader_name or '—'}\n"
        f"💰 إجمالي المبلغ: {total_amt}\n"
        f"📦 الحالة: {status_ar}"
    )
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_quote":
        ...
        if not all((tp.get(f) or "").strip() for f in required_fields):
            ...
            await _alert(q, "تم إرسال طريقة التفعيل بالخاص")
            return
    ...
    if action == "pp_team_goods_confirm":
        ...
        # ✅ جديد: نسخة احتياطية ذكية بعد تأكيد سداد قيمة القطع (بدون بطء + بدون تكرار سريع)
        try:
            app = getattr(context, "application", None)
            if app:
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ توافق خلفي: بعض أجزاء النظام تبحث عن shop_invoice_*
        ...
        # ✅ جديد: رقم الطلب + مبلغ الفاتورة + انتظار إكمال الدفع
        goods_amt = ""
        try:
            b3 = get_order_bundle(order_id2)
            o3 = b3.get("order", {}) or {}
            goods_amt = str(o3.get("goods_amount_sar") or o3.get("quote_goods_amount") or "").strip()
        except Exception:
            goods_amt = ""

        amt_line = f"\n💰 مبلغ الفاتورة: {goods_amt} ر.س" if goods_amt else ""
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Guard: لا نتعامل مع الرسائل النصية داخل المجموعات (منع التداخل)
    # (الأزرار/الكولباك تعمل عبر CallbackQueryHandler)
    ...
    # ===== تمرير رسائل المراسلة الداخلية pp_chat_sessions (قبل أي STAGE) =====
    try:
    ...
    if isinstance(sess, dict):
        peer_id = int(sess.get("peer_id") or 0)
        order_id_sess = (sess.get("order_id") or "").strip()
        role = (sess.get("role") or "").strip()  # client / trader

        # حماية
        if peer_id and order_id_sess and text:
            try:
                prefix = "👤 العميل" if role == "client" else "👤 التاجر"
                await context.bot.send_message(
                    chat_id=peer_id,
                    text=(
                        f"{prefix}\n"
                        f"🧾 رقم الطلب: {order_id_sess}\n"
                        f"💬 {text}"
                    ),
                    disable_web_page_preview=True,
                )
            ...
            # ✅ لا تكمل text_handler حتى لا يدخل في مسارات “مبلغ القطع”
            return

    # ✅ اجلب UD مرة واحدة فقط (بدل تكرار ud / ud_admin)
    ud = get_ud(context, user_id)
    stage = ud.get(STAGE_KEY, STAGE_NONE)

    # ==================================================
    # ✅ استدعاء لوحة طلب بكتابة رقم الطلب مباشرة
    # مثال: PP-260206-0001
    # يعمل فقط إذا لم يكن المستخدم داخل أي مرحلة
    # ==================================================
    if stage == STAGE_NONE:
        t = (update.message.text or "").strip().upper()

        # pp1234 أو PP-xxxx-1234
    ...
    if user_id in ADMIN_IDS and stage == STAGE_SUPPORT_ADMIN_REPLY:
        ...
        # ✅ رقم الطلب (إن وُجد) لربط الرد بطلب
        ...
        try:
            ...
            try:
                await update.message.reply_text("🟥 تعذر إرسال الرد (قد لا يكون المستخدم بدأ البوت)")
            except Exception:
                pass
            return
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_TRADER_MSG:
        tid = int(ud.get("admin_msg_to_trader_id") or 0)
        msg = (update.message.text or "").strip()
        ...
        if _support_should_close_by_time(ud):
            ...
            if cur_stage and cur_stage != STAGE_NONE:
                ...
                if msg:
                    ...
                    # ✅ توحيد الشرطات (بعض الجوالات ترسل — أو – أو ـ)
                    msg_norm = msg
                    try:
                        msg_norm = (msg_norm or "").replace("—", "-").replace("–", "-").replace("ـ", "-")
                    except Exception:
                        msg_norm = msg

                    # ✅ استخراج/تحديد رقم الطلب
                    ...
                    # محاولة التقاط رقم طلب مكتوب داخل الرسالة (PP-xxxx)
                    try:
                    ...
                    # ✅ تحديد هل المرسل تاجر؟ (يسمح له برسالة عامة بدون رقم طلب)
                    ...
                    # ✅ إذا ما لقطنا رقم كامل: جرّب آخر 4 أرقام فقط (0003)
                    if not order_id_support:
                        ...
                        if tail4:
                            # نجمع طلبات المستخدم (عميل/أدمن/تاجر)
                            orders = []
                            try:
                                uid = int(user_id or 0)
                            except Exception:
                            ...
                            if len(matches) == 1:
                                ...
                                try:
                                    await update.message.reply_text(
                                        "🔎 وُجد أكثر من طلب بنفس الرقم، اختر الطلب:",
                                        reply_markup=kb,
                                    )
                                except Exception:
                                    pass
                    ...
                    # ✅ إذا ما فيه رقم طلب: نرفض فقط للعميل، ونسمح للتاجر بدون رقم
                    if not order_id_support and not is_trader_sender:
                        try:
                            await update.message.reply_text(
                                "🟥 لإرسال طلبك للمنصة:\n"
                                "اكتب آخر 4 أرقام من الطلب (مثل 0003) أو رقم الطلب كامل (مثل PP-260208-0003)\n"
                                "ثم اكتب سؤالك.",
                                reply_markup=_support_kb(),
                                disable_web_page_preview=True,
                            )
                    ...
                    # حفظ رقم الطلب (إذا موجود) للرسائل القادمة داخل نفس جلسة الدعم
                    try:
                        if order_id_support:
                    ...
                    # زر الرد للإدارة: بدون رقم طلب نستخدم الشكل القديم pp_support_reply|user_id
                    reply_cb = (
                    ...
                    try:
                        if order_id_support:
                            await update.message.reply_text(
                                f"✅ تم إرسال رسالتك للإدارة\n🧾 رقم الطلب: {order_id_support}",
                                reply_markup=_support_kb(),
                            )
                        else:
                            await update.message.reply_text(
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_SEND_PAYLINK:
        ...
        if not order_id or not client_id:
            await update.message.reply_text(
                f"{name}\n🟥 تعذر تحديد الطلب/العميل، أعد المحاولة",
                disable_web_page_preview=True,
            )
            set_stage(context, user_id, STAGE_NONE)
        ...
        try:
            ...
            await update.message.reply_text(
                f"{name}\n🟥 تعذر إرسال الرابط للعميل (قد لا يكون بدأ المنصة)",
                disable_web_page_preview=True,
            )
            return
    ...
    if (text or "").strip() == "تاجر":
        ...
        # ✅ تعديل (الخطوة 2): لا تمنع "تاجر" بسبب stage
        # نخلي "تاجر" زر رجوع للوحة دائمًا، لكن نبقي استثناء الموقوف كما هو
        if stage_now != STAGE_NONE and not is_disabled:
            # بدل الرجوع الصامت، نصفر المرحلة عشان ما تتعلق الجلسات
            set_stage(context, user_id, STAGE_NONE)
        ...
        if not is_member and not is_admin and not tp and not is_registered_trader:
            # ✅ لا نسكت: نعطي المستخدم سبب واضح
            if not TRADERS_GROUP_ID:
        ...
        # ✅ إبقاء تصفير المرحلة قبل الفتح (احتياط)
        set_stage(context, user_id, STAGE_NONE)
        try:
            await show_trader_panel(update, context, user_id)
        except Exception:
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_FIND_ORDER:
        ...
        await update.message.reply_text(
            msg,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|home")]]),
            parse_mode="HTML",
        )
        return
    ...
    if stage == STAGE_TRADER_PROFILE_EDIT:
        ...
        if field == "iban":
            ...
            # ✅ شرط صارم: SA + 22 رقم (24 خانة)
            if not re.fullmatch(r"SA\d{22}", v):
                await update.message.reply_text(
                    f"{name}\n"
                    "رقم الايبان غير صحيح\n"
                    "الصيغة المطلوبة: يبدأ بـ SA ثم 22 رقم (بدون مسافات)\n"
        ...
        if field == "stc_pay":
            v = re.sub(r"\s+", "", val)
            # ✅ شرط صارم: 10 أرقام ويبدأ بـ 05
            if not re.fullmatch(r"05\d{8}", v):
                await update.message.reply_text(
                    f"{name}\n"
                    "رقم STC Pay غير صحيح\n"
                    "الصيغة المطلوبة: 10 أرقام ويبدأ بـ 05\n"
                    "مثال: 05XXXXXXXX"
                )
                return
            val = v
        ...
        # ✅ جديد: جدولة نسخة احتياطية بعد حفظ ملف التاجر (بدون أي بطء على المستخدم)
        # - إرسال النسخة يتم في الخلفية
        # - ويُمنع التكرار تلقائياً داخل _send_backup_excel عبر PP_BACKUP_MIN_SECONDS
        try:
            app = getattr(context, "application", None)
            if app:
                # منع جدولة متكررة لنفس الحدث بسرعة
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE:
        ...
        if kind == "tracking":
            ...
            # ✅ المطلوب الجديد: السماح بالتخطي (كتابة) بنفس منطق زر التخطي
            # (زر التخطي سيتم ربطه بالكولباك لاحقاً، لكن هذا يمنع تعطل التاجر لو كتب "تخطي")
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        # ✅ مخرج سريع: إلغاء/خروج من وضع عرض السعر إذا كتب المستخدم كلمة إلغاء
        ...
        if txt in ("الغاء", "إلغاء", "cancel", "خروج", "رجوع", "انهاء", "إنهاء"):
            ...
            td.pop("quote_started_at_utc", None)  # ✅ تنظيف وقت البدء
            # ✅ تنظيف جلسة تسعير القطع لهذا الطلب
            try:
                _qreset(td, td.get("quote_order_id") or "")
            except Exception:
                pass
            # ✅ تنظيف تسعير القطع (توافقي)
            td.pop("quote_item_prices", None)
            td.pop("quote_pending_item_idx", None)
            td.pop("quote_pending_item_name", None)
            set_stage(context, user_id, STAGE_NONE)
            await update.message.reply_text(f"{name}\n✅ تم إلغاء وضع عرض السعر.")
            return
        ...
        # ✅ شرط: لا يبدأ/يكمل عرض سعر إلا بعد اكتمال ملف التاجر
        try:
            tp = get_trader_profile(int(user_id or 0)) or {}
        except Exception:
            tp = {}
        ...
        required_fields = ["company_name", "iban", "stc_pay"]
        if not all((tp.get(f) or "").strip() for f in required_fields):
            await update.message.reply_text(f"{name}\n⛔ لا يمكنك تقديم عرض قبل إكمال ملف التاجر (اسم المتجر / IBAN / STC Pay)")
            return

        # ✅ إغلاق تلقائي لو جلس وضع عرض السعر معلق مدة طويلة (قبل أي تحقق مبلغ)
        # ملاحظة: بعد الإغلاق التلقائي نخرج مباشرة حتى لا يكمل نفس البلوك على بيانات قديمة.
        try:
            started = _parse_utc_iso(td.get("quote_started_at_utc") or "")
            if started:
                now = _dt_utc_now()
                if (now - started).total_seconds() > 20 * 60:  # 20 دقيقة
                    td.pop("quote_order_id", None)
                    td.pop("quote_step", None)
                    ...
                    set_stage(context, user_id, STAGE_NONE)
                    await update.message.reply_text(
                        f"{name}\nℹ️ انتهت جلسة عرض السعر بسبب عدم النشاط. ابدأ من جديد من الطلب."
                    )
                    return
        except Exception:
            pass

        # ===== helpers لتسعير القطع (متوافق مع ppq_cb) =====
        def _get_items_for_quote(oid: str):
            # ✅ مهم: تأكيد تحميل الاكسل قبل القراءة (يحل "لا توجد بنود" أحياناً)
            try:
                ensure_workbook()
            except Exception:
                pass
        ...
        def _price_map():
            # ✅ أسعار القطع مرتبطة بالطلب (جلسة لكل Order)
            try:
                qs = _qget(td, order_id, create=True)
            except Exception:
                qs = {}
            ...
            # ✅ حفظ منقح داخل الجلسة + توافقي مع بقية النظام
            try:
        ...
        if step == "it_price":
            ...
            try:
                idx = int(td.get("quote_pending_item_idx") or 0)
            except Exception:
                idx = 0
            ...
            if not items or idx < 1 or idx > len(items):
                ...
                await update.message.reply_text(
                    f"{name}\n"
                    "🟥 تعذر تحديد القطعة. ارجع للكيبورد واعد المحاولة.",
                    disable_web_page_preview=True,
                )
                return
            ...
            await update.message.reply_text(
                ...
                f"🧾 رقم الطلب: {order_id}\n"
                f"💰 إجمالي القطع الحالي: {total_now} ر.س\n\n"
                "اختر قطعة أخرى أو أكمل الخطوات:",
                reply_markup=_items_kb_local(order_id, items, pm),
                disable_web_page_preview=True,
            )
            return

        # ✅ جديد: تسعير الكل بسعر واحد
        ...
        # (موجود سابقاً) — يبقى احتياط لو وصل للمرحلة القديمة
        if step == "amount":
            m_amt = re.search(r"(\d+(?:\.\d+)?)", text)
            if not m_amt:
                await update.message.reply_text(
    ...
    if stage == STAGE_CHAT_TRADER:
        ...
        # ✅ تعديل: فصل رقم الطلب عن المبلغ + ايموجي لكل واحد
        head_lines = [f"💬 {cname}", f"🧾 الطلب: {order_id}"]
        if amt:
            head_lines.append(f"💰 قيمة القطع: {amt}")
    ...
    if stage == STAGE_TRADER_REPLY:
        ...
        if amt_txt:
            head_lines.append(f"💰 قيمة القطع: {html.escape(amt_txt)}")
    ...
    if stage == STAGE_TRADER_CHAT_ADMIN:
        ...
        # ✅ تفاصيل الطلب: العميل / المبلغ / الحالة
        client_name = "—"
        client_id = ""
        total_amt = "—"
        status_txt = "—"
        try:
            ...
            try:
                client_id = str(int(o.get("user_id") or 0)) if int(o.get("user_id") or 0) else ""
            except Exception:
                client_id = ""
            ...
            status_txt = str(o.get("order_status") or "").strip() or str(o.get("goods_payment_status") or "").strip() or "—"
        except Exception:
            pass
        ...
        msg = (
            ...
            f"🧾 رقم الطلب: {order_id}\n"
            f"🧑‍💼 التاجر: {tname} ({user_id})\n"
            f"👤 العميل: {client_name}{f' ({client_id})' if client_id else ''}\n"
            f"💰 مبلغ الطلب: {total_amt}\n"
            f"✅ الحالة: {status_txt}\n"
            "────────────────\n"
            f"{body}"
        )
    ...
    if stage == STAGE_PREPAY_NOTES:
        ...
        await update.message.reply_text(
            f"{name}\n🟩 تم حفظ الملاحظة.\nاختر تخطي للانتقال للشحن أو ارسل ملاحظة جديدة للتعديل.",
            reply_markup=prepay_notes_kb(),
            disable_web_page_preview=True,
        )
        return
    ...
    # ✅ كانت ناقصة عندك: مرحلة رقم جوال الاستلام
        ...
        if fee <= 0 and _is_platform_fee_free_mode():
            ...
            try:
                ...
                await update.message.reply_text(
                    ...
                    + safe_details
                    + "</pre>\n"
                    "<b>✅ شكرا تم استلام طلبك وستصلك العروض قريباً</b>",
                    parse_mode="HTML",
                    reply_markup=track_kb(order_id),
                    disable_web_page_preview=True,
                )
    ...
    # === لو المستخدم كتب نص أثناء مرحلة الصورة: اعتبره اسم قطعة جديدة مباشرة ===
    if stage == STAGE_ASK_ITEM_PHOTO and text:
        ...
        return  # ✅ كان ناقص: يمنع السقوط لباقي البلوكات

    # ===== رد التاجر للإدارة (رسائل ملف التاجر) =====
    if stage == "trader_reply_admin_msg":
        admin_id = int(ud.get("reply_to_admin_id") or 0)
        msg = (update.message.text or "").strip()
...
async def confirm_received_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ============================
    # ⏳ مؤقت 7 أيام (نفس منطق المراسلة)
    # ============================
    # - إذا وجدنا chat_expires_at_utc نلتزم به.
    # - إذا لم يوجد: نثبته أول مرة بعد (الشحن/الدفع المؤكد/الإغلاق).
    if user_id not in ADMIN_IDS:
        now_utc = datetime.now(timezone.utc)
    ...
    # ✅ تحديث الحالة إلى (تم الاستلام) + إغلاق
    fields = {
    ...
    if accepted_tid:
        try:
            # ===== حساب المبلغ الإجمالي (القطع + الشحن) =====
            goods_total = 0
            shipping_fee = 0
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"💰 إجمالي المبلغ (شامل الشحن): {grand_total} ر.س\n\n"
                    "يمكنك متابعة العميل لمدة 7 أيام عبر زر المراسلة.\n"
                    "ثم اضغط (إنهاء/إقفال) لتسجيله ضمن الطلبات المنجزة."
                ),
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "backup_now":
        await _alert(q, "جاري النسخ...")
        try:
            sent = await _send_backup_excel(context.application, reason="manual_admin")

            # ✅ ثبّت آخر نسخة (حتى نقدر نسترجعها بدون file_id مخزن)
            try:
                if sent and PP_BACKUP_CHAT_ID:
    ...
    if action == "restore_last_pinned":
        ...
        if q.message and q.message.chat and q.message.chat.type != ChatType.PRIVATE:
            await _alert(q, "هذا الخيار يعمل في الخاص فقط")
            return

        await _alert(q, "جاري الاسترجاع...")
        try:
            if not PP_BACKUP_CHAT_ID:
                await _alert(q, "PP_BACKUP_CHAT_ID غير مضبوط")
                return

            chat_obj = await context.bot.get_chat(PP_BACKUP_CHAT_ID)
...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, trader_id: int, order_id: str):
    ...
    ship_included = str(order.get("shipping_included") or "").strip().lower() in ("yes", "1", "true", "included")
    ...
    if last_mid:
        try:
            await context.bot.delete_message(chat_id=int(trader_id or 0), message_id=int(last_mid))
        except Exception:
            pass
...
async def _send_backup_excel(app: Application, reason: str = "scheduled"):
    ...
    """
    # 1) تحقق من مجموعة النسخ
    if not PP_BACKUP_CHAT_ID:
        if _should_throttle_notice("last_backup_warn_no_chat_id_utc", 6 * 3600):
            await _notify_admins(
                app,
                "⚠️ النسخ الاحتياطي متوقف: متغير البيئة PP_BACKUP_CHAT_ID غير مضبوط/غير صحيح."
            )
        return None

    # 2) تحقق من ملف الإكسل
    path = _excel_path()
    if not os.path.exists(path):
        if _should_throttle_notice("last_backup_warn_no_excel_utc", 6 * 3600):
            await _notify_admins(
                app,
                f"⚠️ النسخ الاحتياطي لم يتم: ملف الإكسل غير موجود على المسار:\n{path}"
            )
        return None

    # 3) منع التكرار (حد أدنى)
    try:
    ...
    try:
        ...
        if _should_throttle_notice("last_backup_warn_send_error_utc", 30 * 60):
            await _notify_admins(app, f"❌ خطأ أثناء إرسال النسخة الاحتياطية لمجموعة النسخ:\n{e}")
        return None
