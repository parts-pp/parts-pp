...
def set_setting(key: str, value: str, *args, **kwargs):
    with _EXCEL_WRITE_LOCK:
        return _pp_set_setting(key, value, *args, **kwargs)
...
TRADER_SUB_FEE_KEY = "trader_sub_fee_sar"
PLATFORM_FEE_LOW_KEY = "platform_fee_low_sar"
PLATFORM_FEE_HIGH_KEY = "platform_fee_high_sar"
...
def _trader_sub_fee_amount() -> int:
    return _get_setting_int(TRADER_SUB_FEE_KEY, 99)

def _platform_fee_low_amount() -> int:
    return _get_setting_int(PLATFORM_FEE_LOW_KEY, 25)

def _platform_fee_high_amount() -> int:
    return _get_setting_int(PLATFORM_FEE_HIGH_KEY, 39)

def _platform_fee_notice_amount() -> int:
    try:
        amt = int(_platform_fee_low_amount())
        return amt if amt > 0 else 1
    except Exception:
        return 25

def _platform_fee_notice_status_value(prof: dict) -> str:
    try:
        return str((prof or {}).get("platform_fee_notice_status") or "").strip().lower()
    except Exception:
        return ""

def _platform_fee_notice_is_active(prof: dict) -> bool:
    return _platform_fee_notice_status_value(prof) in ("awaiting", "pending", "blocked")

def _platform_fee_notice_status_label(prof: dict) -> str:
    st = _platform_fee_notice_status_value(prof)
    if st == "blocked":
        return "🟥 الحساب موقوف لعدم السداد"
    if st == "pending":
        return "🟨 بانتظار اعتماد السداد"
    if st == "awaiting":
        return "🟧 مهلة سداد قائمة"
    if st == "confirmed":
        return "🟩 تم السداد"
    return "—"

def _platform_fee_notice_due_text(prof: dict) -> str:
    try:
        return str((prof or {}).get("platform_fee_notice_due_at_utc") or "").strip() or "—"
    except Exception:
        return "—"

def _platform_fee_notice_reference(prof: dict, trader_id: int) -> str:
    raw = str((prof or {}).get("platform_fee_notice_id") or "").strip()
    raw = re.sub(r"[^A-Za-z0-9_-]+", "", raw)
    if not raw:
        raw = f"PFEE-{int(trader_id or 0)}"
    return raw

async def _send_platform_fee_notice_message(
    ...
    *,
    ...
    try:
        prof = profile or get_trader_profile(int(trader_id or 0)) or {}
    except Exception:
        prof = profile or {}

    name = (str((prof or {}).get("display_name") or "").strip() or "عزيزي التاجر")
    amount = str((prof or {}).get("platform_fee_notice_amount_sar") or _platform_fee_notice_amount()).strip()
    due_at = str((prof or {}).get("platform_fee_notice_due_at_utc") or "").strip()
    ref = _platform_fee_notice_reference(prof, trader_id)

    if broadcast:
        text = (
            f"🤍 أهلاً {html.escape(name)}\n\n"
            "نود إشعارك بلطف ببدء سياسة رسوم المنصة للتجار.\n"
            "حرصًا على استمرار الحركة في حسابك دون أي توقف، يرجى متابعة أي مطالبة تصلك من المنصة وسدادها خلال المهلة المحددة.\n\n"
            "عند وجود مطالبة فعالة سيظهر لك زر <b>سداد رسوم المنصة</b> داخل لوحة التاجر، كما يمكنك التواصل مع المنصة مباشرة عند الحاجة.\n\n"
            "نشكر لك تعاونك وثقتك."
        )
    elif blocked:
        text = (
            f"🤍 أهلاً {html.escape(name)}\n\n"
            "⛔ <b>تم تقييد الحركة على حسابك مؤقتًا</b> بسبب انتهاء مهلة سداد رسوم المنصة دون اعتماد السداد.\n\n"
            f"💰 المبلغ المطلوب: <b>{html.escape(str(amount))}</b> ريال\n"
            f"🧾 المرجع: <code>{html.escape(ref)}</code>\n"
            f"🕓 تاريخ انتهاء المهلة: <b>{html.escape(due_at or '—')}</b>\n\n"
            "يمكنك الآن استخدام زر السداد لإرسال الإيصال، أو مراسلة المنصة مباشرة."
        )
    elif gentle:
        text = (
            f"🤍 أهلاً {html.escape(name)}\n\n"
            "🔔 <b>تذكير لطيف</b> بسداد رسوم المنصة خلال المهلة المحددة حتى لا تتوقف الحركة على حسابك.\n\n"
            f"💰 المبلغ المطلوب: <b>{html.escape(str(amount))}</b> ريال\n"
            f"🧾 المرجع: <code>{html.escape(ref)}</code>\n"
            f"🕓 آخر موعد للسداد: <b>{html.escape(due_at or '—')}</b>\n\n"
            "بعد السداد أرسل الإيصال عبر زر <b>سداد رسوم المنصة</b> داخل لوحة التاجر."
        )
    else:
        text = (
            f"🤍 أهلاً {html.escape(name)}\n\n"
            "نود إشعارك بوجود مطالبة <b>رسوم منصة</b> على حسابك.\n"
            f"يرجى السداد خلال <b>{PLATFORM_FEE_NOTICE_DAYS}</b> أيام حتى لا يتم تقييد الحركة على الحساب.\n\n"
            f"💰 المبلغ المطلوب: <b>{html.escape(str(amount))}</b> ريال\n"
            f"🧾 المرجع: <code>{html.escape(ref)}</code>\n"
            f"🕓 آخر موعد للسداد: <b>{html.escape(due_at or '—')}</b>\n\n"
            "بعد السداد أرسل الإيصال من زر <b>سداد رسوم المنصة</b> داخل لوحة التاجر."
        )

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💳 سداد رسوم المنصة", callback_data="pp_tprof|pfee|pay")],
        [InlineKeyboardButton("📩 مراسلة المنصة", callback_data="pp_support_open")],
    ])
    try:
        await context.bot.send_message(
            chat_id=int(trader_id),
            text=text,
            parse_mode="HTML",
            reply_markup=kb,
            disable_web_page_preview=True,
...
def _start_platform_fee_notice(trader_id: int, amount: int | None = None) -> dict:
    tid = int(trader_id or 0)
    amount = int(amount or _platform_fee_notice_amount())
    now_dt = _dt_utc_now()
    now_iso = now_dt.isoformat()
    due_iso = (now_dt + timedelta(days=PLATFORM_FEE_NOTICE_DAYS)).isoformat()
    notice_id = f"PFEE-{tid}-{now_dt.strftime('%Y%m%d%H%M%S')}"
    fields = {
        "platform_fee_notice_id": notice_id,
        "platform_fee_notice_status": "awaiting",
        "platform_fee_notice_amount_sar": str(amount),
        "platform_fee_notice_started_at_utc": now_iso,
        "platform_fee_notice_due_at_utc": due_iso,
        "platform_fee_notice_sent_at_utc": now_iso,
        "platform_fee_notice_reminded_at_utc": "",
        "platform_fee_notice_payment_method": "",
        "platform_fee_notice_receipt_file_id": "",
        "platform_fee_notice_receipt_kind": "",
        "platform_fee_notice_receipt_uploaded_at_utc": "",
        "platform_fee_notice_confirmed_at_utc": "",
        "platform_fee_notice_blocked_at_utc": "",
        "updated_at_utc": now_iso,
    }
    upsert_trader_profile(tid, fields)
    return get_trader_profile(tid) or fields

def _clear_platform_fee_notice(trader_id: int, *, mark_confirmed: bool = False) -> None:
    now_iso = utc_now_iso()
    fields = {
        "platform_fee_notice_id": "",
        "platform_fee_notice_status": ("confirmed" if mark_confirmed else ""),
        "platform_fee_notice_amount_sar": "",
        "platform_fee_notice_started_at_utc": "",
        "platform_fee_notice_due_at_utc": "",
        "platform_fee_notice_sent_at_utc": "",
        "platform_fee_notice_reminded_at_utc": "",
        "platform_fee_notice_payment_method": "",
        "platform_fee_notice_receipt_file_id": "",
        "platform_fee_notice_receipt_kind": "",
    ...
    if mark_confirmed:
        fields["platform_fee_notice_confirmed_at_utc"] = now_iso
    else:
        fields["platform_fee_notice_confirmed_at_utc"] = ""
    upsert_trader_profile(int(trader_id or 0), fields)


async def _notify_admins_platform_fee_receipt(
    context: ContextTypes.DEFAULT_TYPE,
    *,
    trader_id: int,
    trader_name: str,
    ...
    prof = profile or {}
    amount = str((prof or {}).get("platform_fee_notice_amount_sar") or _platform_fee_notice_amount()).strip()
    payment_method = str((prof or {}).get("platform_fee_notice_payment_method") or "—").strip() or "—"
    ref = _platform_fee_notice_reference(prof, trader_id)
    due_at = str((prof or {}).get("platform_fee_notice_due_at_utc") or "—").strip() or "—"
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ اعتماد السداد ورفع التقييد", callback_data=f"pp_admin|tpfee|{int(trader_id)}|mark_paid")],
        [InlineKeyboardButton("👤 فتح ملف التاجر", callback_data=f"pp_admin|tview|{int(trader_id)}")],
    ])
    cap = (
        "💳 <b>إيصال رسوم منصة جديد</b>\n"
        f"👤 التاجر: <b>{html.escape(str(trader_name or '—'))}</b>\n"
        f"🆔 trader_id: <code>{int(trader_id or 0)}</code>\n"
        f"🧾 المرجع: <code>{html.escape(ref)}</code>\n"
        f"💰 المبلغ: <b>{html.escape(str(amount))}</b> ريال\n"
        f"💳 الطريقة: <b>{html.escape(payment_method)}</b>\n"
        f"🕓 آخر موعد: <b>{html.escape(due_at)}</b>\n\n"
        "⬇️ راجع الإيصال ثم اعتمد السداد:"
    )
...
async def _handle_trader_platform_fee_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = int(update.effective_user.id)
    file_id = ""
    is_photo = False
    if update.message and update.message.photo:
        try:
            ...
            file_id = ""
    if (not file_id) and update.message and update.message.document:
        doc = update.message.document
        mime = (doc.mime_type or "").lower()
        fname = (doc.file_name or "").lower()
        if mime.startswith("application/pdf") or mime.startswith("image/") or fname.endswith((".pdf", ".jpg", ".jpeg", ".png", ".webp")):
            file_id = doc.file_id
    if not file_id:
    ...
    try:
        prof = get_trader_profile(user_id) or {}
    except Exception:
        prof = {}
    if not _platform_fee_notice_is_active(prof):
        set_stage(context, user_id, STAGE_NONE)
        await update.message.reply_text(f"{_user_name(update)}\nلا توجد مطالبة رسوم منصة نشطة على حسابك حاليا")
        return

    now_iso = utc_now_iso()
    payment_method = str(get_ud(context, user_id).get("platform_fee_notice_payment_method") or prof.get("platform_fee_notice_payment_method") or "").strip()
    try:
        upsert_trader_profile(user_id, {
            "platform_fee_notice_status": "pending",
            "platform_fee_notice_payment_method": payment_method,
            "platform_fee_notice_receipt_file_id": file_id,
            "platform_fee_notice_receipt_kind": ("photo" if is_photo else "document"),
            "platform_fee_notice_receipt_uploaded_at_utc": now_iso,
            "updated_at_utc": now_iso,
        })
    except Exception:
        pass
    ...
    try:
        prof = get_trader_profile(user_id) or prof
    except Exception:
        pass

    trader_name = str((prof or {}).get("display_name") or update.effective_user.full_name or update.effective_user.first_name or "").strip()
    await _notify_admins_platform_fee_receipt(
        context,
        trader_id=user_id,
    ...
    try:
        ...
        ud.pop("platform_fee_notice_payment_method", None)
    except Exception:
        pass

    await update.message.reply_text(
        f"{_user_name(update)}\n✅ تم استلام إيصال رسوم المنصة وسيتم التحقق من الإدارة قريبًا",
        disable_web_page_preview=True,
    )

async def _platform_fee_deadline_job(context: ContextTypes.DEFAULT_TYPE):
    try:
    ...
    for t in traders:
        ...
        try:
            ...
            prof = {}
        st = _platform_fee_notice_status_value(prof)
        # pending = تم رفع الإيصال وينتظر اعتماد الإدارة، فلا نعيد إيقاف الحساب آليًا
        if st != "awaiting":
            continue
        due_dt = _parse_utc_iso(str(prof.get("platform_fee_notice_due_at_utc") or ""))
        if not due_dt or due_dt > now_dt:
            continue
        try:
            ...
            pass
        try:
            upsert_trader_profile(tid, {
                "platform_fee_notice_status": "blocked",
                "platform_fee_notice_blocked_at_utc": now_dt.isoformat(),
                "updated_at_utc": now_dt.isoformat(),
            })
        except Exception:
            pass
        try:
            await _send_platform_fee_notice_message(context, tid, profile=get_trader_profile(tid) or prof, blocked=True)
        except Exception:
            pass
...
def _safe_int(x) -> int:
    """تحويل آمن إلى int بدون رمي أخطاء."""
    try:
        return int(x or 0)
    except Exception:
        return 0
...
def _fmt_money(v: object) -> str:
    """
    تنسيق مبلغ بصيغة موحدة.
    - يعيد نصًا فارغًا للقيم الفارغة/الصفرية حتى يبقى سلوك الواجهات الحالية كما هو.
    - يعتمد على _money() ليخرج بصيغة: 1,234 ر.س
    """
...
def _calc_totals(goods_total_str: object, shipping_fee_str: object) -> tuple[str, str, str]:
    """
    يحسب (القطع، الشحن، الإجمالي) ويعيدها كنصوص رقمية خام مناسبة
    لإعادة تمريرها على _fmt_money عند الحاجة.
    مثال الإخراج: ("150", "25", "175")
    """
    def _to_float_local(x: object) -> float:
        try:
            return float(str(x or "").replace(",", "").strip() or 0)
        except Exception:
            return 0.0

    def _fmt_num_local(x: float) -> str:
        try:
    ...
    g = _to_float_local(goods_total_str)
    s = _to_float_local(shipping_fee_str)
    t = g + s
    return _fmt_num_local(g), _fmt_num_local(s), _fmt_num_local(t)
...
def _pay_status_ar(x: object) -> str:
    ...
    order_map = {
        "awaiting_quotes": "بانتظار تقديم عروض",
        "new": "بانتظار تقديم عروض",
...
def _effective_order_status(o: dict) -> str:
    """إرجاع حالة طلب فعلية حتى لو كان order_status فاضي في الإكسل.
    يعتمد على حقول واقعية محفوظة داخل الطلب بدون أي عشوائية.
    """
    try:
    ...
    # ===== Legacy rows with empty order_status =====

    # الإلغاء
    try:
        by_admin = str((o or {}).get("cancelled_by_admin_id") or "").strip()
        by_client = str((o or {}).get("cancelled_by_client_id") or "").strip()
        cancelled_at = str((o or {}).get("cancelled_at_utc") or (o or {}).get("cancelled_at") or "").strip()
    except Exception:
        by_admin = by_client = cancelled_at = ""

    if by_admin or by_client or cancelled_at:
        return "cancelled"

    # الإغلاق النهائي
    for k in ("closed_at_utc", "closed_at"):
        try:
            if str((o or {}).get(k) or "").strip():
                return "closed"
        except Exception:
            pass

    # تم الاستلام / التسليم
    for k in ("delivered_confirmed_at_utc", "delivered_at_utc"):
        try:
            if str((o or {}).get(k) or "").strip():
                return "delivered"
        except Exception:
    ...
    # تم الشحن
    for k in ("shipped_at_utc", "shipped_at", "shipping_tracking", "tracking_number"):
        try:
            if str((o or {}).get(k) or "").strip():
                return "shipped"
        except Exception:
    ...
    # الدفع المؤكد أو تعيين تاجر = دخلت مرحلة التجهيز
    try:
        gps = str((o or {}).get("goods_payment_status") or (o or {}).get("payment_status") or "").strip().lower()
    except Exception:
        gps = ""

    if gps in ("awaiting_confirm", "confirmed", "paid"):
        return "preparing"

    try:
        accepted_tid = str((o or {}).get("accepted_trader_id") or "").strip()
        accepted_at = str((o or {}).get("accepted_at_utc") or "").strip()
    except Exception:
        accepted_tid = accepted_at = ""

    if accepted_tid or accepted_at:
        return "preparing"

    # الحالة الافتراضية الأولى
...
def _order_status_display(o: dict) -> str:
    ...
    # تخصيص الإغلاق النهائي عند الصفوف القديمة
    if ost == "closed":
        return "مغلق"

    base = _pay_status_ar(ost)
    return base or "—"
...
def _order_snapshot(order_id: str) -> dict:
    ...
    try:
        out["ship_included"] = str(o.get("ship_included") or o.get("shipping_included") or "").strip()
    except Exception as e:
        _swallow(e)
    ...
    try:
        out["status"] = _order_status_display(o) or "—"
    except Exception:
        out["status"] = "—"
...
async def ui_noop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    try:
        await q.answer()
    except Exception:
...
STAGE_TRADER_FIND_ORDER = "pp_trader_find_order"
...
def _parse_finish_dt_utc(v: object) -> datetime | None:
    try:
        s = str(v or "").strip()
        if not s:
            return None
...
def _order_finish_base_dt(order: dict) -> datetime | None:
    o = order or {}
    candidates = [
        o.get("shipped_at_utc"),
        o.get("shipped_at"),
        o.get("delivered_confirmed_at_utc"),
        o.get("delivered_at_utc"),
        o.get("delivered_at"),
        o.get("goods_payment_confirmed_at_utc"),
        o.get("accepted_at_utc"),
        o.get("created_at_utc"),
        o.get("created_at"),
    ]
    for raw in candidates:
        dt = _parse_finish_dt_utc(raw)
        if dt:
            return dt
    return None


def _order_is_closed_final(order: dict) -> bool:
    try:
        st = str((order or {}).get("order_status") or "").strip().lower()
    except Exception:
        st = ""
    if st in ("closed", "delivered"):
        return True
    for k in ("closed_at_utc", "closed_at", "delivered_confirmed_at_utc"):
        try:
            if str((order or {}).get(k) or "").strip():
                return True
        except Exception:
            pass
    return False
...
def price_for_count(c: int) -> int:
    ...
    return _platform_fee_low_amount() if c <= 5 else _platform_fee_high_amount()

# ===== مساعدات تنسيق رسائل الإدارة (بدون تشوه بصري) =====
STAGE_ADMIN_SEND_PAYLINK = "admin_send_paylink"
STAGE_ADMIN_SET_TRADER_SUB_FEE = "pp_admin_set_trader_sub_fee"
STAGE_ADMIN_SET_PLATFORM_FEE_LOW = "pp_admin_set_platform_fee_low"
STAGE_ADMIN_SET_PLATFORM_FEE_HIGH = "pp_admin_set_platform_fee_high"
STAGE_TRADER_PLATFORM_FEE_AWAIT_PAY_METHOD = "pp_trader_platform_fee_await_pay_method"
STAGE_TRADER_PLATFORM_FEE_AWAIT_RECEIPT = "pp_trader_platform_fee_await_receipt"
PLATFORM_FEE_NOTICE_DAYS = 3
...
def _order_is_done_for_panels(o: dict) -> bool:
    try:
        ost = _effective_order_status(o)
    except Exception:
        ost = str((o or {}).get("order_status") or "").strip().lower()
    ...
    try:
        gps = str((o or {}).get("goods_payment_status") or "").strip().lower()
    except Exception:
        gps = ""

    return bool(gps == "confirmed" or ost in ("closed", "delivered"))

def _client_can_cancel_order(o: dict) -> bool:
    try:
        ost = _effective_order_status(o)
    except Exception:
        ost = str((o or {}).get("order_status") or "").strip().lower()

    if ost in ("cancelled", "canceled", "closed", "delivered", "shipped", "ready_to_ship", "ready"):
        return False
    ...
    if gps in ("awaiting_confirm", "confirmed", "paid", "success", "successful", "done", "ok"):
        return False
...
def admin_reply_kb(order_id: str, user_id: int) -> InlineKeyboardMarkup:
    oid = (order_id or "").strip()
    rows = [[InlineKeyboardButton("✍️ رد كالإدارة", callback_data=f"pp_admin_reply|{oid}|{user_id}")]]
    if oid:
        rows.append([InlineKeyboardButton(f"🧾 {oid}", callback_data=f"pp_open_order|{oid}")])
    return InlineKeyboardMarkup(rows)


def _support_order_sort_dt(o: dict) -> datetime:
    """ترتيب آمن لاختيار آخر طلب مرتبط بالطرف."""
    for k in ("updated_at_utc", "created_at_utc", "accepted_at_utc", "cancelled_at_utc"):
        try:
            dt = _parse_utc_iso(str((o or {}).get(k) or "").strip())
            if dt:
                return dt
        except Exception:
            pass
...
def _latest_order_id_for_support_user(user_id: int, is_trader: bool = False) -> str:
    """آخر طلب مرتبط بالمستخدم لدعم /منصة."""
    ...
    try:
        if is_trader:
            orders = list_orders_for_trader(uid) or []
        else:
            all_orders = list_orders() or []
            orders = [o for o in all_orders if _safe_int((o or {}).get("user_id")) == uid]
    except Exception:
        orders = []
    ...
    try:
        latest = sorted(orders, key=_support_order_sort_dt, reverse=True)[0]
    except Exception:
        try:
            latest = orders[-1]
        except Exception:
    ...
    return str((latest or {}).get("order_id") or "").strip()


def _support_admin_notify_kb(user_id: int, order_id: str = "", is_trader_sender: bool = False) -> InlineKeyboardMarkup:
    """كيبورد إشعار الإدارة داخل /منصة."""
    oid = (order_id or "").strip()
    reply_cb = f"pp_support_reply|{oid}|{user_id}" if oid else f"pp_support_reply|{user_id}"

    rows = [[InlineKeyboardButton("✉️ رد", callback_data=reply_cb)]]

    if is_trader_sender:
        rows.append([InlineKeyboardButton("📒 سجل التاجر", callback_data=f"pp_admin|tview|{int(user_id or 0)}")])
...
async def send_trader_ledger_pdf(
    ...
    # ===== سجل الاشتراكات =====
    try:
        subs_all = [s for s in (list_trader_subscriptions() or []) if int(s.get("trader_id") or 0) == tid]
    except Exception:
        subs_all = []

    def _sub_state_txt(v: str) -> str:
        x = str(v or "").strip().lower()
        if x == "confirmed":
            return "مؤكد"
        if x in ("pending", "awaiting"):
            return "قيد التحقق"
        if x == "rejected":
            return "مرفوض"
        return x or "—"

    subs_all = sorted(subs_all, key=lambda s: str(s.get("month") or ""), reverse=True)
    section_header("سجل الاشتراكات")
    if subs_all:
        sub_tbl = [[
            P("<b>الشهر</b>", center),
            P("<b>الحالة</b>", center),
            P("<b>المبلغ</b>", center),
            P("<b>طريقة الدفع</b>", center),
            P("<b>تاريخ السداد</b>", center),
            P("<b>رقم الوثيقة</b>", center),
        ]]
        for s in subs_all[:12]:
            sub_tbl.append([
                P(str(s.get("month") or "—"), center),
                P(_sub_state_txt(str(s.get("payment_status") or "")), center),
                P(_money_tail(s.get("amount_sar") or 0, fb="0"), center),
                P(str(s.get("payment_method") or "—"), center),
                P(str(s.get("paid_at_utc") or s.get("receipt_uploaded_at_utc") or "—"), center),
                P(str(s.get("receipt_file_id") or s.get("sub_id") or "—"), center),
            ])
        t_sub = Table(sub_tbl, colWidths=[full_w * 0.13, full_w * 0.14, full_w * 0.14, full_w * 0.18, full_w * 0.19, full_w * 0.22])
        t_sub.setStyle(
            TableStyle(
                [
                    ("BOX", (0, 0), (-1, -1), 1.05, GRID_BOLD),
        ...
        try:
            for r in range(1, len(sub_tbl)):
                t_sub.setStyle(TableStyle([("BACKGROUND", (0, r), (-1, r), ROW_BG1 if r % 2 == 1 else ROW_BG2)]))
        except Exception:
            pass
        story.append(t_sub)
        story.append(Spacer(1, 6))
    else:
        kv_table([("الاشتراكات المسجلة", "لا توجد سجلات اشتراك محفوظة")])
...
async def send_trader_subscription_invoice_pdf(
    ...
    """Generate and send a simple 1-page PDF invoice for trader subscription. Returns meta dict."""
    try:
    ...
    amount_sar = int(float(amount_sar or _trader_sub_fee_amount()))
    invoice_no = f"SUBINV-{month.replace('-', '')}-{int(trader_id or 0)}"
    paid_at_txt = str(paid_at_utc or utc_now_iso()).strip()
    pay_method_txt = _pay_method_ar(payment_method or "")
    ...
    c.drawRightString(w-15*mm, h-18*mm, _ar("فاتورة سداد اشتراك المنصة"))
    c.setFont(font_name, 10)
    c.drawRightString(w-15*mm, h-26*mm, _ar(f"الشهر: {month}"))

    c.setFillColor(colors.whitesmoke)
    c.rect(15*mm, h-160*mm, w-30*mm, 110*mm, fill=1, stroke=0)
    ...
    c.drawRightString(w-20*mm, h-100*mm, _ar(f"رقم الفاتورة: {invoice_no}"))
    c.drawRightString(w-20*mm, h-115*mm, _ar(f"المرجع: SUB-{trader_id}-{month}"))
    c.drawRightString(w-20*mm, h-130*mm, _ar(f"طريقة السداد: {pay_method_txt}"))
    c.drawRightString(w-20*mm, h-145*mm, _ar(f"تاريخ التحقق: {paid_at_txt}"))

    c.setFont(font_name, 9)
    c.setFillColor(colors.gray)
    c.drawString(15*mm, 15*mm, "PP / GO - Trader Subscription Invoice")

    c.showPage()
    c.save()

    caption = f"🧾 فاتورة اشتراك المنصة — {month} — {amount_sar} ريال\nرقم الفاتورة: {invoice_no}"
    trader_invoice_file_id = ""
    try:
        with open(path, "rb") as f:
            sent = await context.bot.send_document(chat_id=int(trader_id), document=f, caption=caption)
            try:
                trader_invoice_file_id = str((sent.document.file_id if sent and sent.document else "") or "").strip()
            except Exception:
                trader_invoice_file_id = ""
    except Exception as e:
        _swallow(e)
    ...
    return {
        ...
        "invoice_file_id": trader_invoice_file_id,
        "paid_at_utc": paid_at_txt,
    }
...
async def pay_bank_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_SUB_AWAIT_PAY_METHOD:
        ...
        amount = int(float(ud.get("sub_amount_sar") or _trader_sub_fee_amount()))
    ...
    if stage == STAGE_TRADER_PLATFORM_FEE_AWAIT_PAY_METHOD:
        prof = get_trader_profile(int(user_id or 0)) or {}
        amount = int(float(ud.get("platform_fee_notice_amount_sar") or prof.get("platform_fee_notice_amount_sar") or _platform_fee_notice_amount()))
        ref = _platform_fee_notice_reference(prof, user_id)
        ud["platform_fee_notice_payment_method"] = "bank_transfer"
        set_stage(context, user_id, STAGE_TRADER_PLATFORM_FEE_AWAIT_RECEIPT)
        try:
            upsert_trader_profile(user_id, {
                "platform_fee_notice_payment_method": "bank_transfer",
                # لا نعتبرها pending إلا بعد رفع الإيصال فعليًا
                "platform_fee_notice_status": "awaiting",
        ...
        await q.message.reply_text(
            f"🤍 اهلا {_user_name(q)}\n\n"
            f"💳 <b>طريقة الدفع: تحويل بنكي</b>\n\n"
            f"المبلغ المطلوب <b>{amount} ريال</b> مقابل <b>رسوم المنصة</b>\n\n"
            f"🏦 <b>المستفيد</b>:\n<i>{PP_BENEFICIARY}</i>\n\n"
            f"IBAN:\n<code>{PP_IBAN}</code>\n\n"
            f"🧾 <b>رقم المرجع</b>:\n<code>{ref}</code>\n\n"
            "📸 بعد الدفع أرسل <b>صورة/ملف الإيصال</b> هنا مباشرة ليتم مراجعته من الإدارة.\n",
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
        return
...
async def pay_stc_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_PLATFORM_FEE_AWAIT_PAY_METHOD:
        ...
        amount = int(float(ud.get("platform_fee_notice_amount_sar") or prof.get("platform_fee_notice_amount_sar") or _platform_fee_notice_amount()))
        ref = _platform_fee_notice_reference(prof, user_id)
        ud["platform_fee_notice_payment_method"] = "stc_pay"
        set_stage(context, user_id, STAGE_TRADER_PLATFORM_FEE_AWAIT_RECEIPT)
        try:
            upsert_trader_profile(user_id, {
                "platform_fee_notice_payment_method": "stc_pay",
                # لا نعتبرها pending إلا بعد رفع الإيصال فعليًا
                "platform_fee_notice_status": "awaiting",
                "updated_at_utc": utc_now_iso(),
            })
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ سياسة السماح ببدء عرض جديد:
    # تطبق فقط عند بدء عرض جديد أو إنشاء إصدار جديد
    # حتى لا نكسر جلسة بدأت سابقا أثناء التحول بين مجاني/مدفوع
    if action in ("ppq_begin", "ppq_new_version"):
        try:
            can_quote_now, deny_msg = _trader_can_start_quote_now(int(user_id or 0))
        except Exception:
            can_quote_now, deny_msg = True, ""

        if not can_quote_now:
            try:
                await _alert(q, deny_msg or "لا يمكنك بدء عرض سعر جديد حاليا", force=True)
            except Exception as e:
                _swallow(e)
            try:
                await q.message.reply_text(
                    (deny_msg or "لا يمكنك بدء عرض سعر جديد حاليا") +
                    "\n\nراجع الإدارة إذا كان لديك إعفاء أو استثناء لهذا الشهر.",
                    disable_web_page_preview=True,
                )
            except Exception as e:
                _swallow(e)
    ...
    def _save_amounts_to_order(
        ...
        payload = {
            ...
            "shipping_fee_sar": (s_str if ship_fee_norm != "" else ""),
            "total_amount_sar": (t_str if ship_fee_norm != "" else g_str),
        }
    ...
    if action == "ppq_begin":
        ...
        order_status = _effective_order_status(oo)
...
def _trader_can_start_quote_now(user_id: int) -> tuple[bool, str]:
    """
    سياسة السماح ببدء عرض السعر بشكل آمن ومرن:
    1) إذا المنصة حاليا على الوضع المجاني -> يسمح مباشرة
    2) إذا التاجر عليه استثناء/سماح إداري -> يسمح مباشرة
    3) إذا الإدارة وضعت منعًا صريحًا -> يمنع
    4) إذا النظام المدفوع مفعل ولا يوجد استثناء -> يشترط اشتراك شهري صالح
    """
    ...
    def _truthy(v) -> bool:
        return _norm(v) in ("1", "true", "yes", "on", "enabled", "active", "allow", "allowed", "ok", "paid", "approved", "confirmed", "free", "exempt")

    def _month_key_now() -> str:
        try:
            return str(month_key_utc() or "").strip()
        except Exception:
            try:
    ...
    def _month_matches(raw_val: object, now_key: str) -> bool:
        s = _s(raw_val)
        if not s or not now_key:
            return False

        # يقبل: 2026-03 أو 2026/03 أو 202603
        s2 = s.replace("/", "-").strip()
        if s2 == now_key:
            return True

        compact_now = now_key.replace("-", "")
        compact_s = re.sub(r"[^0-9]", "", s2)
        return bool(compact_s and compact_s == compact_now)
    ...
    now_key = _month_key_now()

    # ===== 1) إذا المنصة مجانية الآن -> يسمح =====
    try:
        if _is_platform_fee_free_mode():
            return True, ""
    except Exception:
    ...
    # ===== 2) منع/سماح إداري صريح من ملف التاجر =====
    # لو الإدارة تريد منحه سماحا دائما أو مؤقتا للشهر الحالي
    override_raw = _norm(
        tp.get("quote_access_override")
        or tp.get("subscription_override")
        or tp.get("quote_override")
        or tp.get("pricing_override")
        or ""
    )

    if override_raw in ("deny", "denied", "block", "blocked", "forbid", "forbidden", "off", "disabled", "no"):
        return False, "⛔ تم إيقاف صلاحية تقديم عروض السعر لهذا التاجر من قبل الإدارة"

    if override_raw in ("allow", "allowed", "yes", "true", "1", "on", "enabled", "manual", "admin", "free", "exempt"):
        return True, ""

    # ===== 3) إعفاء/استثناء للشهر الحالي =====
    exempt_flag = (
        tp.get("subscription_exempt")
        or tp.get("quote_access_exempt")
        or tp.get("monthly_subscription_exempt")
        or tp.get("is_subscription_exempt")
        or ""
    )
    if _truthy(exempt_flag):
        return True, ""

    exempt_month = (
        tp.get("subscription_exempt_month")
        or tp.get("quote_access_exempt_month")
        or tp.get("subscription_override_month")
        or tp.get("quote_access_month")
        or tp.get("manual_quote_access_month")
        or ""
    )
    if _month_matches(exempt_month, now_key):
        return True, ""

    override_until = (
        tp.get("subscription_override_until")
        or tp.get("quote_access_until")
        or tp.get("quote_access_until_month")
        or ""
    )
    if _month_matches(override_until, now_key):
        return True, ""

    # ===== 4) فحص الاشتراك الشهري =====
    sub = {}
    try:
        try:
            sub = get_trader_subscription(int(user_id or 0), now_key) or {}
        except TypeError:
            sub = get_trader_subscription(int(user_id or 0)) or {}
    except Exception:
        sub = {}

    # إذا سجل الاشتراك نفسه فيه استثناء أو سماح
    try:
        sub_override = _norm(
            sub.get("quote_access_override")
            or sub.get("subscription_override")
            or ""
        )
    except Exception:
        sub_override = ""

    if sub_override in ("allow", "allowed", "yes", "true", "1", "on", "enabled", "manual", "admin", "free", "exempt"):
        return True, ""

    if sub_override in ("deny", "denied", "block", "blocked", "forbid", "forbidden", "off", "disabled", "no"):
        return False, "⛔ لا يمكنك تقديم عروض السعر حاليا حسب إعدادات الاشتراك"

    try:
        sub_exempt = (
            sub.get("subscription_exempt")
            or sub.get("quote_access_exempt")
            or ""
        )
    except Exception:
        sub_exempt = ""
    if _truthy(sub_exempt):
        return True, ""

    try:
        sub_exempt_month = (
            sub.get("subscription_exempt_month")
            or sub.get("quote_access_exempt_month")
            or sub.get("period_key")
            or sub.get("month_key")
            or sub.get("month")
            or sub.get("billing_month")
            or ""
        )
    except Exception:
        sub_exempt_month = ""
    # لا نعتبر وجود شهر فقط كافيا، لكن نستفيد منه مع حالة الإعفاء أو مع حالة مدفوعة أدناه

    try:
        sub_status = _norm(
            sub.get("status")
            or sub.get("subscription_status")
            or sub.get("payment_status")
            or sub.get("state")
            or ""
        )
    except Exception:
    ...
    # الحالات المسموح بها صراحة
    if sub_status in ("paid", "active", "approved", "confirmed", "success", "successful", "done", "ok"):
        # إذا كان السجل مربوطا بشهر، نتأكد أنه شهرنا الحالي
        sub_month = (
            sub.get("period_key")
            or sub.get("month_key")
            or sub.get("month")
        ...
        if not _s(sub_month) or _month_matches(sub_month, now_key):
            return True, ""

    # إذا الإدارة خزنت حالة الإعفاء داخل الاشتراك نفسه
    if sub_status in ("free", "exempt", "waived"):
        return True, ""

    # السماح بحالة قيد التحقق غير مستحب في الوضع الصارم
    if sub_status in ("pending", "awaiting_confirm", "under_review", "review", "verifying", "pending_payment"):
        return False, "⏳ اشتراكك الشهري ما زال قيد التحقق من الإدارة ولا يمكنك بدء عرض سعر جديد حاليا"

    return False, "💳 لا يمكنك بدء عرض سعر جديد حاليا لأن الاشتراك الشهري غير مفعل لهذا الشهر"
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    try:
        b0 = get_order_bundle(order_id) or {}
        o0 = b0.get("order", {}) or {}
    except Exception:
        o0 = {}
    ...
    # ✅ عداد العروض: نزيده عند كل إرسال عرض جديد
    try:
        current_quotes_count = int(float(str(o0.get("quotes_count") or "0").strip() or "0"))
    except Exception:
        current_quotes_count = 0

    new_quotes_count = current_quotes_count + 1
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_status == "shipped":
        ...
        # ✅ عند زر التخطي: سجل التتبع فارغ + وقت الشحن
        if skip_tracking_btn:
    ...
    if new_status == "delivered":
        fields["delivered_at_utc"] = utc_now_iso()
    ...
    # ✅ سجل قانوني موحد لكل انتقالة رئيسية
    try:
        legal_status_map = {
            "preparing": "جاري تجهيز الطلب",
            "ready_to_ship": "الطلب جاهز للشحن",
        ...
        legal_status_text = legal_status_map.get(new_status, new_status)
        append_legal_log(
            order_id,
            f"تم تحديث حالة الطلب إلى {legal_status_text}",
    ...
    if new_status in ("preparing", "prep"):
        ...
        next_step_client = "✅ <b>تم تأكيد استلام الطلب بنجاح</b>."
        next_step_trader = "✅ <b>تم تأكيد تسليم الطلب بنجاح</b>."
        next_step_admin = "✅ تم تأكيد استلام الطلب."
...
async def order_finish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if actor_id not in ADMIN_IDS and actor_id != accepted_tid:
        try:
            await _alert(q, f"🔒 هذا الزر مخصص للتاجر المستلم أو الإدارة فقط\n🧾 رقم الطلب: {order_id}", force=True)
        except Exception as e:
            _swallow(e)
        return
    ...
    ship_fee = _to_float(order.get("shipping_fee_sar") or order.get("shipping_fee") or 0)
    try:
        ship_inc_raw = str(order.get("shipping_included") or order.get("ship_included") or order.get("shipping_fee_included") or "").strip().lower()
    except Exception:
        ship_inc_raw = ""
    shipping_included = ship_inc_raw in ("yes", "1", "true", "on", "included", "مشمولة", "مشمول")
    has_shipping = (ship_fee > 0.0) or shipping_included
    amount_line = f"💰 إجمالي الطلب (شامل الشحن): {_money(goods_amt + (ship_fee if ship_fee > 0.0 else 0.0))}" if has_shipping else f"💰 مبلغ القطع: {_money(goods_amt)}"

    if _order_is_closed_final(order):
        try:
            await _alert(q, f"✅ الطلب منجز ومغلق نهائيًا بالفعل\n🧾 رقم الطلب: {order_id}\n{amount_line}", force=True)
        except Exception as e:
            _swallow(e)
        try:
            if q.message:
    ...
    is_admin_actor = actor_id in (ADMIN_IDS or [])
    now_utc = datetime.now(timezone.utc)
    base_dt = _order_finish_base_dt(order)

    if (not is_admin_actor) and (not base_dt):
        try:
            await _alert(q, "⏳ لا يمكن إغلاق الطلب الآن.\n\n🔒 سيتم تفعيل زر الإغلاق بعد الشحن أو بعد توفر تاريخ صالح لبداية مدة المتابعة (7 أيام).", force=True)
        except Exception as e:
            _swallow(e)
        return

    if (not is_admin_actor) and base_dt:
        ...
        if now_utc < expires_dt:
            ...
            try:
                await _alert(q, f"⏳ زر الإغلاق غير متاح الآن.\n\n🔒 يتم تفعيل زر الإغلاق بعد انتهاء مدة المتابعة (7 أيام) من تاريخ الشحن أولًا.\n🕒 المتبقي تقريبًا: {days_left} يوم.", force=True)
            except Exception as e:
                _swallow(e)
            return
    ...
    finish_fields = {
        ...
        "delivered_confirmed_at_utc": now_iso,
        "delivered_confirmed_by": str(actor_id),
        "quote_locked": "yes",
    }
    try:
        update_order_fields(order_id, finish_fields)
    except Exception as e:
        _swallow(e)
        try:
            await _alert(q, "تعذر إنهاء الطلب حاليًا", force=True)
        except Exception:
            pass
        return
    ...
    if client_chat_id:
        try:
            await context.bot.send_message(chat_id=client_chat_id, text=(f"{_user_name(update)}\n✅ تم إنجاز وإغلاق الطلب نهائيًا.\n🧾 رقم الطلب: {html.escape(order_id)}\n{amount_line}\n\nنشكر لك اهتمامك وتعاونك."), parse_mode="HTML", disable_web_page_preview=True)
        except Exception as e:
            _swallow(e)

    try:
        log_event("order_finished", order_id=order_id, actor_role="admin" if is_admin_actor else "trader", actor_id=actor_id, actor_name=actor_name, payload={"order_status": "delivered", "policy": "finish_unified"})
    except Exception as e:
        _swallow(e)

    try:
        await _alert(q, f"✅ تم إنجاز وإغلاق الطلب نهائيًا\n🧾 رقم الطلب: {order_id}\n{amount_line}", force=True)
    except Exception as e:
        _swallow(e)
...
async def notify_team(context: ContextTypes.DEFAULT_TYPE, ud: dict):
    ...
    # ✅ اجعل الباندل هو المصدر الأساسي دائمًا
    try:
        b = get_order_bundle(order_id) or {}
    except Exception:
        b = {}
    ...
    bundle_items = (b.get("items", []) or []) if isinstance(b, dict) else []
    ...
    def _normalize_items(src):
        out = []
        for it in (src or []):
            if not isinstance(it, dict):
                continue
            nm = _safe_str(it.get("name"))
            pn = _safe_str(it.get("part_no") or it.get("item_part_no"))
            fid = _safe_str(it.get("photo_file_id") or it.get("file_id"))
            mt = _safe_str(it.get("media_type") or "photo").lower()
            out.append({
                **it,
                "name": nm,
                "part_no": pn,
                "item_part_no": pn,
                "photo_file_id": _safe_str(it.get("photo_file_id")),
                "file_id": fid,
                "media_type": mt or "photo",
            })
        return out

    # ✅ الأساس من order ثم fallback من ud
    user_name = _safe_str(_pick(order.get("user_name"), ud.get("user_name")))
    user_id = _pick(order.get("user_id"), ud.get("user_id"))
    car_name = _safe_str(_pick(order.get("car_name"), ud.get("car_name")))
    car_model = _safe_str(_pick(order.get("car_model"), ud.get("car_model")))
    vin = _safe_str(_pick(order.get("vin"), ud.get("vin")))
    notes = _norm(_pick(order.get("notes"), ud.get("notes"), default=""))

    # ✅ القطع: الباندل أولًا ثم ud
    items = _normalize_items(bundle_items or ud.get("items") or [])
    if not items:
        items = []

    # ✅ مبالغ: الأساس من order لأن هذه القيم تحفظ عادة هناك
    fee = _pick(order.get("price_sar"), ud.get("price_sar"))
    goods_amount = _pick(order.get("goods_amount_sar"), ud.get("goods_amount_sar"))
    ship_fee = _pick(order.get("shipping_fee_sar"), ud.get("shipping_fee_sar"))

    fee_txt = _safe_num_txt(fee)
    goods_txt = _safe_num_txt(goods_amount)
    ship_txt = _safe_num_txt(ship_fee)

    notes_html = f"<i>{html.escape(notes)}</i>" if notes else "<i>—</i>"

    # ✅ بلوك التسليم: استخدم order كأساس و ud فقط كمساند
    try:
        delivery_block = _delivery_brief(order or {}, ud or {}) or "<i>—</i>"
    except Exception:
        delivery_block = "<i>—</i>"
    ...
    for i, it in enumerate(items, start=1):
        ...
        pn = _safe_str(it.get("part_no") or it.get("item_part_no"))
        has_media = bool(_safe_str(it.get("photo_file_id") or it.get("file_id")))
        if has_media:
            media_count += 1
        ...
        items_lines.append(f"{badge} <b>{shown + 1}.</b> {html.escape(nm)}{pn_txt}")
        shown += 1
    ...
    car_txt = html.escape((f"{car_name} {car_model}").strip()) if (car_name or car_model) else "—"
    uname_txt = html.escape(user_name) if user_name else "—"
    uid_txt = html.escape(str(user_id)) if str(user_id or "").strip() else "—"

    # ✅ وضع إعادة النشر
    is_reminder = bool(
        ud.get("_reminder")
        or order.get("_reminder")
        or str(order.get("rebroadcast_count") or "").strip()
    )
    ...
    try:
        reb_no = int(
            _pick(
                ud.get("rebroadcast_no"),
                order.get("rebroadcast_count"),
                default=0,
            ) or 0
        )
    except Exception:
    ...
    # ✅ أبقينا نفس نمط الرسالة حتى لا نكسر التنسيق الحالي
    txt = (
        f"{hdr}\n"
        f"🧾 <b>رقم الطلب</b>: {html.escape(order_id)}\n\n"
    ...
    # ✅ لو كانت هناك مبالغ محفوظة فعلًا نعرضها، وإلا لا نلزم إظهارها
    extra_money_lines = []
    if fee_txt != "—":
        extra_money_lines.append(f"💰 <b>السعر المتوقع</b>: <b>{html.escape(str(fee_txt))}</b>")
    if goods_txt != "—":
        extra_money_lines.append(f"🧾 <b>قيمة القطع</b>: <b>{html.escape(str(goods_txt))}</b>")
    if ship_txt != "—":
        extra_money_lines.append(f"🚚 <b>رسوم الشحن</b>: <b>{html.escape(str(ship_txt))}</b>")

    if extra_money_lines:
        txt += "\n".join(extra_money_lines) + "\n"
    ...
    try:
        ...
        if team_msg_id:
            ...
            try:
                fwd0 = _safe_str(order.get("forwarded_to_team_at_utc"))
                if not fwd0:
                    ...
                    try:
                        aid = int(
                            ...
                            or order.get("forwarded_by_admin_id")
                            or 0
                        )
                    except Exception:
                        aid = 0
                    ...
                    aname = _safe_str(
                        ud.get("assigned_admin_name")
                        or ud.get("forwarded_by_admin_name")
                        or order.get("forwarded_by_admin_name")
                    )
    ...
    for it in items:
        ...
        caption = f"🧩 قطعة {media_idx}: {nm}" if nm else f"🧩 قطعة {media_idx}"
        if pn:
            caption += f" ({pn})"
...
async def _rebroadcast_noquote_orders_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for o in orders:
        ...
        if ost in ("closed", "delivered", "cancelled", "canceled"):
            continue

        # منع إعادة نشر الطلبات المقفلة (قفل صارم)
        ql = str(o.get("quote_locked") or "").strip().lower()
        if ql in ("yes", "true", "1", "locked", "on"):
            continue
        ...
        # نافذة إعادة النشر: 7 أيام من أول وصول الطلب للمجموعة
        age = now - base_ts
        if age >= seven_days:
            try:
                update_order_fields(order_id, {
                    ...
                    "rebroadcast_disabled_by_id": str(o.get("rebroadcast_disabled_by_id") or "system_7d"),
                })
            except Exception as e:
                _swallow(e)
            continue

        # إعادة النشر كل 24 ساعة داخل نافذة 7 أيام فقط
        if age >= one_day:
        ...
        # تنبيه الأدمن كل 24 ساعة داخل نافذة 7 أيام فقط
        if age >= one_day:
    ...
    if admin_need_list:
        ...
        for oid in admin_need_list:
            msg_txt = (
                ...
                "هذا الطلب مضى عليه 24 ساعة بدون أي عروض، وسيتم إعادة نشره تلقائيًا كل 24 ساعة لمدة 7 أيام كحد أقصى.\n"
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _locked_reason_message(oo: dict, tname: str, acting_trader_id: int = 0) -> str:
        try:
            ost = _effective_order_status(oo)
        except Exception:
            ost = str((oo or {}).get("order_status") or "").strip().lower()

        by_admin = str((oo.get("cancelled_by_admin_id") or "")).strip()
        by_client = str((oo.get("cancelled_by_client_id") or "")).strip()
        cancelled_at = str((oo.get("cancelled_at_utc") or oo.get("cancelled_at") or "")).strip()
        quote_locked_raw = str((oo.get("quote_locked") or "")).strip().lower()
        quote_locked = quote_locked_raw in ("yes", "true", "1", "locked", "on")
        goods_pay_status = str((oo.get("goods_payment_status") or "")).strip().lower()
        accepted_name = (oo.get("accepted_trader_name") or oo.get("quoted_trader_name") or "").strip() or "تاجر آخر"
        try:
        ...
        if by_client:
            body = "⛔ هذا الطلب ملغي من قبل العميل ولا يستقبل عروض جديدة حالياً."
        elif by_admin:
            body = "⛔ هذا الطلب ملغي من قبل الإدارة ولا يستقبل عروض جديدة حالياً."
        elif ost in ("canceled", "cancelled", "ملغي") or cancelled_at:
            body = "⛔ هذا الطلب ملغي ولا يستقبل عروض جديدة حالياً."
        elif ost == "closed":
            body = "🔒 هذا الطلب مغلق ولا يستقبل عروض جديدة حالياً."
        elif ost == "delivered":
            body = "✅ تم استلام هذا الطلب واكتملت العملية ولا يستقبل عروضاً جديدة."
        elif ost == "shipped":
            body = "🚚 تم شحن هذا الطلب بالفعل ولا يستقبل عروضاً جديدة."
        elif ost in ("ready_to_ship", "ready"):
            body = "📦 هذا الطلب جاهز للشحن ولا يستقبل عروضاً جديدة حالياً."
        elif goods_pay_status == "awaiting_confirm":
            body = "💳 تم سداد قيمة القطع لهذا الطلب وهو بانتظار تأكيد الاستلام لذلك لا يستقبل عروضاً جديدة."
        elif goods_pay_status in ("confirmed", "paid", "success", "successful", "done", "ok"):
            body = "💳 تم تأكيد دفع قيمة القطع لهذا الطلب ولا يستقبل عروضاً جديدة."
        elif accepted_tid and acting_trader_id and accepted_tid != int(acting_trader_id):
            body = f"🔒 تم اعتماد الطلب لدى {accepted_name} ولا يستقبل عروضاً جديدة حالياً."
        elif accepted_tid:
            body = f"🔒 هذا الطلب معتمد لدى {accepted_name} ولا يستقبل عروضاً جديدة حالياً."
        elif quote_locked:
            body = "🔒 هذا الطلب مقفول من الإدارة ولا يستقبل عروضاً جديدة حالياً."
        else:
            body = "🔒 هذا الطلب مقفول حالياً ولا يستقبل عروضاً جديدة."
    ...
    if action in ("pp_team_locked", "pp_team_quote_locked"):
        try:
            ...
            msg = _locked_reason_message(oo, tname, actor_id)
            await _alert(q, msg, force=True)

        except Exception:
            try:
                tname = (actor_first or actor_name or "").strip() or "عزيزي التاجر"
                await q.answer(f"{tname}\n🔒 تعذر التحقق من حالة الطلب حالياً.", show_alert=True)
            except Exception as e:
                _swallow(e)
        return
    ...
    if action == "pp_team_quote":
        ...
        effective_status = _effective_order_status(oo)

        is_final_locked = (
            effective_status in ("closed", "delivered", "shipped", "ready_to_ship", "ready", "canceled", "cancelled")
            or quote_locked
            or goods_pay_status in ("awaiting_confirm", "confirmed", "paid", "success", "successful", "done", "ok")
            or (accepted_tid and accepted_tid != actor_id)
        )

        if is_final_locked and actor_id not in ADMIN_IDS:
            tname = (actor_first or actor_name or "").strip() or "عزيزي التاجر"
            await _alert(q, _locked_reason_message(oo, tname, actor_id), force=True)
            return
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg:
        ...
        if is_private and kind and file_id:
            ...
            if is_receipt_media and stage not in CHAT_STAGES and stage not in (STAGE_AWAIT_GOODS_RECEIPT, STAGE_AWAIT_RECEIPT, STAGE_TRADER_SUB_AWAIT_RECEIPT, STAGE_TRADER_PLATFORM_FEE_AWAIT_RECEIPT, STAGE_JOIN_CR, STAGE_JOIN_LICENSE):
                try:
                    cand_order_id = ""
                    try:
                        orders = list_orders() or []
        ...
        try:
            BLOCK_SESS_STAGES = {
                ...
                STAGE_TRADER_PLATFORM_FEE_AWAIT_RECEIPT, # إيصال مطالبة رسوم المنصة للتاجر
    ...
    if stage == STAGE_TRADER_SUB_AWAIT_RECEIPT:
        ...
        try:
            upsert_trader_subscription(user_id, month, {
                ...
                "payment_status": "pending",
                "receipt_file_id": file_id,
                "receipt_kind": "photo" if is_photo else "document",
                "receipt_uploaded_at_utc": utc_now_iso(),
            })
        except Exception as e:
            _swallow(e)
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📚 عرض إيصالات الاشتراك", callback_data=f"pp_admin|tsubs|{user_id}")],
            [InlineKeyboardButton("👤 فتح ملف التاجر", callback_data=f"pp_admin|tview|{user_id}")],
        ])
    ...
    # === مرحلة استلام إيصال مطالبة رسوم المنصة للتاجر ===
    if stage == STAGE_TRADER_PLATFORM_FEE_AWAIT_RECEIPT:
        return await _handle_trader_platform_fee_receipt(update, context)
...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, viewer_id: int, order_id: str):
    ...
    def _fmt_days(v):
        ...
        try:
            ...
            if n <= 0:
                return "—"
            return f"{n} يوم" if n == 1 else f"{n} أيام"
        except Exception:
            return s

    def _payment_status_display(o: dict) -> str:
        """
        حالة دفع القطع فقط
        لا تعتمد على payment_status لأنه قد يخص رسوم المنصة
        """
        goods_status = _s(o.get("goods_payment_status")).lower()
        has_goods_receipt = bool(_s(o.get("goods_receipt_file_id")))
        goods_confirmed_at = bool(_s(o.get("goods_payment_confirmed_at_utc")))

        # مؤكد فعليا فقط إذا وُجد تأكيد واضح
        if goods_status in ("confirmed", "paid", "success", "successful", "done", "ok"):
            return "تم تأكيد دفع قيمة القطع"

        # أرسل الإيصال ولم يؤكد بعد
        if goods_status == "awaiting_confirm" or (has_goods_receipt and not goods_confirmed_at):
            return "تم رفع إيصال الدفع وبانتظار تأكيد التاجر"

        # مطلوب من العميل رفع الإيصال
        if goods_status in ("awaiting_receipt",):
            return "بانتظار رفع إيصال دفع قيمة القطع"

        # حالات فشل/رفض خاصة بدفع القطع
        if goods_status == "rejected":
            return "تم رفض إيصال الدفع"
        if goods_status == "failed":
            return "فشل دفع قيمة القطع"
        if goods_status == "expired":
            return "انتهت مهلة دفع قيمة القطع"
        if goods_status == "processing":
            return "جاري التحقق من دفع قيمة القطع"

        # لا نظهر أي نجاح اعتمادا على رسوم المنصة أو quote_status
        return "لم يتم دفع قيمة القطع بعد"

    def _is_payment_unlocked(o: dict) -> bool:
        """
        فتح رقم التواصل للتاجر بعد دفع القطع فقط.
        """
        goods_status = _s(o.get("goods_payment_status")).lower()
        goods_confirmed_at = _s(o.get("goods_payment_confirmed_at_utc"))
        return bool(
            goods_status in ("confirmed", "paid", "success", "successful", "done", "ok")
            or goods_confirmed_at
        )
    ...
    # ---------------- عرض بيانات ----------------
    client_name = _nz(order.get("user_name") or order.get("client_name") or "—", "—")
    ...
    # آخر تاجر قدم عرض
    last_quote_trader_id = _to_int(order.get("quoted_trader_id"))
    last_quote_trader_name = _s(order.get("quoted_trader_name"))
    if not last_quote_trader_name and last_quote_trader_id:
        try:
            last_quote_trader_name = _trader_label(int(last_quote_trader_id), "—")
        except Exception:
            last_quote_trader_name = "—"
    if not last_quote_trader_name and trader_id:
        last_quote_trader_name = trader_name or "—"
    last_quote_trader_name = last_quote_trader_name or "—"

    car_name = _nz(order.get("car_name") or "—", "—")
    car_model = _nz(order.get("car_model") or "—", "—")
    vin = _nz(order.get("vin") or "—", "—")

    city = _nz(
        order.get("ship_city")
        or order.get("shipping_city")
        or order.get("city")
        or order.get("pickup_city")
        or "—",
        "—",
    )

    ship_method = _nz(
        order.get("ship_method")
        or order.get("ship_carrier")
        or order.get("shipping_method")
        or order.get("delivery_type")
        or "—",
        "—",
    )

    parts_type = _nz(order.get("parts_type") or order.get("parts_condition") or "—", "—")

    # بيانات شحن إضافية
    shipping_phone = _s(
        order.get("user_phone")
        or order.get("client_phone")
        or order.get("ship_phone")
        or order.get("phone")
        or order.get("mobile")
    )
    shipping_address = _s(
        order.get("shipping_address")
        or order.get("ship_address")
        or order.get("delivery_details")
    ...
    # عدد العروض
    quotes_count_raw = _s(order.get("quotes_count"))
    try:
        quotes_count = int(float(quotes_count_raw or "0"))
    except Exception:
        quotes_count = 0

    # ✅ fallback بسيط لمنع التناقض البصري إذا نسي النظام تحديث العداد
    if quotes_count <= 0:
        has_any_quote_evidence = any([
            _to_int(order.get("quoted_trader_id")) > 0,
            _to_int(order.get("accepted_trader_id")) > 0,
            _s(order.get("quoted_trader_name")),
            _s(order.get("goods_amount_sar")),
            _s(order.get("quote_goods_amount")),
            _s(order.get("quote_item_prices")),
        ])
        if has_any_quote_evidence:
            quotes_count = 1

    quotes_count_text = f"{quotes_count} عروض" if quotes_count > 0 else "0 عروض"

    status_ar = _order_status_display(order)

    # ✅ توضيح جهة الإلغاء داخل لوحة الطلب
    ost = _effective_order_status(order)
    if ost in ("cancelled", "canceled"):
    ...
    # مدد
    availability_days = _fmt_days(
        order.get("availability_days")
        or order.get("prep_days")
        or order.get("prepare_days")
        or order.get("preparing_days")
    )
    ship_eta = _fmt_days(
        order.get("ship_eta")
        or order.get("shipping_eta")
        or order.get("delivery_eta")
        or order.get("shipping_days")
    )

    # مالي
    ga_raw = _s(order.get("goods_amount_sar") or order.get("quote_goods_amount") or "")
    sf_raw = _s(order.get("shipping_fee_sar") or order.get("shipping_fee") or "")
    tot_raw = _s(order.get("total_amount_sar") or order.get("total_amount") or order.get("quote_total_amount") or "")

    if not tot_raw:
        try:
            g_num = float(re.sub(r"[^0-9.]+", "", ga_raw) or 0)
        except Exception:
            g_num = 0.0
        try:
            s_num = float(re.sub(r"[^0-9.]+", "", sf_raw) or 0)
        except Exception:
            s_num = 0.0
        total_num = g_num + s_num
        if total_num > 0:
            if float(total_num).is_integer():
                tot_raw = str(int(total_num))
            else:
                tot_raw = f"{total_num:.2f}"

    ga = _money_or_dash(ga_raw)
    sf = _money_or_dash(sf_raw)
    tot = _money_or_dash(tot_raw)

    payment_status_ar = _payment_status_display(order)

    # ---------------- سرية البيانات ----------------
    payment_unlocked = _is_payment_unlocked(order)

    # الأدمن يرى كل شيء دائمًا
    # التاجر يرى المدينة والعنوان دائمًا، والهاتف بعد دفع القطع فقط
    # العميل يرى هاتفه وعنوانه دائمًا
    show_phone = False
    show_address = False
    ...
    if role == "admin":
        ...
        show_phone = bool(payment_unlocked)
        show_address = True
        show_client_id = False
    else:  # client
        show_phone = True
    ...
    for idx, it in enumerate(items or [], start=1):
        ...
        partno = (it.get("part_no") or it.get("partno") or it.get("part_number") or it.get("item_part_no") or "").strip()
        price = _pick_item_price(idx, it, pm)

        if _is_unavailable(it):
            unavailable_cnt += 1
            unavailable_lines.append(
                f"{idx}- {html.escape(nm)}"
            ...
            priced_lines.append(
                f"{idx}- {html.escape(nm)}"
                + (f" — <code>{html.escape(partno)}</code>" if partno else "")
                + f" — {html.escape(_money(price) or str(price))}"
            )
        else:
            unpriced_cnt += 1
            unpriced_lines.append(
                f"{idx}- {html.escape(nm)}"
                + (f" — <code>{html.escape(partno)}</code>" if partno else "")
            )

        if (len(priced_lines) + len(unpriced_lines) + len(unavailable_lines)) >= 60:
            break

    if priced_cnt > 0:
        pricing_status = "تم تسعير بعض أو كل القطع"
    else:
        pricing_status = "تم التسعير" if _s(order.get("goods_amount_sar") or order.get("quote_goods_amount")) else "لم يتم التسعير"

    # ---------------- رسالة اللوحة ----------------
    lines = []
    lines.append("🧾 <b>لوحة الطلب</b>")
    lines.append("")
    lines.append("📌 <b>البيانات العامة</b>")
    lines.append("رقم الطلب")
    lines.append(f"<b>{html.escape(order_id)}</b>")
    lines.append("")
    lines.append("الحالة")
    lines.append(f"<b>{html.escape(status_ar)}</b>")

    lines.append("")
    lines.append("🚗 <b>السيارة</b>")
    lines.append(f"{html.escape(car_name)}")
    lines.append(f"{html.escape(car_model)}")
    lines.append(f"VIN <code>{html.escape(vin)}</code>")

    lines.append("")
    lines.append("📊 <b>العروض</b>")
    lines.append("عدد العروض")
    lines.append(f"<b>{html.escape(quotes_count_text)}</b>")
    lines.append("")
    lines.append("آخر تاجر قدم عرض")
    lines.append(f"<b>{html.escape(last_quote_trader_name)}</b>")

    # معلومات الأطراف
    lines.append("")
    lines.append("👥 <b>الأطراف</b>")
    lines.append("العميل")
    lines.append(f"<b>{html.escape(client_name)}</b>")

    if show_client_id and client_id:
        lines.append("")
        lines.append("معرف العميل")
        lines.append(f"<code>{client_id}</code>")

    lines.append("")
    lines.append("التاجر المعتمد")
    lines.append(f"<b>{html.escape(trader_name)}</b>")
    if trader_store:
        lines.append("")
        lines.append("المتجر")
        lines.append(f"<b>{html.escape(trader_store)}</b>")

    if shipping_phone and show_phone:
        lines.append("")
        lines.append("جوال المستلم")
        lines.append(f"<b>{html.escape(shipping_phone)}</b>")

    lines.append("")
    lines.append("🚚 <b>الشحن</b>")
    lines.append("طريقة الشحن")
    lines.append(f"<b>{html.escape(ship_method)}</b>")
    lines.append("")
    lines.append("المدينة")
    lines.append(f"<b>{html.escape(city)}</b>")

    if shipping_address and show_address:
        lines.append("")
        lines.append("عنوان الشحن")
        lines.append(f"<b>{html.escape(shipping_address)}</b>")

    lines.append("")
    lines.append("مدة التجهيز")
    lines.append(f"<b>{html.escape(availability_days)}</b>")

    lines.append("")
    lines.append("مدة الشحن")
    lines.append(f"<b>{html.escape(ship_eta)}</b>")

    if tracking_no:
        lines.append("")
        lines.append("رقم التتبع")
        lines.append(f"<b>{html.escape(tracking_no)}</b>")

    lines.append("")
    lines.append("🧩 <b>القطع</b>")
    lines.append("نوع القطع")
    lines.append(f"<b>{html.escape(parts_type)}</b>")
    lines.append("")
    lines.append("حالة التسعير")
    lines.append(f"<b>{html.escape(pricing_status)}</b>")

    if priced_lines:
        lines.append("")
        lines.append("💰 <b>القطع المسعرة</b>")
        lines.extend(priced_lines)

    if unpriced_lines:
        lines.append("")
        lines.append("⚠️ <b>قطع بدون تسعير</b>")
        lines.extend(unpriced_lines)

    if unavailable_lines:
        lines.append("")
        lines.append("❌ <b>قطع غير متوفرة</b>")
        lines.extend(unavailable_lines)

    if not priced_lines and not unpriced_lines and not unavailable_lines:
        lines.append("")
        lines.append("لا توجد قطع مسجلة لهذا الطلب حاليا")

    lines.append("")
    lines.append("💳 <b>حالة الدفع</b>")
    lines.append(f"<b>{html.escape(payment_status_ar)}</b>")

    lines.append("")
    lines.append("💵 <b>الملخص المالي</b>")
    lines.append("قيمة القطع")
    lines.append(f"<b>{html.escape(ga)}</b>")
    lines.append("")
    lines.append("رسوم الشحن")
    lines.append(f"<b>{html.escape(sf)}</b>")
    lines.append("")
    lines.append("الإجمالي")
    lines.append(f"<b>{html.escape(tot)}</b>")
    ...
    goods_receipt_file_id = _s(order.get("goods_receipt_file_id"))
    platform_invoice_file_id = _s(
        order.get("platform_invoice_file_id")
        or order.get("invoice_platform_pre_file_id")
        or order.get("invoice_platform_ship_file_id")
        or order.get("platform_invoice_pdf_file_id")
        or order.get("invoice_platform_file_id")
    )
    ...
    if role == "admin":
        ...
        extra_row = []
        if goods_receipt_file_id:
            extra_row.append(InlineKeyboardButton("🧾 إيصال دفع القطع", callback_data=f"pp_open_goods_receipt|{order_id}"))
        if platform_invoice_file_id:
            extra_row.append(InlineKeyboardButton("🧾 فاتورة المنصة", callback_data=f"pp_open_platform_invoice|{order_id}"))
        if extra_row:
            rows.append(extra_row)

        rows.append([InlineKeyboardButton("📜 سجل الطلب / آخر تحديث", callback_data=f"pp_order_legal|{order_id}")])
        rows.append([InlineKeyboardButton("🔒 فرض الغاء الطلب قصري", callback_data=f"pp_admin_cancel|{order_id}")])
        rows.append([InlineKeyboardButton("🔁 إعادة نشر الطلب", callback_data=f"pp_admin_republish|{order_id}")])
        ...
        if goods_receipt_file_id:
            rows.append([InlineKeyboardButton("🧾 إيصال دفع القطع", callback_data=f"pp_open_goods_receipt|{order_id}")])
        ...
        if _client_can_cancel_order(order):
            rows.append([InlineKeyboardButton("⛔ الغاء الطلب لم اعد بحاجة له", callback_data=f"pp_rb_stop|{order_id}")])
        rows.append([InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")])
...
def build_order_legal_message(order_id: str, viewer_id: int) -> tuple[str, InlineKeyboardMarkup | None]:
    ...
    def _payment_status_display_local(o: dict) -> str:
        """
        حالة دفع القطع فقط
        لا تعتمد على payment_status لأنه قد يخص رسوم المنصة
        """
    ...
    # أسماء
    client_name = _s(o.get("user_name") or o.get("client_name") or "—") or "—"
    car_name = _s(o.get("car_name") or o.get("vehicle_name") or o.get("car") or "—") or "—"
    car_model = _s(o.get("car_model") or "") or ""
    vin = _s(o.get("vin") or "")

    # التاجر المعتمد
    accepted_tid = _to_int(o.get("accepted_trader_id"))
    accepted_trader_disp = _trader_label(accepted_tid, "—") if accepted_tid else "—"
    try:
        tp_acc = get_trader_profile(accepted_tid) or {}
    except Exception:
        tp_acc = {}
    accepted_trader_store = _s(tp_acc.get("company_name") or tp_acc.get("shop_name") or o.get("trader_store") or o.get("company_name") or "")

    # آخر تاجر قدم عرض
    quoted_tid = _to_int(o.get("quoted_trader_id"))
    quoted_name = _s(o.get("quoted_trader_name"))
    if not quoted_name and quoted_tid:
        try:
            quoted_name = _trader_label(quoted_tid, "—")
        except Exception:
            quoted_name = "—"
    if not quoted_name and accepted_tid:
        quoted_name = accepted_trader_disp
    quoted_name = quoted_name or "—"

    quoted_store = ""
    if quoted_tid:
        try:
            tp_q = get_trader_profile(quoted_tid) or {}
        except Exception:
            tp_q = {}
        quoted_store = _s(tp_q.get("company_name") or tp_q.get("shop_name") or "")

    # حالة الطلب — نوحدها مع لوحة الطلب
    ost = _effective_order_status(o)
    display_status = _order_status_display(o) or "—"

    if ost in ("cancelled", "canceled", "ملغي"):
        if _s(o.get("cancelled_by_admin_id")):
            display_status = "ملغي من قبل الإدارة"
        elif _s(o.get("cancelled_by_client_id")):
    ...
    try:
        ...
        tot = _money(o.get("total_amount_sar") or o.get("total_amount") or o.get("quote_total_amount") or "")
        if not tot:
            try:
                gnum = float(re.sub(r"[^0-9.]+", "", str(o.get("goods_amount_sar") or o.get("quote_goods_amount") or "")) or 0)
            except Exception:
                gnum = 0.0
            try:
                snum = float(re.sub(r"[^0-9.]+", "", str(o.get("shipping_fee_sar") or o.get("shipping_fee") or "")) or 0)
            except Exception:
                snum = 0.0
            if gnum or snum:
        ...
        if sf:
            ship_line = f"• رسوم الشحن: <b>{html.escape(sf)}</b>"
        if tot:
    ...
    # المدد
    availability = _fmt_days(o.get("availability_days") or o.get("quote_availability") or o.get("availability") or "")
    ship_eta = _fmt_days(o.get("ship_eta") or o.get("shipping_eta") or o.get("ship_days") or o.get("shipping_days") or "")

    # حالة الدفع — دفع القطع فقط
    payment_status_ar = _payment_status_display_local(o)
    gps = _s(o.get("goods_payment_status")).lower()

    # عدد العروض
    try:
        qcount = int(float(_s(o.get("quotes_count")) or "0"))
    except Exception:
        qcount = 0

    # fallback بسيط للطلبات القديمة التي فيها عرض فعلي لكن العداد لم يُحدّث
    if qcount <= 0:
        has_any_quote_evidence = any([
            _to_int(o.get("quoted_trader_id")) > 0,
            _to_int(o.get("accepted_trader_id")) > 0,
            _s(o.get("quoted_trader_name")),
            _s(o.get("goods_amount_sar")),
            _s(o.get("quote_goods_amount")),
            _s(o.get("quote_item_prices")),
        ])
        if has_any_quote_evidence:
            qcount = 1

    # تفاصيل القطع المطلوبة — أسماء فقط
    parts_lines = []
    for idx, it in enumerate(items or [], start=1):
        nm = _s(it.get("name") or it.get("item_name") or it.get("part_name"))
        if not nm:
            continue
        partno = _s(it.get("part_no") or it.get("partno") or it.get("part_number") or it.get("item_part_no"))
        line = f"{idx}- {html.escape(nm)}"
        if partno:
            line += f" — <code>{html.escape(partno)}</code>"
    ...
    parts_block = ""
    if parts_lines:
        parts_block = "🧩 <b>تفاصيل القطع المطلوبة</b>\n" + "\n".join(parts_lines)

    # منصة: مدفوع/مجاني
    fee_sar = _s(o.get("price_sar"))
    pm = _s(o.get("payment_method")).lower()
    ps = _s(o.get("payment_status")).lower()
    platform_fee_mode = "مجاني" if (fee_sar in ("", "0", "0.0") or pm == "free") else "مدفوع"
    platform_fee_status = _pay_status_ar(ps) if ps else "—"

    # خط زمني (best effort)
    timeline = []
    ts_created = _fmt_utc_ts_for_humans(_s(o.get("created_at_utc") or o.get("created_at")))
    if ts_created:
        timeline.append(f"🟢 إنشاء الطلب: {ts_created} ({platform_fee_mode})")

    ts_pf = _fmt_utc_ts_for_humans(_s(o.get("payment_confirmed_at_utc") or o.get("platform_payment_confirmed_at_utc")))
    if ts_pf:
        timeline.append(f"💳 تأكيد رسوم المنصة: {ts_pf} ({platform_fee_status})")

    ts_quote = _fmt_utc_ts_for_humans(_s(o.get("quote_sent_at_utc") or o.get("quote_sent_at")))
    if ts_quote:
        timeline.append(f"💬 إرسال عرض السعر للعميل: {ts_quote}")

    ts_accept = _fmt_utc_ts_for_humans(_s(o.get("accepted_at_utc") or o.get("quote_accepted_at_utc")))
    if ts_accept:
        timeline.append(f"✅ قبول العرض: {ts_accept}")

    ts_inv = _fmt_utc_ts_for_humans(_s(o.get("seller_invoice_at") or o.get("shop_invoice_at")))
    if ts_inv:
        timeline.append(f"🧾 رفع فاتورة المتجر: {ts_inv}")

    ts_goods = _fmt_utc_ts_for_humans(_s(o.get("goods_payment_confirmed_at_utc") or o.get("goods_payment_at_utc")))
    if ts_goods:
        timeline.append(f"💰 تأكيد سداد قيمة القطع: {ts_goods} ({html.escape(_payment_status_display_local(o))})")
    elif _s(o.get("goods_receipt_file_id")):
        timeline.append("🧾 تم رفع إيصال دفع قيمة القطع وبانتظار تأكيد التاجر")

    ts_ship = _fmt_utc_ts_for_humans(_s(o.get("shipped_at_utc") or o.get("shipping_at")))
    if ts_ship:
        timeline.append(f"🚚 تم شحن الطلب: {ts_ship}")

    ts_del = _fmt_utc_ts_for_humans(_s(o.get("delivered_at_utc")))
    if ts_del:
        timeline.append(f"📦 تم تسليم الطلب: {ts_del}")

    ts_close = _fmt_utc_ts_for_humans(_s(o.get("closed_at_utc")))
    if ts_close:
        timeline.append(f"🔒 إغلاق الطلب: {ts_close}")

    tracking = _s(o.get("shipping_tracking") or o.get("tracking_no") or o.get("tracking"))
    if tracking:
        timeline.append(f"🔎 رقم التتبع: {html.escape(tracking)}")

    # موقع توقف الطلب
    stop_hint = ""
    if ost in ("", "new", "awaiting_quotes"):
        stop_hint = "بانتظار تقديم العروض"
    elif ost in ("accepted",):
        stop_hint = "تم قبول العرض"
    elif ost in ("quoted",):
        stop_hint = "تم تقديم عرض سعر وبانتظار قرار العميل"
    elif ost in ("preparing", "prep"):
        stop_hint = "بانتظار تجهيز الطلب"
    elif ost in ("ready_to_ship", "ready"):
        inv_file = _s(o.get("seller_invoice_file_id") or o.get("shop_invoice_file_id"))
        if not inv_file:
    ...
    elif ost in ("cancelled", "canceled", "ملغي"):
        stop_hint = "تم إلغاء الطلب"
    else:
        stop_hint = "قيد المتابعة"

    # بناء الرسالة
    lines = []
    lines.append("📜 <b>سجل الطلب</b>")
    lines.append("")
    ...
    lines.append(f"• آخر تاجر قدم عرض: <b>{html.escape(quoted_name)}</b>")
    if quoted_store:
        lines.append(f"• متجر آخر عرض: <b>{html.escape(quoted_store)}</b>")
    lines.append(f"• التاجر المعتمد: <b>{html.escape(accepted_trader_disp)}</b>")
    if accepted_trader_store:
        lines.append(f"• المتجر المعتمد: <b>{html.escape(accepted_trader_store)}</b>")

    lines.append("")
    lines.append("🚗 <b>بيانات المركبة</b>")
    lines.append(f"• السيارة: <b>{html.escape(car_name)}</b>")
    if car_model:
        lines.append(f"• الموديل: <b>{html.escape(car_model)}</b>")
    if vin:
        lines.append(f"• VIN: <code>{html.escape(vin)}</code>")

    lines.append("")
    lines.append("📊 <b>ملخص الطلب</b>")
    lines.append(f"• عدد العروض: <b>{qcount}</b>")
    lines.append(f"• حالة الدفع: <b>{html.escape(payment_status_ar)}</b>")
    ...
    if timeline:
        lines.append("")
        lines.append("⏱️ <b>الخط الزمني</b>")
        lines.append("\n".join(timeline))
...
async def order_legal_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await _alert(q, "جارٍ تحميل السجل...")
    data = (q.data or "").strip()
    try:
        _, oid = data.split("|", 1)
    except Exception:
...
def _order_created_dt_safe(o: dict) -> datetime:
    v = str((o or {}).get("created_at_utc") or "")
    try:
        return datetime.fromisoformat(v.replace("Z", "+00:00"))
    except Exception:
        return datetime.min.replace(tzinfo=timezone.utc)
...
def _filtered_orders_for_panel(orders: list[dict], mode: str) -> list[dict]:
    out = []
    target_done = (str(mode or "pending").strip().lower() == "done")
    for o in (orders or []):
        oid = str((o or {}).get("order_id") or "").strip()
        if not oid:
            continue
        is_done = _order_is_done_for_panels(o or {})
        show = is_done if target_done else (not is_done)
        if show:
            out.append(o)
    out.sort(key=_order_created_dt_safe, reverse=True)
    return out


def _build_paginated_orders_view(orders: list[dict], mode: str, callback_prefix: str, back_callback: str, manual_callback: str, theme: str = "admin", page: int = 1, per_page: int = 5) -> tuple[str, InlineKeyboardMarkup]:
    mode_norm = str(mode or "pending").strip().lower()
    filtered = _filtered_orders_for_panel(orders or [], mode_norm)

    header = "📦 الطلبات المعلقة" if mode_norm != "done" else "✅ الطلبات المنجزة"
    if theme == "trader":
        header = "📦 طلباتك المعلقة" if mode_norm != "done" else "✅ طلباتك المنجزة"
        prefix = "🟩"
        back_label = "↩️ رجوع للوحة التاجر"
    else:
        prefix = "🟥"
        back_label = "↩️ رجوع"
    ...
    for o in chunk:
        ...
        amt = _money(o.get("goods_amount_sar") or o.get("quote_goods_amount") or o.get("quote_amount_sar") or "") or "—"
        st_txt = _order_status_display(o) or "—"
        if theme == "trader":
            lines.append(f"• {oid} — {amt} — {st_txt}")
        else:
            lines.append(f"• {oid} — {uname} — {amt} — {st_txt}")
        kb_rows.append([InlineKeyboardButton(f"🧾 {oid}", callback_data=f"pp_open_order|{oid}")])

    if total:
        body = "\n".join(lines)
        body += f"\n\n📄 الصفحة {page}/{total_pages}"
    else:
        body = "لا يوجد"

    msg = f"{prefix} <b>{html.escape(header)}</b>\n\n{html.escape(body)}"

    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton("⬅️ السابق", callback_data=f"{callback_prefix}|{mode_norm}|{page - 1}"))
    nav_row.append(InlineKeyboardButton(f"{page}/{total_pages}", callback_data="pp_list_noop"))
    if page < total_pages:
        nav_row.append(InlineKeyboardButton("التالي ➡️", callback_data=f"{callback_prefix}|{mode_norm}|{page + 1}"))
    if nav_row:
        kb_rows.append(nav_row)

    kb_rows.append([InlineKeyboardButton("🔎 اكتب رقم الطلب يدوي", callback_data=manual_callback)])
    kb_rows.append([InlineKeyboardButton(back_label, callback_data=back_callback)])

    return msg, InlineKeyboardMarkup(kb_rows)


async def _send_or_edit_orders_view(q, msg: str, kb: InlineKeyboardMarkup):
    try:
        await q.message.edit_text(msg, parse_mode="HTML", reply_markup=kb, disable_web_page_preview=True)
        return
    except Exception:
        pass
    try:
...
async def open_order_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ نفس الدالة تخدم: فتح اللوحة / فتح إيصال القطع / فتح فاتورة المنصة
    try:
    ...
    try:
        if action == "pp_open_order":
            await _alert(q, "جارٍ تحميل الطلب...")
        else:
            await q.answer()
    except Exception:
        try:
    ...
    # ✅ جلب الطلب لتحديد الصلاحية + الملف المطلوب
    try:
        b = get_order_bundle(order_id) or {}
        order = (b.get("order") or {}) if isinstance(b, dict) else {}
    except Exception:
    ...
    is_allowed = bool(
        uid in (ADMIN_IDS or [])
        or (accepted_tid and uid == accepted_tid)
        or (client_id and uid == client_id)
    )
    ...
    # ==============================
    # 2) فتح إيصال دفع القطع
    # ==============================
    if action == "pp_open_goods_receipt":
        file_id = str(
            order.get("goods_receipt_file_id")
            or order.get("goods_payment_receipt_file_id")
            or ""
        ).strip()
        mime = str(order.get("goods_receipt_mime") or "").strip().lower()

        if not file_id:
            try:
                await _alert(q, "لا يوجد إيصال دفع محفوظ لهذا الطلب", force=True)
            except Exception:
                pass
            return

        caption = f"🧾 إيصال دفع القطع\n🧾 رقم الطلب: {order_id}"
        ...
        try:
            ...
            try:
                ...
                _swallow(e)
                try:
                    await _alert(q, "تعذر فتح إيصال الدفع", force=True)
                except Exception:
                    pass
        return
    ...
    # 3) فتح فاتورة المنصة
    # ==============================
    if action == "pp_open_platform_invoice":
        file_id = str(
            order.get("platform_invoice_file_id")
            or order.get("invoice_platform_pre_file_id")
            or order.get("invoice_platform_ship_file_id")
        ...
        if not file_id:
            try:
                await _alert(q, "لا توجد فاتورة منصة محفوظة لهذا الطلب", force=True)
            except Exception:
                pass
            return
        ...
        try:
            await context.bot.send_document(
                chat_id=uid,
                document=file_id,
                caption=f"🧾 فاتورة المنصة\n🧾 رقم الطلب: {order_id}",
            )
        except Exception as e:
            _swallow(e)
            try:
                await _alert(q, "تعذر فتح فاتورة المنصة", force=True)
            except Exception:
                pass
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_JOIN_VAT:
        ...
        try:
            upsert_trader_profile(int(user_id), {
                "trader_id": int(user_id),
                "cr_file_id": str(jd.get("cr_file_id") or "").strip(),
                "cr_kind": str(jd.get("cr_kind") or "").strip(),
                "license_file_id": str(jd.get("license_file_id") or "").strip(),
                "license_kind": str(jd.get("license_kind") or "").strip(),
                "vat_no": vat,
            })
        except Exception as e:
            _swallow(e)
        ...
        if _support_should_close_by_time(ud):
            ...
            if cur_stage and cur_stage != STAGE_NONE:
                ...
                if msg:
                    ...
                    # ✅ لا نفرض رقم الطلب في /منصة أو /help
                    # إذا وُجد رقم طلب نرفقه للإدارة، وإذا لم يوجد تُرسل الرسالة مباشرة.
                    # ✅ وإذا لم يذكره المرسل نحاول إرفاق آخر طلب مرتبط به تلقائيا.

                    if not order_id_support:
                        try:
                            order_id_support = _latest_order_id_for_support_user(user_id, is_trader=is_trader_sender)
                        except Exception as e:
                            _swallow(e)
                            order_id_support = ""
                    ...
                    for aid in (ADMIN_IDS or []):
                        try:
                            await context.bot.send_message(
                                ...
                                reply_markup=_support_admin_notify_kb(
                                    user_id=user_id,
                                    order_id=order_id_support,
                                    is_trader_sender=is_trader_sender,
                                ),
                                disable_web_page_preview=True,
                            )
                        except Exception as e:
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_SEND_PAYLINK:
        ...
        return
    # ==================================================
    # 6.1) إدخال قيم الرسوم من الإدارة
    # ==================================================
    if user_id in ADMIN_IDS and stage in (STAGE_ADMIN_SET_TRADER_SUB_FEE, STAGE_ADMIN_SET_PLATFORM_FEE_LOW, STAGE_ADMIN_SET_PLATFORM_FEE_HIGH):
        raw_val = (text or "").strip()
        try:
            val = int(float(raw_val))
        except Exception:
            await _reply_html("قيمة غير صحيحة", ["⚠️ أرسل رقمًا صحيحًا فقط بدون نصوص إضافية."])
            return

        if val < 0:
            await _reply_html("قيمة غير صحيحة", ["⚠️ لا يمكن إدخال قيمة سالبة."])
            return

        key = ""
        label = ""
        if stage == STAGE_ADMIN_SET_TRADER_SUB_FEE:
            key = TRADER_SUB_FEE_KEY
            label = "اشتراك التاجر الشهري"
        elif stage == STAGE_ADMIN_SET_PLATFORM_FEE_LOW:
            key = PLATFORM_FEE_LOW_KEY
            label = "رسوم المنصة حتى 5 قطع"
        elif stage == STAGE_ADMIN_SET_PLATFORM_FEE_HIGH:
            key = PLATFORM_FEE_HIGH_KEY
            label = "رسوم المنصة من 6 قطع فأكثر"

        try:
            set_setting(key, str(val), actor_id=user_id, actor_name=(update.effective_user.full_name or ""))
        except Exception as e:
            _swallow(e)

        try:
            append_legal_log(user_id, (update.effective_user.full_name or ""), key, str(val))
        except Exception as e:
            _swallow(e)

        set_stage(context, user_id, STAGE_NONE)
        await _reply_html("تم الحفظ", [f"✅ تم تحديث {label} إلى <b>{val}</b> ريال.", "يمكنك الآن الرجوع إلى لوحة الإدارة وفتح قسم الرسوم والاشتراكات."], kb=InlineKeyboardMarkup([[InlineKeyboardButton("💳 فتح الرسوم والاشتراكات", callback_data="pp_admin|fees")]]))
        return
    ...
    if (text or "").strip().lower() == "pp25s":
        ...
        return
    # ==================================================
    # 9) بحث طلب من لوحة التاجر
    # ==================================================
    if stage == STAGE_TRADER_FIND_ORDER:
        ...
        if pr.get("kind") == "full":
            ...
            if not oid:
                ...
                return
            try:
                await _show_order_panel_private(context, user_id, oid)
            except Exception:
                msg, kb = build_order_legal_message(oid, int(user_id))
                await _reply(msg, kb=kb, parse_mode="HTML")
            return

        await _resolve_and_show_order(context, user_id, raw_in)
        return
...
# ==================================================
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        if step == "shipping_fee":
            ...
            await _reply_html(
                "مدة التجهيز",
                _quote_hdr(order_id) + ["👇 حدد مدة التجهيز من الأزرار:"],
...
async def admin_cancel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _alert(q, "⛔ تم إيقاف الطلب من الإدارة بنجاح", force=True)
    try:
        await _show_order_panel_private(context, actor_id, order_id)
    except Exception as e:
        _swallow(e)
async def admin_republish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ جلب بيانات الطلب كاملة قبل إعادة النشر
    try:
        b = get_order_bundle(order_id) or {}
    except Exception:
        b = {}
    ...
    # إذا ما لقينا أصل الطلب فعليًا، نوقف هنا بدل إعادة نشر فارغة
    if not order and not items:
        await _alert(q, "تعذر قراءة بيانات الطلب لإعادة نشره")
        return

    # ✅ بناء payload كامل مثل النشر الأصلي
    try:
        user_id_val = int(order.get("user_id") or 0)
    except Exception:
        user_id_val = 0

    try:
        price_val_raw = order.get("price_sar") or 0
        price_val = float(price_val_raw) if str(price_val_raw).strip() else 0.0
    except Exception:
        price_val = 0.0
    ...
    ud_payload = {
        ...
        "price_sar": price_val,
        "items": items,
        "ship_method": str(order.get("ship_method") or ""),
        "delivery_details": str(order.get("delivery_details") or ""),
    }
    ...
    # ✅ إعادة نشر للمجموعة مع payload كامل
    republish_ok = True
    try:
        await notify_team(context, ud_payload)
    except Exception as e:
    ...
    accepted_tid = 0
    try:
        gps = str(order.get("goods_payment_status") or order.get("payment_status") or "").strip().lower()
        has_paid = gps in ("paid", "confirmed")
        accepted_tid = int(order.get("accepted_trader_id") or 0) if str(order.get("accepted_trader_id") or "").strip().isdigit() else 0
    except Exception:
        has_paid = False
        accepted_tid = 0
    ...
    if uid:
        try:
            await context.bot.send_message(
                ...
                reply_markup=notice_kb_for(
                    context,
                    uid,
                    order_id,
                    include_chat_trader=bool(has_paid),
    ...
    if republish_ok:
        await _alert(q, "✅ تم إعادة نشر الطلب بنجاح", force=True)
    else:
        await _alert(q, "⚠️ تم فتح الطلب لكن فشل نشره للمجموعة", force=True)
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Toast خفيف للأزرار السريعة التي قد تحتاج قراءة إكسل/بناء لوحة
    try:
        if action == "orders":
            await _alert(q, "جارٍ تحميل الطلبات...")
        elif action in ("refresh", "home", ""):
            await _alert(q, "جارٍ فتح لوحة التاجر...")
        elif action == "find_order":
            await _alert(q, "جارٍ فتح البحث...")
        else:
            await _alert(q, "")
    except Exception:
        try:
    ...
    if uid not in ADMIN_IDS and _trader_is_disabled(uid) and action in ("edit", "orders", "sub", "sublist"):
        # Popup واضح
    ...
    if action == "orders":
        ...
        msg, kb = _build_paginated_orders_view(
            orders=orders,
            mode=mode,
            callback_prefix="pp_tprof|orders",
            back_callback="pp_tprof|refresh",
            manual_callback="pp_tprof|find_order",
            theme="trader",
            page=page,
            per_page=5,
        )
        await _send_or_edit_orders_view(q, msg, kb)
        return

    if action == "find_order":
        set_stage(context, uid, STAGE_TRADER_FIND_ORDER)
        await _send_or_edit_orders_view(
            q,
            "🔎 <b>بحث عن طلب</b>\n\nاكتب رقم الطلب الآن:",
            InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع للوحة التاجر", callback_data="pp_tprof|refresh")]]),
        )
        return

    if action == "sublist":
        sel = (sub or "mine").strip()
        try:
            subs = [s for s in (list_trader_subscriptions() or []) if int(s.get("trader_id") or 0) == int(uid or 0)]
        except Exception:
            subs = []

        def _sub_dt(s: dict):
            for k in ("receipt_uploaded_at_utc", "paid_at_utc", "created_at_utc"):
                try:
        ...
        if sel in ("", "mine", "list"):
            subs = [s for s in subs if str(s.get("receipt_file_id") or "").strip()]
            subs = sorted(subs, key=_sub_dt, reverse=True)
            if not subs:
                await _send_or_edit_orders_view(
                    q,
                    "🧾 <b>إيصالات الاشتراك</b>\n\nلا توجد إيصالات محفوظة حتى الآن.",
                    InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع للوحة التاجر", callback_data="pp_tprof|refresh")]]),
                )
                return
            ...
            rows = []
            for s in subs[:24]:
                month = str(s.get("month") or "").strip()
                if not month:
                    continue
                stv = str(s.get("payment_status") or "").strip().lower()
                icon = "🟩" if stv == "confirmed" else ("🟨" if stv in ("pending", "awaiting") else "🟥")
                rows.append([InlineKeyboardButton(f"{icon} {month}", callback_data=f"pp_tprof|sublist|{month}")])
            rows.append([InlineKeyboardButton("↩️ رجوع للوحة التاجر", callback_data="pp_tprof|refresh")])
            await _send_or_edit_orders_view(
                q,
                "🧾 <b>إيصالات الاشتراك</b>\n\nاختر الشهر لفتح الإيصال المحفوظ:",
                InlineKeyboardMarkup(rows),
            )
            return

        try:
            sub_row = get_trader_subscription(int(uid or 0), sel) or {}
        except Exception:
            sub_row = {}

        file_id = str(sub_row.get("receipt_file_id") or "").strip()
        if not file_id:
            await _pop("⚠️ لا يوجد إيصال محفوظ لهذا الشهر")
            return

        kind = str(sub_row.get("receipt_kind") or "").strip().lower()
        stv = str(sub_row.get("payment_status") or "").strip().lower()
        st_txt = "مؤكد" if stv == "confirmed" else ("قيد التحقق" if stv in ("pending", "awaiting") else ("مرفوض" if stv == "rejected" else "—"))
        cap = (
            "🧾 <b>إيصال اشتراك التاجر</b>\n"
            f"📅 الشهر: <b>{html.escape(sel)}</b>\n"
            f"💳 الطريقة: <b>{html.escape(str(sub_row.get('payment_method') or '—'))}</b>\n"
            f"💰 المبلغ: <b>{html.escape(str(sub_row.get('amount_sar') or '—'))}</b> ريال\n"
            f"📌 الحالة: <b>{html.escape(st_txt)}</b>\n"
            f"🕓 تاريخ الرفع: <b>{html.escape(str(sub_row.get('receipt_uploaded_at_utc') or sub_row.get('created_at_utc') or '—'))}</b>"
        )
        try:
            if kind == "photo":
                await context.bot.send_photo(chat_id=uid, photo=file_id, caption=cap, parse_mode="HTML")
            else:
                await context.bot.send_document(chat_id=uid, document=file_id, caption=cap, parse_mode="HTML")
            await _pop(f"✅ تم إرسال إيصال {sel} في الخاص")
        except Exception as e:
            _swallow(e)
            await _pop("⚠️ تعذر فتح الإيصال الآن")
        return

    if action == "sub":
        # 💳 اشتراك شهري للتاجر (99 ر.س)
        month = month_key_utc()
        amount = _trader_sub_fee_amount()
        ud["sub_month"] = month
    ...
    if action == "pfee":
        try:
            prof = get_trader_profile(int(uid or 0)) or {}
        except Exception:
            prof = {}
        if not _platform_fee_notice_is_active(prof):
            await _pop("ℹ️ لا توجد مطالبة رسوم منصة نشطة على حسابك حالياً")
            return
        amount = int(float(prof.get("platform_fee_notice_amount_sar") or _platform_fee_notice_amount()))
        ref = _platform_fee_notice_reference(prof, uid)
        ud["platform_fee_notice_amount_sar"] = amount
        ud["platform_fee_notice_id"] = str(prof.get("platform_fee_notice_id") or "")
        set_stage(context, uid, STAGE_TRADER_PLATFORM_FEE_AWAIT_PAY_METHOD)
        msg = (
            "💳 <b>رسوم المنصة</b>\n"
            f"💰 المبلغ المطلوب: <b>{amount}</b> ريال\n"
            f"🧾 المرجع: <code>{html.escape(ref)}</code>\n"
            f"🕓 آخر موعد للسداد: <b>{html.escape(str(prof.get('platform_fee_notice_due_at_utc') or '—'))}</b>\n\n"
            "⬇️ اختر طريقة الدفع ثم أرسل إيصال السداد هنا."
        )
        await q.message.reply_text(msg, parse_mode="HTML", reply_markup=pay_method_kb(), disable_web_page_preview=True)
        return
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Toast خفيف للأزرار السريعة الثقيلة نسبيًا
    try:
        if action == "orders":
            await _alert(q, "جارٍ تحميل الطلبات...")
        elif action in ("home", ""):
            await _alert(q, "جارٍ فتح لوحة الإدارة...")
        elif action == "find_order":
            await _alert(q, "جارٍ فتح البحث...")
        else:
            await _alert(q, "")
    ...
    # ==================================================================
    # 🔒 زر إنهاء / إقفال الطلب
    # - تم توحيد المسار مع order_finish_cb حتى لا تختلف النتيجة بين الواجهات
    # ==================================================================
    if action == "pp_order_finish":
        await order_finish_cb(update, context)
        return
    ...
    if action == "pp_order_finish":
        ...
        def _finish_base_dt(o: dict):
            ...
            for raw in candidates:
                dt = _parse_finish_dt(raw)
                if dt:
                    return dt
            return None

        def _already_closed(o: dict) -> bool:
            try:
                st = str(o.get("order_status") or "").strip().lower()
            except Exception:
                st = ""
            if st == "closed":
                return True
        ...
        if _already_closed(order):
            try:
                await q.answer("✅ الطلب مغلق مسبقًا.", show_alert=True)
            except Exception as e:
                _swallow(e)
            return

        is_admin_actor = int(uid) in (ADMIN_IDS or [])
        now_utc = datetime.now(timezone.utc)
        base_dt = _finish_base_dt(order)

        if (not is_admin_actor) and (not base_dt):
            try:
                await q.answer(
                    "⏳ لا يمكن إغلاق الطلب الآن.\n\n"
                    "🔒 سيتم تفعيل زر الإغلاق بعد الشحن أو بعد توفر تاريخ صالح لبداية مدة المتابعة (7 أيام).",
                    show_alert=True
                )
            except Exception as e:
                _swallow(e)
            ...
            if now_utc < expires_dt:
                ...
                try:
                    await q.answer(
                        ...
                        "🔒 يتم تفعيل زر الإغلاق بعد انتهاء مدة المتابعة (7 أيام).\n"
        ...
        finish_fields = {
            ...
            "closed_at_utc": now_iso,
            "delivered_confirmed_at_utc": now_iso,
            "delivered_confirmed_by": str(uid),
            "quote_locked": "yes",
        }
        ...
        try:
            ...
            _swallow(e)
            try:
                await q.answer("تعذر إنهاء الطلب حاليًا", show_alert=True)
            except Exception:
                pass
            return
        ...
        try:
            append_legal_log(
                order_id=order_id,
                actor_id=str(uid),
                actor_role=("admin" if is_admin_actor else "trader"),
                action="order_finish",
                note=("admin_final_close" if is_admin_actor else "close_after_7_days"),
            )
        except Exception as e:
            _swallow(e)
        ...
        try:
            if is_admin_actor:
                await q.answer("✅ تم إنجاز وإغلاق الطلب نهائيًا.", show_alert=True)
            else:
    ...
    # ===== الرسوم والاشتراكات =====
    if action == "fees":
        trader_sub_fee = _trader_sub_fee_amount()
        fee_low = _platform_fee_low_amount()
        fee_high = _platform_fee_high_amount()
        free_now = "مفعل" if _is_platform_fee_free_mode() else "غير مفعل"
        msg = (
            "💳 <b>إدارة الرسوم والاشتراكات</b>\n\n"
            f"• اشتراك التاجر الشهري: <b>{trader_sub_fee}</b> ريال\n"
            f"• رسوم المنصة حتى 5 قطع: <b>{fee_low}</b> ريال\n"
            f"• رسوم المنصة من 6 قطع فأكثر: <b>{fee_high}</b> ريال\n"
            f"• وضع المجاني الحالي: <b>{free_now}</b>"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💳 تعديل اشتراك التاجر", callback_data="pp_admin|setfee|sub")],
            [InlineKeyboardButton("💰 تعديل رسوم المنصة 1-5", callback_data="pp_admin|setfee|p1")],
            [InlineKeyboardButton("💰 تعديل رسوم المنصة 6+", callback_data="pp_admin|setfee|p2")],
            [InlineKeyboardButton("📣 إعلان سياسة الرسوم للتجار", callback_data="pp_admin|pfee_broadcast")],
            [InlineKeyboardButton("🎁 العرض المجاني", callback_data="pp_admin|fee_free")],
            [InlineKeyboardButton("🏠 الرئيسية", callback_data="pp_admin|home")],
        ])
        await _admin_edit_or_send(q, msg, kb)
        return
    ...
    if action == "setfee":
        mode = (parts[2] if len(parts) > 2 else "").strip()
        if mode == "sub":
            ud[STAGE_KEY] = STAGE_ADMIN_SET_TRADER_SUB_FEE
            await _admin_edit_or_send(q, "💳 <b>تعديل اشتراك التاجر</b>\n\nأرسل الآن قيمة الاشتراك الجديدة بالأرقام فقط.", InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|fees")]]))
            return
        if mode == "p1":
            ud[STAGE_KEY] = STAGE_ADMIN_SET_PLATFORM_FEE_LOW
            await _admin_edit_or_send(q, "💰 <b>تعديل رسوم المنصة حتى 5 قطع</b>\n\nأرسل الآن القيمة الجديدة بالأرقام فقط.", InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|fees")]]))
            return
        if mode == "p2":
            ud[STAGE_KEY] = STAGE_ADMIN_SET_PLATFORM_FEE_HIGH
            await _admin_edit_or_send(q, "💰 <b>تعديل رسوم المنصة من 6 قطع فأكثر</b>\n\nأرسل الآن القيمة الجديدة بالأرقام فقط.", InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|fees")]]))
            return
    ...
    # ===== ORDERS =====
    if action == "orders":
        mode = parts[2].strip() if len(parts) >= 3 else "pending"
        try:
            page = int(parts[3]) if len(parts) >= 4 else 1
        except Exception:
            page = 1
        ...
        msg, kb = _build_paginated_orders_view(
            orders=orders,
            mode=mode,
            callback_prefix="pp_admin|orders",
            back_callback="pp_admin|home",
            manual_callback="pp_admin|find_order",
            theme="admin",
            page=page,
            per_page=5,
        )
    ...
    if action == "tview":
        ...
        cr_file_id = (prof.get("cr_file_id") or "").strip()
        cr_kind = (prof.get("cr_kind") or "").strip().lower()
        license_file_id = (prof.get("license_file_id") or "").strip()
        license_kind = (prof.get("license_kind") or "").strip().lower()
        vat_no = (prof.get("vat_no") or "").strip()
        ...
        quote_override_raw = str(prof.get("quote_access_override") or "").strip().lower()
        if quote_override_raw in ("allow", "allowed", "yes", "true", "1", "on", "enabled", "manual", "admin", "free", "exempt"):
            quote_override_txt = "🟩 سماح يدوي"
        elif quote_override_raw in ("deny", "denied", "block", "blocked", "forbid", "forbidden", "off", "disabled", "no"):
            quote_override_txt = "🟥 منع يدوي"
        else:
            quote_override_txt = "—"

        perm_exempt_txt = "🟩 مفعل" if str(prof.get("subscription_exempt") or "").strip().lower() in ("1", "true", "yes", "on", "enabled", "active", "allow", "allowed", "ok", "paid", "approved", "confirmed", "free", "exempt") else "—"

        month_exempt_raw = str(prof.get("subscription_exempt_month") or "").strip()
        month_exempt_txt = html.escape(month_exempt_raw or "—")
        month_override_until_txt = html.escape(str(prof.get("subscription_override_until") or "").strip() or "—")
        platform_fee_notice_txt = html.escape(_platform_fee_notice_status_label(prof))
        platform_fee_notice_due_txt = html.escape(_platform_fee_notice_due_text(prof))
        platform_fee_notice_amount_txt = html.escape(str(prof.get("platform_fee_notice_amount_sar") or "—"))
        ...
        msg = (
            ...
            f"🎛️ صلاحية التسعير اليدوية: <b>{quote_override_txt}</b>\n"
            f"🪪 إعفاء دائم: <b>{perm_exempt_txt}</b>\n"
            f"🗓️ إعفاء هذا الشهر: <b>{month_exempt_txt}</b>\n"
            f"⏳ سماح حتى: <b>{month_override_until_txt}</b>\n"
            f"💳 مطالبة رسوم المنصة: <b>{platform_fee_notice_txt}</b>\n"
            f"💰 مبلغ المطالبة: <b>{platform_fee_notice_amount_txt}</b>\n"
            f"🕓 استحقاق المطالبة: <b>{platform_fee_notice_due_txt}</b>\n\n"
        ...
        kb_rows = [
            ...
            [InlineKeyboardButton("⏳ بدء مهلة رسوم المنصة", callback_data=f"pp_admin|tpfee|{tid}|start"), InlineKeyboardButton("🔔 تذكير الآن", callback_data=f"pp_admin|tpfee|{tid}|remind")],
            [InlineKeyboardButton("✅ اعتماد السداد ورفع التقييد", callback_data=f"pp_admin|tpfee|{tid}|mark_paid"), InlineKeyboardButton("🧹 تصفير المطالبة", callback_data=f"pp_admin|tpfee|{tid}|clear")],
            [InlineKeyboardButton("🧾 عرض السجل التجاري", callback_data=f"pp_admin|tcr|{tid}"), InlineKeyboardButton("🏪 عرض رخصة المحل", callback_data=f"pp_admin|tlic|{tid}")],
            [InlineKeyboardButton("🟩 سماح يدوي للتسعير", callback_data=f"pp_admin|tquote|{tid}|allow")],
            [InlineKeyboardButton("🟥 منع يدوي للتسعير", callback_data=f"pp_admin|tquote|{tid}|deny")],
            [InlineKeyboardButton("🪪 إعفاء دائم", callback_data=f"pp_admin|tquote|{tid}|perm_on"), InlineKeyboardButton("🧹 إلغاء الإعفاء الدائم", callback_data=f"pp_admin|tquote|{tid}|perm_off")],
            [InlineKeyboardButton(f"🗓️ إعفاء شهر {month}", callback_data=f"pp_admin|tquote|{tid}|month_on"), InlineKeyboardButton("🧹 إلغاء إعفاء الشهر", callback_data=f"pp_admin|tquote|{tid}|month_off")],
            [InlineKeyboardButton("♻️ تصفير قيود التسعير", callback_data=f"pp_admin|tquote|{tid}|reset")],
            ...
            [InlineKeyboardButton("📚 إيصالات الاشتراك", callback_data=f"pp_admin|tsubs|{tid}")],
    ...
    # ===== TRADER ATTACHMENTS (tcr / tlic) =====
    if action in ("tcr", "tlic"):
        tid = 0
        if len(parts) >= 3:
            try:
                tid = int(parts[2] or 0)
        ...
        if action == "tcr":
            file_id = str(prof.get("cr_file_id") or "").strip()
            kind = str(prof.get("cr_kind") or "").strip().lower()
            title = "السجل التجاري"
        else:
            file_id = str(prof.get("license_file_id") or "").strip()
            kind = str(prof.get("license_kind") or "").strip().lower()
            title = "رخصة المحل"

        if not file_id:
            await _pop(f"⚠️ لا يوجد {title} محفوظ لهذا التاجر")
            return

        sent = await _send_saved_file_to_chat(
            context,
            uid,
            file_id,
            f"🧾 {title}\n👤 التاجر: {html.escape((prof.get('display_name') or '').strip() or str(tid))}\n🆔 ID: {tid}",
            kind=kind,
        )
        if sent:
            await _pop(f"✅ تم إرسال {title} في الخاص")
        else:
            await _pop(f"⚠️ تعذر فتح {title} الآن")
        return

    # ===== TRADER SUBSCRIPTION RECEIPTS (tsubs) =====
    if action == "tsubs":
        tid = 0
        month_sel = ""
        if len(parts) >= 3:
            try:
                ...
                tid = 0
        if len(parts) >= 4:
            month_sel = str(parts[3] or "").strip()
        ...
        try:
            ...
            prof = {}
        trader_label = html.escape((prof.get("display_name") or "").strip() or str(tid))

        try:
            subs = [s for s in (list_trader_subscriptions() or []) if int(s.get("trader_id") or 0) == tid]
        except Exception:
            subs = []

        def _sub_dt_admin(s: dict):
            for k in ("receipt_uploaded_at_utc", "paid_at_utc", "created_at_utc"):
                try:
                    v = str(s.get(k) or "").strip()
                    if v:
        ...
        if not month_sel:
            subs = [s for s in subs if str(s.get("receipt_file_id") or "").strip()]
            subs = sorted(subs, key=_sub_dt_admin, reverse=True)
            if not subs:
                await _admin_edit_or_send(
                    q,
                    f"🧾 <b>إيصالات الاشتراك</b>\n\n👤 التاجر: <b>{trader_label}</b>\nلا توجد إيصالات محفوظة لهذا التاجر.",
                    InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع لملف التاجر", callback_data=f"pp_admin|tview|{tid}")]]),
                )
                return
            ...
            for s in subs[:24]:
                ...
                stv = str(s.get("payment_status") or "").strip().lower()
                icon = "🟩" if stv == "confirmed" else ("🟨" if stv in ("pending", "awaiting") else "🟥")
                rows.append([InlineKeyboardButton(f"{icon} {month}", callback_data=f"pp_admin|tsubs|{tid}|{month}")])
            rows.append([InlineKeyboardButton("↩️ رجوع لملف التاجر", callback_data=f"pp_admin|tview|{tid}")])
            await _admin_edit_or_send(
                q,
                f"🧾 <b>إيصالات الاشتراك</b>\n\n👤 التاجر: <b>{trader_label}</b>\nاختر الشهر لفتح الإيصال:",
                InlineKeyboardMarkup(rows),
            )
            return

        try:
            sub_row = get_trader_subscription(tid, month_sel) or {}
        except Exception:
            sub_row = {}
        ...
        cap = (
            "🧾 <b>إيصال اشتراك التاجر</b>\n"
            f"👤 التاجر: <b>{trader_label}</b>\n"
            f"🆔 ID: <b>{tid}</b>\n"
            f"📅 الشهر: <b>{html.escape(month_sel)}</b>\n"
            f"💳 الطريقة: <b>{html.escape(str(sub_row.get('payment_method') or '—'))}</b>\n"
            f"💰 المبلغ: <b>{html.escape(str(sub_row.get('amount_sar') or '—'))}</b> ريال\n"
            f"📌 الحالة: <b>{html.escape(st_txt)}</b>\n"
            f"🕓 تاريخ الرفع: <b>{html.escape(str(sub_row.get('receipt_uploaded_at_utc') or sub_row.get('created_at_utc') or '—'))}</b>"
        ...
        kb_sub = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ تأكيد الاشتراك", callback_data=f"pp_admin_sub|confirm|{tid}|{month_sel}"),
                InlineKeyboardButton("❌ رفض", callback_data=f"pp_admin_sub|reject|{tid}|{month_sel}"),
            ],
            [InlineKeyboardButton("↩️ رجوع لقائمة الإيصالات", callback_data=f"pp_admin|tsubs|{tid}")],
        ])
        try:
            if kind == "photo":
                await context.bot.send_photo(chat_id=uid, photo=file_id, caption=cap, parse_mode="HTML", reply_markup=kb_sub)
            else:
                await context.bot.send_document(chat_id=uid, document=file_id, caption=cap, parse_mode="HTML", reply_markup=kb_sub)
            await _pop(f"✅ تم إرسال إيصال {month_sel} في الخاص")
        except Exception as e:
            _swallow(e)
            await _pop("⚠️ تعذر فتح الإيصال الآن")
        return

    # ===== TRADER QUOTE ACCESS / EXEMPTIONS (tquote) =====
    if action == "tquote":
        tid = 0
        mode = ""
        if len(parts) >= 4:
            try:
                ...
                tid = 0
            mode = str(parts[3] or "").strip().lower()

        if not tid or not mode:
            await _pop("⚠️ بيانات غير مكتملة")
            return

        month = str(month_key_utc() or "").strip()
        fields = {}
        toast = "✅ تم تحديث صلاحيات التسعير"

        if mode == "allow":
            fields = {"quote_access_override": "allow"}
            toast = "✅ تم منح سماح يدوي للتسعير"
        elif mode == "deny":
            fields = {"quote_access_override": "deny"}
            toast = "⛔ تم منع التاجر من بدء عروض سعر جديدة"
        elif mode == "perm_on":
            fields = {"subscription_exempt": "yes"}
            toast = "✅ تم تفعيل الإعفاء الدائم"
        elif mode == "perm_off":
            fields = {"subscription_exempt": ""}
            toast = "🧹 تم إلغاء الإعفاء الدائم"
        elif mode == "month_on":
            fields = {
                "subscription_exempt_month": month,
                "subscription_override_until": month,
            }
            toast = f"✅ تم منح إعفاء لشهر {month}"
        elif mode == "month_off":
            fields = {
                "subscription_exempt_month": "",
                "subscription_override_until": "",
            }
            toast = "🧹 تم إلغاء إعفاء هذا الشهر"
        elif mode == "reset":
            fields = {
            ...
            toast = "♻️ تم تصفير كل قيود واستثناءات التسعير"
        else:
            await _pop("⚠️ إجراء غير معروف")
            return
        ...
        try:
            ...
            _swallow(e)
            await _pop("⚠️ تعذر حفظ التعديل")
            return

        await _toast(toast)

        try:
            q.data = f"pp_admin|tview|{tid}"
        except Exception:
            pass
        parts[:] = ["pp_admin", "tview", str(tid)]
        action = "tview"

    # ===== PLATFORM FEE POLICY BROADCAST =====
    if action == "pfee_broadcast":
        sent = 0
        failed = 0
        try:
            traders = list_traders() or []
        ...
        for t in traders:
            ...
            if not tid:
                continue
            try:
                ok = await _send_platform_fee_notice_message(context, tid, profile=(get_trader_profile(tid) or t), broadcast=True)
                if ok:
                    sent += 1
                else:
                    failed += 1
        ...
        await _admin_edit_or_send(
            q,
            f"📣 تم إرسال إعلان سياسة الرسوم للتجار.\n\n✅ تم الإرسال: <b>{sent}</b>\n⚠️ تعذر الإرسال: <b>{failed}</b>",
            InlineKeyboardMarkup([[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|fees")]])
        )
        return

    # ===== TRADER PLATFORM FEE NOTICE ACTIONS (tpfee) =====
    if action == "tpfee":
        tid = 0
        mode = ""
        if len(parts) >= 3:
        ...
        if mode == "start":
            amount = int(float((prof or {}).get("platform_fee_notice_amount_sar") or _platform_fee_notice_amount()))
            prof = _start_platform_fee_notice(tid, amount) or {}
            try:
                set_trader_enabled(tid, True)
            except Exception:
                pass
            try:
                ok = await _send_platform_fee_notice_message(context, tid, profile=prof)
            except Exception:
                ok = False
            try:
                append_legal_log(uid, (q.from_user.full_name or ""), "trader_platform_fee_notice_start", f"trader_id={tid}; amount={amount}; sent={'yes' if ok else 'no'}")
            except Exception:
                pass
            await _toast("✅ تم بدء المهلة وإرسال الإشعار")
            try:
                q.data = f"pp_admin|tview|{tid}"
            except Exception:
                pass
            ...
            if not _platform_fee_notice_is_active(prof):
                await _pop("ℹ️ لا توجد مطالبة نشطة لهذا التاجر")
                return
            now_iso = utc_now_iso()
            try:
                upsert_trader_profile(tid, {"platform_fee_notice_reminded_at_utc": now_iso, "updated_at_utc": now_iso})
                prof = get_trader_profile(tid) or prof
            except Exception:
                pass
            try:
                ok = await _send_platform_fee_notice_message(context, tid, profile=prof, gentle=True)
            except Exception:
                ok = False
            try:
                append_legal_log(uid, (q.from_user.full_name or ""), "trader_platform_fee_notice_remind", f"trader_id={tid}; sent={'yes' if ok else 'no'}")
            except Exception:
                pass
            await _toast("🔔 تم إرسال التذكير")
            try:
                q.data = f"pp_admin|tview|{tid}"
            except Exception:
                pass
            ...
            try:
                upsert_trader_profile(tid, {
                    "platform_fee_notice_status": "confirmed",
                    "platform_fee_notice_confirmed_at_utc": now_iso,
                    "updated_at_utc": now_iso,
                })
                _clear_platform_fee_notice(tid, mark_confirmed=True)
            except Exception:
                pass
            try:
                append_legal_log(uid, (q.from_user.full_name or ""), "trader_platform_fee_notice_paid", f"trader_id={tid}")
            except Exception:
                pass
            try:
                await context.bot.send_message(
                    chat_id=int(tid),
                    text="🤍 أهلاً بك\n\n✅ <b>تم اعتماد سداد رسوم المنصة</b> ورفع التقييد عن حسابك.\nيمكنك الآن استخدام لوحة التاجر بشكل طبيعي.",
                    parse_mode="HTML",
                    disable_web_page_preview=True,
                )
            except Exception:
                pass
            await _toast("✅ تم اعتماد السداد ورفع التقييد")
            try:
                q.data = f"pp_admin|tview|{tid}"
            except Exception:
                pass
            ...
            enabled_ok = False
            try:
                _clear_platform_fee_notice(tid, mark_confirmed=False)
                cleared_ok = True
            except Exception:
                pass
            try:
            ...
            try:
                append_legal_log(
                    uid,
                    (q.from_user.full_name or ""),
                    "trader_platform_fee_notice_clear",
                    f"trader_id={tid}; cleared={'yes' if cleared_ok else 'no'}; enabled={'yes' if enabled_ok else 'no'}",
                )
            except Exception:
                pass
            try:
                await context.bot.send_message(
                    chat_id=int(tid),
                    text="🤍 أهلاً بك\n\n🧹 <b>تمت إزالة مطالبة رسوم المنصة</b> عن حسابك.\n✅ تم رفع أي تقييد مرتبط بها وإعادة تفعيل الحساب.",
                    parse_mode="HTML",
                    disable_web_page_preview=True,
                )
            except Exception:
                pass
            await _toast("🧹 تم تصفير المطالبة ورفع التقييد وتفعيل الحساب")
            try:
                q.data = f"pp_admin|tview|{tid}"
            except Exception:
                pass
...
def trader_panel_kb(tid: int) -> InlineKeyboardMarkup:
    ...
    has_platform_fee_notice = _platform_fee_notice_is_active(tp)

    rows += [
        [InlineKeyboardButton("🧾 إرسال إيصال الاشتراك", callback_data="pp_tprof|sub|start")],
        *([[InlineKeyboardButton("💳 سداد رسوم المنصة", callback_data="pp_tprof|pfee|pay")]] if has_platform_fee_notice else []),
        [InlineKeyboardButton("📚 عرض إيصالات الاشتراك", callback_data="pp_tprof|sublist|mine")],
...
def admin_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📦 الطلبات المعلقة", callback_data="pp_admin|orders|pending")],
        [InlineKeyboardButton("✅ الطلبات المنجزة", callback_data="pp_admin|orders|done")],
        ...
        [InlineKeyboardButton("💳 الرسوم والاشتراكات", callback_data="pp_admin|fees")],
        [InlineKeyboardButton("📣 إعلان سياسة الرسوم للتجار", callback_data="pp_admin|pfee_broadcast")],
...
async def admin_sub_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == "confirm":
        ...
        admin_name = (
            str(getattr(q.from_user, "full_name", "") or getattr(q.from_user, "first_name", "") or "")
            .strip()
        ) or "الإدارة"

        try:
            sub_row = get_trader_subscription(trader_id, month) or {}
        except Exception as e:
            _swallow(e)
            sub_row = {}

        try:
            amount = int(float(sub_row.get("amount_sar") or _trader_sub_fee_amount()))
        except Exception:
            amount = _trader_sub_fee_amount()

        payment_method = str(sub_row.get("payment_method") or "").strip()

        try:
            upsert_trader_subscription(trader_id, month, {
                "amount_sar": amount,
                "payment_status": "confirmed",
                "paid_at_utc": confirmed_at_utc,
                "confirmed_by_admin_id": str(actor_id),
                "confirmed_by_admin_name": admin_name,
            })
        except Exception as e:
            _swallow(e)
        ...
        invoice_meta = {}
        try:
            invoice_meta = await send_trader_subscription_invoice_pdf(
                context,
                trader_id,
                month,
                amount,
        ...
        try:
            upsert_trader_subscription(trader_id, month, {
                ...
                "payment_status": "confirmed",
                "paid_at_utc": str(invoice_meta.get("paid_at_utc") or confirmed_at_utc),
                "invoice_no": str(invoice_meta.get("invoice_no") or "").strip(),
                "invoice_file_id": str(invoice_meta.get("invoice_file_id") or "").strip(),
                "confirmed_by_admin_id": str(actor_id),
                "confirmed_by_admin_name": admin_name,
            })
        except Exception as e:
        ...
        invoice_notice = (
            "🧾 تم إرسال فاتورة الاشتراك لك في هذه المحادثة.\n"
            if str(invoice_meta.get("invoice_file_id") or "").strip()
            else "🧾 تم اعتماد الاشتراك، وإذا لم تصلك الفاتورة الآن فستجدها لاحقًا من الإدارة عند الحاجة.\n"
        )
        ...
        try:
            await context.bot.send_message(
                chat_id=trader_id,
                text=(
                    "✅ <b>تم التحقق من سداد اشتراكك</b>\n"
                    f"📅 الشهر: <b>{html.escape(month)}</b>\n"
                    f"💰 قيمة الاشتراك: <b>{html.escape(str(amount))}</b> ريال\n"
                    f"💳 طريقة السداد: <b>{html.escape(_pay_method_ar(payment_method or '—'))}</b>\n"
                    f"{invoice_notice}"
        ...
        try:
            await q.message.reply_text(
                ...
                f"👤 التاجر: <code>{trader_id}</code>\n"
                f"📅 الشهر: <b>{html.escape(month)}</b>\n"
                f"💰 المبلغ: <b>{html.escape(str(amount))}</b> ريال\n"
                f"💳 الطريقة: <b>{html.escape(_pay_method_ar(payment_method or '—'))}</b>",
                parse_mode="HTML",
                disable_web_page_preview=True,
            )
        except Exception as e:
...
async def pp_rb_stop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await _alert(q, "⛔ تم إلغاء الطلب من قبلك ولن يستقبل عروضاً جديدة", force=True)
    except Exception as e:
        _swallow(e)
...
def build_app():
    ...
    app.add_handler(CallbackQueryHandler(open_order_cb, pattern=r"^pp_open_(order|goods_receipt|platform_invoice)\|"))
    app.add_handler(CallbackQueryHandler(ui_noop_cb, pattern=r"^pp_list_noop$"))
    ...
    try:
        if app.job_queue:
            ...
            app.job_queue.run_repeating(
                _platform_fee_deadline_job,
                interval=3600,
                first=900,
                name="platform_fee_deadline_job",
            )
    except Exception as e:
        try:
