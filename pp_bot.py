...
def client_trader_chat_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💬 مراسلة التاجر", callback_data=f"pp_chat_trader|{order_id}")],
        [InlineKeyboardButton("✅ تأكيد استلام القطع", callback_data=f"pp_confirm_received|{order_id}")],
    ])
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "restore_db":
        # حالة التفعيل الحالية (داخل الخاص فقط)
        ok_now = _restore_private_ok(context)

        txt = (
            "🗂 <b>استرجاع قاعدة البيانات</b>\n\n"
            "✅ الطريقة 1: أرسل ملف الإكسل هنا في الخاص (للأدمن فقط).\n"
            "✅ الطريقة 2: أرسل ملف الإكسل داخل مجموعة النسخ (PP_BACKUP_CHAT_ID).\n\n"
            f"🔐 حالة التفعيل الحالية: {'✅ مفعّل الآن' if ok_now else '⛔ غير مفعّل'}\n\n"
            "⚡ للتفعيل السريع استخدم زر (🔓 تفعيل الاسترجاع) ثم أرسل ملف الإكسل مباشرة.\n"
        )

        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔓 تفعيل الاسترجاع (10 دقائق)", callback_data="pp_admin|restore_unlock")],
            [InlineKeyboardButton("📥 استرجاع آخر نسخة من مجموعة النسخ", callback_data="pp_admin|restore_last_backup")],
            [InlineKeyboardButton("🔒 تعطيل الاسترجاع", callback_data="pp_admin|restore_lock")],
            [
    ...
    if action == "restore_last_backup":
        ...
        # نقرأ file_id المحفوظ لآخر نسخة
        try:
            fid = str(get_setting("last_backup_file_id", "") or "").strip()
        except Exception:
            fid = ""

        if not fid:
            await _alert(q, "لا توجد نسخة محفوظة بعد")
            try:
                await q.message.reply_text(
                    "⚠️ لا توجد نسخة محفوظة حتى الآن.\n"
                    "انتظر النسخة التلقائية القادمة أو أعد إرسال ملف الإكسل في مجموعة النسخ مرة واحدة."
                )
            except Exception:
                pass
            return
        ...
        try:
            ...
            try:
                await q.message.reply_text("✅ تم استرجاع آخر نسخة احتياطية بنجاح وتم تشغيلها فورًا.")
            except Exception:
                pass
        except Exception as e:
            try:
                await q.message.reply_text(f"❌ فشل استرجاع آخر نسخة.\n{e}")
            except Exception:
                pass
        return

    if action == "restore_unlock":
        # للأمان: نفعّل فقط من الخاص للأدمن
        if q.message and q.message.chat and q.message.chat.type != ChatType.PRIVATE:
            await _alert(q, "هذا الخيار يعمل في الخاص فقط")
            return

        # تفعيل مؤقت
        context.user_data["restore_ok_until_utc"] = (
            datetime.now(timezone.utc) + timedelta(minutes=PP_RESTORE_OK_MINUTES)
        ).isoformat()

        await _alert(q, "تم تفعيل الاسترجاع مؤقتًا")
        try:
            await _admin_edit_or_send(
                q,
                "✅ تم تفعيل الاسترجاع مؤقتًا.\nالآن أرسل ملف الإكسل هنا مباشرة.",
                InlineKeyboardMarkup([
                    [InlineKeyboardButton("🗂 تحديث الصفحة", callback_data="pp_admin|restore_db")],
                    [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
                ])
            )
        except Exception:
    ...
    if action == "restore_lock":
        try:
            context.user_data.pop("restore_ok_until_utc", None)
        except Exception:
            pass

        await _alert(q, "تم تعطيل الاسترجاع")
        try:
            await _admin_edit_or_send(
                q,
                "🔒 تم تعطيل الاسترجاع.",
                InlineKeyboardMarkup([
                    [InlineKeyboardButton("🗂 صفحة الاسترجاع", callback_data="pp_admin|restore_db")],
                    [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
                ])
            )
        except Exception:
...
async def _send_backup_excel(app: Application, reason: str = "scheduled") -> None:
    ...
    # 3) منع التكرار (حد أدنى) - مفيد عند تعدد restarts
    try:
    ...
    try:
        ...
        # ✅ حفظ file_id لآخر نسخة احتياطية
        try:
...
def _seconds_until_next_riyadh_2am() -> int:
    tz = _riyadh_tz()
    now = datetime.now(tz)
...
async def _backup_loop(app: Application) -> None:
    # ✅ نسخة عند أي إعادة تشغيل
    await asyncio.sleep(10)
    await _send_backup_excel(app, reason="startup")

    # ✅ جدولة يومية الساعة 2:00 صباحاً بتوقيت السعودية
    while True:
        try:
            await asyncio.sleep(_seconds_until_next_riyadh_2am())
            await _send_backup_excel(app, reason="daily_02:00_riyadh")
        except Exception as e:
...
def build_app():
    ...
    try:
        ...
        app.add_handler(CommandHandler("pprs", pprs_cmd))  # ✅ اختصار
    ...
    # 🟢 [TASK] Backup (startup + daily 2AM Riyadh)
...
async def _run_webhook():
    ...
    # ✅ إعداد Webhook URL
    # لازم تعطي واحد من هذه المتغيرات في Render:
    # WEBHOOK_BASE_URL مثال: https://parts-pp-b8ky.onrender.com
    # أو RENDER_EXTERNAL_URL (أحياناً متاح تلقائياً)
    ...
    async def webhook_handler(request: web.Request):
        ...
        try:
            ...
            # لا نرجّع 500 كثير حتى ما يعيد تيليجرام الإرسال بلا نهاية
            try:
...
def main():
    ...
    # PP_WEBHOOK_ENABLED=1  -> webhook + web service
    # غير ذلك -> polling (الوضع القديم)
    enabled = (os.getenv("PP_WEBHOOK_ENABLED") or "").strip().lower() in ("1", "true", "yes", "on")

    if enabled:
        # Webhook mode: لا نشغّل HTTPServer القديم حتى ما يتعارض مع PORT
