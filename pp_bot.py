...
async def admin_forward_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ (1) إذا كان forwarded_to_team_at_utc موجود: Alert + قفل بصري للزر
    if str(order.get("forwarded_to_team_at_utc") or "").strip():
        await _alert(q, "✅ تم ارسال الطلب للتاجر مسبقًا", force=True)
        try:
            # قفل بصري: استبدال الكيبورد بزر توضيحي
            locked_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ تم إرسال الطلب للتاجر مسبقًا", callback_data="pp_ui_close")],
            ])
            await q.message.edit_reply_markup(reply_markup=locked_kb)
        except Exception:
            pass
        return
    ...
    # ✅ (2) بعد نجاح الإرسال وتمييزه forwarded: قفل بصري + Alert نجاح
    try:
        locked_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ تم إرسال الطلب للتاجر مسبقًا", callback_data="pp_ui_close")],
        ])
