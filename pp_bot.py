...
def build_official_quote_text(
    ...
    if ship_included:
        ...
        shipping_txt = _money(shipping_val) if _s(shipping_fee_sar) else "—"
    ...
    if ship_block:
        lines = []
        for ln in ship_block.splitlines():
            if any(k in ln for k in ("قيمة الشحن", "رسوم الشحن", "تكلفة الشحن", "مبلغ الشحن", "سعر الشحن")):
                continue
            lines.append(ln)
        ship_block_clean = "\n".join(lines).strip()
...
async def quote_ok_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if missing_ship and ship_included_norm not in ("yes", "true", "1", "included", "مشمولة"):
        ...
        ship_txt = "مشمول" if ship_included_norm in ("yes", "true", "1", "included", "مشمولة") else _money(raw_shipping_fee)
        total_val = _to_float(goods_amount) + ship_fee_val
        total_txt = _money(total_val)
    ...
    amount_block_trader = (
        ...
        f"🧩 قيمة القطع: {goods_txt}\n"
        f"🚚 الشحن: {ship_txt}\n"
        f"💰 <b>الإجمالي (قطع + شحن)</b>: {total_txt}\n"
    )
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # ✅ نسخة للتاجر (مخصصة): اسم العميل + نص مخصص للتاجر فقط
    ...
    try:
        ...
        # حذف نص العميل بالكامل (سطرين) واستبداله بنص التاجر
        old_block = (
            "يرجى مراجعة العرض ثم اختيار القرار من الأزرار بالأسفل.\n"
            "في حال قبول العرض سيتم فتح قناة تواصل مباشرة بين التاجر والعميل."
        )
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_goods_confirm":
        ...
        try:
            ...
            total3 = _money(o3.get("total_amount_sar") or o3.get("goods_amount_sar") or 0)
            ...
            if tid3:
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"🧾 <b>إجمالي الفاتورة</b>: <b>{html.escape(str(total3))}</b>\n\n"
