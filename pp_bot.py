...
async def _send_client_payment_preview(
    ...
    if pay_scope == "goods":
        ...
        next_line = "✅ الخطوة التالية: بعد التحقق سيتم تحديث حالة الطلب وإكمال إجراءات الشحن."
        icon_scope = "🧩"
    else:
        ...
        next_line = "✅ الخطوة التالية: بعد التحقق سيتم متابعة طلبك وإرساله للتجار/استكمال الإجراء."
        icon_scope = "🧾"
    ...
    if pay_scope == "goods" and isinstance(items, list) and items:
        ...
        parts_summary = (
            "\n"
            f"📌 <b>ملخص القطع</b>: "
            f"✅ مسعّرة <b>{priced_count}</b> / "
            f"🟥 غير متوفرة <b>{unpriced_count}</b> / "
            f"📦 الإجمالي <b>{total_items}</b>\n"
        )
...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    # بناء بلوك تفصيلي (يعرض فقط المسعّر)
    items_block = ""
    try:
        if isinstance(itemsx, list) and itemsx and pm:
            lines = ["🧩 تسعير القطع:"]
            for i in range(1, len(itemsx) + 1):
                ...
                pn = (it.get("part_no") or it.get("partno") or "").strip()
                label = f"{i}) {nm}"
                if pn:
                    label += f" ({pn})"
                lines.append(f"• {label}: {vv} ر.س")
            lines.append(f"💰 إجمالي القطع: {goods_amount} ر.س")
            items_block = "\n".join(lines) + "\n"
    except Exception:
        items_block = ""
