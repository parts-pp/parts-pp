...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "ppq_begin":
        ...
        if (
            ...
            msg_lock = "⛔ الطلب معلق من الإدارة ولا يقبل عروض جديدة" if order_status in ("canceled","cancelled","ملغي") else "🔒 الطلب مقفول حالياً ولا يقبل عروض جديدة"
...
async def goods_receipt_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        _swallow(e)
    try:
        await _lock_team_post_keyboard(context, order_id, reason="🔒 الطلب تحت التحديث (مدفوع)")
    except Exception as e:
        _swallow(e)
...
# ===== زر مقفول (تنبيه فقط) =====
    if action in ("pp_team_locked", "pp_team_quote_locked"):
        try:
            ...
            # ✅ تحديد سبب القفل من بيانات الطلب (3 حالات)
            try:
                ob = get_order_bundle(order_id) or {}
                oo = (ob.get("order", {}) or {}) if isinstance(ob, dict) else {}
            except Exception:
            ...
            cancel_source = str(oo.get("cancel_source") or "").strip().lower()
            goods_pay_status = str(oo.get("goods_payment_status") or "").strip().lower()

            # 01) بعد دفع قيمة القطع/ضمن مراحل التحديث (شحن/تسليم)
            if goods_pay_status in ("awaiting_confirm", "confirmed", "paid") or ost in ("ready_to_ship", "shipped", "delivered", "closed"):
                msg = (
                    f"{tname}\n"
                    "🔒 هذا الطلب تحت التحديث بعد الدفع ولا يستقبل عروض جديدة.\n"
                    "نشكر لك اهتمامك وتعاونك."
                )

            # 02/03) ملغي (إدارة أو عميل)
            elif ost in ("canceled", "cancelled", "ملغي"):
                if cancel_source in ("client", "user", "customer"):
                    msg = (
                        f"{tname}\n"
                        "⛔ هذا الطلب ملغي من قبل العميل ولا يستقبل عروض حالياً.\n"
                        "نشكر لك اهتمامك وتعاونك."
                    )
                else:
                    msg = (
            ...
            # ✅ Popup واضح داخل المجموعة + رسالة خاصة (بدون تلويث المجموعة)
    ...
    if action == "pp_team_goods_confirm":
        ...
        try:
            await _lock_team_post_keyboard(
                context,
                order_id,
                reason="🔒 الطلب تحت التحديث (مدفوع)"
            )
        except Exception as e:
            _swallow(e)
...
async def admin_republish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إشعار العميل (مع لوحة الطلب + مراسلة التاجر عند وجود مبالغ مدفوعة + مراسلة المنصة)
    uid = get_order_user_id(order_id)
    if uid:
        try:
            # هل توجد مبالغ/دفعات مؤكدة؟
            has_paid = False
            try:
                b = get_order_bundle(order_id) or {}
                o = (b.get("order", {}) or {}) if isinstance(b, dict) else {}
            ...
            rows = []
            rows.append([InlineKeyboardButton("📋 لوحة الطلب", callback_data=f"pp_open_order|{order_id}")])
            if has_paid:
                rows.append([InlineKeyboardButton("💬 مراسلة التاجر", callback_data=f"pp_chat_trader|{order_id}")])
            rows.append([InlineKeyboardButton("🔎 مراسلة المنصة", callback_data=f"pp_track|{order_id}")])

            await context.bot.send_message(
                chat_id=uid,
                text=("✅ تم إعادة فتح الطلب وإعادة نشره للتجار\n" f"رقم الطلب: {order_id}"),
...
async def pp_rb_stop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ السماح للأدمن أو لصاحب الطلب فقط
    ...
    # ==========================================================
    # ✅ إذا كان صاحب الطلب (وليس أدمن): هذا زر (إيقاف الطلب - لا أحتاجه)
    # - يلغي الطلب فعليًا
    # - يقفل العروض بصريًا داخل مجموعة التجار
    # ==========================================================
    if (not is_admin) and (owner_id == uid):
        now_iso = utc_now_iso()
        try:
        ...
        try:
            update_order_fields(order_id, {
                ...
                "cancelled_by_client_name": _user_name(q),
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    f"{_user_name(q)}\n"
                    "⛔ تم إيقاف/إلغاء الطلب بناءً على طلبك (لم أعد بحاجة له)\n"
                    f"🧾 رقم الطلب: {_order_id_link_html(order_id)}"
                ),
                parse_mode="HTML",
                disable_web_page_preview=True,
    ...
    # ==========================================================
    # ✅ إذا كان أدمن: هذا الزر فقط لإيقاف إعادة النشر (بدون إلغاء)
    # ==========================================================
    try:
        update_order_fields(order_id, {
            "rebroadcast_disabled": "1",
    ...
    try:
        await context.bot.send_message(
            chat_id=uid,
            text=(
                "⛔ تم إيقاف إعادة النشر تلقائيًا لهذا الطلب (الإدارة)\n"
                f"🧾 رقم الطلب: {_order_id_link_html(order_id)}"
            ),
            parse_mode="HTML",
            disable_web_page_preview=True,
