...
async def finalize_quote_send(context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str):
    ...
    try:
        _qc = int(str(o0.get("quotes_count") or "0").strip() or 0)
    except Exception:
        _qc = 0
    fields_to_update["quotes_count"] = _qc + 1
        ...
        try:
            ...
            if ost in ("canceled", "cancelled", "ملغي"):
                if by_client:
                    msg = (
                        f"{tname}\n"
                        "⛔ هذا الطلب ملغي من قبل العميل ولا يستقبل عروض جديدة حالياً.\n"
                        "نشكر لك اهتمامك وتعاونك."
                    )
                elif by_admin:
                    msg = (
                        f"{tname}\n"
                        "⛔ هذا الطلب ملغي من قبل الإدارة ولا يستقبل عروض جديدة حالياً.\n"
                        "نشكر لك اهتمامك وتعاونك."
                    )
                else:
                    msg = (
                        f"{tname}\n"
                        "⛔ هذا الطلب ملغي ولا يستقبل عروض جديدة حالياً.\n"
                        "نشكر لك اهتمامك وتعاونك."
                    )
            else:
                msg = (
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=msg,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("حسناً", callback_data="pp_ui_close")]
                    ]),
                )
            except Exception:
                pass
...
def _build_order_parts_details(order_id: str, items: list[dict], limit: int = 40) -> str:
    # يلخص القطع: مسعّر / غير مسعّر / غير متوفر مع أرقام القطع إن وجدت
    priced = []
    unpriced = []
    unavailable = []

    try:
        bundle = get_order_bundle(order_id) or {}
        order = (bundle.get("order") or {}) if isinstance(bundle, dict) else {}
    except Exception:
        order = {}

    price_map = {}
    raw_pm = order.get("quote_item_prices") or {}
    try:
        if isinstance(raw_pm, dict):
            ...
            if s.startswith("{") and s.endswith("}"):
                try:
                    price_map = json.loads(s)
                except Exception:
                    try:
                        import ast
                        price_map = ast.literal_eval(s)
                    except Exception:
                        price_map = {}
    except Exception:
        price_map = {}

    def _fmt_line(idx: int, it: dict, price: str = "") -> str:
        nm = (it.get("name") or it.get("item_name") or "").strip()
        pn = (it.get("part_no") or it.get("item_part_no") or it.get("partno") or it.get("part_number") or "").strip()
        line = f"{idx}) {nm}" if nm else f"{idx}) —"
        if pn:
            line += f" — {pn}"
        if price:
            line += f" — {_money(price) or str(price).strip()}"
        return line
    ...
    for idx, it in enumerate((items or []), start=1):
        ...
        price = (it.get("price_sar") or it.get("unit_price_sar") or it.get("quote_price_sar") or it.get("price") or price_map.get(str(idx)) or "")
        price = str(price or "").strip()

        if st in ("na", "not_available", "unavailable", "none", "0", "غير متوفر", "غير_متوفر"):
            unavailable.append(_fmt_line(idx, it))
        elif price and price not in ("0", "0.0", "0.00"):
            priced.append(_fmt_line(idx, it, price))
        else:
            unpriced.append(_fmt_line(idx, it))
    ...
    lines = []
    if priced:
        lines.append("القطع المسعّرة:")
        for x in priced[:25]:
            lines.append(f"• {x}")
    if unpriced:
        if lines:
            lines.append("")
        lines.append("القطع غير المسعّرة:")
        for x in unpriced[:25]:
            lines.append(f"• {x}")
    if unavailable:
        if lines:
            lines.append("")
        lines.append("القطع غير المتوفرة:")
        for x in unavailable[:25]:
            lines.append(f"• {x}")
...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, viewer_id: int, order_id: str):
    ...
    last_trader_id = _to_int((order or {}).get("accepted_trader_id") or (order or {}).get("quoted_trader_id"))
    trader_id = _to_int((order or {}).get("accepted_trader_id"))
    trader_name = "—"
    trader_store = ""
    if last_trader_id:
        trader_name = _trader_label(int(last_trader_id), (order.get("accepted_trader_name") or order.get("quoted_trader_name") or "—"))
        try:
            tp = get_trader_profile(int(last_trader_id)) or {}
        except Exception:
            tp = {}
    ...
    city = (str(order.get("ship_city") or order.get("shipping_city") or order.get("city") or "—") or "—").strip() or "—"
    delivery_type = (str(order.get("ship_method") or order.get("delivery_choice") or order.get("delivery_type") or order.get("shipping_method") or "—") or "—").strip() or "—"
    parts_type = (str(order.get("parts_type") or order.get("parts_condition") or "—") or "—").strip() or "—"

    # بيانات شحن إضافية (لو كانت موجودة بالطلب)
    shipping_phone = _s(order.get("ship_phone") or order.get("user_phone") or order.get("client_phone") or order.get("phone") or order.get("mobile"))
    ...
    ga_raw = order.get("goods_amount_sar") or order.get("quote_goods_amount") or ""
    sf_raw = order.get("shipping_fee_sar") or order.get("shipping_fee") or ""
    ga = _money(ga_raw)
    sf = _money(sf_raw)
    tot = _money(order.get("total_amount_sar") or order.get("total_amount") or order.get("quote_total_amount") or "")
    if not tot:
        try:
            _g = float(re.sub(r"[^0-9.]+", "", str(ga_raw)) or 0)
        except Exception:
            _g = 0.0
        try:
            _sfee = float(re.sub(r"[^0-9.]+", "", str(sf_raw)) or 0)
        except Exception:
            _sfee = 0.0
        if _g or _sfee:
            tot = _money(_g + _sfee)
    ...
    try:
        quotes_count = int(str(order.get("quotes_count") or "").strip() or 0)
    except Exception:
        quotes_count = 0
    if quotes_count <= 0:
        quotes_count = 1 if (last_trader_id or pm or order.get("goods_amount_sar") or order.get("quote_goods_amount")) else 0

    availability_txt = _s(order.get("availability_days") or order.get("quote_availability") or order.get("availability"))
    ship_eta_txt = _s(order.get("ship_eta") or order.get("shipping_eta") or order.get("ship_days"))
    payment_status_ar = _pay_status_ar(order.get("goods_payment_status") or order.get("payment_status") or "")

    quote_ts = _fmt_utc_ts_for_humans(_s(order.get("quote_sent_at_utc") or order.get("quote_sent_at") or order.get("seller_invoice_at")))
    accepted_ts = _fmt_utc_ts_for_humans(_s(order.get("accepted_at_utc")))
    shipped_ts = _fmt_utc_ts_for_humans(_s(order.get("shipped_at_utc") or order.get("shipping_at")))
    delivered_ts = _fmt_utc_ts_for_humans(_s(order.get("delivered_at_utc") or order.get("closed_at_utc")))
    ...
    lines.append(f"📊 عدد العروض المقدمة: <b>{quotes_count}</b>")
    ...
    if role == "admin":
        ...
        gps = str(order.get("goods_payment_status") or "").strip().lower()
        _allow_phone = gps in ("confirmed", "paid", "success", "successful", "done", "ok")
    if shipping_phone and _allow_phone:
        lines.append(f"• جوال العميل: <b>{html.escape(shipping_phone)}</b>")

    lines.append(f"• آخر تاجر قدم عرض: <b>{html.escape(trader_name)}</b>")
    if trader_store:
        lines.append(f"• المتجر: <b>{html.escape(trader_store)}</b>")
    ...
    if availability_txt:
        lines.append(f"• مدة التجهيز: <b>{html.escape(availability_txt)}</b>")
    if ship_eta_txt:
        lines.append(f"• مدة الشحن: <b>{html.escape(ship_eta_txt)}</b>")
    if shipping_address:
    ...
    lines.append("")
    lines.append("💳 <b>الدفع</b>")
    lines.append(f"• الحالة: <b>{html.escape(payment_status_ar or '—')}</b>")
    ...
    timeline = []
    for label, val in (("تقديم آخر عرض", quote_ts), ("قبول العرض", accepted_ts), ("الشحن", shipped_ts), ("التسليم", delivered_ts)):
        if val:
            timeline.append(f"• {label}: <b>{html.escape(val)}</b>")
    if timeline:
        lines.append("")
        lines.append("🕒 <b>آخر التحديثات</b>")
...
def build_order_legal_message(order_id: str, viewer_id: int) -> tuple[str, InlineKeyboardMarkup | None]:
    ...
    car_name = (str(o.get("car_name") or o.get("vehicle_name") or o.get("car") or "—")).strip() or "—"
    car_model = (str(o.get("car_model") or o.get("model") or "—")).strip() or "—"
    vin = (str(o.get("vin") or "—")).strip() or "—"

    tid = 0
    try:
        tid = int(o.get("accepted_trader_id") or o.get("quoted_trader_id") or 0)
    except Exception:
        tid = 0
    ...
    try:
        quotes_count = int(str(o.get("quotes_count") or "").strip() or 0)
    except Exception:
        quotes_count = 0
    if quotes_count <= 0:
        quotes_count = 1 if (tid or o.get("quote_item_prices") or o.get("goods_amount_sar")) else 0

    ship_method = (str(o.get("ship_method") or o.get("delivery_choice") or o.get("delivery_type") or o.get("shipping_method") or "—")).strip() or "—"
    payment_status_ar = _pay_status_ar(o.get("goods_payment_status") or o.get("payment_status") or "")
    ...
    lines.append(f"الموديل: <b>{html.escape(car_model)}</b>")
    lines.append(f"رقم الهيكل VIN: <code>{html.escape(vin)}</code>")
    lines.append(f"آخر تاجر قدم عرض: <b>{html.escape(trader_disp)}</b>")
    if trader_store:
        lines.append(f"المتجر: <b>{html.escape(trader_store)}</b>")
    lines.append(f"الحالة الحالية: <b>{html.escape(display_status)}</b>")
    lines.append(f"عدد العروض المقدمة: <b>{quotes_count}</b>")
    lines.append(f"موقع توقف الطلب: <b>{html.escape(stop_hint)}</b>")
    lines.append(f"طريقة الشحن: <b>{html.escape(ship_method)}</b>")
    lines.append(f"حالة الدفع: <b>{html.escape(payment_status_ar or "—")}</b>")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _order_snapshot(order_id: str) -> dict:
        ...
        # ✅ ship_included: دعم المفتاحين ship_included و shipping_included + لا يتركها فاضية
        try:
        ...
        # ✅ الحالة (موحد + يوضح جهة الإلغاء + لا يتركها فاضية)
        try:
