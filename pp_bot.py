...
def _is_trader_profile_strict(context: ContextTypes.DEFAULT_TYPE = None) -> bool:
    # ✅ يقرأ سياسة الإلزام من Settings Sheet (Safe: مع default لتفادي TypeError)
    try:
        v = str(get_setting("trader_profile_strict", "0") or "").strip().lower()
    except Exception:
        v = ""
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    missing = _trader_profile_missing(tp, strict=bool(strict))
    if missing:
        # رسالة توجيه كاملة + زر فتح لوحة التاجر (بدون رسائل مكررة)
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "trecord_pdf":
        ...
        try:
            ...
            val = ParagraphStyle(
                "val", parent=base, alignment=2
            )  # TA_RIGHT (قريب من العنوان)
            ...
            rows = [
                [P("<b>ID</b>", label), P(str(tid), val)],
                [P("<b>اسم التاجر</b>", label), P(html.escape(tname), val)],
                [P("<b>اسم المتجر</b>", label), P(html.escape(tcompany), val)],
                [P("<b>تاريخ الانضمام</b>", label), P(html.escape(created), val)],
                [P("<b>حالة التفعيل</b>", label), P(html.escape(enabled_txt), val)],
                [
                    P(f"<b>حالة الاشتراك ({month})</b>", label),
                    P(html.escape(sub_status), val),
                ],
                [P("<b>اسم البنك</b>", label), P(html.escape(bank), val)],
                [P("<b>IBAN</b>", label), P(html.escape(iban), val)],
                [P("<b>STC Pay</b>", label), P(html.escape(stc), val)],
                [P("<b>رقم الهوية</b>", label), P(html.escape(nid), val)],
                [P("<b>السجل التجاري</b>", label), P(html.escape(crn), val)],
                [P("<b>الرقم الضريبي</b>", label), P(html.escape(vat), val)],
                [P("<b>آخر تحديث</b>", label), P(html.escape(upd), val)],
            ]
            tbl = Table(rows, colWidths=[full_w * 0.38, full_w * 0.62])
            tbl.setStyle(
                TableStyle(
                    [
                        # خلفية صفوف متناوبة (مثل الفواتير) لسهولة القراءة
                        (
                            "ROWBACKGROUNDS",
                            (0, 0),
                            (-1, -1),
                        ...
                        # تمييز عمود العناوين (اليمين)
                        (
                            "BACKGROUND",
                            (0, 0),
                            (0, -1),
                            colors.Color(0.95, 0.55, 0.10, alpha=0.18),
                        ),
