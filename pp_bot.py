...
def _trader_profile_missing(tp: dict) -> list[str]:
    """يرجع قائمة مفاتيح الحقول الناقصة (ثابت: 7 حقول)."""
    tp = tp or {}
    missing: list[str] = []
    for f in TRADER_PROFILE_REQUIRED_FIELDS:
        try:
            v = str(tp.get(f) or "").strip()
        except Exception:
            v = ""
        if not v:
            missing.append(f)
