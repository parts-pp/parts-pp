...
def _viewer_role_for_order(viewer_id: int, order: dict) -> str:
    ...
    # 2) Client (مالك الطلب)
    client_id = 0
    for k in ("user_id", "client_id", "owner_id", "customer_id"):
        try:
            v = int(o.get(k) or 0)
            if v:
                client_id = v
            ...
            continue
    if client_id and vid == client_id:
        return "client"

    # 3) Trader (التاجر المقبول/المسند)
    trader_id = 0
    for k in ("accepted_trader_id", "trader_id", "seller_id", "assigned_trader_id"):
        try:
            v = int(o.get(k) or 0)
            if v:
                trader_id = v
            ...
            continue
    if trader_id and vid == trader_id:
        return "trader"
