...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all((tp.get(f) or "").strip() for f in required_fields):
        await _need_complete_trader_profile_notice(context, user_id, _user_name(q), td.get("quote_order_id") or "")
        try:
    ...
    if action == "ppq_begin":
        ...
        td["quote_started_at_utc"] = utc_now_iso()  # ✅ جديد: وقت بدء بناء عرض السعر
