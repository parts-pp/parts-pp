...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, trader_id: int, order_id: str):
    ...
    try:
        is_admin = uid in (ADMIN_IDS or [])
    except Exception:
        is_admin = False

    if (not is_admin) and int(uid) != int(accepted_tid or 0):
        try:
            await context.bot.send_message(
                chat_id=uid,
    ...
    trader_store = ""
    if accepted_tid:
        trader_name = _trader_label(int(accepted_tid), "—")
        try:
            tp = get_trader_profile(int(accepted_tid)) or {}
        except Exception:
            tp = {}
    ...
    car_name = (str(order.get("car_name") or order.get("vehicle_name") or order.get("car") or order.get("car_model") or "—") or "—").strip() or "—"
    ost = str(order.get("order_status") or "").strip().lower()
    ...
    tot = _money(order.get("total_amount_sar") or order.get("total_amount") or "")
    ...
    if ga or sf or tot:
        ...
        if sf:
            lines.append(f"🚚 قيمة الشحن: <b>{html.escape(sf)}</b>")
        if tot:
    ...
    if is_admin:
        ...
        try:
            ...
            rows = list(tkb.inline_keyboard or [])
        except Exception:
            rows = []

        rows.append([InlineKeyboardButton("📜 سجل الطلب / آخر تحديث", callback_data=f"pp_order_legal|{order_id}")])

        has_close = any((getattr(btn, "callback_data", "") == "pp_ui_close") for r in rows for btn in (r or []))
        if not has_close:
            rows.append([InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")])
