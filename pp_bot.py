...
from telegram.ext import (
    ...
    ChatJoinRequestHandler,  # ✅ Join Requests (مجموعة التجار)
    filters,
)
...
def _join_portal_applicant_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 فتح البوت وإرسال المستندات", url=_join_portal_open_url())],
    ])
...
def _join_portal_admin_kb(applicant_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("👤 مراسلة المتقدم", url=f"tg://user?id={aid}"),
            InlineKeyboardButton("🔗 فتح البوت", url=_join_portal_open_url()),
        ],
        [
...
async def traders_join_request_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ضمان: لا نتعامل إلا مع مجموعة التجار المحددة
    try:
    ...
    full_name = (" ".join([u.first_name or "", u.last_name or ""])).strip()
    username = (getattr(u, "username", None) or "").strip()
    who = html.escape(full_name or (f"@{username}" if username else f"ID:{uid}"), quote=False)

    brand = _join_portal_brand()

    # ===== رسالة المتقدم (بالخاص إن أمكن) =====
    applicant_text = (
        f"{brand}\n\n"
        "هذه مجموعة خاصة بالتجار وموردي قطع الغيار فقط.\n\n"
        "لإكمال طلب الانضمام يرجى إرسال المستندات التالية عبر البوت:\n"
        "• السجل التجاري\n"
        "• الرقم الضريبي\n"
        "• رخصة/إثبات المتجر\n\n"
    ...
    admin_text = (
        ...
        + "\n"
        "📌 اطلب منه المستندات قبل قبول الطلب:\n"
        "• السجل التجاري\n"
        "• الرقم الضريبي\n"
        "• رخصة/إثبات المتجر\n\n"
...
async def join_portal_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # صفحة التقديم داخل البوت (خاص فقط)
    if not update.message:
        return
    user_id = update.effective_user.id
    set_stage(context, user_id, STAGE_JOIN_DOCS)

    brand = _join_portal_brand()
    msg = (
        f"{brand}\n\n"
        "📎 ارفع المستندات الآن (صور أو PDF) في نفس المحادثة، وسيتم إرسالها للإدارة تلقائياً.\n\n"
        "المطلوب:\n"
        "• السجل التجاري\n"
        "• الرقم الضريبي\n"
        "• رخصة/إثبات المتجر\n\n"
        "بعد الإرسال انتظر مراجعة الإدارة.\n\n"
        f"✅ التقديم والطلبات عبر @{PP_BOT_USERNAME} فقط"
    )

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔒 إنهاء", callback_data="pp_join_done")],
    ])
    try:
...
async def pp_join_done_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get(STAGE_KEY) == STAGE_JOIN_DOCS:
        # إنهاء بسيط بدون لمس التدفقات الأخرى
        try:
            ud.pop(STAGE_KEY, None)
        except Exception:
            pass
    try:
        await q.message.edit_text(f"{_join_portal_brand()}\n\n✅ تم إنهاء التقديم. إذا احتجت، افتح البوت وابدأ من جديد.", disable_web_page_preview=True)
    except Exception as e:
        _swallow(e)
...
async def pp_join_admin_action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if action == "pp_join_appr":
            ...
            txt = f"{_join_portal_brand()}\n\n✅ تم قبول طلب الانضمام.\n🆔 <code>{applicant_id}</code>"
        else:
            await context.bot.decline_chat_join_request(chat_id=int(TRADERS_GROUP_ID), user_id=applicant_id)
            txt = f"{_join_portal_brand()}\n\n⛔ تم رفض طلب الانضمام.\n🆔 <code>{applicant_id}</code>"
    except Exception as e:
        _swallow(e)
        txt = f"{_join_portal_brand()}\n\n⚠️ تعذر تنفيذ الإجراء.\n🆔 <code>{applicant_id}</code>"

    try:
        await q.message.edit_text(txt, parse_mode="HTML", disable_web_page_preview=True)
    except Exception as e:
        _swallow(e)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================
    # (0) join : بوابة التقديم للتجار (رفع المستندات)
    # =========================
    if args and isinstance(args[0], str) and (args[0] == "join" or args[0].startswith("join_")):
        try:
...
async def pay_bank_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================
    # ✅ بوابة التجار: استقبال المستندات وإرسالها للإدارة (خاص فقط)
    # =========================
    if stage == STAGE_JOIN_DOCS:
        msg = getattr(update, "message", None)
        if not msg:
            return

        # التقط وسائط (صورة/PDF/ملف) + كابتشن
        kind = None
        file_id = None
        try:
            ...
            pass
        try:
            if (not file_id) and getattr(msg, "document", None):
                kind = "document"
        ...
        header = (
            ...
            "📥 <b>مستندات تسجيل تاجر</b>\n"
            f"👤 الاسم: <b>{html.escape(full_name or '—', quote=False)}</b>\n"
            f"🆔 ID: <code>{int(user_id)}</code>\n"
            + (f"🔗 المستخدم: @{html.escape(username, quote=False)}\n" if username else "")
        )

        if caption:
            header += f"\n📝 ملاحظة:\n{html.escape(caption, quote=False)}\n"

        kb = _join_portal_admin_kb(int(user_id))
        ...
        for aid in (ADMIN_IDS or []):
            try:
                if kind == "photo" and file_id:
                    ...
                    # إذا لم تكن وسائط (صوت/فيديو) نرسل تنبيه فقط
                    await context.bot.send_message(
                        chat_id=int(aid),
                        text=header + "\n\n⚠️ تم استلام رسالة ليست صورة/PDF. اطلب منه إرسال المستندات كصور أو PDF.",
                        parse_mode="HTML",
                        reply_markup=kb,
                        disable_web_page_preview=True,
                    )
        ...
        try:
            await msg.reply_text(
                f"{_join_portal_brand()}\n\n✅ تم استلام الملف وإرساله للإدارة. يمكنك إرسال بقية المستندات بنفس الطريقة.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔒 إنهاء", callback_data="pp_join_done")]]),
                disable_web_page_preview=True,
            )
        except Exception as e:
            _swallow(e)
    ...
    if msg:
        ...
        if stage == STAGE_JOIN_DOCS:
            ...
            try:
                ...
                if not ch or str(getattr(ch, "type", "")).lower() != "private":
                    return
            except Exception:
                return
            ...
            # إذا ما كانت وسائط مدعومة (مثلا فيديو/صوت) نرسل تنبيه نصي للإدارة
            if not sent_any:
                try:
                    txt = (
                        f"{_join_portal_brand()}\n\n"
                        "⚠️ وصل محتوى غير مدعوم ضمن التقديم (ليس صورة/PDF).\n"
                        f"👤 الاسم: <b>{html.escape(full_name or '—', quote=False)}</b>\n"
                        f"🆔 ID: <code>{int(user_id)}</code>\n"
                        + (f"🔗 المستخدم: @{html.escape(username, quote=False)}\n" if username else "")
                        + "\n"
                        "اطلب من المتقدم إرسال المستندات كصور أو PDF."
                    )
                    await _notify_admins_private(context, txt, kb=kb)
                except Exception as e:
                    _swallow(e)
            ...
            try:
                await msg.reply_text(
                    f"{_join_portal_brand()}\n\n✅ تم استلام المستند وإرساله للإدارة بنجاح. يمكنك إرسال مستندات إضافية (صور أو PDF) أو كتابة ملاحظة.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔒 إنهاء", callback_data="pp_join_done")]]),
                    disable_web_page_preview=True,
                )
            except Exception as e:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================
    # ✅ بوابة التجار: رسائل نصية أثناء التقديم (تصل للإدارة كـ ملاحظة)
    # =========================
    ud = get_ud(context, user_id)
    stage = ud.get(STAGE_KEY)
    if stage == STAGE_JOIN_DOCS:
        ...
        header = (
            f"{_join_portal_brand()}\n\n"
            "📝 <b>ملاحظة من متقدم للتجار</b>\n"
            f"👤 الاسم: <b>{html.escape(full_name or '—', quote=False)}</b>\n"
            f"🆔 ID: <code>{int(user_id)}</code>\n"
            + (f"🔗 المستخدم: @{html.escape(username, quote=False)}\n" if username else "")
            + "\n"
            f"{html.escape(raw_text or '', quote=False)}\n\n"
            f"✅ التقديم والطلبات عبر @{PP_BOT_USERNAME} فقط"
        )
        kb = _join_portal_admin_kb(int(user_id))
        await _notify_admins_private(context, header, kb=kb)
        try:
            await update.message.reply_text(
                f"{_join_portal_brand()}\n\n✅ تم إرسال ملاحظتك للإدارة. إذا عندك مستندات ارسلها كصور أو PDF هنا.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔒 إنهاء", callback_data="pp_join_done")]]),
                disable_web_page_preview=True,
            )
        except Exception as e:
...
def build_app():
    ...
    # 🟢 [HANDLER] Traders Join Requests (لا إشعارات داخل مجموعة التجار)
    app.add_handler(ChatJoinRequestHandler(traders_join_request_cb))

    # 🟢 [HANDLER] UI / Cancel / Close
    # 🟢 [HANDLER] Join Portal (التقديم للتجار)
