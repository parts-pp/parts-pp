...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    """لوحة مراحل الطلب للتاجر (تسلسلية): نفتح زر المرحلة الحالية فقط ونقفل البقية بصريًا 🔒.
    ملاحظة: الضغط على زر مقفول يُعطي Popup داخل trader_status_cb.
    """
    try:
    ...
    ost = ost_map.get(ost, ost)

    gps = str(order.get("goods_payment_status") or "").strip().lower()
    gps_ok = gps in ("confirmed", "paid", "done", "success")

    inv_ok = bool(str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "").strip())

    def _btn(title: str, cb: str, enabled: bool) -> InlineKeyboardButton:
        return InlineKeyboardButton((title if enabled else f"🔒 {title}"), callback_data=cb)

    # تحديد المرحلة الحالية
    # - البداية بعد قبول العرض: نسمح بـ (جاري تجهيز)
    # - بعد تجهيز: نسمح بـ (جاهز)
    # - بعد جاهز: نسمح بـ (تم الشحن) (لكن لو ناقص فاتورة/سداد سيبقى مقفول)
    if ost in ("", "new", "accepted", "accepted_quote", "open", "processing"):
        can_prep = True
        can_ready = False
        can_ship = False
        can_deliver = False
        ...
        can_prep = False
        can_ready = False
        # الشحن يحتاج فاتورة + سداد مؤكد
        can_ship = bool(inv_ok and gps_ok)
        can_deliver = False
        can_chat = False
    elif ost == "shipped":
        can_prep = False
    ...
    return InlineKeyboardMarkup([
        [_btn("🟡 جاري تجهيز الطلب", f"pp_trader_status|prep|{order_id}", can_prep)],
        [_btn("🟢 الطلب جاهز للشحن", f"pp_trader_status|ready|{order_id}", can_ready)],
        [_btn("🚚 تم شحن الطلب", f"pp_trader_status|shipped|{order_id}", can_ship or (ost == "ready_to_ship"))],
        [_btn("✅ تم تسليم الطلب للعميل بنجاح", f"pp_trader_status|delivered|{order_id}", can_deliver)],
        [_btn("💬 مراسلة العميل داخل المنصة", f"pp_chat_open|{order_id}", can_chat)],
    ])
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if actor_id not in ADMIN_IDS and actor_id != accepted_tid:
        await _alert(q, f"🔒 هذا الزر غير متاح لك الآن\n🧾 رقم الطلب: {order_id}\nتم إسناد الطلب إلى: {accepted_name}")
        return
# ===== ✅ حساب الإجمالي الموحد (القطع + الشحن إذا غير مشمول) =====
    ...
    # ✅ حارس تسلسلي: Popup فقط (بدون رسائل نصية)
    async def _need_step_popup(required_btn: str, extra: str = "") -> bool:
        msg = f"لا يمكن تنفيذ هذه الخطوة الآن.\nابدأ أولاً بـ: {required_btn}"
        if extra:
            msg = f"{msg}\n{extra}"
        try:
    ...
    ost_now = _ost_map.get(ost_now, ost_now)


    
    # ✅ تسلسل الأزرار (المطلوب)
    # 1) 🟡 جاري تجهيز الطلب: مسموح كبداية بعد قبول العرض
    if new_status == "preparing":
        if ost_now not in ("", "new", "accepted", "accepted_quote", "open", "processing", "preparing"):
            if await _need_step_popup("🟡 جاري تجهيز الطلب"):
                return

    # 2) 🟢 جاهز للشحن: فقط بعد تجهيز
    if new_status == "ready_to_ship" and ost_now not in ("preparing", "ready_to_ship"):
        if await _need_step_popup("🟡 جاري تجهيز الطلب"):
            return

    # 3) 🚚 تم الشحن: فقط بعد جاهز للشحن (وبشرط الفاتورة + السداد)
    if new_status == "shipped":
        if ost_now != "ready_to_ship":
            if await _need_step_popup("🟢 الطلب جاهز للشحن"):
                return
        inv_ok = bool(str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "").strip())
        if not inv_ok:
            if await _need_step_popup("🧾 رفع فاتورة المتجر", "ارسل فاتورة المتجر بالخاص ثم أعد المحاولة."):
                return
        gps_now = str(order.get("goods_payment_status") or "").strip().lower()
        if gps_now not in ("confirmed", "paid", "done", "success"):
            if await _need_step_popup("💳 تأكيد سداد العميل", "لا يمكن اعتماد (تم الشحن) قبل تأكيد السداد."):
                return

    # 4) ✅ التسليم + 💬 مراسلة العميل: فقط بعد الشحن
    if new_status == "delivered" and ost_now != "shipped":
        if await _need_step_popup("🚚 تم شحن الطلب"):
            return
# بعد الشحن: نقفل (تجهيز/جاهز/تم الشحن) ونسمح فقط (تم التسليم + مراسلة)
    ...
    if new_status == "ready_to_ship":
        ...
        if not inv_file:
            ...
            await _alert(q, "🧾 قبل (🟢 الطلب جاهز للشحن) لازم ترفع فاتورة المتجر بالخاص (PDF/صورة).")
            return
