...
async def send_trader_ledger_pdf(
    ...
    # username
    tuser = _pick("username", "tg_username", "user_name", "telegram_username")
    ...
    # ✅ (6) ترويسة خارج الجدول
    header_top = Table([[
        P("منصة PPARTS احد الخدمات المساندة لنظام GO", tiny_r),
        P("P202126P@HOTMAIL.CPM", tiny_l),
    ]], colWidths=[full_w*0.70, full_w*0.30])
    ...
    # ✅ (1)(2) badges: الوقت/التاريخ بصيغة صحيحة + التاجر بدل رقم السجل
    trader_badge = (tname or str(tid)) + (f" {tuser}" if tuser else "")
    badges = Table([[
        P(f"الوقت: <b>{now.strftime('%H:%M')}</b>", tiny_c),
        P(f"التاريخ: <b>{now.strftime('%Y-%m-%d')}</b>", tiny_c),
        P(f"التاجر: <b>{trader_badge}</b>", tiny_c),
    ...
    def section_header(title: str):
        t = Table([[P(
            f"<b>{title}</b>",
            ParagraphStyle("sec", parent=right, textColor=colors.white, fontSize=9.8, leading=12.0, fontName=font_name)
        )]], colWidths=[full_w])
    ...
    def kv_table(rows: list[tuple[str, str]]):
        """
        ✅ (4) تقريب الأعمدة + محاذاة يمين للقيم
        ✅ (5) تلوين صفوف بالتناوب
        """
        data = []
        for k, v in rows:
            vtxt = v if (v is not None and str(v).strip() != "") else "—"
            # [القيمة, المعرّف] — والقيمة يمين عشان تكون قريبة
            data.append([P(str(vtxt), right), P(f"<b>{k}</b>", right)])

        # أعمدة أقرب + padding أقل
        ...
        for r in range(0, len(data)):
            ts.add("BACKGROUND", (0, r), (-1, r), colors.HexColor(ROW_TINT1 if r % 2 == 0 else ROW_TINT2))
    ...
    kv_table([
        # ✅ (2) لا نكرر TL بالأعلى، نخليه هنا فقط إذا تحبه كسجل داخلي
