...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            await q.message.reply_text(
                _hdr(order_id, snap=snap, goods_total=g) +
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    client_name = (
        (order.get("user_name") or order.get("client_name") or order.get("customer_name") or "").strip()
        or "العميل"
    )
    ...
    if new_status == "ready_to_ship":
        # ✅ السماح فقط إذا كان الطلب فعلاً في مرحلة (جاري تجهيز)
        # بعض الملفات القديمة قد تحفظ الحالة بالعربي، لذلك نتحقق بشكل مرن.
        is_preparing = ost_now in ("preparing", "prep") or ("تجهيز" in ost_now and "شحن" not in ost_now)
        if not is_preparing:
            return
        ...
        if not inv_file:
            ...
            # ✅ تنبيه فوري للتاجر حتى لو كان الإرسال اللاحق بطيئًا
            try:
                await _alert(q, "🧾 ارسل فاتورة المتجر الآن في الخاص")
            except Exception as e:
                _swallow(e)
