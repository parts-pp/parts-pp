...
async def order_finish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _alert(
        q,
        "🔒 زر إقفال الطلب\n"
        "⏳ متاح بعد مرور 7 أيام من الشحن أو التسليم.",
    )
...
async def _send_backup_excel(app: Application, reason: str = "scheduled"):
    ...
    # ✅ تأكد chat_id رقم (وتنظيف مسافات)
    try:
        chat_id = int(str(PP_BACKUP_CHAT_ID).strip())
    except Exception:
        if _should_throttle_notice("last_backup_warn_bad_chat_id_utc", 6 * 3600):
            await _notify_admins(app, f"⚠️ PP_BACKUP_CHAT_ID غير صالح: {PP_BACKUP_CHAT_ID}")
        return None
    ...
    # ✅ تحقق الحجم (أهم شيء لحالة 400 كثيرًا)
    try:
    ...
    if sz <= 0:
        if _should_throttle_notice("last_backup_warn_excel_empty_utc", 30 * 60):
            await _notify_admins(
                app,
                f"❌ فشل النسخ: ملف الإكسل فارغ/تالف.\nPATH: {path}\nSIZE: {sz}"
            )
        return None
    ...
    try:
        ...
        # ✅ اطبع سبب 400 الحقيقي للإدارة (بدون نفخ)
        if _should_throttle_notice("last_backup_warn_send_error_utc", 30 * 60):
            await _notify_admins(
                app,
                "❌ خطأ أثناء إرسال النسخة الاحتياطية لمجموعة النسخ:\n"
                f"{e}\n\n"
                f"CHAT_ID: {chat_id}\nPATH: {path}\nSIZE: {sz}"
            )
        return None
