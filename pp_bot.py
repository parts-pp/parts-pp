...
_pp_is_trader_enabled = is_trader_enabled
...
def _pp_data_xlsx_path() -> str:
    """مسار ملف البيانات الرئيسي (pp_data.xlsx)."""
    try:
        p = os.getenv("PP_DATA_XLSX") or os.getenv("PP_DATA_PATH") or ""
        p = str(p).strip()
        ...
        pass
    try:
        p2 = os.path.join(os.path.dirname(__file__), "pp_data.xlsx")
...
def is_trader_enabled(trader_id: int) -> bool:
    """✅ توحيد قراءة حالة التاجر (الافتراضي: مفعل إذا الخانة فاضية)."""
    tid = 0
    try:
    ...
    # 1) حاول القراءة من طبقة pp_excel أولاً
    try:
        v = _pp_is_trader_enabled(tid)
        # pp_excel قد يرجّع bool أو 0/1 أو نص
        if v is None or v == "":
            return True
        ...
        if s in (
            ...
            return False
        if s in ("1", "yes", "true", "on", "enabled", "enable", "مفعل", "فعال"):
            return True
        # أي قيمة غريبة نعتبرها مفعل (أفضل من حظر تاجر بالغلط)
        return True
    except Exception:
        pass

    # 2) fallback: قراءة مباشرة من الإكسل
    try:
        path = _pp_data_xlsx_path()
        wb = openpyxl.load_workbook(path)
        ws = wb["traders"] if "traders" in wb.sheetnames else None
        if not ws:
            return True
        headers = [str(c.value or "").strip().lower() for c in ws[1]]
        if "trader_id" not in headers or "is_enabled" not in headers:
            return True
        col_id = headers.index("trader_id") + 1
        col_en = headers.index("is_enabled") + 1
        for r in range(2, ws.max_row + 1):
            v_id = ws.cell(r, col_id).value
            try:
                if int(v_id or 0) != tid:
                    continue
            except Exception:
                continue
            v = ws.cell(r, col_en).value
            if v is None or v == "":
                return True
            if isinstance(v, bool):
                return bool(v)
...
def _xlsx_set_trader_enabled_fallback(tid: int, enable: bool) -> bool:
    """كتابة مباشرة للإكسل (حل أخير) لضمان أن زر التفعيل يغيّر قيمة is_enabled فعلياً."""
    try:
        path = _pp_data_xlsx_path()
        wb = openpyxl.load_workbook(path)
        if "traders" not in wb.sheetnames:
            return False
        ws = wb["traders"]
        headers = [str(c.value or "").strip().lower() for c in ws[1]]
        if "trader_id" not in headers:
            return False
        col_id = headers.index("trader_id") + 1
        if "is_enabled" not in headers:
            # أنشئ العمود إذا غير موجود
            ws.cell(1, ws.max_column + 1).value = "is_enabled"
            headers.append("is_enabled")
        col_en = headers.index("is_enabled") + 1

        # updated_at_utc
        col_upd = None
        if "updated_at_utc" in headers:
            col_upd = headers.index("updated_at_utc") + 1

        found_row = None
        for r in range(2, ws.max_row + 1):
            v_id = ws.cell(r, col_id).value
            try:
                if int(v_id or 0) == int(tid):
                    found_row = r
                    break
            except Exception:
                continue
        ...
        if found_row is None:
            ...
            ws.cell(found_row, col_id).value = int(tid)

        ws.cell(found_row, col_en).value = 1 if enable else 0
        if col_upd:
            ws.cell(found_row, col_upd).value = utc_now_iso()
...
def set_trader_enabled(trader_id: int, enable: bool):
    """✅ تحديث حالة التاجر بشكل مضمون (حتى لو pp_excel لا يكتب القيمة كما نتوقع)."""
    tid = 0
    try:
        tid = int(trader_id or 0)
    except Exception:
    ...
    with _EXCEL_WRITE_LOCK:
        ...
        ok = True
        try:
            # بعض تطبيقات pp_excel تتعامل أفضل مع 0/1 بدل bool
            _pp_set_trader_enabled(tid, 1 if en else 0)
        except Exception:
            ok = False

        # 2) تحقق سريع — إذا ما تغيّرت الحالة فعلياً، نفّذ fallback كتابة مباشرة
        try:
            now = is_trader_enabled(tid)
            if now != en:
                _xlsx_set_trader_enabled_fallback(tid, en)
        except Exception:
            if not ok:
                _xlsx_set_trader_enabled_fallback(tid, en)
...
TRADERS_GROUP_ID = (
    int(TRADERS_GROUP_ID_RAW) if TRADERS_GROUP_ID_RAW.lstrip("-").isdigit() else None
)
...
PP_BACKUP_CHAT_ID = (
    int(BACKUP_CHAT_ID_RAW) if BACKUP_CHAT_ID_RAW.lstrip("-").isdigit() else None
)

# ✅ حد أدنى بين نسختين (لمنع التكرار/السبام)
PP_BACKUP_MIN_SECONDS = int(
    (os.getenv("PP_BACKUP_MIN_SECONDS") or "600").strip() or "600"
)
...
async def _is_trader_group_member(
    ...
    try:
        m = await context.bot.get_chat_member(
            chat_id=TRADERS_GROUP_ID, user_id=int(user_id)
        )
...
def _q_is_sent(s: dict) -> bool:
    try:
        return str((s or {}).get("sent") or "").strip().lower() in (
            "1",
            "yes",
            "true",
            "sent",
...
async def _notify_invoice_error(context, order_id: str, stage: str, err: Exception):
    ...
    try:
        log.exception(
            "🧾 فشل الفاتورة | الطلب=%s | المرحلة=%s | الخطأ=%s", order_id, stage, err
        )
    except Exception:
        pass
        ...
        await bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "📩 اتصل بالمنصة", callback_data="pp_support_open"
                        )
                    ],
                ]
            ),
...
def _order_id_link_html(
    order_id: str, context: "ContextTypes.DEFAULT_TYPE" = None
) -> str:
...
def _support_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("🔒 إنهاء", callback_data="pp_support_close")]]
    )
...
def _support_should_close_by_time(ud: dict) -> bool:
    ...
    try:
        ...
        started = datetime.fromisoformat(
            (ud.get("support_started_at_utc") or "").replace("Z", "+00:00")
        )
        last = datetime.fromisoformat(
            (
                ud.get("support_last_at_utc") or ud.get("support_started_at_utc") or ""
            ).replace("Z", "+00:00")
        )
...
async def _support_close(
    update_or_q, context: ContextTypes.DEFAULT_TYPE, user_id: int, reason: str = ""
):
...
def _build_admin_order_caption(
    order_id: str, ud: dict, order: dict, title: str, extra_lines=None
) -> str:
    ...
    delivery_details = (
        ud.get("delivery_details") or order.get("delivery_details") or ""
    ).strip()
...
def prepay_notes_kb():
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⏭️ تخطي والانتقال للشحن", callback_data="pp_prepay_notes_skip"
                )
            ],
            [InlineKeyboardButton("✖️ الغاء الطلب", callback_data="pp_cancel")],
        ]
...
def build_order_preview(ud: dict) -> str:
    ...
    for i, it in enumerate(items, start=1):
        ...
        if nm:
            if pn:
                lines.append(
                    f"  🔹 <b>{i}</b>- <i>{nm}</i> <b>رقم</b>: <code>{pn}</code>"
                )
            else:
...
def trader_quote_start_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("🚀 بدء بناء عرض السعر"),
                    callback_data=f"ppq_begin|{order_id}",
                )
            ],
        ]
    )
...
def trader_quote_type_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("✔️ وكالة"),
                    callback_data=f"ppq_type|{order_id}|agency",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✔️ وكلاء محليين"),
                    callback_data=f"ppq_type|{order_id}|local_dealers",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✔️ تجاري"),
                    callback_data=f"ppq_type|{order_id}|aftermarket",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✔️ مختلط"),
                    callback_data=f"ppq_type|{order_id}|mixed",
                )
            ],
        ]
    )
...
def trader_quote_shipping_method_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("🚚 محلي"),
                    callback_data=f"ppq_ship|{order_id}|local",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✈️ دولي"), callback_data=f"ppq_ship|{order_id}|intl"
                )
            ],
        ]
    )
...
def trader_quote_shipping_included_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("✅ السعر يشمل الشحن"),
                    callback_data=f"ppq_shipinc|{order_id}|yes",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("❌ الشحن غير مشمول"),
                    callback_data=f"ppq_shipinc|{order_id}|no",
                )
            ],
        ]
    )
...
def trader_quote_eta_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("⏱ 1-2 يوم"),
                    callback_data=f"ppq_eta|{order_id}|1-2",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("⏱ 3-5 ايام"),
                    callback_data=f"ppq_eta|{order_id}|3-5",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("⏱ 7-14 يوم"),
                    callback_data=f"ppq_eta|{order_id}|7-14",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✍️ مدة اخرى"),
                    callback_data=f"ppq_eta|{order_id}|custom",
                )
            ],
        ]
    )
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("⏱ 1-2 يوم"),
                    callback_data=f"ppq_avail|{order_id}|1-2",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("⏱ 3-5 ايام"),
                    callback_data=f"ppq_avail|{order_id}|3-5",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("⏱ 7-14 يوم"),
                    callback_data=f"ppq_avail|{order_id}|7-14",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✍️ مدة اخرى"),
                    callback_data=f"ppq_avail|{order_id}|custom",
                )
            ],
        ]
    )
...
def trader_quote_preview_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("✅ إرسال العرض للعميل"),
                    callback_data=f"ppq_preview_send|{order_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("🔁 إعادة بناء العرض"),
                    callback_data=f"ppq_preview_restart|{order_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✖️ إلغاء"),
                    callback_data=f"ppq_preview_cancel|{order_id}",
                )
            ],
        ]
    )
...
def trader_quote_preview_kb_locked(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    _wide_btn_label("✅ تم إرسال العرض للعميل"),
                    callback_data="pp_ui_locked",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("🆕 إنشاء عرض جديد (إصدار جديد)"),
                    callback_data=f"ppq_new_version|{order_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    _wide_btn_label("✖️ إغلاق"), callback_data="pp_ui_close"
                )
            ],
        ]
    )
...
def build_legal_shipping_block(
    method: str, fee_sar: str, eta: str, included: str
) -> str:
    ...
    inc_txt = (
        "مشمولة"
        if included_norm in ("yes", "true", "1", "included", "مشمولة", "مشمول")
        else "غير مشمولة"
    )
...
def build_official_quote_text(
    ...
    total_txt = (
        _money(total_val) if (ship_included or _s(shipping_fee_sar)) else "غير محدد"
    )
...
def quote_client_kb(order_id: str, trader_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ أقبل العرض", callback_data=f"pp_quote_ok|{order_id}|{tid}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "❌ أرفض العرض", callback_data=f"pp_quote_no|{order_id}|{tid}"
                ),
            ],
        ]
    )
...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    try:
        ...
        inv_file = (
            str(
                order.get("seller_invoice_file_id")
                or order.get("shop_invoice_file_id")
                or ""
            )
        ).strip()
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🟡 جاري تجهيز الطلب",
                        callback_data=f"pp_trader_status|prep|{order_id}",
                    )
                ],
            ]
        )
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🟢 الطلب جاهز للشحن",
                        callback_data=f"pp_trader_status|ready|{order_id}",
                    )
                ],
            ]
        )
        ...
        if not goods_paid:
            return InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "⏳ بانتظار سداد العميل لقيمة البضاعة",
                            callback_data="pp_ui_close",
                        )
                    ],
                ]
            )
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🚚 تم شحن الطلب",
                        callback_data=f"pp_trader_status|shipped|{order_id}",
                    )
                ],
            ]
        )
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "✅ تأكيد التسليم بنجاح",
                        callback_data=f"pp_trader_status|delivered|{order_id}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        "🔒 إنهاء / إقفال الطلب (منجز)",
                        callback_data=f"pp_order_finish|{order_id}",
                    )
                ],
            ]
        )
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "📌 مراجعة حالة الطلب", callback_data=f"pp_order_review|{order_id}"
                )
            ],
        ]
    )
        ...
        [
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"pp_chat_open|{order_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ إشعار الإدارة: تم التسليم بنجاح",
                    callback_data=f"pp_trader_status|delivered|{order_id}",
                )
            ],
            [
...
def pay_goods_method_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    ✅ مسموح للعميل مراسلة التاجر من هنا (هذا لا يخالف شرطنا).
    """
    rows = [
        [
            InlineKeyboardButton(
                "🏦 تحويل بنكي", callback_data=f"pp_goods_pay_bank|{order_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "📱 STC Pay", callback_data=f"pp_goods_pay_stc|{order_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "🔗 رابط الدفع", callback_data=f"pp_goods_pay_link|{order_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "💬 مراسلة التاجر", callback_data=f"pp_chat_trader|{order_id}"
            )
        ],
    ]
    return InlineKeyboardMarkup(rows)
        ...
        [
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"pp_team_goods_confirm|{order_id}",
                )
            ]
        ]
    )
...
def admin_free_order_kb(order_id: str, client_id: int) -> InlineKeyboardMarkup:
    ...
    if oid and uid:
        rows.append(
            [
                InlineKeyboardButton(
                    "💬 مراسلة العميل", callback_data=f"pp_admin_reply|{oid}|{uid}"
                )
            ]
        )
    ...
    if oid:
        rows.append(
            [
                InlineKeyboardButton(
                    "⛔ إلغاء الطلب", callback_data=f"pp_admin_cancel|{oid}"
                )
            ]
        )
...
def delivery_kb():
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "📍 استلام من الموقع", callback_data="pp_delivery_pickup"
                )
            ],
        ]
    )
...
def track_kb(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🔎 مراسلة المنصة", callback_data=f"pp_track|{order_id}"
                )
            ],
        ]
    )
...
def admin_reply_done_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ انهاء الرد", callback_data="pp_admin_reply_done"
                )
            ],
        ]
    )
...
def client_preview_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ تأكيد ومتابعة", callback_data="pp_client_confirm_preview"
                )
            ],
            [
                InlineKeyboardButton(
                    "↩️ رجوع لتعديل العنوان", callback_data="pp_back|delivery"
                )
            ],
            [InlineKeyboardButton("✖️ إلغاء", callback_data="pp_cancel")],
        ]
...
async def client_confirm_preview_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mode = str(
        ud.get("preview_mode") or ""
    ).strip()  # "free_ship" / "free_pickup" / "pay_ship" / "pay_pickup"
    ...
    if mode.startswith("pay_"):
        ...
        try:
            ...
            await q.message.reply_text(
                "⬇️ اختر طريقة دفع رسوم المنصة", reply_markup=pay_method_kb()
            )
        return
    ...
    try:
        await send_platform_invoice_pdf(
            context, order_id, kind="preliminary", admin_only=False
        )
    except Exception as e:
        _swallow(e)
...
async def send_invoice_pdf(
    ...
    try:
        ...
        await _notify_invoice_error(
            context, order_id, "استيراد مكتبات PDF (reportlab)", e
        )
        return
    ...
    try:
        ...
        await _notify_invoice_error(
            context, order_id, "قراءة بيانات الطلب من الإكسل", e
        )
        return
    ...
    def _load_item_prices_map() -> dict:
        """
        يحاول قراءة أسعار القطع من:
        - حقول داخل item نفسه (price_sar / item_price / price ... )
        - أو خريطة محفوظة في order (dict أو JSON أو نص dict بايثون)
        المفاتيح المتوقعة: "1","2"... (حسب ترتيب البنود)
        """
        pm = {}
        try:
            import ast
    ...
    def _pick_item_price(i: int, it: dict, pm: dict) -> str:
        ...
        # 2) من خريطة الأسعار (حسب رقم البند)
        vs = _s(pm.get(str(i)))
        if vs and vs not in ("0", "0.0", "0.00"):
            return vs
    ...
    # ---------------- IDs / dates ----------------
    client_id = (
        int(order.get("user_id") or 0) if _s(order.get("user_id")).isdigit() else 0
    )
    trader_id = (
        int(order.get("accepted_trader_id") or 0)
        if _s(order.get("accepted_trader_id")).isdigit()
        else 0
    )
    ...
    if invoice_for_norm == "trader":
        sent_flag_field = (
            ...
            if kind_norm == "preliminary"
            else "invoice_trader_ship_pdf_sent"
        )
        ...
        sent_flag_field = (
            ...
            if kind_norm == "preliminary"
            else "invoice_platform_ship_pdf_sent"
        )
    ...
    if (_is_yes(order.get(sent_flag_field)) or _is_yes(order.get(legacy_flag))) and (
        not debug
    ):
        return
    ...
    trader_name = _s(
        order.get("accepted_trader_name") or order.get("quoted_trader_name")
    )
    ...
    ship_method = (
        _s(
            ...
            or order.get("ship_method")
            or order.get("delivery_choice")
        )
        or "—"
    )
    ...
    delivery_blob = _s(
        order.get("delivery_details")
        or order.get("address_text")
        or order.get("full_address")
    ...
    # ✅ رسوم الشحن: قيمة يحددها التاجر فقط (بدون أي رقم افتراضي)
    ship_included = str(order.get("ship_included") or "").strip().lower()
    raw_shipping_fee = order.get("shipping_fee_sar")

    # إذا غير موجودة القيمة
    if raw_shipping_fee is None or str(raw_shipping_fee).strip() == "":
        # مشمول = 0 ، غير ذلك = بدون قيمة (يحددها التاجر)
        raw_shipping_fee = (
            0
            if ship_included in ("yes", "true", "1", "included", "مشمولة", "مشمول")
            else ""
        )
    ...
    if invoice_for_norm == "trader":
        pay_method = _s(order.get("goods_payment_method")) or _s(
            order.get("payment_method")
        )
        pay_status_raw = _s(order.get("goods_payment_status")) or _s(
        ...
        _ = _money_safe(
            gt_val, fb=goods_amount if goods_amount != "0" else "0"
        )  # لا نغيّر المنطق

        inv_title = "فاتورة تاجر - داخلية - قطع + شحن"
    else:
        pay_method = _s(order.get("payment_method")) or _s(
            order.get("goods_payment_method")
        )
        pay_status_raw = _s(order.get("payment_status")) or _s(
            order.get("goods_payment_status")
        )
        pay_status = _pay_status_ar(pay_status_raw)
    ...
    if kind_norm == "shipping":
        inv_title = (
            "فاتورة شحن" if invoice_for_norm == "trader" else "فاتورة شحن - منصة"
        )
    ...
    pdf_path = os.path.join(
        tmpdir, f"pp_invoice_{order_id}_{kind_norm}_{uuid.uuid4().hex[:6]}.pdf"
    )
    ...
    if invoice_for_norm == "trader":
        ...
        STAMP = colors.HexColor("#DC2626")  # ✅ الختم: أحمر
        ROW_TINT1 = "#ECFDF5"
        ROW_TINT2 = "#E6FFFA"
    else:
    ...
    def _with_alpha(c, a: float):
        try:
            return colors.Color(
                c.red, c.green, c.blue, alpha=max(0.0, min(1.0, float(a)))
            )
        except Exception:
            return c
    ...
    header_tbl = Table(
        [[logo_cell if logo_cell else P("PPARTS", center)]], colWidths=[full_w]
    )
    header_tbl.setStyle(
        TableStyle(
            [
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 0),
                        (-1, -1),
                        [_hexA("#FFFFFF", 0.00), _hexA("#FFFFFF", 0.00)],
                    ),
                ]
            )
        )
    ...
    pmethod = (
        _s(order.get("goods_payment_method")) or _s(order.get("payment_method")) or ""
    )
    if pmethod:
    ...
    if kind_norm == "shipping":
        rows_ship.append(
            ("رقم التتبع", (tracking_number or order.get("shipping_tracking") or "—"))
        )
    ...
    # ✅ (تعديل مطلوب): عمود سعر القطعة + إظهار كل القطع في فاتورة التاجر
    ...
    if invoice_for_norm == "trader":
        parts_rows = [
            [
                P(
                    "<b>سعر القطعة</b>",
                    ParagraphStyle("ph0", parent=parts_cell_r, textColor=colors.white),
                ),
                P(
                    "<b>رقم القطعة</b>",
                    ParagraphStyle("ph1", parent=parts_cell_r, textColor=colors.white),
                ),
                P(
                    "<b>اسم القطعة</b>",
                    ParagraphStyle("ph2", parent=parts_cell_r, textColor=colors.white),
                ),
                P(
                    "<b>#</b>",
                    ParagraphStyle("ph3", parent=parts_cell_r, textColor=colors.white),
                ),
            ]
        ]
        ...
        if items:
            for i, it in enumerate(items, start=1):
                ...
                # ✅ إذا ما له سعر نكتب غير متوفرة (بدون حذف البند)
                if pr:
        ...
        # ✅ أقرب/أضيق: السعر أقرب لرقم القطعة واسم القطعة
        col_w = [0.20 * full_w, 0.24 * full_w, 0.48 * full_w, 0.08 * full_w]
    else:
        # (بدون أي تغيير) — فاتورة المنصة كما هي
        parts_rows = [
            [
                P(
                    "<b>رقم القطعة</b>",
    ...
    row_h = 0.62 * cm
    parts_tbl = Table(
        parts_rows, colWidths=col_w, rowHeights=[row_h] * len(parts_rows), repeatRows=1
    )
    parts_tbl.setStyle(
        TableStyle(
            [
                ...
                ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                # ✅ لصق البيانات بخط العمود (أقرب/أجمل)
                ("LEFTPADDING", (0, 0), (-1, -1), 1.2),
                ("RIGHTPADDING", (0, 0), (-1, -1), 1.2),
                ("TOPPADDING", (0, 0), (-1, -1), 1),
                ("BOTTOMPADDING", (0, 0), (-1, -1), 1),
                # رقم # (آخر عمود) تضييق أكثر
                ...
                (
                    ...
                    (0, 1),
                    (-1, -1),
                    [_hexA(ROW_TINT1, 0.18), _hexA(ROW_TINT2, 0.12)],
                ),
            ]
        )
    )
        ...
        if draw_stamp:
            ...
            try:
                ...
                canvas.drawCentredString(
                    stamp_cx, stamp_cy + 0.04 * cm, _ar("منصة قطع الغيار PP")
                )
                ...
                canvas.drawCentredString(
                    stamp_cx, stamp_cy - 0.34 * cm, _ar("الخدمات المساندة GO")
                )
            except Exception as e:
                _swallow(e)
    ...
    for cid in targets:
        try:
            ...
            try:
                log_event(
                    "تم إرسال فاتورة PDF بنجاح", order_id=order_id, target_chat_id=cid
                )
            except Exception as e:
                _swallow(e)

        except Exception as e:
            # ✅ التقط رسالة Telegram الدقيقة لو كانت موجودة
            emsg = getattr(e, "message", None) or str(e)
            failed.append((cid, emsg))
            try:
                log_event(
                    "فشل إرسال فاتورة PDF",
                    order_id=order_id,
                    target_chat_id=cid,
                    error=emsg,
                )
...
def client_trader_chat_done_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ انهاء المراسلة", callback_data="pp_chat_trader_done"
                )
            ],
        ]
    )
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "💬 رد على العميل",
                    callback_data=f"pp_trader_reply|{order_id}|{user_id}",
                )
            ],
        ]
    )
...
def trader_reply_done_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ انهاء الرد", callback_data="pp_trader_reply_done"
                )
            ],
        ]
    )
...
def team_group_kb(
    order_id: str, bot_username: str | None = None
) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "💰 تقديم عرض سعر ➜", callback_data=f"pp_team_quote|{order_id}"
                )
            ],
        ]
    )
...
def _user_name(update_or_q) -> str:
    try:
        u = (
            update_or_q.effective_user
            if hasattr(update_or_q, "effective_user")
            else update_or_q.from_user
        )
...
def _order_parties(order_id: str) -> tuple[str, str]:
    ...
    # trader
    trader_name = str(
        o.get("accepted_trader_name") or o.get("quoted_trader_name") or ""
    ).strip()
    if not trader_name:
        try:
        ...
        if tid:
            try:
                ...
                trader_name = (tp.get("display_name") or "").strip() or (
                    tp.get("company_name") or ""
                ).strip()
            except Exception:
                trader_name = ""
    trader_name = trader_name or "—"
...
async def send_trader_subscription_invoice_pdf(
    ...
    c.drawRightString(
        w - 20 * mm, h - 100 * mm, _ar(f"المرجع: SUB-{trader_id}-{month}")
    )
    ...
    try:
        with open(path, "rb") as f:
            await context.bot.send_document(
                chat_id=int(trader_id), document=f, caption=caption
            )
    except Exception as e:
        _swallow(e)
...
async def _send_client_payment_preview(
    ...
    try:
        trader_id0 = int(
            order.get("accepted_trader_id") or order.get("quoted_trader_id") or 0
        )
    except Exception:
        trader_id0 = 0
    ...
    if missing_ship and ship_included_norm not in (
        "yes",
        "true",
        "1",
        "included",
    ...
    delivery_block = ""
    dd = (
        _sanitize_delivery_details(delivery_details, hide_phone=True)
        if delivery_details
        else ""
    )
    if ship_method or dd:
        ...
        if availability_txt:
            delivery_block += (
                f"• 🛠 مدة التجهيز: <b>{html.escape(availability_txt)}</b>\n"
            )
        if dd:
    ...
    if pay_scope == "goods" and isinstance(items, list) and items:
        total_items = len(
            [
                x
                for x in items
                if isinstance(x, dict) and _s(x.get("name") or x.get("item_name"))
            ]
        )
        parts_summary = (
            "━━━━━━━━━━━━\n"
    ...
    msg = (
        ...
        + (
            ...
            if pay_scope == "goods"
            else f"💰 <b>رسوم المنصة</b>: <b>{amt_txt}</b>\n"
        )
        + ship_line
        ...
        f"{parts_txt}\n"
        f"\n🟦 <b>ماذا الآن؟</b>\n"
        f"• {html.escape(next_line)}" + delivery_block
    )

    kb_client = (
        track_kb(order_id) if pay_scope != "goods" else client_trader_chat_kb(order_id)
    )
...
def _delivery_brief(order: dict, ud: dict) -> str:
    ship_method = (str(order.get("ship_method") or "")).strip() or (
        str(ud.get("ship_method") or "")
    ).strip()
    ship_city = (str(order.get("ship_city") or "")).strip() or (
        str(ud.get("ship_city") or "")
    ).strip()
    pickup_city = (str(order.get("pickup_city") or "")).strip() or (
        str(ud.get("pickup_city") or "")
    ).strip()
    pickup_loc = (str(order.get("pickup_location") or "")).strip() or (
        str(ud.get("pickup_location") or "")
    ).strip()

    d_choice = (
        (str(order.get("delivery_choice") or ud.get("delivery_choice") or ""))
        .strip()
        .lower()
    )
    d_details = (
        str(order.get("delivery_details") or ud.get("delivery_details") or "")
    ).strip()
...
async def alert_nice(
    q, body: str, role: str = "user", force: bool = False, name: str = ""
):
    """
...
def _is_trader_profile_strict(context: ContextTypes.DEFAULT_TYPE) -> bool:
    try:
        v = str(get_setting("trader_profile_strict") or "").strip().lower()
    except Exception:
        v = ""
    return v in ("1", "yes", "true", "on", "enabled", "enable")


def _trader_profile_required_fields(strict: bool = False):
    # ✅ الحد الأدنى المطلوب لتقديم عرض (الآن)
    base = [
    ...
    # ✅ وضع صارم (سياسة قانونية/توثيق) — حقول مستقبلية نجعلها إلزامية عند التفعيل
    extra = [
...
def _trader_profile_missing(tp: dict, strict: bool = False):
    missing = []
    for k, label in _trader_profile_required_fields(strict):
        try:
            v = (tp.get(k) or "").strip()
        except Exception:
            v = ""
...
async def _need_complete_trader_profile_notice(
    context: ContextTypes.DEFAULT_TYPE,
    user_id: int,
    name: str,
    order_id: str = "",
    ...
    if strict is None:
        strict = await _is_trader_profile_strict(context)
    ...
    if missing is None:
        try:
            ...
            tp = {}
        missing = _trader_profile_missing(tp, strict=bool(strict))

    miss_lines = ""
    if missing:
        miss_lines = "\n".join([f"• {label}" for _, label in missing])
        miss_lines = f"\n\n📌 <b>الحقول الناقصة:</b>\n{html.escape(miss_lines)}"

    if strict:
        head = (
            f"{name}\n"
            "⛔ <b>لا يمكنك تقديم/إرسال عرض سعر</b> قبل إكمال <b>جميع</b> حقول ملف التاجر.\n"
            "⚖️ <b>سياسة المنصة القانونية</b>: توثيق بيانات التاجر إلزامي قبل التعاملات."
        )
    else:
        head = (
            f"{name}\n"
            "⛔ لا يمكنك تقديم عرض سعر قبل إكمال ملف التاجر وطرق الدفع.\n"
            "✅ المطلوب فقط لتقديم عرض:"
        )

    if not strict:
        steps = (
            "\n\n"
            "1) افتح لوحة التاجر من الزر\n"
            "2) أدخل: اسم المتجر\n"
            "3) أدخل: اسم البنك\n"
            "4) أدخل: رقم IBAN\n"
            "5) أدخل: رقم STC Pay\n"
            "\nثم ارجع واضغط (تقديم عرض سعر)."
        )
    else:
        steps = "\n\nثم ارجع وأعد المحاولة بعد إكمال الحقول."

    msg = head + miss_lines + steps + extra

    kb = InlineKeyboardMarkup(
        [[InlineKeyboardButton("🧑‍💼 فتح لوحة التاجر", url=panel_url)]]
    )
    try:
        await context.bot.send_message(
            chat_id=int(user_id),
    ...
    try:
        ...
        if isinstance(update, Update) and update.callback_query:
            await update.callback_query.answer(
                "⚠️ حدث خطأ غير متوقع\nيرجى المحاولة مرة أخرى", show_alert=True
            )
            return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and isinstance(args[0], str) and args[0].startswith("ppq_"):
        ...
        try:
            ...
            # طريقة الشحن + المدينة
            ship_method = (
                order.get("quote_ship_method")
                or td.get("quote_ship_method")
                or order.get("ship_method")
                or order.get("shipping_method")
                or ""
            ).strip()
            delivery_details = (
                order.get("delivery_details") or order.get("address") or ""
            ).strip()
    ...
    if args and isinstance(args[0], str) and args[0].startswith("ppopen_"):
        ...
        accepted_name = (
            order.get("accepted_trader_name") or order.get("quoted_trader_name") or ""
        ).strip()
        ...
        try:
            if acc and actor_id == acc:
                notified = (
                    str(order.get("accepted_trader_notified") or "").strip().lower()
                    == "yes"
                )
                if not notified:
                    await context.bot.send_message(
                    ...
                    try:
                        update_order_fields(
                            order_id, {"accepted_trader_notified": "yes"}
                        )
                    except Exception as e:
                        _swallow(e)
        except Exception as e:
...
async def start_trader_deeplink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    has_company = bool((tp.get("company_name") or "").strip())
    has_iban = bool((tp.get("iban") or "").strip())
    has_stc = bool((tp.get("stc_pay") or "").strip())

    if tp and has_company and has_iban and has_stc:
        try:
            await update.message.reply_text(
                f"{name}\n✅ بياناتك مسجلة مسبقًا. يمكنك فتح لوحة التاجر الآن."
            )
        except Exception as e:
            _swallow(e)
    else:
        try:
            await update.message.reply_text(
                f"{name}\n⚠️ أكمل بياناتك لبدء تقديم العروض (اسم المتجر / IBAN / STC Pay)."
            )
        except Exception as e:
            _swallow(e)
    ...
    try:
        if _trader_is_disabled(int(user_id or 0)):
            ...
            try:
                ...
                tp = {}
            tname = (
                (tp.get("display_name") or "").strip()
                or (tp.get("company_name") or "").strip()
                or _user_name(q)
            )
        ...
        await q.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "✅ إنهاء", callback_data="pp_support_admin_done"
                        )
                    ]
                ]
            ),
    ...
    try:
        ...
        if _support_is_open(ud0):
            await _support_close(
                ...
                context,
                user_id,
                reason="(تم إغلاقها لأنك بدأت طلباً جديداً)",
            )
    except Exception as e:
        _swallow(e)
...
async def cancel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reset_flow(context, user_id)
    await q.message.reply_text(
        "تم الغاء العملية\للبداء بطلب قطع غيار  ارسل كلمة pp فقط"
    )
...
async def more_no_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await q.message.reply_text(
            build_order_preview(ud), parse_mode="HTML", disable_web_page_preview=True
        )
    except Exception as e:
        _swallow(e)
...
async def pay_bank_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _save_order_once(ud)
    update_order_fields(
        ud["order_id"], {"payment_method": "bank_transfer", "payment_status": "pending"}
    )
...
async def pay_stc_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _save_order_once(ud)
    update_order_fields(
        ud["order_id"], {"payment_method": "stc_pay", "payment_status": "pending"}
    )
...
async def pay_link_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cap = _build_admin_order_caption(
            ...
            extra_lines=["المطلوب: اضغط الزر ثم الصق رابط الدفع ليتم إرساله للعميل"],
        )
...
async def quote_ok_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        gps in ("awaiting_confirm", "confirmed")
        or locked_now
        or ost in ("closed", "delivered")
    ):
        await q.message.reply_text(
    ...
    if prev_tid:
        try:
            ...
            pn = (
                (pp.get("display_name") or "").strip()
                or (order.get("accepted_trader_name") or "").strip()
                or "التاجر"
            )
...
def trader_quote_items_kb(
    order_id: str, items: list[dict], selected: set[int]
) -> InlineKeyboardMarkup:
    rows = []
    ...
    for i, it in enumerate(items or [], start=1):
        ...
        rows.append(
            [InlineKeyboardButton(label, callback_data=f"ppq_it|{order_id}|{i}")]
        )

    rows.append(
        [
            InlineKeyboardButton(
                _wide_btn_label("☑️ تحديد الكل"), callback_data=f"ppq_it_all|{order_id}"
            )
        ]
    )
    rows.append(
        [
            InlineKeyboardButton(
                _wide_btn_label("🧹 مسح الكل"), callback_data=f"ppq_it_none|{order_id}"
            )
        ]
    )
    rows.append(
        [
            InlineKeyboardButton(
                _wide_btn_label("✅ التالي"), callback_data=f"ppq_it_done|{order_id}"
            )
        ]
    )
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _trader_is_disabled(user_id):
        await _deny_disabled_trader_q(
            q, "لا يمكنك تقديم أو تعديل عروض السعر لأن حسابك موقوف"
        )
        return

    # ✅ لا يبدأ/يكمل عرض سعر إلا بعد اكتمال ملف التاجر
    # 🛡️ strict لازم يكون مُعرّف دائمًا (حتى لو فشل تحديد الوضع الصارم)
    strict = False
    try:
        _tmp = _is_trader_profile_strict(context)
        strict = await _tmp if asyncio.iscoroutine(_tmp) else _tmp
    except Exception as e:
        _swallow(e, "ppq_strict")
        strict = False
    ...
    missing = _trader_profile_missing(tp, strict=bool(strict))
    if missing:
        # رسالة توجيه كاملة + زر فتح لوحة التاجر
        ...
        try:
            ...
            panel_url = f"https://t.me/{bot_username}?start=trader_{int(user_id)}"
            await q.message.reply_text(
                "⛔ أكمل ملف التاجر ثم ارجع وابدأ عرض السعر.",
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("🧑‍💼 فتح لوحة التاجر", url=panel_url)]]
                ),
                disable_web_page_preview=True,
    ...
    def _order_snapshot(order_id: str) -> dict:
        ...
        try:
            out["goods_amount"] = str(
                oo.get("goods_amount_sar") or oo.get("quote_goods_amount") or ""
            ).strip()
        except Exception as e:
            _swallow(e)

        try:
            out["shipping_fee"] = str(
                oo.get("shipping_fee_sar") or oo.get("shipping_fee") or ""
            ).strip()
        except Exception as e:
            _swallow(e)
        ...
        try:
            out["ship_included"] = str(
                oo.get("ship_included") or oo.get("shipping_included") or ""
            ).strip()
        except Exception as e:
            _swallow(e)
    ...
    def _save_amounts_to_order(
        ...
        payload = {
            ...
            "shipping_fee_sar": (
                s_str if ship_fee_norm != "" else ""
            ),  # نخليها فاضية إذا غير محددة
            "total_amount_sar": (
                t_str if ship_fee_norm != "" else g_str
            ),  # إذا ما تحدد شحن -> الإجمالي = القطع
        }
    ...
    def _hdr(
        ...
        goods_now = (
            str(goods_total or "").strip()
            or str(snap.get("goods_amount") or "").strip()
        )
    ...
    def _get_items(order_id: str):
        ...
        for it in its:
            if isinstance(it, dict):
                ...
                pn = (
                    it.get("part_no") or it.get("partno") or it.get("part_number") or ""
                ).strip()
    ...
    def _items_kb(order_id: str, items: list, price_map: dict):
        ...
        for i, it in enumerate(items, start=1):
            ...
            label += tail
            rows.append(
                [InlineKeyboardButton(label, callback_data=f"ppq_it|{oid}|{i}")]
            )

        rows.append(
            [
                InlineKeyboardButton(
                    "✅ اكمال خطوات العرض", callback_data=f"ppq_it_done|{oid}"
                )
            ]
        )
        return InlineKeyboardMarkup(rows)
    ...
    def _q_is_sent_local(ss: dict) -> bool:
        try:
            return str((ss or {}).get("sent") or "").strip().lower() in (
                "1",
                "yes",
                "true",
                "sent",
        ...
        return InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        _wide_btn_label("🆕 إنشاء عرض جديد (إصدار جديد)"),
                        callback_data=f"ppq_new_version|{oid}",
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    if _q_is_sent_local(s) and action not in ("ppq_new_version", "pp_ui_close"):
        await _alert(
            q, "⚠️ تم إرسال العرض للعميل ولا يمكن تعديله. أنشئ عرضًا جديدًا (إصدار جديد)."
        )
        try:
    ...
    if action == "ppq_new_version":
        if not _q_is_sent_local(s):
            await _alert(
                q, "ℹ️ العرض الحالي لم يُرسل بعد. يمكنك تعديله من الأزرار الحالية."
            )
            return
        ...
        await _alert(
            q, f"✅ تم إنشاء عرض جديد (إصدار V{new_v}). ابدأ تسعير القطع من البداية."
        )
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            g, s_fee, t = _save_amounts_to_order(
                ...
                its,
                pm,
                shipping_fee=ship_fee_existing,
                ship_included=ship_inc,
            )
        ...
        try:
            ...
            await q.message.edit_reply_markup(
                reply_markup=_items_kb(order_id, its, pm2)
            )
        except Exception as e:
            _swallow(e)
    ...
    if action == "ppq_shipinc":
        ...
        if v_inc == "yes":
            ...
            try:
                update_order_fields(
                    order_id, {"ship_included": "yes", "shipping_fee_sar": "0"}
                )
            except Exception as e:
                _swallow(e)
        ...
        try:
            update_order_fields(
                order_id, {"ship_included": "no", "shipping_fee_sar": ""}
            )
        except Exception as e:
            _swallow(e)
    ...
    if action == "ppq_eta":
        ...
        try:
            ...
            ship_fee = str(
                ...
                or _order_snapshot(order_id).get("shipping_fee")
                or ""
            ).strip()
            ship_inc = str(
                td.get("quote_ship_included")
                or _order_snapshot(order_id).get("ship_included")
                or ""
            ).strip()
            _save_amounts_to_order(
                order_id, its, pm, shipping_fee=ship_fee, ship_included=ship_inc
            )
        except Exception as e:
            _swallow(e)
...
async def show_quote_preview(
    context: ContextTypes.DEFAULT_TYPE, trader_id: int, message, order_id: str
):
    ...
    try:
        trader_name = (
            getattr(getattr(message, "from_user", None), "full_name", "") or ""
        ).strip() or "التاجر"
    except Exception:
        trader_name = "التاجر"
    ...
    official = build_official_quote_text(
        ...
        ship_included=(
            str(ship_included).lower()
            in ("yes", "true", "1", "included", "مشمولة", "مشمول")
        ),
    )
...
async def finalize_quote_send(
    ...
    # ✅ منع إرسال العرض إذا ملف التاجر ناقص (حسب وضع الإلزام)
    try:
        tp = get_trader_profile(int(trader_id or 0)) or {}
    except Exception:
        tp = {}
    strict = _is_trader_profile_strict(context)
    missing = _trader_profile_missing(tp, strict=bool(strict))
    if missing:
        await _need_complete_trader_profile_notice(
            context,
        ...
        try:
            await message.reply_text("⛔ أكمل ملف التاجر ثم أعد إرسال عرض السعر.")
        except Exception as e:
            _swallow(e)
        return
    ...
    if (
        locked0
        or gps0 in ("awaiting_confirm", "confirmed")
        or ost0 in ("closed", "delivered")
    ):
        try:
            tname = (
    ...
    try:
        ...
        client_name = (
            o0.get("user_name") or o0.get("client_name") or o0.get("name") or ""
        ).strip() or "—"
    except Exception:
        client_name = "—"
    ...
    try:
        ...
        if anchor in official_with_trader:
            official_with_trader = official_with_trader.replace(
                anchor, f"{trader_header}\n\n{anchor}", 1
            )
        else:
            anchor2 = "يرجى مراجعة"
            if anchor2 in official_with_trader:
                official_with_trader = official_with_trader.replace(
                    anchor2, f"{trader_header}\n\n{anchor2}", 1
                )
            else:
                official_with_trader = (
                    official_with_trader.rstrip() + "\n\n" + trader_header
                )
    except Exception:
        official_with_trader = official2
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trader_store = (
        order.get("accepted_store_name")
        or order.get("shop_name")
        or order.get("store_name")
        or ""
    ).strip()
    ...
    if (not accepted_name or not trader_store) and accepted_tid:
        try:
            ...
            tp = {}
        if not accepted_name:
            accepted_name = (tp.get("display_name") or "").strip() or (
                tp.get("company_name") or ""
            ).strip()
        if not trader_store:
            trader_store = (
                (tp.get("company_name") or "").strip()
                or (tp.get("store_name") or "").strip()
                or (tp.get("display_name") or "").strip()
            )
    ...
    try:
        goods_total = int(
            float(order.get("goods_total_sar") or order.get("goods_amount_sar") or 0)
        )
    except Exception:
        goods_total = 0
    ...
    shipping_included = ship_inc_raw in (
        "yes",
        "1",
        "true",
        "on",
        ...
        if isinstance(items, list) and items:
            for i, it in enumerate(items, start=1):
                ...
                if pr:
                    ...
                    unavail_count += 1
                    # ✅ إزالة 🟥 (تشوه بصري) واستبدالها بنمط أنظف
    ...
    try:
        if total_items > 0:
            ...
            parts_block_lines.append(
                f"✅ المتوفر: <b>{priced_count}</b> / {total_items}"
            )
            if unavail_count:
                parts_block_lines.append(
                    f"⚠️ غير متوفر: <b>{unavail_count}</b> / {total_items}"
                )
            if priced_count:
                parts_block_lines.append(
                    f"💰 إجمالي القطع المتوفرة: <b>{_money_tail(parts_total, fb='0')}</b>"
                )
            parts_block_lines.append("")
            if priced_lines:
    ...
    if new_status in ("preparing", "prep"):
        ...
        next_step_trader = (
            "➡️ <b>الخطوة القادمة</b>: بعد التجهيز اضغط (🟢 الطلب جاهز للشحن)."
        )
        ...
        next_step_trader = (
            "➡️ <b>الخطوة القادمة</b>: بعد التسليم اضغط (✅ تأكيد التسليم بنجاح)."
        )
        ...
        try:
            ...
            for row in kb.inline_keyboard:
                ...
                for btn in row:
                    ...
                    # أي زر شات/اتصال يتم حجبه هنا (لن يظهر للتاجر إلا من رسالة إيصال الدفع لاحقاً)
    ...
    if client_id:
        try:
            ...
            # ✅ زر مراسلة التاجر للعميل: يظهر فقط بعد أول تحديث (جاري تجهيز) وما بعده
            show_client_chat = (
                new_status
                in (
                    "preparing",
            ...
            ) and bool(_assigned_trader_id(order_id))
            await context.bot.send_message(
                chat_id=client_id,
                text=msg,
                parse_mode="HTML",
                reply_markup=(
                    client_trader_chat_kb(order_id) if show_client_chat else None
                ),
                disable_web_page_preview=True,
            )
        except Exception as e:
...
async def tsu_skip_tracking_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        goods_total = 0
    try:
        shipping_fee = int(float(order.get("shipping_fee_sar") or 0))
    except Exception:
        shipping_fee = 0
    grand_total = goods_total + shipping_fee
    ...
    if not already_shipped:
        ...
        # ✅ إشعار العميل: تم الشحن بدون تتبع
        if client_id:
            try:
                lines = [
                    "📦 تم شحن طلبك",
                ...
                if grand_total > 0:
                    lines.append(f"💰 إجمالي المبلغ (شامل الشحن): {grand_total} ر.س")
        ...
        for aid in ADMIN_IDS:
            try:
                alines = [
                    "📦 تحديث حالة الطلب",
                    f"🧾 رقم الطلب: {order_id}",
                    f"🧑‍💼 التاجر: {tname} ({accepted_tid or user_id})",
                    "✅ الحالة: تم الشحن",
                    "🔎 التتبع: غير متوفر",
                ]
                if grand_total > 0:
                    alines.append(f"💰 إجمالي (شامل الشحن): {grand_total} ر.س")
    ...
    if grand_total > 0:
        done_text += f"\n💰 إجمالي (شامل الشحن): {grand_total} ر.س"
...
async def order_finish_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    goods_amt = _to_float(
        order.get("goods_amount_sar") or order.get("goods_total_sar") or 0
    )
    ...
    shipped_dt = _parse_iso(
        order.get("shipped_at_utc") or order.get("shipped_at") or ""
    )
    delivered_dt = _parse_iso(
        order.get("delivered_at_utc") or order.get("delivered_at") or ""
    )
...
async def _open_chat_session(
    context: ContextTypes.DEFAULT_TYPE, order_id: str, client_id: int, trader_id: int
):
    ...
    try:
        await context.bot.send_message(chat_id=client_id, text=client_open_txt)
    except Exception as e:
        _swallow(e)
    try:
        await context.bot.send_message(chat_id=trader_id, text=trader_open_txt)
    except Exception as e:
        _swallow(e)

    context.bot_data.setdefault("pp_chat_sessions", {})[str(client_id)] = {
        "order_id": order_id,
        "peer_id": trader_id,
        "role": "client",
    }
    context.bot_data.setdefault("pp_chat_sessions", {})[str(trader_id)] = {
        "order_id": order_id,
        "peer_id": client_id,
        "role": "trader",
    }
...
def admin_contact_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "💬 مراسلة العميل", callback_data=f"pp_admin_chat_client|{order_id}"
                ),
                InlineKeyboardButton(
                    "🧑‍🔧 مراسلة التاجر",
                    callback_data=f"pp_admin_chat_trader|{order_id}",
                ),
            ],
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✍️ رد للإدارة",
                    callback_data=f"pp_trader_chat_admin|{order_id}|{admin_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    "✖️ إنهاء", callback_data="pp_trader_chat_admin_done"
                )
            ],
        ]
    )
...
async def chat_open_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if actor_id == trader_id and actor_id not in ADMIN_IDS:
        if not goods_paid:
            await _alert(
                q, "🔒 لا يمكن مراسلة العميل أو عرض بياناته قبل تأكيد سداد قيمة القطع"
            )
            return
    ...
    # ============================
    try:
        goods_amt_num = _to_float(
            order.get("goods_total_sar") or order.get("goods_amount_sar") or 0
        )
    except Exception:
        goods_amt_num = 0.0
    ...
    ship_line = (
        "🚚 الشحن: مشمولة ✅"
        if shipping_included
        else f"🚚 الشحن: {_money(ship_fee_effective) if ship_fee_effective else '—'}"
    )
    ...
    if actor_id not in ADMIN_IDS:
        ...
        if not expires_dt:
            ...
            if base_dt:
                ...
                try:
                    update_order_fields(
                        order_id, {"chat_expires_at_utc": expires_dt.isoformat()}
                    )
                except Exception as e:
                    _swallow(e)
    ...
    try:
        ...
        tp = {}
    tname = (
        (tp.get("display_name") or "").strip() or actor_first or actor_name or "التاجر"
    )
    tco = (tp.get("company_name") or "").strip()
    tline = f"👤 <b>{html.escape(tname)}</b>" + (
        f" • 🏢 <b>{html.escape(tco)}</b>" if tco else ""
    )
    ...
    try:
        for i, it in enumerate(items, start=1):
            ...
            nm = (it.get("name") or it.get("item_name") or "").strip()
            pn = (
                it.get("part_no") or it.get("item_part_no") or it.get("number") or ""
            ).strip()
            if nm and pn:
...
async def goods_pay_bank_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ship_included = _is_yes(
        order.get("ship_included") or order.get("shipping_included") or ""
    )
...
async def goods_pay_stc_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    update_order_fields(
        order_id,
        {"goods_payment_method": "stc_pay", "goods_payment_status": "awaiting_receipt"},
    )
    set_stage(context, user_id, STAGE_AWAIT_GOODS_RECEIPT)
...
async def goods_receipt_photo_handler(
    ...
    try:
        await _lock_team_post_keyboard(
            context, order_id, reason="🔒 تم إيقاف العروض الطلب منتهي"
        )
    except Exception as e:
        _swallow(e)
    ...
    ship_method = (
        o_addr.get("delivery_type")
        or o_addr.get("ship_method")
        or o_addr.get("delivery_choice")
        or ""
    ).strip()
    ...
    if delivery_details:
        ...
        safe_details = re.sub(
            r"(\+?9665\d{8}|9665\d{8}|05\d{8})", "*********", delivery_details
        )
    ...
    if ship_fee_raw is None or str(ship_fee_raw).strip() == "":
        ship_fee_raw = (
            0
            if ship_included_norm in ("yes", "true", "1", "included", "مشمولة", "مشمول")
            else ""
        )
    ...
    try:
        await _send_client_payment_preview(
            context, user_id, order_id, pay_scope="goods"
        )
    except Exception as e:
        _swallow(e)
...
async def goods_receipt_document_handler(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    user_id = update.effective_user.id
    ud = get_ud(context, user_id)
...
async def notify_team(context: ContextTypes.DEFAULT_TYPE, ud: dict):
    ...
    goods_txt = (
        f"{goods_amount} ريال"
        if str(goods_amount).strip() not in ("", "0", "0.0")
        else "—"
    )
    ship_txt = (
        f"{ship_fee} ريال" if str(ship_fee).strip() not in ("", "0", "0.0") else "—"
    )
    ...
    # شارات سريعة
    car_txt = (
        html.escape((car_name + " " + car_model).strip())
        if (car_name or car_model)
        else "—"
    )
...
def _parse_item_name_partno(raw: str) -> tuple[str, str]:
    ...
    if len(toks) >= 2:
        ...
        if (
            re.search(r"\d", last)
            and (len(last) >= 5)
            and re.fullmatch(r"[A-Za-z0-9\-_/\.]+", last)
        ):
...
async def _rebroadcast_noquote_orders_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for o in orders:
        ...
        base_ts = (
            _parse_utc_iso(fwd)
            or _parse_utc_iso(str(o.get("created_at_utc") or ""))
            or None
        )
        base_ts = _as_utc_aware(base_ts)  # ✅ تحصين
        if not base_ts:
            continue
        ...
        if (now - base_ts) >= one_hour:
            last_b = (
                _parse_utc_iso(str(o.get("last_group_broadcast_at_utc") or "")) or None
            )
            last_b = _as_utc_aware(last_b)  # ✅ تحصين
            ...
            if (not last_b) or ((now - last_b) >= one_hour):
                ...
                try:
                    log_event(
                        "إعادة نشر طلب بدون عروض (تذكير للتجار)", order_id=order_id
                    )
                except Exception as e:
                    _swallow(e)
                ...
                try:
                    ...
                    try:
                        log_event(
                            "فشل إعادة نشر الطلب لمجموعة التجار",
                            order_id=order_id,
                            error=e,
                        )
                    except Exception as e:
                ...
                try:
                    update_order_fields(
                        order_id, {"last_group_broadcast_at_utc": utc_now_iso()}
                    )
                except Exception as e:
                    _swallow(e)
                ...
                if client_id:
                    last_ping = (
                        _parse_utc_iso(
                            str(o.get("last_noquote_user_ping_at_utc") or "")
                        )
                        or None
                    )
                    last_ping = _as_utc_aware(last_ping)  # ✅ تحصين
                    ...
                    if (not last_ping) or ((now - last_ping) >= one_day):
                        ...
                        try:
                            update_order_fields(
                                order_id,
                                {"last_noquote_user_ping_at_utc": utc_now_iso()},
                            )
                        except Exception as e:
                            _swallow(e)
        ...
        if (now - base_ts) >= one_day:
            last_admin = (
                _parse_utc_iso(str(o.get("admin_noquote_24h_sent_at_utc") or ""))
                or None
            )
            last_admin = _as_utc_aware(last_admin)  # ✅ تحصين

            if (not last_admin) or ((now - last_admin) >= one_day):
                admin_need_list.append(order_id)
                try:
                    update_order_fields(
                        order_id, {"admin_noquote_24h_sent_at_utc": utc_now_iso()}
                    )
                except Exception as e:
                    _swallow(e)
    ...
    if admin_need_list:
        ...
        text = (
            ...
            + "\n".join([f"• {html.escape(oid)}" for oid in admin_need_list])
        )
        for aid in ADMIN_IDS:
            try:
                await context.bot.send_message(
...
async def notify_admins_goods_receipt(
    ...
    try:
        ...
        ship_included = _is_yes(
            o.get("ship_included") or o.get("shipping_included") or ""
        )
    ...
    for aid in ADMIN_IDS:
        try:
            m = (mime or "").lower()
            is_img = m.startswith("image/") or m.endswith(
                ("jpg", "jpeg", "png", "webp")
            )
            if is_img:
                await context.bot.send_photo(
...
def admin_forward_kb(order_id: str, client_id: int = 0) -> InlineKeyboardMarkup:
    rows = [
        [
            InlineKeyboardButton(
                "📤 ارسال الطلب للتاجر", callback_data=f"pp_admin_forward|{order_id}"
            )
        ],
    ]
    ...
    if client_id:
        rows.append(
            [
                InlineKeyboardButton(
                    "💬 مراسلة العميل",
                    callback_data=f"pp_admin_reply|{order_id}|{client_id}",
                )
            ]
        )
    ...
    rows.append(
        [
            InlineKeyboardButton(
                "⛔ الغاء الطلب", callback_data=f"pp_admin_cancel|{order_id}"
            )
        ]
    )
...
def admin_forward_kb_locked(order_id: str, client_id: int = 0) -> InlineKeyboardMarkup:
    rows = [
        [
            InlineKeyboardButton(
                "✅ تم إرسال الطلب للتاجر مسبقًا", callback_data="pp_ui_close"
            )
        ],
    ]
...
async def notify_admins_receipt(
    ...
    ship_method = (
        ud.get("delivery_choice")
        or order.get("delivery_choice")
        or ud.get("ship_method")
        or order.get("ship_method")
        or ""
    ).strip()
    delivery_details = (
...
async def notify_admins_free_order(
    ...
    delivery_choice = (
        ud.get("delivery_choice") or order.get("delivery_choice") or ""
    ).strip()
    delivery_details = (
        ud.get("delivery_details") or order.get("delivery_details") or ""
    ).strip()
    ...
    try:
        ...
        if not trader_name:
            try:
                tid = (
                    int(order.get("accepted_trader_id") or 0)
                    if str(order.get("accepted_trader_id") or "").isdigit()
                    else 0
                )
                if tid:
                    tp = get_trader_profile(int(tid)) or {}
...
async def receipt_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await _send_client_payment_preview(
            context, user_id, order_id, pay_scope="platform"
        )
    except Exception as e:
        _swallow(e)
...
async def delivery_ship_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (not order_id) or (not items):
        ...
        try:
            await q.message.reply_text(
                "⚠️ انتهت جلسة الطلب أو تم فتح زر قديم.\nابدأ من جديد بكتابة: pp"
            )
        except Exception as e:
            _swallow(e)
        return
...
def team_locked_kb(
    order_id: str, reason: str = "🔒 الطلب مقفول"
) -> InlineKeyboardMarkup:
...
async def _lock_team_post_keyboard(
    context: ContextTypes.DEFAULT_TYPE, order_id: str, reason: str = "🔒 الطلب مقفول"
) -> None:
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== مكان التنفيذ =====
    in_team_group = bool(
        TEAM_CHAT_ID and q.message and q.message.chat_id == TEAM_CHAT_ID
    )
    ...
    if action == "pp_team_quote":
        ...
        required_fields = ["company_name", "iban", "stc_pay"]
        if not all((tp.get(f) or "").strip() for f in required_fields):
            await _need_complete_trader_profile_notice(
                context, actor_id, _user_name(q), order_id
            )
            ...
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "💰 فتح شاشة العرض بالخاص", url=quote_url
                            )
                        ],
    ...
    if action == "pp_trader_open":
        ...
        accepted_name = (
            order.get("accepted_trader_name") or ""
        ).strip() or "التاجر المستلم"
    ...
    if action == "pp_team_goods_confirm":
        ...
        try:
            ...
            try:
                goods_num3 = float(
                    o3.get("goods_amount_sar") or o3.get("goods_total_sar") or 0
                )
            except Exception:
                goods_num3 = 0.0
            ...
            try:
                ship_num3 = float(
                    o3.get("shipping_fee_sar") or o3.get("shipping_fee") or 0
                )
            except Exception:
                ship_num3 = 0.0
            ...
            try:
                ship_included = str(
                    o3.get("ship_included") or o3.get("shipping_included") or ""
                ).strip().lower() in ("yes", "y", "true", "1", "مشمول", "included")
            except Exception:
                ship_included = False
            ...
            client_username3 = (
                o3.get("user_username")
                or o3.get("username")
                or o3.get("client_username")
                or ""
            ).strip()
        ...
        try:
            ...
            try:
                goods_num4 = float(
                    o4.get("goods_amount_sar") or o4.get("goods_total_sar") or 0
                )
            except Exception:
                goods_num4 = 0.0
            try:
                ship_num4 = float(
                    o4.get("shipping_fee_sar") or o4.get("shipping_fee") or 0
                )
            except Exception:
                ship_num4 = 0.0
            try:
                ship_included4 = str(
                    o4.get("ship_included") or o4.get("shipping_included") or ""
                ).strip().lower() in ("yes", "y", "true", "1", "مشمول", "included")
            except Exception:
            ...
            trader_name_admin = (
                o4.get("accepted_trader_name") or o4.get("quoted_trader_name") or ""
            ).strip()
            if not trader_name_admin:
                ...
                if tid_admin:
                    try:
                        ...
                        trader_name_admin = (tp.get("display_name") or "").strip() or (
                            tp.get("company_name") or ""
                        ).strip()
                    except Exception as e:
                        _swallow(e)
        ...
        if uid:
            try:
                ...
                try:
                    goods_num5 = float(
                        o5.get("goods_amount_sar") or o5.get("goods_total_sar") or 0
                    )
                except Exception:
                    goods_num5 = 0.0
                try:
                    ship_num5 = float(
                        o5.get("shipping_fee_sar") or o5.get("shipping_fee") or 0
                    )
                except Exception:
                    ship_num5 = 0.0
                try:
                    ship_included5 = str(
                        o5.get("ship_included") or o5.get("shipping_included") or ""
                    ).strip().lower() in ("yes", "y", "true", "1", "مشمول", "included")
                except Exception:
                ...
                if ship_included5:
                    ...
                    ship_line_client = (
                        f"🚚 <b>الشحن</b>: <b>{html.escape(str(ship5))}</b>\n"
                    )
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _calc_goods_ship_total(order: dict, ud_local: dict | None = None) -> dict:
        """
        يرجع:
        goods_val, ship_val, total_val
        goods_txt, ship_txt, total_txt
        """
        ...
        # لو فيه حقل إجمالي محفوظ سابقًا نستخدمه إذا كان أكبر من (goods+ship) أو إذا ما نقدر نحسب
        total_saved = _num(
            order.get("total_amount_sar") or order.get("total_amount") or ""
        )
        total_val = (
            goods_val + ship_val if (goods_val > 0 or ship_val > 0) else total_saved
        )
    ...
    if (
        stage == STAGE_TRADER_STATUS_UPDATE
        and (ud.get("tsu_kind") or "").strip() == "seller_invoice"
    ):
        ...
        try:
            await send_invoice_pdf(
                context, order_id2, kind="preliminary", admin_only=True
            )
        except Exception as e:
            _swallow(e)
        ...
        try:
            ...
            o2 = b2.get("order", {}) or {}
            client_id2 = (
                int(o2.get("user_id") or 0)
                if str(o2.get("user_id") or "").isdigit()
                else 0
            )

            # ✅ حساب مبالغ صحيحة (قيمة القطع + الشحن = الإجمالي)
            am = _calc_goods_ship_total(o2, ud_local=ud)

            # ✅ محاولة حفظ الإجمالي بالحقل (بدون لمس رسوم المنصة)
            # (رسوم المنصة منفصلة وليست ضمن هذا الإجمالي)
            try:
                if am.get("total_val", 0) > 0:
                    update_order_fields(
                        order_id2,
                        {
                            "total_amount_sar": am.get("total_val"),
                        },
                    )
            except Exception as e:
                _swallow(e)

            # تجهيز نص مختصر للمبالغ
            money_lines = []
            if am.get("goods_txt"):
            ...
            # 1) إرسال فاتورة التاجر للعميل فقط (مع توضيح المبالغ)
            if client_id2:
                try:
                    cap_client = (
                        f"🧾 فاتورة التاجر - الطلب {order_id2}" f"{money_block}"
                    )
                    if is_photo:
                        await context.bot.send_photo(
                            chat_id=client_id2, photo=file_id, caption=cap_client
                        )
                    else:
                        await context.bot.send_document(
                            chat_id=client_id2, document=file_id, caption=cap_client
                        )
                except Exception as e:
                    _swallow(e)

            # 2) نسخة فاتورة التاجر للإدارة (مع توضيح المبالغ)
            for aid in ADMIN_IDS:
                try:
                    cap_admin = (
                        f"🧾 فاتورة تاجر (نسخة للمنصة) - الطلب {order_id2}"
                        f"{money_block}"
                    )
                    if is_photo:
                        await context.bot.send_photo(
                            chat_id=aid, photo=file_id, caption=cap_admin
                        )
                    else:
                        await context.bot.send_document(
                            chat_id=aid, document=file_id, caption=cap_admin
                        )
                except Exception as e:
                    _swallow(e)
            ...
            if client_id2:
                ...
                update_order_fields(
                    order_id2, {"goods_payment_status": "awaiting_method"}
                )

                # ✅ رسالة العميل لازم تحتوي المبالغ الصحيحة (مع الشحن إن وجد)
                client_lines = [
                    f"📦 الطلب {order_id2} جاهز للشحن ✅",
                    "تم إرسال فاتورة التاجر",
                ]
                if am.get("goods_txt"):
                    client_lines.append(f"💰 قيمة القطع: {am['goods_txt']}")
                if am.get("ship_val", 0) > 0 and am.get("ship_txt"):
                    client_lines.append(f"🚚 رسوم الشحن: {am['ship_txt']}")
                if am.get("total_txt"):
                    client_lines.append(f"🧾 الإجمالي: {am['total_txt']}")
                client_lines.append("اختر طريقة دفع قيمة القطع لاستكمال الشحن")
                ...
                # ✅ تم حذف (إشعار إداري إضافي مختصر) لتجنب تكدس رسائل مكررة عند الإدارة
        ...
        # ✅ جديد: رقم الطلب + مبالغ صحيحة (قيمة القطع + الشحن + الإجمالي) للتاجر
        goods_amt = ""
        ship_fee = ""
        total_amt = ""
        try:
            b3 = get_order_bundle(order_id2)
            o3 = b3.get("order", {}) or {}
            am3 = _calc_goods_ship_total(o3, ud_local=ud)

            goods_amt = am3.get("goods_txt") or ""
            ship_fee = am3.get("ship_txt") or ""
            total_amt = am3.get("total_txt") or ""
            ship_val3 = am3.get("ship_val", 0) or 0
        except Exception:
            goods_amt = ""
            ship_fee = ""
            total_amt = ""
        ...
        amt_lines = []
        if goods_amt:
            amt_lines.append(f"💰 قيمة القطع: {goods_amt}")
        if ship_val3 > 0 and ship_fee:
            amt_lines.append(f"🚚 رسوم الشحن: {ship_fee}")
        if total_amt:
            amt_lines.append(f"🧾 الإجمالي: {total_amt}")
        amt_block2 = ("\n".join(amt_lines)).strip()
        if amt_block2:
            amt_block2 = "\n" + amt_block2
    ...
    if stage == STAGE_AWAIT_GOODS_RECEIPT:
        ...
        if doc:
            ...
            is_img = mime.startswith("image/") or fname.endswith(
                (".jpg", ".jpeg", ".png", ".webp")
            )
            if is_pdf or is_img:
    ...
    if stage == STAGE_TRADER_SUB_AWAIT_RECEIPT:
        ...
        amount = int(float(ud.get("sub_amount_sar") or 99))
        pm = (
            str(ud.get("sub_payment_method") or ud.get("payment_method") or "").strip()
            or "—"
        )
        ...
        kb = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "✅ تأكيد الاشتراك",
                        callback_data=f"pp_admin_sub|confirm|{user_id}|{month}",
                    ),
                    InlineKeyboardButton(
                        "❌ رفض", callback_data=f"pp_admin_sub|reject|{user_id}|{month}"
                    ),
                ]
            ]
        )
        ...
        for aid in ADMIN_IDS:
            try:
                ...
                try:
                    await context.bot.send_message(
                        chat_id=aid, text=cap, parse_mode="HTML", reply_markup=kb
                    )
                except Exception as e:
                    _swallow(e)
...
def _admin_to_trader_reply_kb(admin_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "💬 رد للإدارة", callback_data=f"pp_trader_reply_admin|{admin_id}"
                )
            ],
...
async def _resolve_and_show_order(
    context: ContextTypes.DEFAULT_TYPE, user_id: int, tail4: str
):
    tail4 = str(tail4).strip()
    if not tail4.isdigit():
        return
    ...
    if not matches:
        await context.bot.send_message(
            chat_id=uid,
            text=f"🔍 لا يوجد طلب ينتهي بالرقم {tail4}",
        )
        return

    # لو طلب واحد فقط → اعرضه
    if len(matches) == 1:
        await _show_order_panel_private(context, uid, matches[0])
        return

    # لو أكثر من طلب بنفس النهاية → نخليه يختار
    kb = InlineKeyboardMarkup(
        [
    ...
    await context.bot.send_message(
        chat_id=uid,
        text="🔎 وُجد أكثر من طلب بنفس الرقم، اختر الطلب:",
        reply_markup=kb,
    )
...
async def open_order_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    order_id = (q.data or "").split("|", 1)[1]
    await _show_order_panel_private(context, q.from_user.id, order_id)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _reply(
        msg: str, kb=None, parse_mode: str = None, web_preview: bool = False
    ):
        try:
            await update.message.reply_text(
                msg,
    ...
    def _order_snapshot(order_id: str) -> dict:
        ...
        try:
            out["trader_name"] = (
                _trader_label(out["trader_id"], "—") if out["trader_id"] else "—"
            )
        except Exception:
            out["trader_name"] = "—"
        ...
        try:
            ga = str(
                o.get("goods_amount_sar") or o.get("quote_goods_amount") or ""
            ).strip()
            out["goods_amount"] = ga
        except Exception as e:
            _swallow(e)
    ...
    # =========================================================
    try:
        sess = (context.bot_data.get("pp_chat_sessions") or {}).get(str(user_id))
    except Exception:
        sess = None
    ...
    # ==================================================
    # 2) استدعاء لوحة طلب بكتابة رقم الطلب مباشرة (فقط إذا STAGE_NONE)
    # مثال: PP-260206-0001 أو PP1234
    # ==================================================
    if stage == STAGE_NONE:
        t = (raw_text or "").strip().upper()
        m_short = re.match(r"^PP(\d{4})$", t, flags=re.I)
        m_full = re.match(r"^PP-\d{6}-(\d{4,})$", t, flags=re.I)
        if m_short or m_full:

            tail = (m_short or m_full).group(1)
            await _resolve_and_show_order(context, user_id, tail)
            return
    ...
    if user_id in ADMIN_IDS and stage == STAGE_SUPPORT_ADMIN_REPLY:
        ...
        try:
            ...
            await _reply_html(
                "تعذر الإرسال", ["⚠️ قد لا يكون المستخدم بدأ البوت أو قام بحظر البوت."]
            )
            return
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_TRADER_MSG:
        ...
        try:
            ...
            await _reply_html(
                "تعذر الإرسال", ["⚠️ قد لا يكون التاجر بدأ البوت أو قام بحظر البوت."]
            )
            return
    ...
    if _support_is_open(ud):
        if _support_should_close_by_time(ud):
            await _support_close(
                ...
                context,
                user_id,
                reason="ℹ️ تم الإغلاق تلقائياً لانتهاء مدة الجلسة.",
            )
        else:
            ...
            if cur_stage and cur_stage != STAGE_NONE:
                await _support_close(
                    ...
                    context,
                    user_id,
                    reason="ℹ️ تم الإغلاق تلقائياً لأنك بدأت عملية أخرى.",
                )
            else:
                msg = _clean(raw_text)
                if msg:
                    ...
                    try:
                        m = re.search(
                            r"(PP-\d{6}-\d{4,})", msg_norm, flags=re.IGNORECASE
                        )
                        if m:
                        ...
                        if tail4:
                            ...
                            if len(matches) == 1:
                                ...
                                kb = InlineKeyboardMarkup(
                                    [
                                        [
                                            InlineKeyboardButton(
                                                f"🧾 {oid}",
                                                callback_data=f"pp_open_order|{oid}",
                                            )
                                        ]
                                        for oid in matches[:10]
                                    ]
                                ...
                                await _reply_html(
                                    "اختر الطلب",
                                    [
                                        "🔎 وُجد أكثر من طلب بنفس الرقم، اختر الطلب من القائمة:"
                                    ],
                                    kb=kb,
                                )
                                return
    ...
    if user_id in ADMIN_IDS and stage == STAGE_ADMIN_SEND_PAYLINK:
        ...
        if not (link.startswith("http://") or link.startswith("https://")):
            await _reply_html(
                "رابط غير صحيح", ["⚠️ ارسل رابط يبدأ بـ <code>https://</code>"]
            )
            return
        ...
        if not order_id or not client_id:
            await _reply_html(
                "تعذر الإرسال", ["⚠️ تعذر تحديد الطلب/العميل، أعد المحاولة."]
            )
            set_stage(context, user_id, STAGE_NONE)
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("🔗 فتح رابط الدفع", url=link)]]
                ),
                disable_web_page_preview=True,
            )
        except Exception:
            await _reply_html(
                "تعذر الإرسال", ["⚠️ تعذر إرسال الرابط للعميل (قد لا يكون بدأ البوت)."]
            )
            return
    ...
    if (text or "").strip() == "تاجر":
        ...
        if not is_member and not is_admin and not tp and not is_registered_trader:
            if not TRADERS_GROUP_ID:
                await _reply_html(
                    "تنبيه إعدادات",
                    ["⚠️ متغير <code>PP_TRADERS_GROUP_ID</code> غير موجود في .env"],
                )
            else:
                await _reply_html(
                    "غير مصرح",
                    ["⛔ تأكد أنك منضم لمجموعة التجار، وأن البوت مشرف داخلها."],
                )
            return
        ...
        try:
            ...
            await _reply_html(
                "تعذر الفتح", ["⚠️ تعذر فتح لوحة التاجر حالياً. حاول لاحقاً."]
            )
        return
    # ==================================================
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE:
        ...
        tname = (
            (tprof.get("display_name") or "").strip()
            or (name or "").strip()
            or "التاجر"
        )

        goods_amt = str(
            o2.get("goods_amount_sar") or o2.get("quote_goods_amount") or ""
        ).strip()
        ...
        if kind == "tracking":
            ...
            if tracking and tracking.lower() in [w.lower() for w in skip_words]:
                    ...
                    try:
                        ...
                        if goods_amt:
                            details_lines.append(
                                f"💰 قيمة القطع: {_fmt_money(goods_amt) or (goods_amt + ' ر.س')}"
                            )
                ...
                for aid in ADMIN_IDS:
                    try:
                        ...
                        if goods_amt:
                            admin_lines.append(
                                f"💰 قيمة القطع: <b>{html.escape(_fmt_money(goods_amt) or goods_amt + ' ر.س')}</b>"
                            )
                        if ship_method or ship_city:
                            admin_lines.append(
                                f"🚚 التسليم: {html.escape((ship_method + ' - ' + ship_city).strip(' -'))}"
                            )
            ...
            if len(tracking) < 4:
                await _reply_html(
                    "رقم تتبع غير واضح",
                    ["⚠️ اكتب رقم التتبع بشكل صحيح (مثال: <code>7845123690</code>)."],
                )
                return
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        def _quote_hdr(oid: str, goods_total: str = "") -> list:
            snap = (
                _order_snapshot(oid)
                if "_order_snapshot" in globals() or "_order_snapshot" in locals()
                else {}
            )
            client_name = (
                str(
                    (snap or {}).get("client_name")
                    or (snap or {}).get("user_name")
                    or "—"
                ).strip()
                or "—"
            )
            ...
            try:
                ship_val = (
                    float(str(ship_fee).strip())
                    if str(ship_fee).strip() not in ("", "—")
                    else None
                )
            except Exception:
                ship_val = None
            ...
            if (
                ship_txt
                and ship_txt not in ("0 ر.س", "0.0 ر.س")
                and ship_val is not None
                and ship_val > 0
            ):
            ...
            try:
                g = (
                    float(str(goods_now).strip())
                    if str(goods_now).strip() not in ("", "—")
                    else None
                )
            except Exception:
                g = None
        ...
        if step == "start":
            await _reply_html(
                "بناء عرض السعر",
                _quote_hdr(order_id)
                + ["👇 اضغط زر <b>(بدء بناء عرض السعر)</b> ثم اتبع الخطوات بالترتيب."],
        ...
        if step == "it_price":
            ...
            if not m_amt:
                await _reply_html(
                    ...
                    _quote_hdr(order_id)
                    + [
                        "ℹ️ اكتب السعر بالأرقام فقط.",
                        "مثال: <code>120</code> أو <code>120.50</code>",
                    ],
                )
                return
            ...
            if not items or idx < 1 or idx > len(items):
                ...
                await _reply_html(
                    "تعذر تحديد القطعة",
                    _quote_hdr(order_id) + ["ℹ️ ارجع للكيبورد واختر القطعة مرة أخرى."],
                )
                return
            ...
            await _reply_html(
                ...
                + ["✅ تم حفظ سعر القطعة.", "👇 اختر قطعة أخرى أو أكمل الخطوات:"],
        ...
        if step == "it_all_price":
            ...
            if not m_amt:
                await _reply_html(
                    ...
                    + [
                        "ℹ️ اكتب السعر بالأرقام فقط.",
                        "مثال: <code>50</code> أو <code>75.5</code>",
                    ],
                )
                return
            ...
            if not items:
                ...
                await _reply_html(
                    "لا توجد بنود",
                    _quote_hdr(order_id) + ["ℹ️ لا توجد قطع لتسعيرها حالياً."],
                )
                return
            ...
            await _reply_html(
                ...
                _quote_hdr(order_id, goods_total=total_now)
                + [
                    "✅ تم تطبيق السعر على جميع القطع.",
                    "👇 اضغط (اكمل خطوات العرض) للمتابعة:",
                ],
                kb=_items_kb_local(order_id, items, pm),
            )
            return
        ...
        if step == "amount":
            ...
            if not m_amt:
                await _reply_html(
                    ...
                    _quote_hdr(order_id)
                    + [
                        "ℹ️ اكتب مبلغ القطع بالأرقام فقط.",
                        "مثال: <code>850</code> أو <code>850.50</code>",
                    ],
                )
                return
            ...
            await _reply_html(
                "نوع القطع",
                _quote_hdr(order_id, goods_total=amount)
                + ["👇 اختر نوع القطع من الأزرار:"],
        ...
        if step == "shipping_fee":
            ...
            if not m_fee:
                await _reply_html(
                    ...
                    _quote_hdr(order_id)
                    + [
                        "ℹ️ اكتب قيمة الشحن بالأرقام فقط.",
                        "مثال: <code>25</code> أو <code>40.5</code>",
                    ],
                )
                return
            ...
            await _reply_html(
                ...
                _quote_hdr(order_id)
                + [
                    f"🚚 الشحن: <b>{html.escape(fee)}</b> ر.س",
        ...
        if step == "eta_custom":
            ...
            if len(v) < 2:
                await _reply_html(
                    "مدة غير واضحة",
                    _quote_hdr(order_id)
                    + ["ℹ️ اكتبها بصيغة مفهومة.", "مثال: <code>2-3 أيام</code>"],
                )
                return
            ...
            try:
                ...
                await _reply_html(
                    "تنبيه",
                    _quote_hdr(order_id)
                    + ["ℹ️ تعذر عرض المعاينة الآن، حاول مرة أخرى من الأزرار."],
                )
            return
        ...
        if step == "avail_custom":
            ...
            if len(v) < 2:
                await _reply_html(
                    "مدة غير واضحة",
                    _quote_hdr(order_id)
                    + ["ℹ️ اكتبها بصيغة مفهومة.", "مثال: <code>5 أيام</code>"],
                )
                return
            ...
            await _reply_html(
                ...
                _quote_hdr(order_id)
                + [
                    f"⏳ مدة التجهيز: <b>{html.escape(v)}</b>",
        ...
        await _reply_html(
            "تنبيه",
            _quote_hdr(order_id)
            + ["ℹ️ أكمل الخطوات باستخدام الأزرار حتى لا تتداخل المراحل."],
        )
        return
    ...
    if stage == STAGE_AWAIT_GOODS_PAY_METHOD:
        await _reply_html(
            "اختيار طريقة الدفع", ["👇 اختر طريقة الدفع من الأزرار بالأسفل."]
        )
        return
    ...
    if stage == STAGE_CHAT_TRADER:
        ...
        try:
            ...
            ship_included = _is_yes(
                o.get("ship_included")
                or o.get("shipping_included")
                or snap.get("ship_included")
                or ""
            )
        except Exception:
        ...
        await _reply_html("تم الإرسال", ["✅ تم إرسال رسالتك للتاجر."])
        return
    # ==================================================
    ...
    if stage == STAGE_TRADER_REPLY:
        ...
        if not to_uid or not order_id:
            await _reply_html(
                "تعذر الإرسال", ["⚠️ تعذر تحديد العميل المرتبط بهذه المراسلة."]
            )
            set_stage(context, user_id, STAGE_NONE)
            return
        ...
        try:
            ...
            await _reply_html(
                "تم الإرسال", ["✅ تم إرسال ردّك للعميل."], kb=trader_reply_done_kb()
            )
        except Exception:
            await _reply_html(
                "تعذر الإرسال", ["⚠️ قد لا يكون العميل بدأ البوت أو قام بحظر البوت."]
            )
        return
    # ==================================================
    ...
    if stage == STAGE_ADMIN_CHAT:
        ...
        try:
            if role == "client":
                ...
                try:
                    context.bot_data.setdefault("pp_admin_trader_sessions", {})[
                        str(peer_id)
                    ] = {
                        "order_id": order_id,
    ...
    if stage == STAGE_TRADER_CHAT_ADMIN:
        ...
        try:
            if snap.get("client_id"):
                client_id_part = (
                    f" (<code>{html.escape(str(snap.get('client_id')))}</code>)"
                )
        except Exception:
            client_id_part = ""
    ...
    if stage == STAGE_TRACK_ORDER:
        ...
        await _reply_html(
            "تم الاستلام",
            [
                f"✅ تم استلام رسالتك وسيتم الرد عليك من <b>{html.escape(PP_SUPPORT_LABEL)}</b>."
            ],
        )
        return
    # ==================================================
    ...
    if stage == STAGE_PREPAY_NOTES:
        ...
        await _reply_html(
            ...
            [
                "✅ تم حفظ الملاحظة.",
                "يمكنك إرسال ملاحظة جديدة للتعديل أو اختيار (تخطي) للمتابعة.",
            ],
            kb=prepay_notes_kb(),
        )
        return
    ...
    if stage == STAGE_ASK_PICKUP_CITY:
        ...
        await _reply_html(
            "رقم الجوال", ["📱 اكتب رقم الجوال للاستلام. مثال: <code>05xxxxxxxx</code>"]
        )
        return
    ...
    if stage == STAGE_ASK_PICKUP_PHONE:
        ...
        if not (phone.startswith("05") and len(phone) == 10):
            await _reply_html(
                "رقم غير صحيح",
                [
                    "⚠️ اكتب الرقم أرقام فقط ويبدأ بـ 05 ويكون 10 أرقام.",
                    "مثال: <code>05xxxxxxxx</code>",
                ],
            )
            return
        ...
        if not order_id:
            await _reply_html(
                "تعذر الربط",
                ["⚠️ تعذر ربط بيانات الاستلام بالطلب. اعد المحاولة من البداية."],
            )
            set_stage(context, user_id, STAGE_NONE)
            return
        ...
        try:
            ...
            await _reply_html(
                "معاينة الطلب", ["🔎 راجع طلبك قبل المتابعة"], kb=client_preview_kb()
            )
    ...
    if stage == STAGE_ASK_CAR:
        if len(text) < 3:
            await _reply_html("بيانات غير واضحة", ["⚠️ اكتب اسم السيارة بشكل أوضح."])
            return
        ud["car_name"] = text
        set_stage(context, user_id, STAGE_ASK_MODEL)
        await _reply_html(
            "سنة الموديل",
            ["📌 اكتب سنة الموديل فقط (4 أرقام). مثال: <code>2023</code>"],
        )
        return
    ...
    if stage == STAGE_ASK_MODEL:
        ...
        if not re.fullmatch(r"(19|20)\d{2}", s):
            await _reply_html(
                "صيغة غير صحيحة",
                ["⚠️ اكتب سنة الموديل 4 أرقام فقط. مثال: <code>2023</code>"],
            )
            return
        ...
        await _reply_html(
            "رقم الهيكل VIN",
            ["🔎 اكتب رقم الهيكل (17 خانة). مثال: <code>LVVDC12B4RD012345</code>"],
        )
        return
    ...
    if stage == STAGE_ASK_VIN:
        ...
        if not _looks_like_vin(vin):
            await _reply_html(
                "VIN غير صحيح",
                [
                    "⚠️ رقم الهيكل يجب أن يكون 17 خانة. مثال: <code>LVVDC12B4RD012345</code>"
                ],
            )
            return
        ...
        ud["vin"] = vin
        set_stage(context, user_id, STAGE_ASK_ITEM_NAME)
        await _reply_html("اسم القطعة", ["🧩 اكتب اسم القطعة رقم 1"])
        return
    # ==================================================
    ...
    if stage == STAGE_CONFIRM_MORE and text:
        ...
        if len(items) >= MAX_ITEMS:
            await _reply_html(
                "تم الوصول للحد",
                [
                    f"⚠️ وصلت للحد الأقصى من القطع ({MAX_ITEMS}).",
                    "اختر (إنهاء وإرسال الطلب).",
                ],
                kb=more_kb(),
            )
            return
        ...
        if len(text) < 2:
            await _reply_html("اسم غير واضح", ["⚠️ اكتب اسم القطعة بشكل أوضح."])
            return
        ...
        await _reply_html(
            "رقم القطعة", ["اكتب رقم القطعة (اختياري) أو اختر (تخطي)."], kb=partno_kb()
        )
        return
    # ==================================================
    # 26) إدخال اسم القطعة
    ...
    if stage == STAGE_ASK_ITEM_PARTNO:
        ...
        await _reply_html(
            "تمت الإضافة",
            [
                f"✅ تمت إضافة القطعة رقم <b>{item_no}</b>.",
                "📷 ارسل صورة (اختياري) أو اكتب اسم القطعة التالية مباشرة.",
            ],
            kb=photo_prompt_kb(),
        )
        return
    ...
    await _reply_html(
        ...
        [
            "ℹ️ لم أفهم الرسالة.",
            "اكتب <code>pp</code> لبدء طلب جديد، أو اكتب رقم الطلب لفتح لوحة الطلب.",
        ],
        kb=_support_kb(),
    )
    return
...
async def confirm_received_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        ...
        if not expires_dt:
            ...
            if not base_dt and (
                gps == "confirmed" or st in ("shipped", "delivered", "closed")
            ):
                base_dt = now_utc
    ...
    try:
        goods_total = int(
            float(order.get("goods_amount_sar") or order.get("goods_total_sar") or 0)
        )
    except Exception:
        goods_total = 0
    ...
    try:
        shipping_fee = int(
            float(order.get("shipping_fee_sar") or order.get("shipping_fee") or 0)
        )
    except Exception:
        shipping_fee = 0
...
async def trader_reply_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tname = (
        (tp.get("display_name") or "").strip()
        or (q.from_user.first_name or q.from_user.full_name or "").strip()
        or "التاجر"
    )
    tco = (tp.get("company_name") or "").strip()
    tline = f"👤 <b>{html.escape(tname)}</b>" + (
    ...
    if assigned and actor_id not in (assigned, *ADMIN_IDS):
        intruder_name = (
            q.from_user.first_name or q.from_user.full_name or ""
        ).strip() or "هذا التاجر"
...
async def trader_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        and _trader_is_disabled(uid)
        and action in ("edit", "orders", "sub")
    ):
        # Popup واضح
...
async def _notify_admins(
    context: ContextTypes.DEFAULT_TYPE, text: str, exclude_id: int = 0
):
    for aid in ADMIN_IDS:
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _admin_show_traders_manage():
        ...
        for t in trs:
            ...
            rows.append(
                [
                    InlineKeyboardButton(
                        f"👤 ملف — {tlabel}", callback_data=f"pp_admin|tview|{tid}"
                    )
                ]
            )
        ...
        strict = _is_trader_profile_strict(context)
        strict_txt = (
            "🔒 إلزام كل الحقول: <b>مفعل</b>"
            if strict
            else "🔓 إلزام كل الحقول: <b>غير مفعل</b>"
        )
        rows2 = [
            [
                InlineKeyboardButton(
                    ("🔓 تعطيل الإلزام" if strict else "🔒 تفعيل الإلزام"),
                    callback_data=f"pp_admin|tstrict|{'off' if strict else 'on'}",
                )
            ],
        ]

        msg = f"🧑‍💼 <b>إدارة التجار</b>\n{strict_txt}\nاختر تاجر لفتح ملفه أو تفعيل/تعطيل:"
        kb = InlineKeyboardMarkup(
            rows2
            + rows
            + [[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|home")]]
    ...
    if action == "fee_free":
        ...
        status = (
            "✅ مفعل (رسوم المنصة = 0)" if enabled else "⛔ غير مفعل (الرسوم طبيعية)"
        )
        ...
        kb = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "✅ تفعيل المجاني", callback_data="pp_admin|fee_free_on"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "⛔ إلغاء المجاني", callback_data="pp_admin|fee_free_off"
                    )
                ],
    ...
    if action == "fee_free_on":
        ...
        await _admin_edit_or_send(
            ...
            InlineKeyboardMarkup(
                [[InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|fee_free")]]
            ),
        )
        return
    ...
    if action == "backup_now":
        ...
        try:
            ...
            # ✅ لا تطبع نجاح إذا فشل الإرسال
            if not sent:
                try:
                    await q.message.reply_text(
                        "❌ فشل إرسال النسخة الاحتياطية إلى مجموعة النسخ."
                    )
                except Exception as e:
                    _swallow(e)
                return
            ...
            try:
                await q.message.reply_text(
                    "✅ تم إرسال نسخة احتياطية الآن إلى مجموعة النسخ وتم تثبيتها."
                )
            except Exception as e:
                _swallow(e)
    ...
    if action == "restore_last_pinned":
        ...
        try:
            ...
            try:
                await q.message.reply_text(
                    "✅ تم استرجاع آخر نسخة مثبتة وتشغيلها فورًا."
                )
            except Exception as e:
                _swallow(e)
    ...
    if action == "maint":
        ...
        kb = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🟧 تفعيل الصيانة", callback_data="pp_admin|maint_on"
                    ),
                    InlineKeyboardButton(
                        "🟩 إيقاف الصيانة", callback_data="pp_admin|maint_off"
                    ),
                ],
                [InlineKeyboardButton("↩️ رجوع", callback_data="pp_admin|home")],
            ]
    ...
    if action in ("maint_on", "maint_off"):
        ...
        try:
            set_setting(
                ...
                "on" if on else "off",
                actor_id=uid,
                actor_name=(q.from_user.full_name or ""),
            )
            try:
                append_legal_log(
                    uid,
    ...
    if action == "traders":
        ...
        if not per_amt:
            ...
            lines = []
            for tid, amt in sorted(
                per_amt.items(), key=lambda x: float(x[1] or 0), reverse=True
            )[:30]:
                tlabel = _trader_label(int(tid), "")
                lines.append(
                    f"• {tlabel} — {_money(amt)} — {int(per_cnt.get(tid, 0) or 0)} طلب"
                )
            msg = "👥 <b>احصائيات التجار</b>\n\n" + html.escape("\n".join(lines))
    ...
    if action == "orders":
        ...
        for o in orders_sorted:
            ...
            amt = (
                _money(o.get("goods_amount_sar") or o.get("quote_amount_sar") or "")
                or "—"
            )
    ...
    if action == "subs":
        ...
        for t in traders:
            try:
                ...
                tid = 0
            name = (
                t.get("display_name") or t.get("company_name") or ""
            ).strip() or str(tid)
            if tid in confirmed:
    ...
    # ===== TRADER PROFILE STRICT TOGGLE =====
    if action == "tstrict":
        mode = ""
        if len(parts) >= 3:
            mode = str(parts[2] or "").strip().lower()
        try:
            set_setting("trader_profile_strict", "1" if mode == "on" else "0")
        except Exception as e:
            _swallow(e)
        try:
            await _alert(q, "✅ تم تحديث سياسة الإلزام")
        except Exception as e:
            _swallow(e)
        await _admin_show_traders_manage()
        return
    ...
    if action == "tview":
        ...
        created = (prof.get("created_at_utc") or "").strip()
        nid = (prof.get("national_id") or "").strip()
        crn = (prof.get("cr_number") or "").strip()
        vat = (prof.get("vat_number") or "").strip()
        ...
        msg = (
            ...
            f"📅 تاريخ الانضمام: <b>{html.escape(created or '—')}</b>\n\n"
            ...
            f"🪪 رقم الهوية: <b>{html.escape(nid or '—')}</b>\n"
            f"🏷️ السجل التجاري: <b>{html.escape(crn or '—')}</b>\n"
            f"🧾 الرقم الضريبي: <b>{html.escape(vat or '—')}</b>\n"
        ...
        kb_rows = [
            [
                InlineKeyboardButton(
                    "💬 مراسلة التاجر", callback_data=f"pp_admin|tmsg|{tid}"
                )
            ],
            [
                InlineKeyboardButton(
                    "📄 سجل التاجر PDF", callback_data=f"pp_admin|trecord_pdf|{tid}"
                )
            ],
            [
                InlineKeyboardButton(
                    "📤 كشف معاملات (CSV)", callback_data=f"pp_admin|texport|{tid}"
                )
            ],
            [
                InlineKeyboardButton(
                    "📦 آخر طلبات التاجر", callback_data=f"pp_admin|torders|{tid}"
                )
            ],
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    "↩️ رجوع لقائمة التجار", callback_data="pp_admin|traders_manage"
                )
            ],
            [InlineKeyboardButton("🏠 الرئيسية", callback_data="pp_admin|home")],
        ]
    ...
    # ===== TRADER RECORD PDF (trecord_pdf) =====
    if action == "trecord_pdf":
        tid = 0
        if len(parts) >= 3:
            try:
                tid = int(parts[2] or 0)
        ...
        # بيانات أساسية
        tname = (prof.get("display_name") or "").strip() or "التاجر"
        tcompany = (prof.get("company_name") or "").strip() or "—"
        bank = (prof.get("bank_name") or "").strip() or "—"
        iban = (prof.get("iban") or "").strip() or "—"
        stc = (prof.get("stc_pay") or "").strip() or "—"
        created = (prof.get("created_at_utc") or "").strip() or "—"
        upd = (prof.get("updated_at_utc") or "").strip() or "—"
        nid = (prof.get("national_id") or "").strip() or "—"
        crn = (prof.get("cr_number") or "").strip() or "—"
        vat = (prof.get("vat_number") or "").strip() or "—"
        ...
        # توليد PDF بنفس هوية الفاتورة (خط Amiri + شعار)
        try:
            import os
            from reportlab.lib.pagesizes import A4
            from reportlab.lib.units import cm
            ...
            font_name = "Helvetica"
            try:
                f1 = os.path.join(os.path.dirname(__file__), "Amiri-Regular.ttf")
                if os.path.exists(f1):
                    pdfmetrics.registerFont(TTFont("Amiri", f1))
                    font_name = "Amiri"
            ...
            styles = getSampleStyleSheet()
            # ✅ label يمين / value يسار (كما هو مطلوب في السجل العربي)
            base = ParagraphStyle(
                "base",
                parent=styles["Normal"],
                fontName=font_name,
            ...
            label = ParagraphStyle("label", parent=base, alignment=2)
            val = ParagraphStyle("val", parent=base, alignment=0)  # TA_LEFT
            center = ParagraphStyle("center", parent=base, alignment=1)
            small = ParagraphStyle(
                "small", parent=base, fontSize=9, leading=12, textColor=colors.gray
            )
            ...
            out_path = os.path.join("/tmp", f"PP_TRADER_RECORD_{tid}.pdf")
            doc = SimpleDocTemplate(
                out_path,
                pagesize=A4,
                leftMargin=1.6 * cm,
            ...
            try:
                if logo_path and os.path.exists(logo_path):
                    img = RLImage(logo_path)
                    img.drawHeight = 2.6 * cm
                    img.drawWidth = 2.6 * cm
                    logo_cell = img
            except Exception:
                logo_cell = ""

            header_tbl = Table(
                [[logo_cell if logo_cell else P("<b>PPARTS</b>", center)]],
                colWidths=[full_w],
            )
            header_tbl.setStyle(
                TableStyle(
            ...
            title_bar.setStyle(
                TableStyle(
                    [
                        ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#F28C28")),
                        ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                        ("TOPPADDING", (0, 0), (-1, -1), 6),
                        ("BOTTOMPADDING", (0, 0), (-1, -1), 6),
                        ("BOX", (0, 0), (-1, -1), 0.25, colors.HexColor("#F28C28")),
                    ]
                )
            )
            story.append(title_bar)
            ...
            # Data table (Key / Value) — RTL right aligned
            rows = [
                [P(str(tid), val), P("<b>ID</b>", label)],
                [P(html.escape(tname), val), P("<b>اسم التاجر</b>", label)],
                [P(html.escape(tcompany), val), P("<b>اسم المتجر</b>", label)],
                [P(html.escape(created), val), P("<b>تاريخ الانضمام</b>", label)],
                [P(html.escape(enabled_txt), val), P("<b>حالة التفعيل</b>", label)],
                [
                    P(html.escape(sub_status), val),
                    P(f"<b>حالة الاشتراك ({month})</b>", label),
                ],
                [P(html.escape(bank), val), P("<b>اسم البنك</b>", label)],
                [P(html.escape(iban), val), P("<b>IBAN</b>", label)],
                [P(html.escape(stc), val), P("<b>STC Pay</b>", label)],
                [P(html.escape(nid), val), P("<b>رقم الهوية</b>", label)],
                [P(html.escape(crn), val), P("<b>السجل التجاري</b>", label)],
                [P(html.escape(vat), val), P("<b>الرقم الضريبي</b>", label)],
                [P(html.escape(upd), val), P("<b>آخر تحديث</b>", label)],
            ]

            tbl = Table(rows, colWidths=[full_w * 0.66, full_w * 0.34])
            tbl.setStyle(
                TableStyle(
                    [
                        (
                            ...
                            (-1, -1),
                            [
                                colors.Color(0.95, 0.55, 0.10, alpha=0.08),
                                colors.Color(1, 1, 1, alpha=0.0),
                            ],
                        ),
                        ("BOX", (0, 0), (-1, -1), 0.45, colors.HexColor("#D9A15B")),
                        (
                            "INNERGRID",
                            (0, 0),
                            (-1, -1),
                            0.25,
                            colors.HexColor("#E6C79B"),
                        ),
                        ("ALIGN", (0, 0), (0, -1), "LEFT"),
                        ("ALIGN", (1, 0), (1, -1), "RIGHT"),
                        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ...
            story.append(P("الخدمات المساندة GO", small))
            story.append(P("منصة PParts قطع غيار السيارات — سجل داخلي", small))

            # Watermark text on each page
            def _wm(canv, doc_):
                try:
                    canv.saveState()
                    # نفس بصمة الفواتير: لوجو كبير شفاف + مرفوع للأعلى
                    try:
                        if logo_path and os.path.exists(logo_path):
                            from reportlab.lib.utils import ImageReader
                            ...
                            # fallback: نص مائي برتقالي
                            try:
                                canv.setFillAlpha(0.10)
                            except Exception:
                                pass
                            canv.setFont(font_name, 60)
                            canv.setFillColor(colors.HexColor("#F28C28"))
                            canv.translate(A4[0] / 2, A4[1] / 2 + (3.2 * cm))
                            canv.rotate(30)
                            canv.drawCentredString(0, 0, _ar("PPARTS"))
                    finally:
                        canv.restoreState()
                except Exception:
                    try:
            ...
            cap = f"📄 سجل التاجر PDF — ID: {tid}"
            try:
                with open(out_path, "rb") as f:
                    await context.bot.send_document(
                        chat_id=q.message.chat_id, document=f, caption=cap
                    )
            except Exception as e:
                _swallow(e)
        except Exception as e:
            _swallow(e)
            await _pop("⚠️ تعذر إنشاء PDF حالياً")
        return
    ...
    if action == "torders":
        ...
        kb = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "↩️ رجوع لملف التاجر", callback_data=f"pp_admin|tview|{tid}"
                    )
                ],
                [
                    InlineKeyboardButton(
        ...
        try:
            ...
            try:
                append_legal_log(
                    ...
                    (q.from_user.full_name or ""),
                    "export_trader_csv",
                    f"trader_id={tid}; rows={len(orders or [])}",
                )
            except Exception as e:
                _swallow(e)
        ...
        await _admin_edit_or_send(
            ...
            InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "👤 فتح ملف التاجر", callback_data=f"pp_admin|tview|{tid}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "↩️ رجوع لقائمة التجار",
                            callback_data="pp_admin|traders_manage",
                        )
                    ],
                ]
            ),
        ...
        await _admin_edit_or_send(
            ...
            InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "🏠 الرئيسية", callback_data="pp_admin|home"
                        )
                    ],
                ]
            ),
...
def trader_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🧑‍💼 تعديل اسم التاجر", callback_data="pp_tprof|edit|display_name"
                )
            ],
            [
                InlineKeyboardButton(
                    "🏢 تعديل اسم المتجر", callback_data="pp_tprof|edit|company_name"
                )
            ],
            [
                InlineKeyboardButton(
                    "🏦 تعديل اسم البنك", callback_data="pp_tprof|edit|bank_name"
                )
            ],
            [
                InlineKeyboardButton(
                    "💳 تعديل رقم الايبان", callback_data="pp_tprof|edit|iban"
                )
            ],
            [
                InlineKeyboardButton(
                    "📱 تعديل رقم STC Pay", callback_data="pp_tprof|edit|stc_pay"
                )
            ],
            # [InlineKeyboardButton("💳 سداد اشتراك المنصة (99 ر.س)", callback_data="pp_tprof|sub|start")],
            [
                InlineKeyboardButton(
                    "📦 طلباتي المعلقة", callback_data="pp_tprof|orders|pending"
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ طلباتي المنجزة", callback_data="pp_tprof|orders|done"
                )
            ],
            [InlineKeyboardButton("📩 اتصل بالمنصة", callback_data="pp_support_open")],
            [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],  # ✅ جديد
        ]
    )
...
def admin_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "👥 إدارة التجار", callback_data="pp_admin|traders_manage"
                )
            ],
            [
                InlineKeyboardButton(
                    "📊 التقارير المالية", callback_data="pp_admin|finance"
                )
            ],
            [
                InlineKeyboardButton(
                    "🔎 بحث عن طلب", callback_data="pp_admin|find_order"
                )
            ],
            [
                InlineKeyboardButton(
                    "🎁 عرض مجاني لرسوم المنصة", callback_data="pp_admin|fee_free"
                )
            ],
            # ✅ الزران المطلوبان فقط
            [
                InlineKeyboardButton(
                    "🗂 نسخ احتياطي الآن", callback_data="pp_admin|backup_now"
                )
            ],
            [
                InlineKeyboardButton(
                    "♻️ استرجاع آخر نسخة مثبتة",
                    callback_data="pp_admin|restore_last_pinned",
                )
            ],
...
async def show_trader_panel(
    update_or_q, context: ContextTypes.DEFAULT_TYPE, trader_id: int
):
    ...
    try:
        chat_type = None
        if (
            hasattr(update_or_q, "message")
            and update_or_q.message
            and update_or_q.message.chat
        ):
        ...
        if chat_type and str(chat_type).lower() != "private":
            ...
            try:
                if hasattr(update_or_q, "answer"):
                    await update_or_q.answer(
                        "افتح الخاص لعرض لوحة التاجر", show_alert=True
                    )
                elif hasattr(update_or_q, "message") and update_or_q.message:
    ...
    if not tp and (is_member or is_admin):
        try:
            upsert_trader_profile(
                int(trader_id or 0), {"trader_id": int(trader_id or 0)}
            )
            tp = get_trader_profile(int(trader_id or 0)) or {}
        except Exception:
            tp = tp or {}
    ...
    dn = (
        (tp.get("display_name") or "").strip()
        or (
            getattr(update_or_q, "from_user", None).full_name
            if getattr(update_or_q, "from_user", None)
            else ""
        )
        or "التاجر"
    )
    ...
    try:
        if old_msg_id:
            await context.bot.delete_message(
                chat_id=int(old_chat_id or trader_id or 0), message_id=int(old_msg_id)
            )
    except Exception as e:
        _swallow(e)
...
async def _show_order_panel_private(
    context: ContextTypes.DEFAULT_TYPE, trader_id: int, order_id: str
):
    order_id = (order_id or "").strip()
    if not order_id:
        return
    ...
    # صلاحية: فقط التاجر المقبول أو الأدمن
    if int(trader_id or 0) not in (ADMIN_IDS or []) and int(trader_id or 0) != int(
        accepted_tid or 0
    ):
        try:
            await context.bot.send_message(
                chat_id=int(trader_id or 0),
                text=("🔒 هذا الطلب ليس على حسابك.\n" f"🧾 رقم الطلب: {order_id}"),
                disable_web_page_preview=True,
            )
        except Exception as e:
            _swallow(e)
    ...
    # ✅ سطر إجمالي موحد (شامل الشحن إذا موجود/مشمـول)
    def _num(v):
        try:
    ...
    goods = _num(order.get("goods_amount_sar") or order.get("goods_amount") or 0)
    ship_fee = _num(order.get("shipping_fee_sar") or order.get("shipping_fee") or 0)
    ship_included = str(order.get("shipping_included") or "").strip().lower() in (
        "yes",
        "1",
        "true",
        "included",
    ...
    total = goods + (ship_fee if (ship_fee > 0 and not ship_included) else 0)
    total_txt = _money(total) if total else (_money(goods) or "")

    # حذف اللوحة السابقة للتاجر (منع التكدس)
    td = context.user_data.setdefault(int(trader_id or 0), {})
    last_mid = td.get("last_order_panel_mid")
    if last_mid:
        try:
            await context.bot.delete_message(
                chat_id=int(trader_id or 0), message_id=int(last_mid)
            )
        except Exception as e:
            _swallow(e)
    ...
    # ✅ آخر تحديث (الأقرب للأحدث)
    last_ts = (
        order.get("closed_at_utc")
        or order.get("delivered_at_utc")
        or order.get("shipped_at_utc")
        or order.get("updated_at_utc")
        or ""
    )
    ...
    text = (
        ...
        + f"📌 الحالة الحالية: {status_line}\n"
    )

    if last_ts:
        try:
            last_ts = last_ts.replace("T", " ").split(".")[0]
            text += f"🕒 آخر تحديث: {last_ts}\n"
        except Exception as e:
            _swallow(e)
    ...
    try:
        ...
        try:
            td["last_order_panel_mid"] = int(m.message_id)
        except Exception as e:
            _swallow(e)
    except Exception as e:
        _swallow(e)
...
async def show_admin_panel(
    update_or_q, context: ContextTypes.DEFAULT_TYPE, admin_id: int
):
    ...
    try:
        ...
        # Message
            ...
            msg = update_or_q
        # Update
        elif hasattr(update_or_q, "effective_message") and getattr(
    ...
    for aid in ids:
        try:
            await app.bot.send_message(
                chat_id=int(aid), text=text, disable_web_page_preview=True
            )
        except Exception as e:
            _swallow(e)
...
async def _send_backup_excel(app: Application, reason: str = "scheduled"):
    ...
    chat_id_raw = backup_chat_id_raw or (
        str(PP_BACKUP_CHAT_ID).strip() if PP_BACKUP_CHAT_ID else ""
    )
    if not chat_id_raw:
        if _should_throttle_notice("last_backup_warn_no_chat_id_utc", 6 * 3600):
            await _notify_admins(
                app, "⚠️ النسخ الاحتياطي متوقف: PP_BACKUP_CHAT_ID غير مضبوط."
            )
        return None
    ...
    if sz <= 0:
        if _should_throttle_notice("last_backup_warn_excel_empty_utc", 30 * 60):
            await _notify_admins(
                app, f"❌ ملف الإكسل فارغ/تالف.\nPATH: {path}\nSIZE: {sz}"
            )
        return None

    # 2) منع التكرار (حد أدنى)
    try:
    ...
    try:
        if last:
            ...
            ).total_seconds() < PP_BACKUP_MIN_SECONDS:
                return None
    except Exception as e:
        _swallow(e)
    ...
    try:
        ...
        try:
            if sent and getattr(sent, "document", None):
                ...
                set_setting(
                    ...
                    sent.document.file_name or os.path.basename(path),
                )
                ...
                try:
                    ...
                    try:
                        if sent2 and getattr(sent2, "document", None):
                            ...
                            set_setting(
                                "last_backup_file_name",
                                sent2.document.file_name or os.path.basename(path),
                            )
                            set_setting("last_backup_at_utc", _utc_now_iso())
                    except Exception as e:
                        _swallow(e)

                    if _should_throttle_notice(
                        "last_backup_info_migrated_utc", 6 * 3600
                    ):
                        await _notify_admins(
                            app,
                    ...
                    if _should_throttle_notice(
                        "last_backup_warn_send_error_utc", 30 * 60
                    ):
                        await _notify_admins(
                            app, f"❌ فشل الإرسال حتى بعد تحديث chat_id.\n{e2}"
                        )
                    return None
        ...
        if _should_throttle_notice("last_backup_warn_send_error_utc", 30 * 60):
            await _notify_admins(
                app,
                f"❌ BadRequest أثناء إرسال النسخة:\n{msg}\nCHAT_ID: {chat_id}\nPATH: {path}\nSIZE: {sz}",
            )
        return None
...
async def _restore_excel_from_message(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    """
...
def build_app():
    ...
    app.add_handler(
        MessageHandler(filters.Regex(r"^/منصة(?:@\w+)?(?:\s|$)"), support_cmd)
    )
    ...
    app.add_handler(
        CallbackQueryHandler(
            client_confirm_preview_cb, pattern="^pp_client_confirm_preview$"
        )
    )

    # 🟢 [HANDLER] Support Close / Admin Reply / Done + Open from Button
    app.add_handler(
        CallbackQueryHandler(support_close_cb, pattern="^pp_support_close$")
    )
    app.add_handler(
        CallbackQueryHandler(pp_support_reply_cb, pattern=r"^pp_support_reply\|")
    )
    app.add_handler(
        CallbackQueryHandler(support_admin_done_cb, pattern="^pp_support_admin_done$")
    )
    ...
    app.add_handler(
        CallbackQueryHandler(prepay_notes_skip_cb, pattern="^pp_prepay_notes_skip$")
    )
    ...
    app.add_handler(
        CallbackQueryHandler(admin_reply_done_cb, pattern="^pp_admin_reply_done$")
    )

    app.add_handler(CallbackQueryHandler(chat_trader_cb, pattern=r"^pp_chat_trader\|"))
    app.add_handler(
        CallbackQueryHandler(chat_trader_done_cb, pattern="^pp_chat_trader_done$")
    )
    app.add_handler(
        CallbackQueryHandler(trader_reply_cb, pattern=r"^pp_trader_reply\|")
    )
    app.add_handler(
        CallbackQueryHandler(trader_reply_done_cb, pattern=r"^pp_trader_reply_done$")
    )
    app.add_handler(
        CallbackQueryHandler(trader_reply_admin_cb, pattern=r"^pp_trader_reply_admin\|")
    )

    app.add_handler(CallbackQueryHandler(copy_iban_cb, pattern="^pp_copy_iban$"))
    app.add_handler(
        CallbackQueryHandler(copy_beneficiary_cb, pattern="^pp_copy_beneficiary$")
    )
    ...
    app.add_handler(
        CallbackQueryHandler(admin_paylink_cb, pattern=r"^pp_admin_paylink\|")
    )
    app.add_handler(CallbackQueryHandler(admin_sub_cb, pattern=r"^pp_admin_sub\|"))

    app.add_handler(
        CallbackQueryHandler(goods_pay_bank_cb, pattern=r"^pp_goods_pay_bank\|")
    )
    app.add_handler(
        CallbackQueryHandler(goods_pay_stc_cb, pattern=r"^pp_goods_pay_stc\|")
    )
    app.add_handler(
        CallbackQueryHandler(trader_status_cb, pattern=r"^pp_trader_status\|")
    )
    app.add_handler(
        CallbackQueryHandler(tsu_skip_tracking_cb, pattern=r"^pp_tsu_skip_tracking\|")
    )
    app.add_handler(CallbackQueryHandler(chat_open_cb, pattern=r"^pp_chat_open\|"))
    app.add_handler(
        CallbackQueryHandler(order_finish_cb, pattern=r"^pp_order_finish\|")
    )

    app.add_handler(
        CallbackQueryHandler(admin_chat_client_cb, pattern=r"^pp_admin_chat_client\|")
    )
    app.add_handler(
        CallbackQueryHandler(admin_chat_trader_cb, pattern=r"^pp_admin_chat_trader\|")
    )
    app.add_handler(
        CallbackQueryHandler(admin_chat_done_cb, pattern=r"^pp_admin_chat_done$")
    )
    app.add_handler(
        CallbackQueryHandler(trader_chat_admin_cb, pattern=r"^pp_trader_chat_admin\|")
    )
    app.add_handler(
        CallbackQueryHandler(
            trader_chat_admin_done_cb, pattern=r"^pp_trader_chat_admin_done$"
        )
    )
    ...
    app.add_handler(
        CallbackQueryHandler(goods_pay_link_cb, pattern=r"^pp_goods_pay_link\|")
    )

    app.add_handler(
        CallbackQueryHandler(delivery_ship_cb, pattern="^pp_delivery_ship$")
    )
    app.add_handler(
        CallbackQueryHandler(delivery_pickup_cb, pattern="^pp_delivery_pickup$")
    )

    app.add_handler(
        CallbackQueryHandler(admin_forward_cb, pattern=r"^pp_admin_forward\|")
    )
    app.add_handler(
        CallbackQueryHandler(admin_cancel_cb, pattern=r"^pp_admin_cancel\|")
    )

    app.add_handler(
        CallbackQueryHandler(team_cb, pattern=r"^(pp_team_|pp_trader_open\|)")
    )

    app.add_handler(
        CallbackQueryHandler(confirm_received_cb, pattern=r"^pp_confirm_received\|")
    )
    ...
    try:
        app.add_handler(
            MessageHandler(filters.Document.ALL, _restore_excel_from_message), group=0
        )
    except Exception as e:
        try:
...
async def _run_webhook():
    ...
    if not base_url:
        log.warning(
            "⚠️ WEBHOOK_BASE_URL غير موجود. شغّل WEBHOOK_BASE_URL=https://<your-service>.onrender.com"
        )
    ...
    log.info(
        "🌐 Web server running on 0.0.0.0:%s | webhook_path=/%s", port, webhook_path
    )
...
def main():
    ...
    enabled = (os.getenv("PP_WEBHOOK_ENABLED") or "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
