...
async def send_trader_ledger_pdf(
    ...
    # ✅ (02) badges: ترتيب RTL مضبوط (الكلمة أولاً ثم الرقم/القيمة)
    RLM = "\u200f"
    LRM = "\u200e"

    trader_name = (tname or str(tid)).strip()
    trader_badge = f"{trader_name}"
    if tuser:
        trader_badge = f"{trader_badge} {LRM}{tuser}{LRM}"

    badges = Table([[
        P(f"{RLM}<b>الوقت</b> {LRM}{now.strftime('%H:%M')}{LRM}", tiny_c),
        P(f"{RLM}<b>التاريخ</b> {LRM}{now.strftime('%Y-%m-%d')}{LRM}", tiny_c),
        P(f"{RLM}<b>التاجر</b> {trader_badge}", tiny_c),
