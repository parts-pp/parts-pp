...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    # ✅ كيبورد ديناميكي حسب السيناريو الجديد (بدون تنبيهات):
    # 1) بعد موافقة العميل: زر واحد (🟡 جاري تجهيز)
    # 2) بعد تجهيز: زر واحد (🟢 جاهز للشحن)
    # 3) عند (جاهز للشحن): يطلب فاتورة المتجر (إجباري) ثم ينتظر السداد
    # 4) بعد رفع الفاتورة وقبل السداد: يظهر (⏳ بانتظار السداد) + زر (📞 الاتصال بالعميل) لتفعيل المراسلة
    # 5) بعد تأكيد دفع البضاعة: زرين فقط (🚚 تم الشحن + 📞 الاتصال بالعميل)
    # 6) بعد الشحن: زر واحد (✅ تأكيد التسليم) + زر (📞 الاتصال بالعميل) لمدة 7 أيام (حسب chat_expires_at_utc في هاندر المراسلة)
    try:
    ...
    # بعد الموافقة (أو البداية)
    if ost in ("", "new", "accepted"):
        return InlineKeyboardMarkup([
    ...
    # جاهز للشحن / انتظار السداد
    if ost in ("ready_to_ship", "ready"):
        # لو الفاتورة غير موجودة: نبقي زر الجاهزية (سيطلب فاتورة)
        if not inv_file:
            return InlineKeyboardMarkup([
                [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready|{order_id}")],
            ])

        # قبل السداد: (انتظار) + تفعيل المراسلة/الاتصال
        if not goods_paid:
            return InlineKeyboardMarkup([
                [InlineKeyboardButton("⏳ بانتظار إيصال السداد من العميل", callback_data="pp_ui_close")],
                [InlineKeyboardButton("📞 الاتصال بالعميل", callback_data=f"pp_chat_open|{order_id}")],
            ])

        # بعد السداد: زرين فقط (تم الشحن + الاتصال)
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("🚚 تم شحن الطلب", callback_data=f"pp_trader_status|shipped|{order_id}")],
            [InlineKeyboardButton("📞 الاتصال بالعميل", callback_data=f"pp_chat_open|{order_id}")],
        ])

    # بعد الشحن: زر تأكيد التسليم + الاتصال (7 أيام حسب منطق الهاندر)
    if ost in ("shipped",):
        return InlineKeyboardMarkup([
    ...
    # بعد التسليم/الإغلاق: الاتصال فقط (إن كان متاحًا ضمن مدة المراسلة)
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ترجمة الزر إلى حالة داخلية للحفظ
    ...
    # ✅ سماح للتاجر المقبول فقط أو الادمن
    if actor_id not in ADMIN_IDS and actor_id != accepted_tid:
        return

    # ===== حالة دفع البضاعة (مفتاح الإظهار/المنع) =====
    gps = str(order.get("goods_payment_status") or "").strip().lower()
    goods_paid = gps in ("confirmed", "paid", "success", "successful", "done", "ok")
    ...
    # ===== تسلسل السيناريو الجديد (بدون أي تنبيهات) =====
    # 1) تجهيز: مسموح فقط بعد accepted/new
    if new_status == "preparing":
        if ost_now not in ("", "new", "accepted"):
            return

    # 2) جاهز للشحن: هذا الزر وظيفته فتح مرحلة رفع الفاتورة (الحالة الفعلية تتغير إلى ready_to_ship عند رفع الفاتورة)
    if new_status == "ready_to_ship":
        ...
        inv_file = (str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "")).strip()
        if not inv_file:
            # افتح مرحلة رفع فاتورة المتجر (إجباري)
            try:
            ...
            # ⏳ تفعيل مؤقت المراسلة 7 أيام من الآن (يبدأ بعد استخدام زر جاهز للشحن)
            try:
                if not (order.get("chat_expires_at_utc") or "").strip():
                    expires = datetime.now(timezone.utc) + timedelta(days=7)
                    update_order_fields(order_id, {"chat_expires_at_utc": expires.isoformat()})
            except Exception:
                pass

            # رسالة واحدة فقط للتاجر بالخاص لطلب الفاتورة (ضرورية)
            try:
                await context.bot.send_message(
                    chat_id=actor_id,
                    text=(
                        f"{_user_name(q)}\n"
                        "ارسل فاتورة المتجر الأصلية (PDF/صورة) لهذا الطلب هنا بالخاص:\n"
                        f"🧾 رقم الطلب: {order_id}"
                    ),
                    reply_markup=InlineKeyboardMarkup([
        ...
        # لو الفاتورة موجودة أصلاً: التحويل إلى ready_to_ship يتم من هاندر رفع الفاتورة
        return

    # 3) تم الشحن: ممنوع 100% قبل السداد + لازم تكون الحالة ready_to_ship
    ...
    # ===== نص الحالة =====
    display_status2 = _pay_status_ar(new_status)
    ...
    header_block = (
        f"🧾 رقم الطلب: {order_id}\n"
        f"👤 العميل: {(order.get('user_name') or order.get('client_name') or order.get('customer_name') or 'العميل')}\n"
        f"🧑‍💼 التاجر: {accepted_name}\n"
        f"📌 الحالة: {display_status2}\n"
    )
    if grand_total_line:
    ...
    for aid in ADMIN_IDS:
        try:
            admin_msg = ("📌 تحديث حالة من التاجر\n" + header_block + f"🧑‍💼 تم التحديث بواسطة: {_user_name(q)}").strip()
            if parts_block:
                admin_msg += "\n\n" + parts_block
    ...
    # إشعار للتاجر (واضح + الخطوة القادمة)
    try:
        await context.bot.send_message(
            chat_id=actor_id,
            text=(
                ...
                + header_block
                + (
                    "➡️ الخطوة التالية: اضغط (🟢 الطلب جاهز للشحن).\n"
                    if new_status in ("preparing", "prep") else
                    "➡️ الخطوة التالية: بعد تسليم العميل اضغط (✅ تأكيد التسليم بنجاح).\n"
                    if new_status == "shipped" else
                    "✅ تم تأكيد التسليم وإغلاق الطلب.\n"
                    if new_status in ("delivered", "closed") else
                    ""
                )
            ).strip(),
            reply_markup=trader_status_kb(order_id),
