...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _order_snapshot(order_id: str) -> dict:
        ...
        try:
            out["total_amount"] = str(oo.get("total_amount_sar") or oo.get("price_sar") or "").strip()
        except Exception:
            pass

        try:
            out["ship_included"] = str(oo.get("shipping_included") or "").strip()
        except Exception:
            pass

        return out

    def _hdr(order_id: str, snap: dict = None, goods_total: str = "") -> str:
        snap = snap or _order_snapshot(order_id)
        client_name = (snap.get("client_name") or "—").strip()
        goods_now = goods_total.strip() if str(goods_total or "").strip() else (snap.get("goods_amount") or "").strip()
        ...
        if goods_now:
            lines.append(f"💰 إجمالي القطع: {goods_now} ر.س")

        # الشحن/الإجمالي إن توفر
        ship_fee = _money_txt(snap.get("shipping_fee"))
        if ship_fee and ship_fee not in ("0 ر.س", "0.0 ر.س"):
            lines.append(f"🚚 الشحن: {ship_fee}")

        total_amt = _money_txt(snap.get("total_amount"))
        if total_amt and total_amt not in ("0 ر.س", "0.0 ر.س"):
            lines.append(f"🧾 الإجمالي: {total_amt}")
    ...
    if action == "ppq_begin":
        ...
        await q.message.reply_text(
            _hdr(order_id, snap=snap, goods_total=total_now) + "\n\n🧩 اختر القطعة لتسعيرها:",
    ...
    if action in ("ppq_it", "ppq_it_all", "ppq_it_none", "ppq_it_done"):
        ...
        if action == "ppq_it":
            ...
            await q.message.reply_text(
                _hdr(order_id, snap=snap, goods_total=total) +
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(sess, dict):
        ...
        if peer_id and order_id_sess and msg_body:
            ...
            # ✅ لا تكمل text_handler حتى لا يدخل في مسارات أخرى
            return
        ...
        if _support_should_close_by_time(ud):
            ...
            if cur_stage and cur_stage != STAGE_NONE:
                ...
                if msg:
                    ...
                    # محاولة التقاط رقم طلب كامل من الرسالة
                    try:
                    ...
                    # إذا ما فيه رقم كامل: جرّب آخر 4 أرقام
                    ...
                    # إذا ما فيه رقم: نرفض فقط للعميل، ونسمح للتاجر بدون رقم
    ...
    if stage == STAGE_TRADER_SET_QUOTE:
        ...
        def _fmt_money_local(v: object) -> str:
            ...
            if not s or s in ("0", "0.0"):
                return ""
            try:
                return _fmt_money(s)  # إذا عندك helper جاهز
            except Exception:
                try:
                    return _money(s)
                except Exception:
        ...
        def _quote_hdr(oid: str, goods_total: str = "") -> list:
            ...
            # إجمالي القطع: الأولوية لما حسبناه الآن، ثم المخزن
            ...
            total_amt = str((snap or {}).get("total_amount") or (snap or {}).get("total_amount_sar") or "").strip()
            ...
            if ship_txt and ship_txt not in ("0 ر.س", "0.0 ر.س"):
                lines.append(f"🚚 الشحن: <b>{html.escape(ship_txt)}</b>")

            tot_txt = _fmt_money_local(total_amt)
            if tot_txt and tot_txt not in ("0 ر.س", "0.0 ر.س"):
                lines.append(f"🧾 الإجمالي: <b>{html.escape(tot_txt)}</b>")
        ...
        # إغلاق تلقائي لو جلسة عرض السعر طالت
        try:
        ...
        # (قديم/احتياط) — يفضل يبقى لتوافق أي مسار قديم
        if step == "amount":
            m_amt = re.search(r"(\d+(?:\.\d+)?)", text)
            if not m_amt:
                await _reply_html(
