...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    ...
    try:
        ...
        pay_link = (
            str(order.get("seller_invoice_pay_link") or order.get("shop_invoice_pay_link") or "").strip()
        )
    ...
    if ost in ("ready_to_ship", "ready"):
        # قبل إدخال وسيلة الدفع (فاتورة/رابط): يبقى نفس الزر فقط
        if not (inv_file or pay_link):
            return InlineKeyboardMarkup([
                [InlineKeyboardButton("🟢 الطلب جاهز للشحن", callback_data=f"pp_trader_status|ready|{order_id}")],
            ])
        ...
        if not goods_paid:
            rows = [[InlineKeyboardButton("⏳ بانتظار سداد العميل لقيمة البضاعة", callback_data="pp_ui_close")]]
            # ✅ يسمح للتاجر بتعديل وسيلة الدفع (فاتورة/رابط) قبل السداد
            rows.append([InlineKeyboardButton("✏️ تعديل طريقة الدفع", callback_data=f"pp_tsu_payment_menu|{order_id}")])
...
async def _need_complete_trader_profile_notice(context: ContextTypes.DEFAULT_TYPE, user_id: int, name: str, order_id: str = "") -> bool:
    """
    إشعار اكتمال ملف التاجر قبل تقديم عرض السعر.
    - الحقول الأساسية إلزامية.
    - حقول الدفع (IBAN/STC/اسم البنك) اختيارية (لا تمنع تقديم العرض) لكن نذكّر التاجر بها حتى تظهر للعميل لاحقًا.
    Returns:
        True  -> يجب إيقاف التدفق (نواقص أساسية)
        False -> لا نوقف التدفق (مكتمل أساسياً، أو لا يوجد شيء)
    """
    ...
    # ✅ الحقول الأساسية (إلزامية للعروض)
    core_schema = [
    ...
    missing_core = []
    for k, label, ex in core_schema:
        v = (tp.get(k) or "").strip()
        if not v:
            missing_core.append((label, ex))

    missing_pay = []
    for k, label, ex in pay_schema:
        v = (tp.get(k) or "").strip()
        if not v:
            missing_pay.append((label, ex))

    # لا شيء ناقص
    if not missing_core and not missing_pay:
        return False

    def _fmt_missing(items):
        lines = []
        for i, (label, ex) in enumerate(items or [], start=1):
            lines.append(f"{i}) {label}\n   {ex}")
        return "\n".join(lines).strip()

    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🧑‍💼 فتح لوحة التاجر", url=panel_url)]])

    # 1) نواقص أساسية => منع تقديم العرض
    if missing_core:
        msg = (
            f"{name}\n"
            "⛔ لا يمكنك تقديم عرض سعر قبل إكمال بيانات التاجر الأساسية.\n\n"
            "🔻 الحقول الأساسية الناقصة لديك الآن:\n"
            + _fmt_missing(missing_core)
        )

        # تذكير الدفع (اختياري) إذا كان ناقصاً
        if missing_pay:
            msg += (
                "\n\nℹ️ بيانات الدفع اختيارية ولا تمنع تقديم العرض،"
                " لكن يُفضّل إضافتها لتظهر للعميل (IBAN / STC Pay) أثناء تدفق الطلب.\n"
                "🔸 بيانات الدفع غير المضافة حالياً:\n"
                + _fmt_missing(missing_pay)
            )

        msg += "\n\nافتح لوحة التاجر من الزر بالأسفل وأكمل البيانات، ثم ارجع وابدأ عرض السعر." + extra
    ...
    # 2) فقط الدفع ناقص => لا منع، مجرد تذكير
    if missing_pay:
        msg = (
            f"{name}\n"
            "ℹ️ بيانات الدفع غير مكتملة في لوحة التاجر.\n\n"
            "يمكنك تقديم عرض السعر بدونها،"
            " لكن لن تظهر للعميل خيارات التحويل/‏STC Pay تلقائياً إلا بعد إضافتها.\n\n"
            "🔸 بيانات الدفع غير المضافة حالياً:\n"
            + _fmt_missing(missing_pay)
            + "\n\n(اختياري) افتح لوحة التاجر وأضف بيانات الدفع الآن."
            + extra
        )
        try:
            await context.bot.send_message(
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all((tp.get(f) or "").strip() for f in required_fields):
        block = await _need_complete_trader_profile_notice(
            context,
            user_id,
            _user_name(q),
        ...
        if block:
            try:
                await q.message.reply_text("أكمل بيانات التاجر الأساسية ثم ارجع وابدأ عرض السعر.")
            except Exception as e:
                _swallow(e)
            return
    else:
        # ✅ حتى لو مكتمل أساسياً: قد تكون بيانات الدفع ناقصة (اختيارية) فنذكّره فقط بدون منع
        try:
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2) جاهز للشحن: فتح خيارات الدفع (فاتورة متجر / رابط دفع إلكتروني) في الخاص
    # ✅ لا نجبر التاجر على رفع فاتورة إذا كان سيستخدم رابط دفع
    # ✅ حتى لو كان الطلب مسبقًا (ready_to_ship) بعد إعادة تشغيل: نسمح بفتح القائمة دائمًا
    if new_status == "ready_to_ship":
        if ost_now not in ("preparing", "prep", "ready_to_ship", "ready"):
            return

        # ثبّت الحالة (آمن حتى لو هي نفسها)
        try:
            update_order_fields(order_id, {"order_status": "ready_to_ship"})
        except Exception as e:
            _swallow(e)

        inv_file = (str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "")).strip()
        pay_link = (str(order.get("seller_invoice_pay_link") or order.get("shop_invoice_pay_link") or "")).strip()

        # رسالة تعليمات مختصرة + زرين (يرجعون لنفس مرحلة الإدخال)
        try:
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🧾 إرسال فاتورة متجر (PDF/صورة)", callback_data=f"pp_trader_status|inv|{order_id}")],
                [InlineKeyboardButton("🔗 إرسال رابط دفع إلكتروني", callback_data=f"pp_trader_status|plink|{order_id}")],
                [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
            ])
            ...
            try:
                if pay_link:
                    hint_current = "✅ يوجد رابط دفع محفوظ حالياً لهذا الطلب (يمكنك استبداله).\n"
                elif inv_file:
                    hint_current = "✅ توجد فاتورة متجر محفوظة حالياً لهذا الطلب (يمكنك استبدالها).\n"
            except Exception:
                hint_current = ""

            msg_private = (
                "🟢 <b>الطلب جاهز للشحن</b>\n\n"
                ...
                + "\n"
                + f"{goods_line}\n{ship_line}\n{total_line}\n\n"
                + "➡️ <b>اختر طريقة الدفع</b>:\n"
                + "• <b>فاتورة متجر</b>: ارفع PDF/صورة (تُرسل للعميل).\n"
                + "• <b>رابط دفع</b>: أرسل الرابط (يُرسل للعميل بدون معاينة).\n\n"
                + (hint_current or "")
                + "🧩 ملاحظة: يمكنك استبدال أي فاتورة/رابط سابق في أي وقت قبل سداد العميل."
            )
        ...
        # حدّث كيبورد رسالة المجموعة/اللوحة ليتوافق مع الحالة الجديدة
        try:
...
# ==================================================
# ✅ إدارة رابط دفع التاجر (داخل مرحلة جاهز للشحن)
# - الاستمرار على الرابط السابق
# - استبدال الرابط وإرسال رابط جديد
# الهدف: منع لخبطة بعد إعادة التشغيل/وجود رابط محفوظ مسبقاً
# ==================================================

# ==================================================
# ✅ قائمة وسيلة الدفع للتاجر (فاتورة متجر / رابط دفع)
# الهدف: زر واحد واضح + استبدال مباشر لأي بيانات سابقة
# ==================================================
async def tsu_payment_menu_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await _alert(q, "")
    ...
    try:
        ud = get_ud(context, actor_id)
        ud["tsu_kind"] = "seller_invoice"
        ud["tsu_order_id"] = order_id
        ud.pop("tsu_paylink_replace", None)
        set_stage(context, actor_id, STAGE_TRADER_STATUS_UPDATE)
    except Exception as e:
        _swallow(e)

    client_name = "العميل"
    accepted_name = _user_name_from_q(q) or "التاجر"
    trader_store = "—"
    try:
        b = get_order_bundle(order_id) or {}
        o = (b.get("order") or {}) if isinstance(b, dict) else {}
        client_name = (str(o.get("user_name") or o.get("client_name") or o.get("customer_name") or "")).strip() or "العميل"
        try:
            tid = int(o.get("accepted_trader_id") or 0)
        except Exception:
            tid = 0
        try:
            ...
            tprof = {}
        accepted_name = (str(tprof.get("display_name") or "").strip() or accepted_name or "التاجر")
        trader_store = (str(tprof.get("company_name") or "").strip() or "—")
    except Exception:
        pass

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📄 إرسال فاتورة متجر (PDF/صورة)", callback_data=f"pp_tsu_invoice|{order_id}")],
        [InlineKeyboardButton("🔗 إرسال رابط دفع إلكتروني", callback_data=f"pp_tsu_paylink|{order_id}")],
        [InlineKeyboardButton("✖️ إغلاق", callback_data="pp_ui_close")],
    ])
    ...
    text = (
        ...
        f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>\n"
        f"👤 العميل: <b>{html.escape(client_name)}</b>\n"
        f"🧑‍💼 التاجر: <b>{html.escape(accepted_name)}</b>\n"
        f"🏪 المتجر: <b>{html.escape(trader_store)}</b>\n\n"
        "اختر أحد الخيارين ثم أرسل المطلوب:\n"
        "• 📄 فاتورة متجر: ارفع <b>PDF/صورة</b>.\n"
        "• 🔗 رابط دفع: ارسل <b>الرابط</b> كنص.\n\n"
        "✅ أي خيار جديد <b>يستبدل السابق مباشرة</b> لنفس الطلب."
    )
...
async def tsu_invoice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await _alert(q, "")
    ...
    try:
        await context.bot.send_message(
            chat_id=actor_id,
            text=(
                "📄 <b>إرسال فاتورة المتجر</b>\n\n"
                f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>\n"
                "ارسل الآن <b>فاتورة المتجر</b> كـ <b>PDF</b> أو <b>صورة</b>.\n"
                "✅ سيتم اعتمادها مباشرة واستبدال أي رابط/فاتورة سابقة لهذا الطلب."
            ),
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
...
async def tsu_paylink_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await _alert(q, "")
    ...
    try:
        await context.bot.send_message(
            chat_id=actor_id,
            text=(
                "🔗 <b>إرسال رابط دفع إلكتروني</b>\n\n"
                f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id)}</b>\n"
                "ارسل الآن <b>رابط الدفع</b> كنص (https://...).\n"
                "✅ سيتم اعتماده مباشرة واستبدال أي رابط/فاتورة سابقة لهذا الطلب."
            ),
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
...
async def tsu_paylink_keep_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await _alert(q, "")
    ...
    try:
        await q.message.reply_text("✅ تم. سيتم الاستمرار على رابط الدفع السابق لهذا الطلب.", disable_web_page_preview=True)
    except Exception:
        pass


async def tsu_paylink_replace_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    توافق خلفي: كان هذا الزر للاستبدال فقط.
    الآن نفس منطق (إرسال رابط دفع) مع الاستبدال المباشر.
    """
    await tsu_paylink_cb(update, context)
...
async def team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "pp_team_quote":
        ...
        required_fields = ["display_name","company_name","shop_phone","cr_no","vat_no"]
        if not all((tp.get(f) or "").strip() for f in required_fields):
            block = await _need_complete_trader_profile_notice(context, actor_id, _user_name(q), order_id)
            if block:
                await _alert(q, "تم إرسال طريقة التفعيل بالخاص", force=True)
                return
        else:
            # ✅ تذكير اختياري ببيانات الدفع (بدون منع)
            try:
...
async def media_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
        ...
        if not file_id:
            name = _user_name(update)
            await update.message.reply_text(f"{name}\nارسل فاتورة المتجر (PDF/صورة) أو ارسل رابط الدفع الإلكتروني كنص")
            return
        ...
        update_order_fields(order_id2, {
            ...
            # ✅ إذا كان يوجد رابط دفع سابق، نستبدله بهذه الفاتورة
            "seller_invoice_pay_link": "",
            "shop_invoice_pay_link": "",
            "seller_invoice_kind": "file",
            "shop_invoice_kind": "file",
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ==================================================
    # ✅ تحديث حالة التاجر: استقبال رابط دفع إلكتروني بدل فاتورة المتجر
    # نفس مرحلة رفع فاتورة التاجر لكن بالنص (URL)
    # ==================================================
    if stage == STAGE_TRADER_STATUS_UPDATE and (ud.get("tsu_kind") or "").strip() == "seller_invoice":
        order_id2 = (ud.get("tsu_order_id") or "").strip()
        if order_id2:
            ...
            url = ""
            try:
                murl = re.search(r"(https?://\S+)", raw_text or "")
                url = (murl.group(1) if murl else "").strip()
            except Exception:
                url = ""

            # ✅ إذا أرسل رابط: نعتمده كبديل للفاتورة (بدون ملفات)
            if url:

                # خزّن الرابط + تحديث الحالة
                try:
                    update_order_fields(order_id2, {
                        # ✅ نضع علامة حتى يعتبر النظام أن "الفاتورة" موجودة (بديل عبر رابط)
                        # هذا يمنع إعادة فتح زر (جاهز للشحن) بعد الإرسال ويثبت التدفق حتى سداد العميل.
                        "seller_invoice_file_id": "PAYLINK",
                        "seller_invoice_mime": "text/url",
                        "shop_invoice_file_id": "PAYLINK",
                        "shop_invoice_mime": "text/url",

                        "seller_invoice_pay_link": url,
                        "seller_invoice_kind": "pay_link",
                        "seller_invoice_at": utc_now_iso(),

                        # توافق خلفي
                        "shop_invoice_pay_link": url,
                        "shop_invoice_kind": "pay_link",
                        "shop_invoice_at": utc_now_iso(),
                ...
                # فاتورة منصة داخلية (للإدارة فقط)
                try:
                    await send_invoice_pdf(context, order_id2, kind="preliminary", admin_only=True)
                except Exception as e:
                    _swallow(e)

                # قراءة بيانات الطلب لبناء رسائل واضحة
                try:
                    b2 = get_order_bundle(order_id2) or {}
                    o2 = (b2.get("order") or {}) if isinstance(b2, dict) else {}
                except Exception:
                    o2 = {}

                client_id2 = 0
                try:
                    client_id2 = int(o2.get("user_id") or 0)
                except Exception:
                    client_id2 = 0
                ...
                try:
                    ...
                    tprof = {}
                tname = (str(tprof.get("display_name") or "").strip() or name or "التاجر")
                tcompany = (str(tprof.get("company_name") or "").strip())
                tlabel = tname + (f" ({tcompany})" if tcompany else "")

                # حساب مبالغ مختصر (قيمة القطع + الشحن)
                goods_str = str(o2.get("goods_amount_sar") or o2.get("goods_price") or o2.get("goods_amount") or "").strip()
                ship_str = str(o2.get("shipping_fee_sar") or o2.get("shipping_fee") or o2.get("ship_fee") or "").strip()
                ship_included = str(o2.get("ship_included") or "").strip().lower() in ("1", "yes", "true", "included", "with_goods")
                ...
                try:
                    ship_for_total = "0" if ship_included else (ship_str or "0")
                    _, _, tnum = _calc_totals(goods_str or "0", ship_for_total or "0")
                    total_txt = _money_txt(tnum) or ""
                except Exception:
                    total_txt = ""

                money_lines = []
                if goods_txt and goods_txt != "—":
                    money_lines.append(f"💰 قيمة القطع: {goods_txt}")
                if ship_txt:
                    money_lines.append(f"🚚 رسوم الشحن: {ship_txt}")
                if total_txt:
                    money_lines.append(f"🧾 الإجمالي: {total_txt}")
                money_block = ("\n".join(money_lines)).strip()
                if money_block:
                    money_block = "\n" + money_block + "\n"
                # ✅ عندما يكون الدفع عبر رابط: نثبت طريقة الدفع (رابط) وننقل العميل مباشرة لمرحلة إرسال الإيصال
                if client_id2:
                    try:
                ...
                # 1) رسالة للعميل (رابط الدفع)
                if client_id2:
                    try:
                        cap_client = (
                            "💳 <b>رابط دفع المتجر</b>\n\n"
                            f"🧾 رقم الطلب: <b>{_order_id_link_html(order_id2)}</b>\n"
                            f"👤 العميل: <b>{html.escape(client_name2)}</b>\n"
                            f"🧑‍💼 التاجر: <b>{html.escape(tlabel)}</b>\n"
                            f"🚗 السيارة: <b>{html.escape(car_name2)}</b>\n"
                            f"📌 الحالة: <b>{_pay_status_ar('ready_to_ship')}</b>\n"
                            + (f"🛠 مدة التجهيز: <b>{html.escape(availability2)}</b>\n" if availability2 else "")
                            + (f"⏱ مدة الشحن: <b>{html.escape(ship_eta2)}</b>\n" if ship_eta2 else "")
                            + money_block
                            + f"🔗 <b>رابط دفع إلكتروني</b>: <a href=\"{html.escape(url, quote=True)}\">اضغط هنا</a>\n\n"
                            "⬇️ بعد السداد ارسل <b>صورة/ملف إيصال الدفع</b> هنا لإكمال الشحن."
                        )
                        await context.bot.send_message(
                            chat_id=client_id2,
                            text=cap_client,
                ...
                await _reply("✅ تم تسجيل رابط الدفع وإرساله للعميل. الآن بانتظار إيصال السداد.")
                return
...
def build_app():
    ...
    app.add_handler(CallbackQueryHandler(tsu_payment_menu_cb, pattern=r"^pp_tsu_payment_menu\|"))
    app.add_handler(CallbackQueryHandler(tsu_invoice_cb, pattern=r"^pp_tsu_invoice\|"))
    app.add_handler(CallbackQueryHandler(tsu_paylink_cb, pattern=r"^pp_tsu_paylink\|"))
    app.add_handler(CallbackQueryHandler(tsu_paylink_keep_cb, pattern=r"^pp_tsu_paylink_keep\|"))
    app.add_handler(CallbackQueryHandler(tsu_paylink_replace_cb, pattern=r"^pp_tsu_paylink_replace\|"))
