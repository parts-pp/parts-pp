...
async def send_trader_ledger_pdf(
    ...
    trader_id: int,
    admin_chat_id: int,
    max_orders_table: int = 15,
):
    '''
    PDF "سجل التاجر" (ثيم برتقالي) للأدمن فقط:
    - كل حقول التاجر (بما فيها السجل/الضريبي/هاتف المتجر/تاريخ الانضمام)
    - حالة التفعيل + حالة الاشتراك (هذا الشهر)
    - ملخص الطلبات (منجزة/معلقة) + الإجماليات
    - جدول آخر N طلبات (مختصر)
    '''
    import os, re, tempfile
    from datetime import datetime, timezone

    # --- Arabic shaping (آمن: إذا فشل يطبع النص كما هو) ---
    try:
        import arabic_reshaper
        from bidi.algorithm import get_display
    ...
    # --- Load trader profile ---
    try:
        prof = get_trader_profile(int(trader_id or 0)) or {}
    except Exception:
        prof = {}
    ...
    shop_phone = (prof.get("shop_phone") or prof.get("phone") or "").strip()
    cr_no = (prof.get("cr_no") or prof.get("cr_number") or "").strip()
    vat_no = (prof.get("vat_no") or prof.get("vat_number") or "").strip()
    joined = (prof.get("joined_at_utc") or prof.get("created_at_utc") or "").strip()
    upd = (prof.get("updated_at_utc") or "").strip()

    # --- Enabled status ---
    try:
        enabled = is_trader_enabled(int(trader_id))
    except Exception:
        enabled = True
    ...
    try:
        ...
        for s in subs:
            try:
                if int(s.get("trader_id") or 0) != int(trader_id):
                    continue
            except Exception:
                continue
            ...
            if stv == "confirmed":
                ...
                sub_status = "متأخر"
            try:
                sub_amt = _fmt_money(s.get("amount_sar") or "")
            except Exception:
                sub_amt = ""
            break
    except Exception:
    ...
    # --- Orders stats ---
    try:
        orders = list_orders_for_trader(int(trader_id)) or []
    except Exception:
        orders = []
    ...
    def _status_done(o: dict) -> bool:
        st = str(o.get("order_status") or o.get("status") or "").strip().lower()
        return st in ("closed", "delivered")
    ...
    for o in orders:
        if _status_done(o):
            ...
            pending_orders += 1
        sum_goods += _safe_float(o.get("goods_amount_sar") or o.get("price_sar") or 0)
        sum_shipping += _safe_float(o.get("shipping_fee_sar") or 0)
    ...
    table_orders = orders_sorted[: max(0, int(max_orders_table or 0))]

    fd, out_path = tempfile.mkstemp(prefix=f"pp_trader_{trader_id}_", suffix=".pdf")
    os.close(fd)
    ...
    ORANGE = colors.Color(0.95, 0.45, 0.05)
    ORANGE_DARK = colors.Color(0.80, 0.30, 0.05)
    GRAY = colors.Color(0.25, 0.25, 0.25)
    ...
    c.rect(margin, y, w - 2 * margin, 32, stroke=0, fill=1)
    c.setFillColor(colors.white)
    c.setFont(font_name, 16)
    c.drawRightString(w - margin - 10, y + 10, _ar("سجل التاجر"))
    c.setFont(font_name, 10)
    c.drawString(margin + 10, y + 10, f"TRADER ID: {trader_id}")
    ...
    def section(title: str):
        ...
        c.drawRightString(w - margin, y, _ar(title))
        y -= 18
    ...
    def kv(rows):
        ...
        for lab, val in rows:
            if y < 120:
                ...
                y = h - 60
            c.setFillColor(GRAY)
            c.drawRightString(label_x, y, _ar(lab))
            c.setFillColor(colors.black)
            c.drawString(value_x, y, _ar(val if val else "—"))
            y -= line_h
        y -= 6

    section("معلومات التاجر")
    kv([
        ("اسم التاجر", tname),
        ("اسم المتجر", tcompany),
        ("رقم اتصال المتجر", shop_phone),
        ("رقم السجل التجاري", cr_no),
        ("الرقم الضريبي", vat_no),
        ("اسم البنك", bank),
        ("رقم الايبان", iban),
        ("STC Pay", stc),
        ("تاريخ الانضمام", _iso_to_local_str(joined)),
        ("آخر تحديث", _iso_to_local_str(upd)),
    ])

    section("التفعيل والاشتراك")
    sub_line = sub_status + (f" — {sub_amt} ر.س" if sub_amt else "")
    kv([
        ("حالة الحساب", enabled_txt),
        ("اشتراك الشهر", month),
        ("حالة الاشتراك", sub_line),
    ])

    section("ملخص الطلبات")
    kv([
        ("إجمالي الطلبات", str(total_orders)),
        ("طلبات منجزة", str(done_orders)),
        ("طلبات معلقة", str(pending_orders)),
        ("إجمالي قيمة القطع", f"{_fmt_money(sum_goods)} ر.س"),
        ("إجمالي الشحن", f"{_fmt_money(sum_shipping)} ر.س"),
        ("الإجمالي", f"{_fmt_money(sum_goods + sum_shipping)} ر.س"),
    ])

    section(f"آخر {len(table_orders)} طلب")
    if y < 180:
        c.showPage()
        y = h - 60

    c.setFillColor(colors.whitesmoke)
    c.rect(margin, y - 18, w - 2 * margin, 20, stroke=0, fill=1)
    c.setFillColor(GRAY)
    c.setFont(font_name, 10)

    col_x = [margin, margin + 150, margin + 260, margin + 360, margin + 455]
    headers = ["رقم الطلب", "التاريخ", "الحالة", "قيمة القطع", "الشحن"]
    for i, head in enumerate(headers):
        c.drawString(col_x[i] + 2, y - 14, _ar(head))
    y -= 26
    ...
    for o in table_orders:
        ...
        dt_s = _iso_to_local_str(str(o.get("created_at_utc") or ""))
        st = str(o.get("order_status") or o.get("status") or "—").strip()
        goods = _fmt_money(o.get("goods_amount_sar") or o.get("price_sar") or 0)
        ship = _fmt_money(o.get("shipping_fee_sar") or 0)

        c.drawString(col_x[0] + 2, y, oid)
        c.drawString(col_x[1] + 2, y, dt_s)
        c.drawString(col_x[2] + 2, y, _ar(st))
        c.drawString(col_x[3] + 2, y, goods)
        c.drawString(col_x[4] + 2, y, ship)
        y -= 14

    c.setFillColor(GRAY)
    c.setFont(font_name, 9)
    c.drawCentredString(w / 2, 30, _ar("الخدمات المساندة GO — منصة PP"))
    c.save()

    try:
        cap = f"🧾 سجل التاجر — {tname or trader_id}"
        await context.bot.send_document(
            chat_id=int(admin_chat_id),
            document=open(out_path, "rb"),
            filename=f"trader_{trader_id}_ledger.pdf",
            caption=cap,
        )
    finally:
        try:
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "tledgerpdf":
        ...
        # يولد PDF ويرسله للأدمن فقط
        try:
            await send_trader_ledger_pdf(context, trader_id=tid, admin_chat_id=int(q.from_user.id))
            await _toast("✅ تم إرسال سجل التاجر PDF")
        except Exception as e:
            _swallow(e)
            await _pop("❌ تعذر إنشاء سجل التاجر")
        return
