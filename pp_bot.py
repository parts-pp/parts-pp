...
async def admin_chat_client_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✖️ إنهاء المراسلة", callback_data="pp_admin_chat_done")]
        ]),
        disable_web_page_preview=True,
    )
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    allow_search = (stage == STAGE_NONE) or (user_id in (ADMIN_IDS or []))

    if allow_search and (stage not in BLOCK_SEARCH_STAGES or (user_id in (ADMIN_IDS or []))):        if pr.get("kind") == "tail":
