...
def _pay_status_ar(v: object) -> str:
    """ترجمة موحّدة لحالات الدفع وحالات الطلب (تظهر في الرسائل/الفواتير/الأزرار)."""
    ...
    # fallback: إذا كانت قيمة عربية جاهزة أو حالة غير معروفة نعيدها كما هي
...
def trader_status_kb(order_id: str) -> InlineKeyboardMarkup:
    # ✅ لوحة ديناميكية: تظهر الأقفال حسب مرحلة الطلب لمنع اللبس
    try:
        b = get_order_bundle(str(order_id or "").strip())
    ...
    done = ost in ("delivered", "closed")
    allow_prep = not done and ost not in ("shipped",)
    allow_ready = not done and ost in ("preparing", "ready_to_ship")
    allow_shipped = (not done) and (ost == "ready_to_ship") and bool(inv_file) and (gps in ("confirmed", "paid", "done", "success"))
    allow_delivered = (not done) and (ost == "shipped")

    def _lbl(ok: bool, txt: str) -> str:
        return txt if ok else ("🔒 " + txt.replace("🟡 ", "").replace("🟢 ", "").replace("🚚 ", "").replace("✅ ", "").replace("💬 ", "").strip())
...
async def send_invoice_pdf(
    ...
    def _pay_status_ar(x: object) -> str:
        ...
        # لو النص عربي جاهز نرجعه كما هو
...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بعد الشحن: نقفل (تجهيز/جاهز/تم الشحن) ونسمح فقط (تم التسليم + مراسلة)
    if ost_now in ("shipped", "delivered", "closed") and new_status in ("preparing", "ready_to_ship", "shipped"):
        try:
            await _alert(
                q,
                (
                    "🔒 هذا الإجراء منتهي لهذا الطلب.\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"{grand_total_line}\n"
            ...
            try:
                await q.answer("🔒 هذا الإجراء منتهي", show_alert=True)
            except Exception:
                pass
        return

    # بعد التسليم/الإغلاق: أي تحديث حالة يعتبر منتهي
    if ost_now in ("delivered", "closed"):
        try:
            await _alert(
                q,
                (
                    "🔒 الطلب منتهي ولا يمكن تحديث حالته.\n"
                    f"🧾 رقم الطلب: {order_id}\n"
                    f"{grand_total_line}\n"
                    f"📌 الحالة الحالية: {_pay_status_ar(ost_now)}"
                ).strip(),
            ...
            try:
                await q.answer("🔒 الطلب منتهي", show_alert=True)
            except Exception:
                pass
        return

    
    # ✅ حارس تسلسلي: منع تخطي الخطوات + توجيه التاجر للزر المطلوب
    ...
    async def _need_step(required_btn: str, extra: str = "") -> bool:
        msg = (
            f"⚠️ لا يمكن اختيار ({display_status}) الآن\n"
            f"🧾 رقم الطلب: {order_id}\n"
            f"ابدأ أولاً بخطوة: {required_btn}"
        )
        if extra:
            msg += f"\n\n{extra}"
        try:
            ...
            pass
        try:
            # رسالة ثابتة بالخاص (تساعد من لا ينتبه للتنبيه)
            await context.bot.send_message(chat_id=actor_id, text=msg, disable_web_page_preview=True)
        except Exception:
            pass
        return True

    # ✅ الطلب منتهي: لا يسمح بتحديثات جديدة
    if current_status in ("delivered", "closed"):
        await _alert(q, "🔒 الطلب منتهي")
        return

    # ✅ (جاهز للشحن) لا يأتي إلا بعد (تجهيز الطلب)
    if new_status == "ready_to_ship" and current_status not in ("preparing", "ready_to_ship"):
        if await _need_step("🟡 جاري تجهيز الطلب"):
            return

    # ✅ (تم الشحن) لا يأتي إلا بعد (جاهز للشحن) + فاتورة + تأكيد سداد العميل
    if new_status == "shipped":
        if current_status != "ready_to_ship":
            if await _need_step("🟢 الطلب جاهز للشحن"):
                return

        # يلزم رفع فاتورة المتجر قبل الشحن
        inv_file_guard = (str(order.get("seller_invoice_file_id") or order.get("shop_invoice_file_id") or "")).strip()
        if not inv_file_guard:
            if await _need_step("🟢 الطلب جاهز للشحن", "🧾 لازم ترفع فاتورة المتجر أولاً ثم أكمل (تم الشحن)."):
                return

        # يلزم تأكيد دفع العميل (للقطع/الشحن) قبل اعتماد الشحن
        if gps not in ("confirmed", "paid", "done", "success"):
            if await _need_step("💳 انتظار سداد العميل", "📌 لا يمكن اعتماد (تم الشحن) قبل تأكيد سداد العميل وإرسال الإيصال للتاجر."):
                return

    # ✅ (تم التسليم) لا يأتي إلا بعد (تم الشحن)
    if new_status == "delivered" and current_status != "shipped":
        if await _need_step("📦 تم الشحن"):
            return
...
# # ✅ شرط الفاتورة قبل (جاهز للشحن) فقط
    ...
    # ✅ (1) المطلوب الجديد: عند (تم الشحن) اطلب رقم التتبع أو تخطي ثم نكمل الإشعار
    if new_status == "shipped":
        # إذا لم يتم طلب التتبع بعد ولم يتم تسجيل رقم تتبع مسبقًا => افتح مرحلة إدخال تتبع + زر تخطي
        try:
    ...
    # ✅ بعد اعتماد (تم الشحن): تحديث بصري للوحة الطلب => إبقاء (تسليم + مراسلة) فقط
    if new_status == "shipped":
        try:
    ...
    def _load_item_prices_map_from_order(o: dict) -> dict:
        """
        يقرأ الأسعار من الحقول داخل الطلب.
        يدعم:
        - dict مباشر
        - JSON صحيح
        - نص dict بايثون مثل: {'1': '50'}  (الأكثر شيوعًا عندك)
        """
        pm = {}
        try:
            import ast
    ...
    pm = _load_item_prices_map_from_order(order)  # ✅ أهم سطر: يدعم dict-text
    ...
    if client_id:
        try:
            msg = (
                f"📦 تحديث حالة الطلب رقم {order_id}\n"
                f"الحالة: {display_status}\n"
            )
            if grand_total_line:
                msg += f"{grand_total_line}\n"
            if parts_block:
                msg += "\n" + parts_block
    ...
    for aid in ADMIN_IDS:
        try:
            ...
            if grand_total_line:
                admin_msg += f"{grand_total_line}\n"
            admin_msg += f"التاجر: {_user_name(q)}"

            if parts_block:
                admin_msg += "\n\n" + parts_block
            await context.bot.send_message(chat_id=aid, text=admin_msg, disable_web_page_preview=True)
        except Exception:
            pass
