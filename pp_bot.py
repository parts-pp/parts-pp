...
async def _show_order_panel_private(context: ContextTypes.DEFAULT_TYPE, viewer_id: int, order_id: str):
    ...
    def _payment_status_display(o: dict) -> str:
        raw = (
            o.get("goods_payment_status")
            or o.get("payment_status")
            or o.get("quote_status")
            or ""
        )
        v = _s(raw).lower()
        ...
        mapping = {
            ...
            "awaiting_confirm": "بانتظار تأكيد الدفع",
            "processing": "قيد المعالجة",
        ...
        return mapping.get(v, _pay_status_ar(raw) or "—")

    def _is_payment_unlocked(o: dict) -> bool:
        """
        فتح رقم التواصل للتاجر بعد الدفع فقط.
        """
        vals = [
            _s(o.get("goods_payment_status")).lower(),
            _s(o.get("payment_status")).lower(),
            _s(o.get("goods_payment_confirmed_at_utc")),
            _s(o.get("payment_confirmed_at_utc")),
            _s(o.get("goods_receipt_file_id")),
            _s(o.get("receipt_file_id")),
        ]

        if vals[0] in ("confirmed", "paid"):
            return True
        if vals[1] in ("confirmed", "paid"):
            return True
        if vals[2] or vals[3] or vals[4] or vals[5]:
            return True
        return False
    ...
    # التاجر يرى المدينة والعنوان دائمًا، والهاتف بعد الدفع فقط
...
def build_order_legal_message(order_id: str, viewer_id: int) -> tuple[str, InlineKeyboardMarkup | None]:
    ...
    # حالة الدفع
    payment_status_ar = _pay_status_ar(o.get("goods_payment_status") or o.get("payment_status") or "") or "—"
