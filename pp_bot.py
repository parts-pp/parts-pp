...
async def send_trader_ledger_pdf(
    ...
    try:
        ...
        def _ar(s: str) -> str:
            ...
            if not s:
                return s
            # ✅ تجنّب تخريب وسوم reportlab (<b>..</b> وغيرها)
            # بعض بيئات Render قد يتوفر فيها arabic_reshaper/bidi مما يسبب كسر الـ Paragraph عند وجود وسوم.
            if "<" in s and ">" in s:
                return s
            try:
    ...
    inv_no = f"TL-{tid}-{now.strftime('%y%m%d')}"
    platform_bar = "منصة قطع غيار PARTS"
    ...
    base = ParagraphStyle("base", parent=ss["Normal"], fontName=font_name, fontSize=9.6, leading=12.0, textColor=colors.HexColor("#0B1220"))
    ...
    C_DARK    = colors.HexColor("#9A3412")   # orange/brown
    ...
    badges = Table([[
        P(f"رقم السجل: <b>{inv_no}</b>", tiny_c),
        P(f"التاريخ: <b>{now.strftime('%Y-%m-%d')}</b>", tiny_c),
        P(f"الوقت: <b>{now.strftime('%H:%M UTC')}</b>", tiny_c),
    ]], colWidths=[full_w/3.0]*3)
    ...
    def section_header(title: str):
        t = Table([[P(f"<b>{title}</b>", ParagraphStyle("sec", parent=right, textColor=colors.white, fontSize=9.8, leading=12.0, fontName=font_name))]], colWidths=[full_w])
    ...
    def kv_table(rows: list[tuple[str, str]]):
        data = []
        for k, v in rows:
            data.append([P(f"<b>{k}</b>", right), P(v if v else "—", left)])
        t = Table(data, colWidths=[full_w*0.34, full_w*0.66])
        t.setStyle(TableStyle([
            ("BOX", (0, 0), (-1, -1), 0.8, C_BORDER),
    ...
    kv_table([
        ...
        ("الاسم", label),
        ("رقم المتجر", shop_phone or "—"),
        ("السجل التجاري", cr_no or "—"),
    ...
    tbl = [[P("<b>رقم الطلب</b>", center), P("<b>التاريخ</b>", center), P("<b>الحالة</b>", center), P("<b>قيمة القطع</b>", center), P("<b>الشحن</b>", center)]]
    for o in last15:
    ...
    # تلوين صفوف بالتناوب (مثل الفواتير)
    try:
        for r in range(1, len(tbl)):
            t.setStyle(TableStyle([
                ("BACKGROUND", (0, r), (-1, r), colors.HexColor(ROW_TINT1 if r % 2 == 1 else ROW_TINT2)),
            ]))
    except Exception:
        pass
    ...
    # ===== watermark on pages (same style as invoices) =====
    def _wm(canvas, docx):
        try:
    ...
    try:
        with open(pdf_path, "rb") as f:
            await context.bot.send_document(
                ...
                caption=f"🧾 سجل التاجر: {tname or tid}",
            )
    except Exception as e:
        try:
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "tview":
        ...
        # ✅ التصحيح: إرسال الرسالة هنا داخل فرع tview
    ...
    if action == "tledgerpdf":
        ...
        # ✅ يولّد PDF "سجل التاجر" ويرسله للأدمن فقط
        try:
