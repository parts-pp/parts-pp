...
async def send_trader_ledger_pdf(
    ...
    # =========================
    # ✅ (01) BADGES FIX (نهائي):
    # - عمود الكلمة ضيق + عمود القيمة واسع -> تقارب شديد
    # - إزالة رموز LRM الظاهرة واستبدالها بـ Isolate (FSI/PDI) غير مرئي
    # =========================
    FSI = "\u2068"
    PDI = "\u2069"

    def _val_iso(x: str) -> str:
        s = "" if x is None else str(x)
        s = s.strip()
        return f"{FSI}{s}{PDI}" if s else "—"

    trader_name = (tname or str(tid)).strip() or "—"
    trader_value = trader_name
    if tuser:
        trader_value = f"{trader_name}  {_val_iso(tuser)}"

    time_value = _val_iso(now.strftime("%H:%M"))
    date_value = _val_iso(now.strftime("%Y-%m-%d"))

    # ✅ widths: (قيمة عريضة + كلمة ضيقة) × 3
    VAL_W = full_w * 0.2333333333
    LBL_W = full_w * 0.10
