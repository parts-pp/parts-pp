...
async def trader_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ اسم العميل من الطلب (بدون كسر/تعطيل)
    client_name = ""
    try:
        client_name = (order.get("user_name") or order.get("client_name") or order.get("customer_name") or order.get("name") or "").strip()
    except Exception:
        client_name = ""
    if not client_name:
        client_name = f"عميل #{client_id}" if client_id else "العميل"
    ...
    # ✅ اسم التاجر المعتمد (لجميع الإشعارات)
    trader_display_name = accepted_name
    ...
    # ✅ هيدر موحد لازم يدخل في كل إشعار (عميل/تاجر/إدارة)
    def _notify_header(status_ar: str) -> str:
        out = []
        out.append(f"🧾 رقم الطلب: {order_id}")
        out.append(f"👤 العميل: {client_name}")
        out.append(f"🧑‍💼 التاجر: {trader_display_name}")
        out.append(f"📌 الحالة: {status_ar}")
        if grand_total_line:
            out.append(grand_total_line)
        return "\n".join(out).strip()

    # ✅ سماح للتاجر المقبول فقط او الادمن
    if actor_id not in ADMIN_IDS and actor_id != accepted_tid:
        try:
            ost_now0 = str(order.get("order_status") or "").strip().lower()
        except Exception:
            ost_now0 = ""
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"{_notify_header(_pay_status_ar(ost_now0))}\n\n"
                    "تم إسناد الطلب إلى تاجر آخر لذلك تم تجميد أزرار تحديث الحالة عن حسابك.\n\n"
    ...
    # ===== ✅ حارس تسلسل الأزرار (Popup فقط) =====
    # تجهيز -> جاهز للشحن -> تم الشحن -> (تأكيد التسليم + مراسلة العميل)
    if new_status == "ready_to_ship" and ost_now not in ("preparing", "prep"):
        await _alert(
            ...
            (
                ...
                f"{_notify_header(_pay_status_ar(ost_now))}"
            ).strip(),
            force=True
        )
        return

    if new_status == "shipped" and ost_now not in ("ready_to_ship", "ready"):
        await _alert(
            q,
            (
    ...
    if new_status == "delivered" and ost_now not in ("shipped",):
        await _alert(
            q,
            (
                "⚠️ لا يمكن اختيار (✅ تم تسليم الطلب) قبل تنفيذ (🚚 تم شحن الطلب).\n\n"
                f"{_notify_header(_pay_status_ar(ost_now))}"
            ).strip(),
            force=True
        )
        ...
        if (not already_tracking) and (not already_requested):
            ...
            try:
                await q.message.reply_text(
                    ...
                    f"{_notify_header(display_status)}\n\n"
