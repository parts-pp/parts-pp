...
async def _is_trader_profile_strict(context: ContextTypes.DEFAULT_TYPE) -> bool:
    try:
...
async def start_trader_deeplink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # فحص ملف التاجر (موجود للاتساق حتى لو ما استخدمناه هنا)
    try:
        _ = get_trader_profile(int(user_id or 0)) or {}
    except Exception:
        pass

    # ✅ افتح لوحة التاجر مباشرة (بدون رسائل إضافية تسبب إرباك)
    set_stage(context, user_id, STAGE_NONE)
    try:
        await show_trader_panel(update, context, user_id)
    except Exception:
        try:
            await update.message.reply_text(f"{name}\nتعذر فتح لوحة التاجر حاليًا")
        except Exception as e:
            _swallow(e)
...
async def ppq_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ جلب ملف التاجر + تحديد النواقص بشكل صحيح (حتى لو فشل الجلب)
    try:
        tp = get_trader_profile(int(user_id or 0)) or {}
    except Exception as e:
        _swallow(e, "ppq_tp")
    ...
    missing = _trader_profile_missing(tp, strict=bool(strict))
    if missing:
        # ✅ بدون رسائل إضافية مزعجة: تنبيه فقط + منع تقديم العرض حتى يكمل البيانات
        try:
            miss_labels = [lbl for _, lbl in (missing or [])][:5]
            miss_txt = "، ".join(miss_labels) if miss_labels else "—"
            await _alert(
                q,
                "⛔ أكمل بيانات لوحة التاجر أولاً.\n"
                "يرجى إدخال البيانات الناقصة ثم إعادة المحاولة.",
                force=True,
            )
        except Exception as e:
            _swallow(e)
            try:
                await q.answer("⛔ أكمل بيانات لوحة التاجر أولاً", show_alert=True)
            except Exception:
                pass
        return
...
if action == "tstrict":
    ...
    # ✅ تنفيذ صامت: بدون نافذة Alert مزعجة — تنبيه صغير + رسالة للأدمن فقط
    try:
        await q.answer(
                ...
                TableStyle(
                    [
                        (
                            ...
                            (-1, -1),
                            [
                                colors.Color(0.95, 0.55, 0.10, alpha=0.10),
                                colors.Color(0.95, 0.55, 0.10, alpha=0.05),
                            ],
                        ),
...
def trader_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "🏷️ تعديل السجل التجاري", callback_data="pp_tprof|edit|cr_number"
                )
            ],
            [
                InlineKeyboardButton(
                    "🧾 تعديل الرقم الضريبي", callback_data="pp_tprof|edit|vat_number"
                )
            ],
            [
                InlineKeyboardButton(
...
async def show_trader_panel(
    ...
    # ✅ وضع الإلزام (من لوحة الأدمن): إذا مفعل => حقول الهوية/السجل/الضريبي تصبح مطلوبة
    strict = False
    try:
        _tmp = _is_trader_profile_strict(context)
        strict = await _tmp if asyncio.iscoroutine(_tmp) else _tmp
    ...
    national_id = (tp.get("national_id") or "").strip()
    cr_number = (tp.get("cr_number") or "").strip()
    vat_number = (tp.get("vat_number") or "").strip()

    def _val_or_missing(v: str) -> str:
        if v:
            return html.escape(v)
        return "⛔ غير مدخل" if strict else "اختياري"
    ...
    txt = (
        ...
        f"🪪 رقم الهوية: <b>{_val_or_missing(national_id)}</b>\n"
        f"🏷️ السجل التجاري: <b>{_val_or_missing(cr_number)}</b>\n"
        f"🧾 الرقم الضريبي: <b>{_val_or_missing(vat_number)}</b>\n"
