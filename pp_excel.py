...
def update_order_status(order_id: str, status: str, path: str = DEFAULT_PATH) -> None:
    """
    Backward/Forward compatible status update.
    - Legacy code used column: status
    - New flow uses column: order_status
    We write BOTH to avoid silent mismatches.
    """
    update_order_fields(order_id, {"status": status, "order_status": status}, path=path)
...
def generate_order_id(prefix: str = "PP", path: str = DEFAULT_PATH) -> str:
    ...
    # ✅ local date (KSA, fixed +03:00) for the visible part of the ID
    ...
    try:
        ...
        if "key" in headers and "value" in headers:
            ...
            if col_upd:
                # يبقى UTC كما هو (مجرد طابع زمني للتحديث في الإعدادات)
...
def get_trader_profile(trader_id: int, path: str = DEFAULT_PATH) -> dict:
    ...
    trader_id, display_name, company_name, shop_phone, cr_no, vat_no, bank_name, iban, stc_pay,
