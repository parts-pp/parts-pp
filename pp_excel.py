...
TRADERS_HEADERS = [
    ...
    # ✅ حقول اختيارية (غير إلزامية الآن) — جاهزة للتفعيل لاحقًا عبر Strict Mode
    "national_id",     # رقم الهوية
    "cr_number",       # رقم السجل التجاري
    "vat_number",      # الرقم الضريبي

    "created_at_utc",  # تاريخ الانضمام (يُثبت أول مرة)
    "is_enabled",
    "updated_at_utc",
]
SETTINGS_HEADERS = [
...
def generate_order_id(prefix: str = "PP", path: str = DEFAULT_PATH) -> str:
    ...
    ✅ NEW BEHAVIOR (Fix): the numeric sequence does NOT reset daily.
    ...
    Backward-safe:
    - If old orders exist (<= 9) and there is no stored counter yet, next number starts from 0010.
    - Uses settings key: order_seq (stores last used global seq).
    """
    SEQ_KEY = "order_seq"
    ...
    # local date for the visible part of the ID
    today = datetime.now(timezone.utc).astimezone().strftime("%y%m%d")

    # 1) read stored global sequence from settings
    stored_seq = 0
    try:
        ws_set = wb[SHEET_SETTINGS]
        headers = [str(ws_set.cell(row=1, column=c).value or "").strip() for c in range(1, ws_set.max_column + 1)]
        if "key" in headers and "value" in headers:
            col_key = headers.index("key") + 1
            col_val = headers.index("value") + 1
            for r in range(2, ws_set.max_row + 1):
                k = str(ws_set.cell(row=r, column=col_key).value or "").strip()
                if k == SEQ_KEY:
                    v = str(ws_set.cell(row=r, column=col_val).value or "").strip()
                    try:
                        stored_seq = int(re.sub(r"[^0-9]+", "", v) or 0)
                    except Exception:
                        stored_seq = 0
                    break
    except Exception:
    ...
    # 2) recover max sequence from existing orders (works even after restore/backup)
    max_existing = 0
    pat_any_day = re.compile(rf"^{re.escape(prefix)}-\d{{6}}-(\d+)$")
    for row in range(2, ws.max_row + 1):
        oid = str(ws.cell(row=row, column=order_col).value or "").strip()
        m = pat_any_day.match(oid)
        if not m:
            continue
        try:
    ...
    # 3) choose next seq (start from 10 if legacy orders are <= 9)
    base = max(stored_seq, max_existing, 9)
    seq = base + 1

    # 4) persist updated seq back to settings (same workbook, atomic save)
    try:
        ws_set = wb[SHEET_SETTINGS]
        headers = [str(ws_set.cell(row=1, column=c).value or "").strip() for c in range(1, ws_set.max_column + 1)]
        if "key" in headers and "value" in headers:
            ...
            if target_row is None:
                target_row = ws_set.max_row + 1
                ws_set.cell(row=target_row, column=col_key).value = SEQ_KEY

            ws_set.cell(row=target_row, column=col_val).value = str(seq)
            if col_upd:
                ws_set.cell(row=target_row, column=col_upd).value = datetime.now(timezone.utc).isoformat()
    except Exception:
        pass

    _save_and_close(wb, path)

    seq_str = str(int(seq)).zfill(4)  # expands automatically when seq > 9999
...
def get_trader_profile(trader_id: int, path: str = DEFAULT_PATH) -> dict:
    """Return trader profile dict. Keys: trader_id, display_name, company_name, bank_name, iban, stc_pay."""
    ...
    for r in range(2, ws.max_row + 1):
        ...
        try:
            if int(v or 0) == trader_id:
                out = {}
                for k in ["trader_id", "display_name", "company_name", "bank_name", "iban", "stc_pay", "updated_at_utc"]:
                    col = idx.get(k)
                    if col:
                        out[k] = ws.cell(row=r, column=col).value or ""
                return out
        except Exception:
            continue
    return {}
...
def upsert_trader_profile(trader_id: int, fields: dict, path: str = DEFAULT_PATH) -> None:
    ...
    if target_row is None:
        ...
        try:
            if "created_at_utc" in idx:
                cur = ws.cell(row=target_row, column=idx["created_at_utc"]).value
                if not (str(cur or "").strip()):
                    ws.cell(row=target_row, column=idx["created_at_utc"]).value = datetime.now(timezone.utc).isoformat()
        except Exception:
            pass
