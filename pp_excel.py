...
ORDERS_HEADERS = [
    ...
    "quote_locked",
    "goods_amount_sar",
    "quote_item_prices",  # أسعار القطع التفصيلية (JSON dict: {"1":"100","2":"55"})
    "parts_type",
    "ship_method",
    "ship_carrier",
    "ship_eta",
    ...
    # ✅ أعلام منع تكرار إرسال PDF (تفصيل حسب النوع)
...
def generate_order_id(prefix: str = "PP", path: str = DEFAULT_PATH) -> str:
    """Generate human-friendly order id: PP-YYMMDD-#### based on existing rows (per day)."""
    wb = _load(path)
    ws = wb[SHEET_ORDERS]
    h = _header_index(ws)
    order_col = h.get("order_id", 1)

    today = datetime.now(timezone.utc).astimezone().strftime("%y%m%d")  # local date
    pat = re.compile(rf"^{re.escape(prefix)}-{today}-(\d{{4}})$")

    max_seq = 0
    for row in range(2, ws.max_row + 1):
        oid = str(ws.cell(row=row, column=order_col).value or "")
        m = pat.match(oid)
        if m:
            try:
