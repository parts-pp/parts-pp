    ...
    for r in range(2, ws.max_row + 1):
        ...
        try:
            if int(v or 0) == trader_id:
                out = {}
                for k in ["trader_id","display_name","company_name","shop_phone","cr_no","vat_no","bank_name","iban","stc_pay","joined_at_utc","is_enabled","updated_at_utc"]:
                    col = idx.get(k)
                    if col:
...
def upsert_trader_profile(trader_id: int, fields: dict, path: str = DEFAULT_PATH) -> None:
    ...
    if target_row is None:
        ...
        # ✅ تاريخ الانضمام: يثبت مرة واحدة عند إنشاء السجل
        try:
            jcol = idx.get("joined_at_utc")
            if jcol and not (ws.cell(row=target_row, column=jcol).value or ""):
                ws.cell(row=target_row, column=jcol).value = datetime.now(timezone.utc).isoformat()
        except Exception:
            pass
